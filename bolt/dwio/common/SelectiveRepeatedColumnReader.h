...
class SelectiveListColumnReader : public SelectiveRepeatedColumnReader {
 public:
  ...
  void resetWhenFilterAll() override {
    SelectiveRepeatedColumnReader::resetWhenFilterAll();
    if (child_) {
      child_->resetWhenFilterAll();
    }
  }
  ...
  void getValues(const RowSet& rows, VectorPtr* FOLLY_NULLABLE result) override;
...
class SelectiveMapColumnReader : public SelectiveRepeatedColumnReader {
 public:
  ...
  void resetWhenFilterAll() override {
    SelectiveRepeatedColumnReader::resetWhenFilterAll();
    if (keyReader_) {
      keyReader_->resetWhenFilterAll();
    }
    if (elementReader_) {
      elementReader_->resetWhenFilterAll();
    }
  }
