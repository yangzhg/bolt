...
OperatorReplayerBase::OperatorReplayerBase(
    ...
    : queryId_(std::string(std::move(queryId))),
      ...
      executor_(executor) {
  BOLT_USER_CHECK(!taskTraceDir_.empty());
  BOLT_USER_CHECK(!taskId_.empty());
  BOLT_USER_CHECK(!nodeId_.empty());
  BOLT_USER_CHECK(!operatorType_.empty());
  if (operatorType_ == "HashJoin") {
    BOLT_USER_CHECK_EQ(pipelineIds_.size(), 2);
  } else {
    BOLT_USER_CHECK_EQ(pipelineIds_.size(), 1);
  }
