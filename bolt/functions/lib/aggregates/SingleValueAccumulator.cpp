...
// An accumulator for a single complex type value (a map, an array
// or a struct).
...
void SingleValueAccumulator::read(
    const VectorPtr& vector,
    vector_size_t index,
    bool exactSize) const {
  BOLT_CHECK_NOT_NULL(start_.header);

  auto stream = HashStringAllocator::prepareRead(start_.header);
  exec::ContainerRowSerde::deserialize(*stream, index, vector.get(), exactSize);
}
