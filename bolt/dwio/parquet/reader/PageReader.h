...
class PageReader {
 public:
  ...
  const dwio::common::DictionaryValues& getDictionary() const {
    return dictionary_;
  }
  ...
  void seekToPage(int64_t row, bool keepRepDefRawData = false);
 ...
 private:
  ...
  void callDecoder(
      ...
      bool& nullsFromFastPath,
      Visitor visitor) {
    BOLT_CHECK(!isDictionary(), "BOOLEAN types are never dictionary-encoded")
    if (nulls) {
      nullsFromFastPath = false;
  ...
  int64_t erasedLeafNullWords_{0};
