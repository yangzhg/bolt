...
#include "bolt/functions/lib/ArraySort.h"
  ...
  context.applyToSelectedNoThrow(rows, [&](vector_size_t row) {
    ...
    std::sort(
        ...
        [&](vector_size_t& a, vector_size_t& b) {
          ...
          bool nullAtA = decodedElements->isNullAt(a);
          bool nullAtB = decodedElements->isNullAt(b);
...
void applyScalarType(
  ...
  BOLT_DCHECK(kind == inputElements->typeKind());
  const SelectivityVector inputElementRows =
      toElementRows(inputElements->size(), rows, inputArray);
  ...
  resultElements->copy(
      inputElements.get(), inputElementRows, /*toSourceRow=*/nullptr, true);
...
class ArraySortFunction : public exec::VectorFunction {
 ...
 private:
  VectorPtr applyFlat(
    ...
    if constexpr (bolt::TypeTraits<Kind>::isPrimitiveType) {
...
core::TypedExprPtr rewriteArraySortCall(
  ...
  if (auto comparison = checker->isSimpleComparison(prefix, *lambda)) {
    ...
    if (!comparison->expr->type()->isOrderable()) {
      BOLT_USER_FAIL(kNotSupported, lambda->toString());
    }
