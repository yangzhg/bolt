...
std::vector<std::tuple<std::string, std::string>> getUpperUnicodeTestData() {
  return {
      {"àáâãäåæçċèéêëėìíîïi̇ðñòóôõöøùúûüýþ",
       "ÀÁÂÃÄÅÆÇĊÈÉÊËĖÌÍÎÏİÐÑÒÓÔÕÖØÙÚÛÜÝÞ"},
...
std::vector<std::tuple<std::string, std::string>> getLowerUnicodeTestData() {
  return {
      {"ÀÁÂÃÄÅÆÇĊÈÉÊËĖÌÍÎÏİÐÑÒÓÔÕÖØÙÚÛÜÝÞ",
       "àáâãäåæçċèéêëėìíîïi̇ðñòóôõöøùúûüýþ"},
...
TEST_F(StringFunctionsTest, concat_ws) {
...

  {
    ...
    auto c1 = data->childAt(1)->as<FlatVector<StringView>>()->rawValues();
    auto result = evaluate<SimpleVector<StringView>>(
        "concat_ws_spark(',', '', '', 'ccc', 'ddd', 'eee', 'fff')", data);
    ...
    result = evaluate<SimpleVector<StringView>>(
        "concat_ws_spark(',', '', null, 'ccc', null, 'eee', 'fff')", data);
    expected = makeFlatVector<StringView>(1'000, [&](auto row) {
      value = ",ccc,eee,fff";
      return StringView(value);
    });
    test::assertEqualVectors(expected, result);
    result = evaluate<SimpleVector<StringView>>(
        "concat_ws_spark(',', 'aaa', 'bbb', c0, 'ccc', 'ddd', c1, 'eee', 'fff')",
        data);
    ...
    test::assertEqualVectors(expected, result);
    result = evaluate<SimpleVector<StringView>>(
        "concat_ws_spark(',',c1,'A somewhat long string.', 'bar')", data);
...
// Test LocalSelectivityVector reuse
TEST_F(StringFunctionsTest, local_selectivity_vector) {
  ...
  exec::EvalCtx evalCtx(&execCtx_, &exprSet, inputRows.get());
  {
    ...
    auto rows = exec::LocalSelectivityVector(evalCtx, size);
    ASSERT_EQ(rows->size(), size);
    ASSERT_EQ(rows->end(), size);
    rows->clearAll();
  }
  {
    vector_size_t size = 100;
...
TEST_F(StringFunctionsTest, chr) {
  ...
  EXPECT_THROW(
      testChrFlatVector(invalidInputTest), bytedance::bolt::BoltUserError);
...
TEST_F(StringFunctionsTest, ascinessOnDictionary) {
  using S = StringView;
  BOLT_REGISTER_VECTOR_FUNCTION(udf_multi_string_function, "multi_string_fn")
...
TEST_F(StringFunctionsTest, parseUrl) {
  auto parse_url = [&](std::optional<std::string> url,
    ...
    if (key == std::nullopt) {
      return evaluateOnce<std::string>("parse_url(c0, c1)", url, part);
    }
    return evaluateOnce<std::string>("parse_url(c0, c1, c2)", url, part, key);
  };
  ...
  EXPECT_EQ(
      parse_url(
          "http://facebook.com/path1/p.php?k1=v1&k2=v2#Ref1", "QUERY", "k2"),
      "v2");
  EXPECT_EQ(
      parse_url(
          "http://facebook.com/path1/p.php?k1=v1&k2=v2#Ref1", "QUERY", "k1"),
      "v1");
  EXPECT_EQ(
      parse_url(
          "http://facebook.com/path1/p.php?k1=v1&k2=v2#Ref1", "QUERY", "k3"),
      std::nullopt);
  EXPECT_EQ(
  ...
  EXPECT_EQ(
      parse_url(
          "http://facebook.com/path1/p.php?k1=v1&k2=v2#Ref1", "AUTHORITY"),
      "facebook.com");
...
template <typename... TArgs>
void StringFunctionsTest::testPrintf(
    std::optional<std::string> expected,
    std::string format,
    TArgs... args) {
  auto result = evaluateOnce<std::string>(format, std::make_optional(args)...);
  ...
  if (result.has_value() && expected.has_value()) {
    ...
    EXPECT_EQ("null", expected.value());
  }
}

template <typename... TArgs>
void StringFunctionsTest::testPrintfOptional(
    std::optional<std::string> expected,
    std::string format,
    std::optional<TArgs>... args) {
  auto result = evaluateOnce<std::string>(format, args...);
  EXPECT_EQ(result.has_value(), expected.has_value());
  if (result.has_value() && expected.has_value()) {
    EXPECT_EQ(result.value(), expected.value());
  } else if (result.has_value() && !expected.has_value()) {
...
TEST_F(StringFunctionsTest, printfTest) {
  testPrintfOptional(
      "String Null: null, Integer Null:             null, Double Null:             null, Double Null: nu",
      "printf('String Null: %s, Integer Null: %16d, Double Null: %16f, Double Null: %.2f', c0, c1, c2, c3)",
  ...
  testPrintfOptional(
      ...
      "printf('Color %s, String Null: %s, number1 %d, number2 %05d, Integer Null: %d, hex %#x, float %5.2f Double Null: %f', c0, c1, c2, c3, c4, c5, c6, c7)",
      std::make_optional(std::string("red")),
      std::optional<std::string>(),
      std::make_optional(123456),
      std::make_optional(89),
      std::optional<int32_t>(),
      std::make_optional(255),
      std::make_optional(3.14159),
      std::optional<double>());

  setQueryTimeZone("Asia/Shanghai");
  testPrintf(
      "timestamp: 1970-01-01 08:00:00.000",
      "printf('timestamp: %s', c0)",
      Timestamp(0, 0));

  setQueryTimeZone("Etc/Greenwich");
  testPrintf(
      "timestamp: 1970-01-01 00:00:00.000",
      "printf('timestamp: %s', c0)",
      Timestamp(0, 0));

  auto input = std::vector<std::string>{"abc", "def"};
  auto expected =
      std::vector<std::string>{"varbinary: 61 62 63", "varbinary: 64 65 66"};
  auto inputVec = makeFlatVector(input, VARBINARY());
  auto inputRow = makeRowVector({inputVec});
  auto expectedVec = makeFlatVector(expected);

  auto result = evaluate("printf('varbinary: %s', c0)", inputRow);
  test::assertEqualVectors(expectedVec, result);
}

TEST_F(StringFunctionsTest, locate) {
  auto locate_test = [&](std::optional<std::string_view> substr,
                         std::optional<std::string_view> str,
                         std::optional<int32_t> pos = 1) {
    return evaluateOnce<int32_t>("locate(c0, c1, c2)", substr, str, pos);
  };

  EXPECT_EQ(locate_test("abc", "abc"), 1);
  EXPECT_EQ(locate_test("", "abc"), 1);
  EXPECT_EQ(locate_test("", "abc", 3), 1);
  EXPECT_EQ(locate_test("abc", "", 3), 0);
  // locate is registered by spark udf
  EXPECT_EQ(locate_test("", "", 3), 1);
  EXPECT_EQ(locate_test("g", "aggregate"), 2);
  EXPECT_EQ(locate_test("g", "aggregate", 3), 3);
  EXPECT_EQ(locate_test("g", "aggregate", 4), 6);
  EXPECT_EQ(locate_test("g", "aggregate", 7), 0);
  EXPECT_EQ(locate_test("b", "aggregate"), 0);
}
