...
FOLLY_ALWAYS_INLINE bool upper(TOutString& output, const TInString& input) {
  if constexpr (ascii) {
    ...
    // auto size =
    //     upperUnicode(output.data(), output.size(), input.data(),
    //     input.size());
    // output.resize(size);
    std::string upperStr = toUpper<ascii>(input);
    output.resize(upperStr.size());
    std::memcpy(output.data(), upperStr.data(), upperStr.size());
  }
  return true;
}
...
FOLLY_ALWAYS_INLINE int32_t charToCodePoint(const T& inputString) {
  ...
  int size;
  auto codePoint = stringCore::utf8proc_codepoint(
...
std::vector<int32_t> stringToCodePoints(const T& inputString) {
  ...
  while (inputIndex < length) {
    ...
    int size;
    codepoint = stringCore::utf8proc_codepoint(
...
FOLLY_ALWAYS_INLINE int64_t stringPosition(
    ...
    int64_t instance) {
  BOLT_USER_CHECK_GT(instance, 0, "'instance' must be a positive number");
  if (subString.size() == 0) {
    return 1;
  }
...
FOLLY_ALWAYS_INLINE bool md5_radix(
  ...
  md5Context.update((const uint8_t*)input.data(), input.size());
  output.reserve(kMaxTextLength);
  int size = 0;
  switch (radix) {
    case 16:
      size = md5Context.digestHex(output.data(), false);
      break;
    case 10:
      size = md5Context.digestDec(output.data(), false);
      break;
    default:
      BOLT_USER_FAIL(
...
FOLLY_ALWAYS_INLINE int endsWithUnicodeWhiteSpace(
  ...
  if (size >= 3) {
    ...
    auto codePoint = stringCore::utf8proc_codepoint(
        data + size - 3, data + size, &codePointSize);
...
FOLLY_ALWAYS_INLINE void pad(
  ...
  BOLT_USER_CHECK(
      ...
      padMaxSize);
  BOLT_USER_CHECK(padString.size() > 0, "padString must not be empty");
