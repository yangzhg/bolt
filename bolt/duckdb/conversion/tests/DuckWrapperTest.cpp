...
using namespace bytedance::bolt::duckdb;
...
TEST_F(BaseDuckWrapperTest, duckToBoltDecimal) {
  // Test SMALLINT decimal to UnscaledShortDecimal conversion.
  verifyDuckToBoltDecimal<int64_t>(
  ...
  verifyDuckToBoltDecimal<int128_t>(
...
TEST_F(BaseDuckWrapperTest, decimalDictCoversion) {
  ...
  auto actual = toBoltVector(size, data, decimalType, pool_.get());
...
TEST_F(BaseDuckWrapperTest, dictConversion) {
  ...
  auto actual = toBoltVector(5, data, VarcharType::create(), pool_.get());
