...
class SortBuffer {
 public:
  ...
  std::optional<common::SpillReadStats> spillReadStats() const {
    ...
    if (spillMerger_ != nullptr) {
      ...
      spillReadStats.spillDecompressTimeUs =
          spillMerger_->getSpillDecompressTime();
      spillReadStats.spillReadIOTimeUs = spillMerger_->getSpillReadIOTime();
    } else if (rowBasedSpillMerger_ != nullptr) {
      spillReadStats.spillReadTimeUs = rowBasedSpillMerger_->getSpillReadTime();
      spillReadStats.spillDecompressTimeUs =
          rowBasedSpillMerger_->getSpillDecompressTime();
      spillReadStats.spillReadIOTimeUs =
          rowBasedSpillMerger_->getSpillReadIOTime();
    } else {
      return std::nullopt;
    }
  ...
  std::optional<common::SortStats> sortStats() const {
    ...
    sortStats.sortOutputTimeUs = getSortOutputTime();
    return sortStats;
  }

  int64_t getSortOutputTime() const {
    return sortOutputTimeUs_;
  }

  int64_t getSortColToRowTime() const {
    return sortColToRowTimeUs_;
  }

  int64_t getSortInSortTime() const {
