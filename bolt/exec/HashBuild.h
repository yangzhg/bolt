...
#include "bolt/exec/SpillOperatorGroup.h"
...
class HashBuild final : public Operator {
 public:
  ...
  void addSpilledRowInput(std::vector<char*>& rows, uint32_t size);
  ...
  bool supportSkewPartition() const {
    bool supportSkew = false;
    if (supportRangePartitionSkewed_) {
      switch (joinType_) {
 ...
 private:
  ...
  void setupSpiller(
      SpillPartition* spillPartition = nullptr,
  ...
  bool reserveMemory(const RowVectorPtr& input, SpilledRows spilledRows);
  ...
  bool calculateJoinBits(
      uint8_t maxjoinBits = 12,
      uint64_t maxPartitionRowCount = 0);

  void estimateSpillTotalSize();

  void recordHashBuildSpillStats();
  ...
  // map<partitionBitOffset, joinPartitionBits>
  std::map<uint8_t, uint8_t> offsetTojoinBits_;

  std::set<uint8_t> hasBeenCalculated_;

  bool isFirstSpill_{true};

  std::optional<uint64_t> spillThreshold_;
  std::optional<uint64_t> memoryUsedForFirstSpill_;

  uint32_t useAdaptiveHashBuild_{0};

  uint64_t spillLevel_{0};

  bool restoringRangePartition_{false};
  bool supportRangePartitionSkewed_{false};
...
template <>
struct fmt::formatter<bytedance::bolt::exec::HashBuild::State>
    : formatter<std::string> {
  auto format(bytedance::bolt::exec::HashBuild::State s, format_context& ctx) {
    return formatter<std::string>::format(
        bytedance::bolt::exec::HashBuild::stateName(s), ctx);
  }
};
