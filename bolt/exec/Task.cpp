...
#include "bolt/experimental/codegen/CodegenLogger.h"
#endif
...
std::shared_ptr<Task> Task::create(
  ...
  if (task->queryCtx()->queryConfig().enableDynamicConcurrencyAdjustment()) {
    // if gluten assigned more tasks than default concurrency,
    // Bolt is able to schedule tasks byself
    task->taskReadyToRunFuture(
            task->queryCtx()->queryConfig().enableBoltTaskScheduling())
        .wait();
    task->setConcurrency(
        ExecutorTaskScheduler::instance().getCurrentConcurrency());
  }
  return task;
}
...
Task::~Task() {
  folly::json::serialization_opts opts;
  opts.allow_nan_inf = true;
  VLOG(1) << "Task finished: " << taskId_ << ", state: "
          << folly::json::serialize(taskStats().serialize(), opts);
  ...
  bytedance::bolt::process::ThreadDebugInfo debugInfoForTask{
...
#define CLEAR(_action_)   \
  ...
  CLEAR(taskReadyToRunPromises_.clear());
...
void Task::init(std::optional<common::SpillDiskOptions>&& spillDiskOpts) {
  BOLT_CHECK(driverFactories_.empty());
  initTaskPool();
...
void Task::setSpillDiskConfig(
  ...
  BOLT_CHECK(spillDiskOpts->spillDirCreated || spillDiskOpts->spillDirCreateCb);
  BOLT_CHECK_NULL(spillDirectoryCallback_);
  BOLT_CHECK(!spillDirectoryCreated_);
  BOLT_CHECK(spillDirectory_.empty());
...
bolt::memory::MemoryPool* Task::getOrAddNodePool(
    const core::PlanNodeId& planNodeId) {
...
bolt::memory::MemoryPool* Task::addOperatorPool(
    const core::PlanNodeId& planNodeId,
    uint32_t splitGroupId,
    int pipelineId,
    uint32_t driverId,
    const std::string& operatorType) {
  bolt::memory::MemoryPool* nodePool;
...
bolt::memory::MemoryPool* Task::addConnectorPoolLocked(
    const core::PlanNodeId& planNodeId,
    int pipelineId,
    uint32_t driverId,
    const std::string& operatorType,
...
bolt::memory::MemoryPool* Task::addMergeSourcePool(
    const core::PlanNodeId& planNodeId,
    uint32_t pipelineId,
    uint32_t sourceId) {
...
bolt::memory::MemoryPool* Task::addExchangeClientPool(
    const core::PlanNodeId& planNodeId,
    uint32_t pipelineId) {
  auto* nodePool = getOrAddNodePool(planNodeId);
  childPools_.push_back(nodePool->addLeafChild(
...
RowVectorPtr Task::next(ContinueFuture* future) {
  ...
  if (!splitsStates_.empty() &&
      !queryCtx_->queryConfig().isSingleThreadedPartialExecutionEnabled()) {
    for (const auto& it : splitsStates_) {
      BOLT_CHECK(
  ...
  for (;;) {
    ...
    for (auto i = 0; i < numDrivers; ++i) {
      ...
      if (futures_[i].valid() && !futures_[i].isReady()) {
      ...
      if (blockingState) {
        if (blockingState->reason() == BlockingReason::kWaitForData) {
          SourceOperator* source =
              dynamic_cast<SourceOperator*>(blockingState->op());
          if (source) {
            source->resetBlockingReason();
          }
          return nullptr;
        }
        futures_[i] = blockingState->future();
      }
    ...
    if (runnableDrivers == 0) {
      if (blockedDrivers > 0) {
        if (!future) {
          ...
          for (auto& continueFuture : futures_) {
...
void Task::start(uint32_t maxDrivers, uint32_t concurrentSplitGroups) {
  bytedance::bolt::process::ThreadDebugInfo threadDebugInfo{
      queryCtx()->queryId(), taskId_, nullptr};
  bytedance::bolt::process::ScopedThreadDebugInfo scopedInfo(threadDebugInfo);
  ...
  try {
    ...
    if (isRunning()) {
      ...
      maybeRemoveFromOutputBufferManager();
      {
        ...
        // finished.
        std::unique_lock<std::timed_mutex> l(mutex_);
        BOLT_CHECK_EQ(numRunningDrivers_, 0);
        BOLT_CHECK_EQ(numFinishedDrivers_, 0);
...
void Task::checkExecutionMode(ExecutionMode mode) {
  BOLT_CHECK_EQ(mode, mode_, "Inconsistent task execution mode.");
}

void Task::createDriverFactoriesLocked(uint32_t maxDrivers) {
  BOLT_CHECK(isRunningLocked());
  BOLT_CHECK(driverFactories_.empty());
  ...
  for (auto& factory : driverFactories_) {
    if (factory->groupedExecution) {
      ...
      numTotalDrivers_ += factory->numTotalDrivers;
    } else if (!factory->isMorselDriven()) {
      numDriversUngrouped_ += factory->numDrivers;
      numTotalDrivers_ += factory->numTotalDrivers;
    }
    taskStats_.pipelineStats.emplace_back(
...
void Task::createAndStartDrivers(uint32_t concurrentSplitGroups) {
  ...
  BOLT_CHECK(
      ...
      errorMessageLocked());
  BOLT_CHECK(!driverFactories_.empty());
  BOLT_CHECK_EQ(concurrentSplitGroups_, 1);
  BOLT_CHECK(drivers_.empty());
...
void Task::initializePartitionOutput() {
  ...
  if (partitionedOutputNode != nullptr) {
    BOLT_CHECK(hasPartitionedOutput());
    BOLT_CHECK_GT(numOutputDrivers, 0);
    ...
    if (self->isRunningLocked()) {
      for (auto& driver : self->drivers_) {
        if (driver != nullptr) {
          ...
          BOLT_CHECK(!driver->isOnThread() && !driver->isTerminated());
          if (!driver->state().hasBlockingFuture &&
            ...
            // Since it's likely that we are in Serial execution.
            //
      ...
      for (auto& driver : self->drivers_) {
        ...
        if (driver->isOnThread()) {
          BOLT_CHECK(driver->isTerminated());
          continue;
        }
...
void Task::createSplitGroupStateLocked(uint32_t splitGroupId) {
  ...
  for (auto pipeline = 0; pipeline < numPipelines; ++pipeline) {
    ...
    // We only equip localExchangeQueue with driver creation function when
    // pipeline can be morsel-driven
    if (factory->isMorselDriven()) {
      // This is two lambda functions creating & running a driver for current
      // pipeline, and send notification once driver creator ends
      auto create_driver_callback =
          makeCreateDriverCallback(factory, splitGroupId, pipeline);
      auto finish_driver_creator_callback =
          makeFinishDriverCreatorCallback(factory);

      // If the pipeline outputs to partitioned output buffer. It make sure it
      // is morsel-driven enabled.
      if (factory->needsPartitionedOutput() != nullptr) {
        auto bufferManager = bufferManager_.lock();
        bufferManager->setMorselDriven(taskId());
      }
...
std::function<void(std::shared_ptr<LocalExchangeQueue>, const int, const bool)>
Task::makeCreateDriverCallback(
    std::unique_ptr<DriverFactory>& factory,
    uint32_t splitGroupId,
    uint32_t pipeline) {
  auto self = shared_from_this();
  std::function<void(
      std::shared_ptr<LocalExchangeQueue>, const int, const bool)>
      create_driver_callback = [self, &factory, pipeline, splitGroupId](
                                   std::shared_ptr<LocalExchangeQueue> queue,
                                   const int partitionId,
                                   const bool isLast) {
        uint32_t inputQueueSize = queue->size();
        uint32_t driverId = 0;

        {
          std::lock_guard<std::timed_mutex> taskLock(self->mutex_);
          // create driver
          driverId = isLast ? factory->numMorselDrivenDrivers - 1
                            : factory->numMorselDrivenDrivers++;
          auto driver = factory->createDriver(
              std::make_unique<DriverCtx>(
                  self, driverId, pipeline, splitGroupId, partitionId),
          ...
          // Initialize the operator stats for operators upon creation of the
          // first morsel-driven driver in this pipeline.
          if (driverId == 0) {
            driver->initializeOperatorStats(
          ...
          ++self->numTotalDrivers_;
          ++self->numDriversUngrouped_;
          // execute driver
          ++self->numRunningDrivers_;
          self->drivers_.emplace_back(std::move(driver));
          Driver::enqueue(self->drivers_.back());
        }
        VLOG(1) << "Task[" << self->taskId_
                << "] start a new driver pipeline=" << pipeline
                << " partition=" << partitionId << " morsels=" << inputQueueSize
                << ". runningDrivers=" << self->numRunningDrivers_
                << " finishedDrivers=" << self->numFinishedDrivers_
                << " totalDrivers=" << self->numTotalDrivers_
                << " actualDrivers=" << self->drivers_.size()
                << " task_addr=" << self.get();
        // Label the corresponding ParitionedOutputBuffer MorselDriven (if not
        // the last driver).
        if (!isLast && factory->needsPartitionedOutput() != nullptr) {
          auto bufferManager = self->bufferManager_.lock();
          bufferManager->updateNumDrivers(self->taskId_, driverId + 1);
        }
      };
  return create_driver_callback;
}

std::function<void(const bool)> Task::makeFinishDriverCreatorCallback(
    std::unique_ptr<DriverFactory>& factory) {
  auto self = shared_from_this();
  std::function<void(const bool)> finish_driver_creator_callback =
      [self, &factory](const bool isLast) {
        if (factory->needsPartitionedOutput() != nullptr) {
          auto bufferManager = self->bufferManager_.lock();
          bufferManager->addFinishedPushBasedDriverSchedulers(self->taskId_);
        }
        std::lock_guard<std::timed_mutex> taskLock(self->mutex_);
        if (isLast) {
          // Update numDrivers in factory and outputBuffer in advance if driver
          // dispatcher closed and this is the last driver
          ++factory->numMorselDrivenDrivers;
          if (factory->needsPartitionedOutput() != nullptr) {
            auto bufferManager = self->bufferManager_.lock();
            bufferManager->updateNumDrivers(
                self->taskId_, factory->numMorselDrivenDrivers);
          }
        }
        ++factory->numFinishedPushBasedDriverSchedulers;
        ++self->numFinishedPushBasedDriverSchedulers_;
        VLOG(1) << "Task[" << self->taskId_
                << "] finished a driver creator. numFinished="
                << self->numFinishedPushBasedDriverSchedulers_
                << " numTotal=" << self->numPushBasedDriverSchedulers_;
      };
  return finish_driver_creator_callback;
}
...
std::vector<std::shared_ptr<Driver>> Task::createDriversLocked(
  ...
  for (auto pipeline = 0; pipeline < numPipelines; ++pipeline) {
    ...
    // we only create drivers for pipelines that cannot be morsel-driven
    if (factory->isMorselDriven()) {
      VLOG(1) << "Will enable morsel-driven for task=" << self->taskId()
              << " pipeline=" << pipeline;
      continue;
    }
...
std::unique_ptr<ContinuePromise> Task::addSplitLocked(
  ...
  if (split.connectorSplit) {
    BOLT_CHECK_NULL(split.connectorSplit->dataSource);
    taskStats_.updateMultiSplitInnerNum(
        split.getMultiSplitUuid(), split.getMultiSplitInnerSize());
  }
...
BlockingReason Task::getSplitOrFutureLocked(
  ...
  if (splitsStore.splits.empty()) {
    ...
    auto [splitPromise, splitFuture] = makeBoltContinuePromiseContract(
...
int Task::getOutputPipelineId() const {
  ...
  BOLT_FAIL("Output pipeline not found");
}
...
bool Task::checkIfFinishedLocked() {
  ...
  bool allFinished = hasMorselDrivenPipeline
      ? numFinishedPushBasedDriverSchedulers_ ==
              numPushBasedDriverSchedulers_ &&
          numFinishedDrivers_ == numTotalDrivers_
      : numFinishedDrivers_ == numTotalDrivers_;
...
std::vector<Operator*> Task::findPeerOperators(
  ...
  for (auto& driver : drivers_) {
    ...
    BOLT_CHECK_EQ(peer->operatorType(), operatorType);
...
bool Task::allPeersFinished(
  ...
  auto pipelineId = caller->driverCtx()->pipelineId;
  const auto numPeers = isMorselDrivenPipeline(pipelineId)
      ? numMorselDrivenDrivers(pipelineId)
      : numDrivers(pipelineId);
...
void Task::addHashJoinBridgesLocked(
  ...
  auto& splitGroupState = splitGroupStates_[splitGroupId];
  auto isMorselDrivenEnabled =
      this->queryCtx()->queryConfig().morselDrivenEnabled();
  for (const auto& planNodeId : planNodeIds) {
    splitGroupState.bridges.emplace(
        planNodeId, std::make_shared<HashJoinBridge>(isMorselDrivenEnabled));
...
ContinueFuture Task::terminate(TaskState terminalState) {
  ...
  std::vector<std::shared_ptr<ExchangeClient>> exchangeClients;
  {
    ...
    if (queryCtx_->queryConfig().enableDynamicConcurrencyAdjustment()) {
      ExecutorTaskScheduler::instance().scheduleNewTasksIfAny(
          shared_from_this(),
          (terminalState == TaskState::kFinished),
          queryCtx_->queryConfig().enableBoltTaskScheduling());
    }
  ...
  calculateInputPositionsCoefficientsOfVariation();
...
void Task::calculateInputPositionsCoefficientsOfVariation() {
  // weight = sum of inputPositions of all operators in this pipeline
  // taskWeightsSum = W1 + W2 + W3, which is the sum of "weight"s of all
  // pipelines in the task
  uint64_t taskWeightsSum = 0;
  // taskVarianceSum = W1P1 + W2P2 + W3P3, which is the weighted sum of its
  // pipeline's coefficients of variation
  double taskVarianceSum = 0;

  // each operator has variance over its drivers.
  // pipelineVarianceSum is the total variance over each
  // pipeline's operators.
  double pipelineVarianceSum;
  // number of operators (with nonzero inputpositions) in the pipeline
  uint64_t pipelineNonZeroOperatorCount;
  for (int pipelineIndex = 0; pipelineIndex < taskStats_.pipelineStats.size();
       pipelineIndex++) {
    pipelineVarianceSum = 0;
    pipelineNonZeroOperatorCount = 0;
    uint64_t currentPipelineWeight = 0;
    for (int operatorIndex = 0; operatorIndex <
         taskStats_.pipelineStats[pipelineIndex].operatorStats.size();
         operatorIndex++) {
      uint64_t currentOperatorInputPositions =
          taskStats_.pipelineStats[pipelineIndex]
              .operatorStats[operatorIndex]
              .inputPositions;
      if (currentOperatorInputPositions > 0) {
        // This calculation is the coefficient of variation. For a group, keep
        // track of the sum, sum of squares, and count. Then the coefficient of
        // variance is = sqrt(sum of squares * count / sum^2 - 1)
        // Explanation for the formula here:
        // https://bytedance.us.larkoffice.com/docx/AYFrd4FqbowFk7xXRlhuC2eLsj2
        double operatorVariance =
            (double)(taskStats_.pipelineStats[pipelineIndex]
                         .operatorStats[operatorIndex]
                         .sumSquaredInputPositions *
                     taskStats_.pipelineStats[pipelineIndex]
                         .operatorStats[operatorIndex]
                         .nonZeroInputPositionsNumDrivers) /
                (currentOperatorInputPositions *
                 currentOperatorInputPositions) -
            1.0;
        if (operatorVariance <= 0) {
          operatorVariance = 0;
        }
        pipelineVarianceSum += std::sqrt(operatorVariance);

        pipelineNonZeroOperatorCount += 1;
        currentPipelineWeight += currentOperatorInputPositions;
      }
    }
    if (pipelineNonZeroOperatorCount > 0) {
      taskVarianceSum += currentPipelineWeight * pipelineVarianceSum /
          (double)pipelineNonZeroOperatorCount;
      taskWeightsSum += currentPipelineWeight;
      taskStats_.pipelineStats[pipelineIndex]
          .pipelineInputSizeCoefficientOfVariation =
          pipelineVarianceSum / (double)pipelineNonZeroOperatorCount;
    }
  }

  // divide by 0 checking
  if (taskWeightsSum > 0) {
    taskStats_.taskInputSizeCoefficientOfVariation =
        taskVarianceSum / (double)taskWeightsSum;
  }
}
...
ContinueFuture Task::makeFinishFutureLocked(const char* comment) {
  auto [promise, future] = makeBoltContinuePromiseContract(comment);
...
void Task::createLocalExchangeQueuesLocked(
    ...
    std::function<void(const bool)> finish_driver_creator_callback) {
  BOLT_CHECK(
      (create_consuming_driver_callback == nullptr &&
       finish_driver_creator_callback == nullptr) ||
      (create_consuming_driver_callback != nullptr &&
       finish_driver_creator_callback != nullptr));
  auto& splitGroupState = splitGroupStates_[splitGroupId];
  BOLT_CHECK(
  ...
  for (auto i = 0; i < numPartitions; ++i) {
    // If the pipeline can be morsel-driven, then increment
    // numPushBasedDriverSchedulers_ by number of localExchangeQueues.
    if (create_consuming_driver_callback) {
      ++numPushBasedDriverSchedulers_;
      std::shared_ptr<PushBasedEvent> e = std::make_shared<PushBasedEvent>(
          create_consuming_driver_callback, finish_driver_creator_callback);

      exchange.queues.emplace_back(std::make_shared<LocalExchangeQueue>(
          exchange.memoryManager,
          i,
          queryCtx_->queryConfig().morselDrivenEnabled(),
          queryCtx_->queryConfig().morselSize(),
          queryCtx_->queryConfig().morselDrivenPrimedQueueSize(),
          e));
    } else {
      exchange.queues.emplace_back(std::make_shared<LocalExchangeQueue>(
          exchange.memoryManager,
...
void Task::leave(
  ...
  BOLT_CHECK_EQ(reason, StopReason::kTerminate);
  BOLT_CHECK_NOT_NULL(driverCb);
...
StopReason Task::enterSuspended(ThreadState& state) {
  BOLT_CHECK(!state.hasBlockingFuture);
  BOLT_CHECK(state.isOnThread());
...
StopReason Task::leaveSuspended(ThreadState& state) {
  ...
  for (;;) {
    {
      std::lock_guard<std::timed_mutex> l(mutex_);
      BOLT_CHECK_GT(state.numSuspensions, 0);
...
std::shared_ptr<ExchangeClient> Task::getExchangeClientLocked(
  ...
  BOLT_CHECK_LT(pipelineId, exchangeClients_.size());
...
std::optional<trace::TraceConfig> Task::maybeMakeTraceConfig() const {
  ...
  BOLT_USER_CHECK(!traceNodes.empty(), "Query trace nodes are not set");
...
std::shared_ptr<SpillOperatorGroup> Task::getSpillOperatorGroupLocked(
  ...
  BOLT_CHECK(it != groups.end(), "Split group is not set {}", splitGroupId);
...
ContinueFuture Task::taskReadyToRunFuture(bool doSchedule) {
  auto self = shared_from_this();
  if (!ExecutorTaskScheduler::instance().waitForScheduling(self, doSchedule)) {
    return ContinueFuture();
  }
  auto [promise, future] = makeBoltContinuePromiseContract(
      fmt::format("Task::taskReadyToRunFuture {}", taskId_));
  taskReadyToRunPromises_.emplace_back(std::move(promise));
  return std::move(future);
}

void Task::notifyReadyToRun() {
  EventCompletionNotifier readyToRunNotifier;
  readyToRunNotifier.activate(std::move(taskReadyToRunPromises_));
  readyToRunNotifier.notify();
}
...
uint64_t Task::MemoryReclaimer::reclaim(
  ...
  BOLT_CHECK_EQ(task->pool()->name(), pool->name());
...
uint64_t Task::MemoryReclaimer::reclaimTask(
    ...
    memory::MemoryReclaimer::Stats& stats) {
  BOLT_CHECK(
      !isAsyncPreloadThread(),
      "Memory reclamation should not be called from async load thread {}, id {}, task id {}, stack {}",
  ...
  auto resumeGuard = folly::makeGuard([&]() {
    try {
      Task::resume(task);
    } catch (const BoltRuntimeError& exception) {
