...
//     BOLT_ASSERT_THROW(
//         spiller_->spill(SpillPartitionNumSet{0}), "Spiller has been
//         finalized");
//     BOLT_ASSERT_THROW(
//         spiller_->spill(RowContainerIterator{}), "Spiller has been
//         finalized");
//   }
//   {
//     setupSpillData(rowType_, numKeys_, 1'000, 1, nullptr, {});
...
//     verifySortedSpillData();
//     BOLT_ASSERT_THROW(
//         spiller_->spill(SpillPartitionNumSet{}), "Spiller has been
//         finalized");
//     BOLT_ASSERT_THROW(
//         spiller_->spill(RowContainerIterator{}), "Spiller has been
//         finalized");
...
TEST_P(RowBasedSpillTest, string_view_format) {
  // row based spill would clear StringView's data part, so we make sure we
  // clear the right part
  std::string s(40, 'a');
  StringView sv(s.data(), s.size());
  ASSERT_TRUE(sv.getString() == s);
  *(char**)((char*)&sv + sizeof(uint64_t)) = nullptr;
  ASSERT_TRUE(sv.data() == nullptr);
  ASSERT_EQ(sv.size(), s.size());
  *(char**)((char*)&sv + sizeof(uint64_t)) = s.data();
  ASSERT_TRUE(sv.getString() == s);
}
