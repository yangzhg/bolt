...
class SignatureVariable {
 public:
  ...
  bool knownTypesOnly() const {
    BOLT_USER_CHECK(isTypeParameter());
...
namespace std {
template <>
struct hash<bytedance::bolt::exec::SignatureVariable> {
  using argument_type = bytedance::bolt::exec::SignatureVariable;
...
template <>
struct hash<bytedance::bolt::exec::TypeSignature> {
  using argument_type = bytedance::bolt::exec::TypeSignature;
  using result_type = std::size_t;
...
template <>
struct hash<bytedance::bolt::exec::FunctionSignature> {
  using argument_type = bytedance::bolt::exec::FunctionSignature;
  using result_type = std::size_t;

  result_type operator()(const argument_type& key) const noexcept {
    auto variablesHasher =
        std::hash<bytedance::bolt::exec::SignatureVariable>{};
    auto typeSignatureHasher =
        std::hash<bytedance::bolt::exec::TypeSignature>{};
