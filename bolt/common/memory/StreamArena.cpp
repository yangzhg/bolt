...
void StreamArena::newRange(
    ...
    ByteRange* /*lastRange*/,
    ByteRange* range) {
  BOLT_CHECK_GT(bytes, 0, "StreamArena::newRange can't be zero length");
  ...
  BOLT_DCHECK_LE(currentOffset_, run.numBytes());
...
void StreamArena::newTinyRange(
    ...
    ByteRange* /*lastRange*/,
    ByteRange* range) {
  BOLT_CHECK_GT(bytes, 0, "StreamArena::newTinyRange can't be zero length");
