...
class JoinFuzzer {
 public:
  ...
  struct PlanWithSplits {
    ...
    std::unordered_map<
        core::PlanNodeId,
        std::vector<std::shared_ptr<bolt::connector::ConnectorSplit>>>
        splits;
  };
 ...
 private:
  ...
  const bool enableDuckdbVerification_;
  size_t numFailures_{0};
  size_t numFailuresInDuckdb_{0};
  size_t numFailuresInEquivalentPlans_{0};
};
  ...
  for (const auto& probe : probeInput) {
    ...
    for (auto i = 0; i < probeKeys.size(); ++i) {
      ...
      build->childAt(i)->copy(
          probe->childAt(i).get(), rows, rowNumbers.data(), false);
    }
...
JoinFuzzer::PlanWithSplits makeDefaultPlanWithTableScan(
    ...
    const std::vector<std::shared_ptr<bolt::connector::ConnectorSplit>>&
        probeSplits,
...
void JoinFuzzer::verify(core::JoinType joinType) {
  ...
  LOG(INFO) << "The default plan:";
  auto expected = execute(plan, false /*injectSpill*/);

  if (enableDuckdbVerification_) {
    ...
    if (auto duckDbResult =
      ...
      try {
        BOLT_CHECK(
            ...
            "Bolt and DuckDB results don't match");
      } catch (BoltRuntimeError& e) {
        // TODO(join-fuzzer-test): Implement how to save inputs and plans.
        LOG(ERROR) << e.what();
        ++numFailures_;
        ++numFailuresInDuckdb_;
      }
    }
  }
...
void JoinFuzzer::go() {
  ...
  size_t iteration = 0;
  std::map<core::JoinType, size_t> joinStats;
  ...
  while (!isDone(iteration, startTime)) {
    ...
    if (joinStats.find(joinType) != joinStats.end()) {
      ++joinStats[joinType];
    } else {
      joinStats[joinType] = 1;
    }
  ...
  LOG(INFO) << "==============================> Total join stats:";
  for (auto stat : joinStats) {
    LOG(INFO) << joinTypeName(stat.first) << ": " << stat.second
              << " iteration(s)";
  }

  BOLT_CHECK_EQ(
      numFailures_, numFailuresInDuckdb_ + numFailuresInEquivalentPlans_);
  LOG(INFO) << "==============================> Total iteration stats:";
  LOG(INFO) << "Number of iterations: " << iteration;
  LOG(INFO) << "Number of failures: " << numFailures_
            << " (there can be multiple failures in one single iterations)";
  LOG(INFO) << "Number of failures when comparing with DuckDB: "
            << numFailuresInDuckdb_;
  LOG(INFO) << "Number of failures when comparing with equivalent plans: "
            << numFailuresInEquivalentPlans_;
}
