...
class RoundTest : public functions::test::FunctionBaseTest {
 protected:
  ...
  std::vector<std::tuple<T, int32_t, T>> testRoundWithDecFloatData() {
    return {{25.1, 2, 25.1},        {1.122112, 0, 1},
  ...
  std::vector<std::tuple<double, int32_t, double>>
  testSpecialRoundDoubleData() {
    return {
        {0.575, 2, 0.58},
        {1.8824999999999998, 3, 1.882},
        {-0.00032500243751833218, 3, 0.0}};
  }

  std::vector<std::tuple<float, int32_t, float>> testSpecialRoundFloatData() {
    return {{0.575, 2, 0.57}, {1.8824999999999998, 3, 1.883}};
  }
...
TEST_F(RoundTest, roundWithDecimal) {
  ...
  runRoundWithDecimalTest<float>(testSpecialRoundFloatData());
  runRoundWithDecimalTest<double>(testRoundWithDecFloatData<double>());
  runRoundWithDecimalTest<double>(testSpecialRoundDoubleData());
