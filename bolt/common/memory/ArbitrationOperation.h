...
class ArbitrationOperation {
 public:
  ...
  void recordGlobalArbitrationStartTime() {
    BOLT_CHECK_EQ(globalArbitrationStartTimeNs_, 0);
...
template <>
struct fmt::formatter<bytedance::bolt::memory::ArbitrationOperation::State>
    : formatter<std::string> {
  auto format(
      bytedance::bolt::memory::ArbitrationOperation::State state,
      format_context& ctx) const {
    return formatter<std::string>::format(
        bytedance::bolt::memory::ArbitrationOperation::stateName(state), ctx);
  }
};
