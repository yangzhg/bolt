...
RowVectorPtr Limit::getOutput() {
  ...
  if (remainingOffset_ > 0) {
    ...
    RowVectorPtr output;
    // Either use dictionary encoding to support offset or use zerocopy buffer
    // slice.
    if (this->operatorCtx_->execCtx()
            ->queryCtx()
            ->queryConfig()
            .limitOffsetDictionaryEncoding()) {
      BufferPtr indices = allocateIndices(outputSize, pool());
      auto rawIndices = indices->asMutable<vector_size_t>();
      std::iota(rawIndices, rawIndices + outputSize, remainingOffset_);

      output = fillOutput(outputSize, indices);
    } else {
      output = std::dynamic_pointer_cast<RowVector>(
          input_->slice(remainingOffset_, outputSize));
    }
    remainingOffset_ = 0;
