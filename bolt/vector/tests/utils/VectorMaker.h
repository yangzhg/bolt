...
#include "bolt/vector/tests/utils/VectorMakerStats.h"
...
T jsonValue(const folly::dynamic& jsonValue) {
  ...
  BOLT_UNSUPPORTED("Unsupported type VectorMaker: {}.", typeid(T).name());
}
...
class SimpleVectorLoader : public VectorLoader {
 public:
  ...
  void loadInternal(
    ...
    BOLT_CHECK(!hook, "SimpleVectorLoader doesn't support ValueHook");
...
class VectorMaker {
 public:
  ...
  std::shared_ptr<LazyVector> lazyFlatVector(
    ...
    return std::make_shared<LazyVector>(
        ...
        std::make_unique<SimpleVectorLoader>(
            [this, valueAt, isNullAt](RowSet rowSet) {
  ...
  template <typename T>
  FlatVectorPtr<T> allNullFlatVectorWithNullValues(vector_size_t size) {
  ...
  SimpleVectorPtr<EvalType<T>> encodedVector(
    ...
    switch (vecType) {
      ...
      default:
        BOLT_UNSUPPORTED("Unsupported encoding type for VectorMaker.");
    }
    return nullptr;
  }
  ...
  ArrayVectorPtr arrayVectorNullableImpl(
    ...
    BOLT_CHECK(type->isArray(), "Type must be an array: {}", type->toString());
  ...
  constantRow(const RowTypePtr& rowType, variant value, vector_size_t size) {
    BOLT_CHECK_EQ(value.kind(), TypeKind::ROW);

    std::vector<VectorPtr> fields(rowType->size());
    for (auto i = 0; i < rowType->size(); i++) {
      fields[i] = BOLT_DYNAMIC_SCALAR_TYPE_DISPATCH(
  ...
  static std::shared_ptr<T> flatten(const VectorPtr& vector) {
    ...
    flatVector->copy(vector.get(), allRows, nullptr, true);
    return flatVector;
  }
...
#include "bolt/vector/tests/utils/VectorMaker-inl.h"
