...
struct ArrowOptions {
  ...
  bool useLargeString{false};
  // Only takes effect when exportToView is true.
  // When exporting bolt StringView to arrow StringView, FlatVector's
  // StringBuffers will be reused, and if StringViewCopyValues is false,
  // directly updating the values of FlatVector to arrow format may cause
  // FlatVector to be unreadable; When StringViewCopyValues is true, copy the
...
///
/// NOTE: Since Arrow couples type and encoding, we need both Bolt type and
...
/// Import an ArrowSchema into a Bolt Type object.
///
...
/// ArrowSchema (which describes the data type), since a Bolt vector needs
...
///
/// The new Bolt vector returned contains only references to the underlying
...
/// Import an ArrowArray and ArrowSchema into a Bolt vector, acquiring
...
template <>
struct fmt::formatter<TimestampUnit> : formatter<string_view> {
  // parse is inherited from formatter<string_view>.
  template <typename FormatContext>
  auto format(TimestampUnit t, FormatContext& ctx) const {
