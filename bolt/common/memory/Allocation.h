...
class Allocation {
  ...
  class PageRun {
   public:
    ...
    PageRun(void* address, MachinePageCount numPages) {
      ...
      BOLT_CHECK_LE(numPages, kMaxPagesInRun);
      BOLT_CHECK_EQ(
 ...
 private:
  FOLLY_ALWAYS_INLINE void sanityCheck() const {
    BOLT_CHECK_EQ(numPages_ == 0, runs_.empty());
    BOLT_CHECK(numPages_ != 0 || pool_ == nullptr);
  }
  ...
  BOLT_FRIEND_TEST(MemoryAllocatorTest, allocationClass1);
  BOLT_FRIEND_TEST(MemoryAllocatorTest, allocationClass2);
  BOLT_FRIEND_TEST(AllocationTest, append);
  BOLT_FRIEND_TEST(AllocationTest, appendMove);
  BOLT_FRIEND_TEST(AllocationTest, maxPageRunLimit);
};
...
class ContiguousAllocation {
 ...
 private:
  FOLLY_ALWAYS_INLINE void sanityCheck() const {
    BOLT_CHECK_EQ(size_ == 0, data_ == nullptr);
    BOLT_CHECK(size_ != 0 || pool_ == nullptr);
  }
