...
#include "bolt/dwio/dwrf/common/Compression.h"
...
#include "bolt/dwio/dwrf/writer/IntegerDictionaryEncoder.h"
#include "bolt/dwio/dwrf/writer/PhysicalSizeAggregator.h"
#include "bolt/dwio/dwrf/writer/RatioTracker.h"
...
class WriterContext : public CompressionBufferPool {
 public:
  ...
  IntegerDictionaryEncoder<T>& getIntDictionaryEncoder(
      ...
      bolt::memory::MemoryPool& dictionaryPool,
      bolt::memory::MemoryPool& generalPool) {
  ...
  std::unique_ptr<dwio::common::DataBuffer<char>> getBuffer(
    ...
    BOLT_CHECK_NOT_NULL(compressionBuffer_);
    BOLT_CHECK_GE(compressionBuffer_->size(), size);
  ...
  void returnBuffer(
    ...
    BOLT_CHECK_NULL(compressionBuffer_);
  ...
  class LocalDecodedVector {
   ...
   private:
    ...
    std::unique_ptr<bolt::DecodedVector> vector_;
  };
  ...
  SelectivityVector& getSharedSelectivityVector(bolt::vector_size_t size) {
    if (FOLLY_UNLIKELY(selectivityVector_ == nullptr)) {
      selectivityVector_ = std::make_unique<bolt::SelectivityVector>(size);
    } else {
 ...
 private:
  ...
  std::unique_ptr<bolt::DecodedVector> getDecodedVector() {
    if (decodedVectorPool_.empty()) {
      return std::make_unique<bolt::DecodedVector>();
    }
  ...
  void releaseDecodedVector(std::unique_ptr<bolt::DecodedVector>&& vector) {
  ...
  std::vector<std::unique_ptr<bolt::DecodedVector>> decodedVectorPool_;
  // Reusable SelectivityVector
  std::unique_ptr<bolt::SelectivityVector> selectivityVector_;
  ...
  BOLT_FRIEND_TEST(WriterContextTest, GetIntDictionaryEncoder);
  BOLT_FRIEND_TEST(WriterContextTest, RemoveIntDictionaryEncoderForNode);
};
