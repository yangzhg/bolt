...
using ::bytedance::bolt::exec::EvalCtx;
using ::bytedance::bolt::exec::Expr;
...
class Re2Match final : public VectorFunction {
 public:
  ...
  void apply(
    ...
    context.applyToSelectedNoThrow(rows, [&](vector_size_t row) {
      if (UNLIKELY(patterns->isNullAt(row) || toSearch->isNullAt(row))) {
        result.setNull(row, true);
      } else {
        auto pattern = patterns->valueAt<StringView>(row);
        std::shared_ptr<RE2> impl = nullptr;
        {
          std::lock_guard<std::mutex> lock(mutex_);
          auto it = compiledRegularExpressions_.find(pattern);
          if (it != compiledRegularExpressions_.end()) {
            impl = it->second;
          } else {
            auto regex =
            ...
            auto regexResult =
                compiledRegularExpressions_.insert(pattern, std::move(regex));
            BOLT_CHECK(regexResult.second);
            impl = regexResult.first->second;
          }
        }
 ...
 private:
  // LRU cache to store most recently parsed regexp
...
void checkForBadGroupId(int64_t groupId, const RE2& re) {
  if (UNLIKELY(groupId < 0 || groupId > re.NumberOfCapturingGroups())) {
    BOLT_USER_FAIL("No group {} in regex '{}'", groupId, re.pattern());
  }
}
...
class LikeGeneric final : public VectorFunction {
 public:
  LikeGeneric() : compiledRegularExpressions_(kMaxCompiledRegexes) {}
  ...
  void apply(
    ...
    auto applyWithRegex = [&](const StringView& input,
      ...
      auto it = compiledRegularExpressions_.find(key);
      if (it == compiledRegularExpressions_.end()) {
        RE2::Options opt{RE2::Quiet};
        opt.set_dot_nl(true);
        ...
        BOLT_USER_CHECK(
            validEscapeUsage,
            "Escape character must be followed by '%', '_' or the escape character itself");
        auto result = compiledRegularExpressions_.insert(
            key, std::make_unique<RE2>(toStringPiece(regex), opt));
        BOLT_CHECK(result.second);
        it = result.first;
      }
...
class PatternStringIterator {
 ...
 private:
  ...
  char charAt(size_t index) const {
    BOLT_DCHECK(index >= 0 && index < pattern_.size())
