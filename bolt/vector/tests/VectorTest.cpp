...
class TestingLoader : public VectorLoader {
 public:
  ...
  void loadInternal(
    ...
    *result = data_;
    BOLT_CHECK_GE(data_->size(), resultSize);
...
class VectorTest : public testing::Test, public test::VectorTestBase {
 protected:
  ...
  void testCopyEncoded(VectorPtr source) {
    ...
    target->copy(source.get(), even, &evenSource[0], false);
    target->copy(source.get(), odd, &oddSource[0], false);
  ...
  void testCopyFromAllNulls(
    ...
    for (auto canCopyAll : {false, true}) {
      vector->copy(
          allNullSource.get(), rowsToCopy, toSourceRow.data(), canCopyAll);

      rowsToCopy.applyToSelected([&](auto row) {
        EXPECT_TRUE(vector->isNullAt(row)) << "at " << row;
      });
...
TEST_F(VectorTest, createOther) {
  ...
  testFlat<TypeKind::INTEGER>(INTERVAL_YEAR_MONTH(), vectorSize_);
}
...
TEST_F(VectorTest, row) {
  ...
  EXPECT_THROW(rowVector->childAt(3)->size(), BoltRuntimeError);
  BOLT_ASSERT_THROW(
      rowVector->childAt(3)->size(),
      "Trying to access non-existing child in RowVector:");

  EXPECT_THROW(rowVector->childAt(3)->resize(vectorSize_), BoltRuntimeError);
  BOLT_ASSERT_THROW(
...
TEST_F(VectorTest, unknown) {
  ...
  intVector->copy(constUnknownVector.get(), rows, nullptr, false);
  ...
  EXPECT_ANY_THROW(
      constUnknownVector->copy(intVector.get(), rows, nullptr, false));
  ...
  EXPECT_ANY_THROW(unknownVector->copy(intVector.get(), rows, nullptr, false));

  // Copying flat UNKNOWN into integer vector nulls out all the copied
  // element.
  ...
  intVector->copy(unknownVector.get(), rows, nullptr, false);
  for (int i = 0; i < intVector->size(); ++i) {
    ASSERT_TRUE(intVector->isNullAt(i));
  }

  // It is okay to copy to a non-constant UNKNOWN vector.
  unknownVector->copy(constUnknownVector.get(), rows, nullptr, false);
  for (int i = 0; i < unknownVector->size(); ++i) {
    ASSERT_TRUE(unknownVector->isNullAt(i)) << i;
  }
  ...
  // Can't copy to constant.
  EXPECT_ANY_THROW(
      constUnknownVector->copy(unknownVector.get(), rows, nullptr, false));
...
TEST_F(VectorTest, copyBoolAllNullFlatVector) {
  ...
  for (auto canCopyAll : {false, true}) {
    copy->copy(allNulls.get(), rows, nullptr, canCopyAll);
...
TEST_F(VectorTest, copyToAllNullsFlatVector) {
  ...
  for (auto canCopyAll : {false, true}) {
    ...
    allNulls->copy(source.get(), rows, nullptr, canCopyAll);
...
TEST_F(VectorTest, rowResize) {
  ...
  EXPECT_THROW(rowWithLazyChild->resize(20), BoltException);
}
  ...
  for (auto canCopyAll : {false, true}) {
    {
      ...
      target->copy(source.get(), rows, nullptr, canCopyAll);
    }
...
TEST_F(VectorTest, copyAscii) {
  for (auto canCopyAll : {false, true}) {
    ...
    other->copy(source.get(), all, nullptr, canCopyAll);
    ...
    some.updateBounds();
    other->copy(source.get(), some, nullptr, canCopyAll);
    ...
    other->copy(largerSource.get(), some, sourceMappings, canCopyAll);
    ascii = other->isAscii(all);
    ASSERT_TRUE(ascii.has_value());
...
TEST_F(VectorTest, copyComplextVecWithNullChild) {
  std::vector<VectorPtr> children;
  auto child1 = makeFlatVector<int32_t>({1, 2, 3});
  children.push_back(child1);
  children.push_back(nullptr);
  auto source = std::make_shared<RowVector>(
      pool(), ROW({INTEGER(), INTEGER()}), nullptr, 100, children);
  ASSERT_NO_THROW(BaseVector::copy(*source));
}
...
TEST_F(VectorCreateConstantTest, null) {
  ...
  testNullConstant<TypeKind::INTEGER>(INTERVAL_YEAR_MONTH());
...
TEST_F(VectorTest, setStringToNull) {
  for (auto canCopyAll : {false, true}) {
    ...
    rows.updateBounds();
    target->copy(unknownNull.get(), rows, nullptr, canCopyAll);
    EXPECT_TRUE(target->isNullAt(2));

    rows.setValid(4, true);
    rows.updateBounds();
    target->copy(stringNull.get(), rows, nullptr, canCopyAll);
    ...
    rows.updateBounds();
    target->copy(flatNulls.get(), rows, nullptr, canCopyAll);
...
TEST_F(VectorTest, createVectorWithNullType) {
  ...
  BOLT_ASSERT_THROW(BaseVector::create(nullptr, 100, pool()), kErrorMessage);
  BOLT_ASSERT_THROW(
  ...
  BOLT_ASSERT_THROW(RowVector::createEmpty(nullptr, pool()), kErrorMessage);
...
#ifndef BOLT_ENABLE_ARROW_VECTORS
TEST_F(VectorTest, testCopyWithZeroCount) {
#else
TEST_F(VectorTest, DISABLED_testCopyWithZeroCount) {
#endif
...
#ifndef NDEBUG
  BOLT_ASSERT_THROW(data->findDuplicateValue(2, 8, flags), "Size is too large");
  BOLT_ASSERT_THROW(
...
SelectivityVector toSelectivityVector(
  ...
  for (const auto& range : ranges) {
    BOLT_CHECK_EQ(range.sourceIndex, range.targetIndex);
...
TEST_F(VectorTest, operatorEqual) {
  auto v1 = makeArrayVector<int128_t>(
      {{HugeInt::build(31, 23178)}, {HugeInt::build(31, 23178)}},
      DECIMAL(30, 15));
  auto v2 = makeArrayVector<int128_t>(
      {{HugeInt::build(31, 23178)}, {HugeInt::build(31, 23178)}},
      DECIMAL(30, 15));
  EXPECT_TRUE(v1->type()->equivalent(*v2->type().get()));
  EXPECT_TRUE(*v1->type() == *v2->type());
  EXPECT_EQ(*v1->type(), *v2->type());
}

TEST_F(VectorTest, sliceUnknownVector) {
  auto baseVector = BaseVector::create(UNKNOWN(), 1024, pool());
  auto v = baseVector->as<FlatVector<UnknownValue>>();
  EXPECT_EQ(v->values(), nullptr);
  EXPECT_NE(v->mutableValues(16), nullptr);
