...
using Operation = bytedance::bolt::exec::ProbeState::Operation;
...
HashTable<ignoreNullKeys>::HashTable(
  ...
  if (enableJit_) {
    if (rows_->JITable(keys)) {
      auto [jitMod, fn] = rows_->codegenRowEqVectors(keys, !ignoreNullKeys);
      jitModule_ = std::move(jitMod);
      rowEqVectorsFunc_ = (RowEqVectors)jitModule_->getFuncPtr(fn);
      auto [jitMod1, fn1] = rows_->codegenCompare(
          keys, {}, bytedance::bolt::jit::CmpType::EQUAL, !ignoreNullKeys);
      jitModuleRow_ = std::move(jitMod1);
      rowEqRowFunc_ = (RowRowCompare)jitModuleRow_->getFuncPtr(fn1);
    }
  }
#endif
}
...
void ProbeState::preProbe(const Table& table, uint64_t hash, int32_t row) {
  ...
  __builtin_prefetch(reinterpret_cast<uint8_t*>(table.table_) + bucketOffset_);
}

template <Operation op, typename Table>
void ProbeState::firstProbe(const Table& table, int32_t firstKey) {
...
char* ProbeState::fullProbe(
  ...
  BOLT_DCHECK(partitionInfo == nullptr || op == Operation::kInsert);

  // if firstProbe have load hited group
  ...
  for (int64_t numProbedBuckets = 0; numProbedBuckets < table.numBuckets();
    ...
    if (empty > 0) {
      ...
      if (op == Operation::kErase) {
        BOLT_FAIL("Erasing non-existing entry");
      }
...
template <typename Table>
char* ProbeState::joinNormalizedKeyFullProbe(
    const Table& table,
    const uint64_t* keys) {
  ...
  BOLT_FAIL("Have looped through all the buckets in table");
}

template <Operation op, typename Table>
void ProbeState::loadNextHit(Table& table, int32_t firstKey) {
...
void ProbeState::eraseHit(Table& table, int64_t& numTombstones) {
  const auto kEmptyGroup = BaseHashTable::TagVector::broadcast(kEmptyTag);
  const bool hasEmptyGroup = simd::toBitMask(tagsInTable_ == kEmptyGroup) != 0;
      ...
      *this,
      ...
      [&](char* group, int32_t row) {
        ...
        if (rowEqVectorsFunc_) {
          ...
          std::stringstream ss;
          if (expected != jited) {
            LOG(ERROR) << " JITED MISMATCH  details: " << std::endl;
            ss << row << " expected: " << expected << " jited: " << jited;
            ss << "rows: " << rows_->toString(group) << " |vec:";
            for (auto d = 0; d < lookup.hashers.size(); d++) {
              auto& decoded = lookup.hashers[d]->decodedVector();
              // get cpp type from types_[d]
              auto type = rows_->keyTypes()[d];
              auto null = decoded.isNullAt(row);
              if (null) {
                ss << " null = " << (int)null;
              ...
              switch (type->kind()) {
                case TypeKind::BIGINT:
                  ss << ",val= " << decoded.valueAt<int64_t>(row);
                  break;
                case TypeKind::INTEGER:
                  ss << ",val= " << decoded.valueAt<int32_t>(row);
                  break;
                case TypeKind::SMALLINT:
                  ss << ",val= " << decoded.valueAt<int16_t>(row);
                  break;
                case TypeKind::TINYINT:
                  ss << ",val= " << decoded.valueAt<int8_t>(row);
                  break;
                case TypeKind::REAL:
                  ss << ",val= " << decoded.valueAt<float>(row);
                  break;
                case TypeKind::DOUBLE:
                  ss << ",val= " << decoded.valueAt<double>(row);
                  break;
                case TypeKind::VARCHAR:
                  ss << ",val= " << decoded.valueAt<StringView>(row);
                  break;
                case TypeKind::TIMESTAMP:
                  ss << ",val= " << decoded.valueAt<Timestamp>(row);
                  break;
                default:
          ...
          BOLT_CHECK(expected == jited, ss.str());
          return expected;
        } else {
          return compareKeys(group, lookup, row);
        }
#else
        return rowEqVectorsFunc_
            ? rowEqVectorsFunc_(group, row, lookup.getDecodedVectors())
            : compareKeys(group, lookup, row);
#endif
      },
      [&](int32_t row, uint64_t index) {
        return isJoin ? nullptr : insertEntry(lookup, index, row);
...
void HashTable<ignoreNullKeys>::arrayGroupProbe(HashLookup& lookup) {
  ...
  for (; i < numProbes; ++i) {
    ...
    BOLT_DCHECK_LT(index, capacity_);
...
void HashTable<ignoreNullKeys>::checkSize(
  ...
  BOLT_CHECK(
      ...
      "capacity {}, numDistinct {}, numTombstoneRows {}, hashMode {}",
      capacity_,
      numDistinct_,
      numTombstones_,
      hashMode_);
...
bool HashTable<ignoreNullKeys>::canApplyParallelJoinBuild() const {
  ...
  // In Bolt, a latent assumption is number of drivers in each pipeline must be
  // less than 256, otherwise, Bolt will encounter the "1 + otherTables_.size()
  // <= std::numeric_limits<uint8_t>::max() (256 vs. 255)" when doing parallel
  // hashbuild. Add this line to make sure when # of drivers >= 256 (possible in
  // Morsel-driven Bolt), parallel hashbuild is disabled.
  if (1 + otherTables_.size() > std::numeric_limits<uint8_t>::max()) {
    return false;
  }
...
void HashTable<ignoreNullKeys>::parallelJoinBuild() {
  ...
  for (auto i = 0; i < numPartitions; ++i) {
    ...
    BOLT_CHECK_GE(
        buildPartitionBounds_[i],
        0,
        "Turn on BOLT_ENABLE_INT64_BUILD_PARTITION_BOUND to avoid integer overflow in buildPartitionBounds_");
  }
...
int32_t findPartition(
  ...
  BOLT_UNREACHABLE("Partition index out of range");
}
} // namespace
...
void HashTable<ignoreNullKeys>::rehash(bool initNormalizedKeys) {
  ...
  for (int32_t i = 0; i <= otherTables_.size(); ++i) {
    ...
    int32_t numGroups;
    auto rowContainer = (i == 0 ? this : otherTables_[i - 1].get())->rows();
    do {
      numGroups = rowContainer->listRows(&iterator, kHashBatchSize, groups);
      if (!insertBatch(
...
void HashTable<ignoreNullKeys>::enableRangeWhereCan(
  ...
  boost::sort::pdqsort(indices.begin(), indices.end(), [&](auto i, auto j) {
...
uint64_t HashTable<ignoreNullKeys>::setHasherMode(
  ...
  for (int i = 0; i < hashers.size(); ++i) {
    ...
    BOLT_CHECK_NE(multiplier, VectorHasher::kRangeTooLarge);
  }
  return multiplier;
}
...
std::string HashTable<ignoreNullKeys>::toString(
  ...
  BOLT_CHECK_GE(startBucket, 0);
  BOLT_CHECK_GT(numBuckets, 0);
...
int32_t HashTable<ignoreNullKeys>::listJoinResultsInternal(
    JoinResultIterator& iter,
    bool includeMisses,
    folly::Range<vector_size_t*> inputRows,
    folly::Range<char**> hits,
  ...
  if constexpr (hasMatchFlags) {
    flags = matchFlags->as<FlatVector<bool>>();
  }
  ...
  while (iter.lastRowIndex < iter.rows->size()) {
    if (!iter.nextHit) {
      ...
      if (!iter.nextHit) {
        ...
        if (includeMisses) {
          if constexpr (hasMatchFlags) {
            // the probe side has ever matched with build side
            // do not include it
...
void HashTable<ignoreNullKeys>::checkConsistency() const {
  BOLT_CHECK_GE(capacity_, numDistinct_);
...
template <bool ignoreNullKeys>
void HashTable<ignoreNullKeys>::directAddRows(
    HashLookup& lookup,
