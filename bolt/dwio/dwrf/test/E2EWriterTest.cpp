...
#include "bolt/dwio/common/encryption/TestProvider.h"
#include "bolt/dwio/common/tests/utils/BatchMaker.h"
#include "bolt/dwio/common/tests/utils/MapBuilder.h"
...
using namespace bytedance::bolt::dwio::common::encryption;
using namespace bytedance::bolt::dwio::common::encryption::test;
using namespace bytedance::bolt::test;
using namespace bytedance::bolt::dwrf::encryption;
...
// buck test bolt/dwio/dwrf/test:bolt_dwrf_e2e_writer_tests --
...
class E2EEncryptionTest : public E2EWriterTest {
 protected:
  ...
  std::unique_ptr<::bytedance::bolt::dwrf::DwrfReader> writeAndRead(
    ...
    // write file to memory
    auto config = std::make_shared<::bytedance::bolt::dwrf::Config>();
    // make sure we always write dictionary to test stride index
    config->set(
        ::bytedance::bolt::dwrf::Config::DICTIONARY_STRING_KEY_SIZE_THRESHOLD,
        1.0f);
    config->set(
        ::bytedance::bolt::dwrf::Config::ENTROPY_KEY_STRING_SIZE_THRESHOLD,
        0.0f);
    auto sink = std::make_unique<MemorySink>(
        16 * 1024 * 1024,
        dwio::common::FileSink::Options{.pool = leafPool_.get()});
    sink_ = sink.get();
    ::bytedance::bolt::dwrf::WriterOptions options;
    options.config = config;
    options.schema = type;
  ...
  const DecryptionHandler& getDecryptionHandler(
      const ::bytedance::bolt::dwrf::DwrfReader& reader) const {
...
TEST_F(E2EEncryptionTest, EncryptRoot) {
  ...
  auto dwrfRowReader =
      dynamic_cast<::bytedance::bolt::dwrf::DwrfRowReader*>(rowReader.get());
  bool preload = true;
  for (int32_t i = 0; i < reader->getNumberOfStripes(); ++i) {
  ...
  for (bool enableReclaim : {false, true}) {
    ...
    SCOPED_TESTVALUE_SET(
        ...
        std::function<void(dwrf::Writer*)>([&](dwrf::Writer* writer) {
          BOLT_CHECK(writer->testingNonReclaimableSection());
...
TEST_F(E2EWriterTest, memoryReclaimAfterClose) {
  ...
  for (const auto& testData : testSettings) {
    ...
    if (testData.abort) {
      writer->abort();
      BOLT_ASSERT_THROW(writer->abort(), "Writer is not running: ABORTED");
      BOLT_ASSERT_THROW(writer->close(), "Writer is not running: ABORTED");
    } else {
      writer->close();
      BOLT_ASSERT_THROW(writer->abort(), "Writer is not running: CLOSED");
      BOLT_ASSERT_THROW(writer->close(), "Writer is not running: CLOSED");
    }
    // Verify append or write after close or abort will fail.
    BOLT_ASSERT_THROW(writer->write(vectors[0]), "Writer is not running");
...
TEST_F(E2EWriterTest, memoryReclaimThreshold) {
  SCOPED_TESTVALUE_SET(
      "bytedance::bolt::dwrf::Writer::MemoryReclaimer::reclaimableBytes",
      std::function<void(dwrf::Writer*)>([&](dwrf::Writer* writer) {
