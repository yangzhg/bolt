...
class SysMemoryReclaimer : public memory::MemoryReclaimer {
 public:
  ...
  void abort(MemoryPool* pool, const std::exception_ptr& error) override {
    BOLT_UNSUPPORTED("SysMemoryReclaimer::abort is not supported");
  }
...
MemoryManager::MemoryManager(const MemoryManager::Options& options)
    : allocator_{createAllocator(options)},
      ...
      useMemoryPoolForGluten_(options.useMemoryPoolForGluten),
      enableDynamicMemoryQuotaManager_(options.enableDynamicMemoryQuotaManager),
      poolDestructionCb_([&](MemoryPool* pool) { dropPool(pool); }),
      sysRoot_{
          options.useMemoryPoolForGluten
              ? std::make_shared<MemoryPoolForGluten>(
                    this,
                    std::string(kSysRootName),
                    MemoryPool::Kind::kAggregate,
                    nullptr,
                    nullptr,
                    // NOTE: the default root memory pool has no capacity
                    // limit, and it is used for system usage in production
                    // such as disk spilling.
                    MemoryPool::Options{
                        ...
                        .enableDynamicMemoryQuotaManager =
              ...
              : std::make_shared<MemoryPoolImpl>(
                    ...
                    nullptr,
                    nullptr,
                    // NOTE: the default root memory pool has no capacity limit,
                    // and it is used for system usage in production such as
                    // disk spilling.
                    MemoryPool::Options{
                        .alignment = alignment_,
                        .maxCapacity = kMaxMemory,
  ...
  BOLT_USER_CHECK_GE(capacity(), 0);
  BOLT_CHECK_GE(allocator_->capacity(), arbitrator_->capacity());
...
MemoryManager* MemoryManager::getInstance() {
  ...
  BOLT_CHECK_NOT_NULL(instance, "The memory manager is not set");
  return instance;
}
...
std::shared_ptr<MemoryPool> MemoryManager::addRootPool(
  ...
  options.singleAllocationThreshold = singleAllocationThreshold_;
  options.accumulativeAllocationThreshold = accumulativeAllocationThreshold_;
  options.enableDynamicMemoryQuotaManager = enableDynamicMemoryQuotaManager_;
  ...
  if (!disableMemoryPoolTracking_) {
    try {
      ...
      if (pools_.find(poolName) != pools_.end()) {
        BOLT_FAIL("Duplicate root pool name found: {}", poolName);
      }
...
void MemoryManager::dropPool(MemoryPool* pool) {
  ...
  BOLT_DCHECK_EQ(pool->reservedBytes(), 0);
  ...
  if (it == pools_.end()) {
    BOLT_FAIL("The dropped memory pool {} not found", pool->name());
  }
  pools_.erase(it);
}
...
std::string MemoryManager::treeMemoryUsage() const {
  std::string out;
  out += sysRoot_->treeMemoryUsage(false, false);
  std::vector<std::shared_ptr<MemoryPool>> pools = getAlivePools();
  for (const auto& pool : pools) {
    out += pool->treeMemoryUsage(false);
  }
