...
class ColumnStats : public AbstractColumnStats {
  ...
  std::unique_ptr<Filter> filter(
    ...
    switch (filterSpec.filterKind) {
      case FilterKind::kIsNull:
        filter = std::make_unique<bolt::common::IsNull>();
        break;
      case FilterKind::kIsNotNull:
        filter = std::make_unique<bolt::common::IsNotNull>();
        break;
    ...
    for (auto hit : hits) {
      ...
      if (bolt::common::applyFilter(*filter, values->valueAt(row))) {
        hits[numHits++] = hit;
      }
    }
    hits.resize(numHits);
 ...
 private:
  ...
  std::unique_ptr<Filter> makeRangeFilter(const FilterSpec& filterSpec) {
    if (values_.empty()) {
      if (filterSpec.allowNulls_) {
        ...
        return std::make_unique<bolt::common::BigintRange>(0, 0, false);
      }
    }
    int32_t lowerIndex;
    int32_t upperIndex;
    ...
    if (!filterSpec.allowNulls_) {
      return std::make_unique<bolt::common::BigintRange>(
    ...
    if (upperIndex - lowerIndex < 1000 && ++counter_ % 10 <= 3) {
      ...
      if (counter_ % 2 == 1 && filterSpec.selectPct < 100.0) {
        return bolt::common::createNegatedBigintValues(in, true);
      }
      return bolt::common::createBigintValues(in, true);
    }
    // sometimes make a negated filter instead (1/4 chance)
    if (counter_ % 4 == 1 && filterSpec.selectPct < 100.0) {
      return std::make_unique<bolt::common::NegatedBigintRange>(
  ...
  std::unique_ptr<Filter> makeRandomFilter(const FilterSpec& filterSpec) {
    BOLT_FAIL("This method is only used in specific types.");
  }
...
class ComplexColumnStats : public AbstractColumnStats {
 public:
  ...
  std::unique_ptr<Filter> filter(
    ...
    bool isNull = filter->kind() == bolt::common::FilterKind::kIsNull;
    int32_t previousBatch = -1;
    for (auto hit : hits) {
      auto batch = batchNumber(hit);
      if (batch != previousBatch) {
