...
struct BetaCDFFunction {
  ...
  call(double& result, double a, double b, double value) {
    ...
    BOLT_USER_CHECK_GT(a, 0, "a must be > 0");
    BOLT_USER_CHECK_GT(b, 0, "b must be > 0");
    BOLT_USER_CHECK_GE(value, 0, "value must be in the interval [0, 1]");
    BOLT_USER_CHECK_LE(value, 1, "value must be in the interval [0, 1]");
...
struct NormalCDFFunction {
  ...
  call(double& result, double m, double sd, double value) {
    BOLT_USER_CHECK_GT(sd, 0, "standardDeviation must be > 0");
...
struct BinomialCDFFunction {
  ...
  call(double& result, TValue numOfTrials, double successProb, TValue value) {
    ...
    BOLT_USER_CHECK_GT(numOfTrials, 0, "numberOfTrials must be greater than 0");
...
struct CauchyCDFFunction {
  ...
  call(double& result, double median, double scale, double value) {
    ...
    if (std::isnan(median) || std::isnan(value)) {
      ...
      BOLT_USER_CHECK_GE(scale, 0, "scale must be greater than 0");
...
struct GammaCDFFunction {
  ...
  call(double& result, double shape, double scale, double value) {
    ...
    BOLT_USER_CHECK_GE(value, 0, "value must be greater than, or equal to, 0");
    BOLT_USER_CHECK_GT(shape, 0, "shape must be greater than 0");
    BOLT_USER_CHECK_GT(scale, 0, "scale must be greater than 0");
...
struct InverseBetaCDFFunction {
  ...
  FOLLY_ALWAYS_INLINE void call(double& result, double a, double b, double p) {
    ...
    BOLT_USER_CHECK((a > 0) && (a != kInf), "a must be > 0");
    BOLT_USER_CHECK((b > 0) && (b != kInf), "b must be > 0");
...
struct ChiSquaredCDFFunction {
  ...
  FOLLY_ALWAYS_INLINE void call(double& result, double df, double value) {
    BOLT_USER_CHECK_GT(df, 0, "df must be greater than 0");
    BOLT_USER_CHECK_GE(value, 0, "value must non-negative");
...
struct FCDFFunction {
  ...
  call(double& result, double df1, double df2, double value) {
    BOLT_USER_CHECK_GE(value, 0, "value must non-negative");
    BOLT_USER_CHECK_GT(df1, 0, "numerator df must be greater than 0");
    BOLT_USER_CHECK_GT(df2, 0, "denominator df must be greater than 0");
...
struct PoissonCDFFunction {
  ...
  FOLLY_ALWAYS_INLINE void call(double& result, double lambda, TValue value) {
    BOLT_USER_CHECK_GE(value, 0, "value must be a non-negative integer");
    BOLT_USER_CHECK_GT(lambda, 0, "lambda must be greater than 0");
...
struct WeibullCDFFunction {
  ...
  call(double& result, double a, double b, double value) {
    ...
    BOLT_USER_CHECK_GT(a, 0, "a must be greater than 0");
    BOLT_USER_CHECK_GT(b, 0, "b must be greater than 0");
