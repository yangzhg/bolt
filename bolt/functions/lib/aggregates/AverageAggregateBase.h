...
SelectivityVector translateToInnerRows(
  ...
  BOLT_DCHECK(!decoded.isIdentityMapping());
...
class AverageAggregateBase : public exec::Aggregate {
 public:
  ...
  FLATTEN void toIntermediate(
    ...
    assignToIntermediate<TInput, TAccumulator>(
        rows, args, result->as<RowVector>()->childAt(0));
    countToIntermediate<false>(rows, args, result->as<RowVector>()->childAt(1));
  ...
  FLATTEN void extractAccumulators(
    ...
    BOLT_CHECK_NOT_NULL(
        sumVector,
        "Average Aggregate cannot cast accumulatorType {} with resultType {}",
        mapTypeKindToName(rowVector->childAt(0)->typeKind()),
        typeid(TAccumulator).name());
 ...
 protected:
  ...
  void addSingleGroupIntermediateResultsImpl(
    ...
    if (decodedPartial_.isConstantMapping()) {
      ...
      const uint64_t* nulls = nullptr;
      if (decodedPartial_.nulls() != nullptr &&
          decodedPartial_.size() == rows.size()) {
        nulls = decodedPartial_.nulls();
      }
      rows.applyToSelected(
          [&](vector_size_t i) {
