...
struct DropValues {
  ...
  constexpr bool acceptsNulls() const {
    return true;
  }
...
class ColumnVisitor {
 public:
  ...
  bool allowNulls() {
    if constexpr (ExtractValues::kSkipNulls && TFilter::deterministic) {
      return false;
    } else {
  ...
  FOLLY_ALWAYS_INLINE vector_size_t checkAndSkipNulls(
    ...
    if constexpr (isDense) {
      if (nullBit == 63) {
        ...
        nullWord &= ~bolt::bits::lowMask(nullBit);
      }
      for (;;) {
        ...
        if (nextNonNull < 64) {
          BOLT_CHECK_LE(rowIndex_, rowOfNullWord + nextNonNull);
      ...
      for (;;) {
        ...
        if ((nullWord & (1UL << nullBit))) {
          toSkip +=
              __builtin_popcountll(nullWord & bolt::bits::lowMask(nullBit));
          current = testRow;
          return toSkip;
        }
        if (++rowIndex_ >= numRows_) {
          ...
          toSkip += __builtin_popcountll(
              nullWord & bolt::bits::lowMask(testRow - rowOfNullWord));
          atEnd = true;
          return toSkip;
        }
      }
  ...
  vector_size_t processNull(bool& atEnd) {
    ...
    if constexpr (TFilter::deterministic && isDense) {
      return 0;
    }
  ...
  FOLLY_ALWAYS_INLINE std::optional<vector_size_t> processLength(
    ...
    if (filter_.hasTestLength() && filter_.testLength(length)) {
      return std::nullopt;
    }
  ...
  FOLLY_ALWAYS_INLINE vector_size_t process(T value, bool& atEnd) {
    if constexpr (!TFilter::deterministic) {
      ...
      if (bolt::common::applyFilter(filter_, value)) {
        filterPassed(value);
      } else {
        filterFailed();
      }
  ...
  void filterPassed(T value) {
    addResult(value);
    if (!std::is_same_v<TFilter, bolt::common::AlwaysTrue>) {
  ...
  process(typename make_index<T>::type value, bool& atEnd) {
    ...
    if constexpr (std::is_same_v<TFilter, bolt::common::AlwaysTrue>) {
      ...
      if (TFilter::deterministic &&
        ...
        if (bolt::common::applyFilter(super::filter_, valueInDictionary)) {
          super::filterPassed(valueInDictionary);
          if (TFilter::deterministic) {
            filterCache()[value] = FilterResult::kSuccess;
          }
    ...
    if constexpr (isDense && TFilter::deterministic) {
      return 0;
    } else {
      ...
      if (inDict()) {
        ...
        if (mask != allTrue) {
          ...
          while (bits) {
            ...
            if (bolt::common::applyFilter(super::filter_, input[i + index])) {
              passed |= 1 << index;
            }
          }
        }
      }
      // We know xsimd::batch<int32_t>::size compare results. If all false,
      // process next batch.
      if (!passed) {
        continue;
  ...
  FOLLY_ALWAYS_INLINE vector_size_t process(int32_t value, bool& atEnd) {
    ...
    if (++super::rowIndex_ >= super::numRows_) {
      ...
      if constexpr (TFilter::deterministic && isDense) {
        ...
        return super::rows_[super::numRows_ - 1] - previous;
      }
    }
