...
std::shared_ptr<exec::VectorFunction> makeArrayShuffleWithCustomSeed(
    ...
    const std::vector<exec::VectorFunctionArg>& inputArgs,
    const core::QueryConfig& config) {
  BOLT_USER_CHECK_EQ(inputArgs.size(), 2);
  BOLT_USER_CHECK_EQ(inputArgs[1].type->kind(), TypeKind::BIGINT);
  BOLT_USER_CHECK_NOT_NULL(inputArgs[1].constantValue);
  BOLT_CHECK(!inputArgs[1].constantValue->isNullAt(0));
