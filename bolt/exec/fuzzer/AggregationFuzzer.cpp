...
AggregationFuzzer::AggregationFuzzer(
    ...
    : AggregationFuzzerBase{
          ...
          queryConfigs,
          std::move(referenceQueryRunner)} {
  BOLT_CHECK(!signatureMap.empty(), "No function signatures available.");
...
bool AggregationFuzzer::verifyWindow(
  ...
  try {
    ...
    if (resultOrError.userExceptionPtr) {
      ++stats_.numFailed;
    }
      ...
      if (resultOrError.result) {
        ...
        if (auto expectedResult = referenceResult.first) {
          ...
          BOLT_CHECK(
              ...
              "Bolt and reference DB results don't match");
    ...
    return resultOrError.userExceptionPtr != nullptr;
  } catch (...) {
...
bool AggregationFuzzer::verifySortedAggregation(
  ...
  if (customVerification &&
      (customVerifier == nullptr || customVerifier->supportsVerify())) {
...
bool AggregationFuzzer::compareEquivalentPlanResults(
  ...
  try {
    ...
    // If Bolt successfully executes a plan, we attempt to verify
    ...
    //    supports Bolt vectors then we have the reference DB execute the plan
    ...
    if (resultOrError.result != nullptr) {
      if (!customVerification) {
      ...
      } else if (referenceQueryRunner_->supportsBoltVectorResults()) {
        if (isSupportedType(firstPlan->outputType()) &&
          ...
          if (referenceResult.first) {
            bolt::fuzzer::ResultOrError expected;
            expected.result =
