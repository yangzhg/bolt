...
void SelectiveTimestampColumnReader::readHelper(
  ...
  if (version_ == bolt::dwrf::RleVersion_1) {
    decodeWithVisitor<bolt::dwrf::RleDecoderV1<true>>(seconds_.get(), visitor);
  } else {
    decodeWithVisitor<bolt::dwrf::RleDecoderV2<true>>(seconds_.get(), visitor);
  }
  ...
  numValues_ = 0;
  if (version_ == bolt::dwrf::RleVersion_1) {
    decodeWithVisitor<bolt::dwrf::RleDecoderV1<false>>(nano_.get(), visitor);
  } else {
    decodeWithVisitor<bolt::dwrf::RleDecoderV2<false>>(nano_.get(), visitor);
  }
  ...
  switch (
    ...
    case common::FilterKind::kMultiRange:
      ...
      break;
    default:
      BOLT_UNSUPPORTED("Unsupported filter.");
  }
}
