...
TEST_F(ArrayConcatTest, singleElementArray) {
  const auto arrayVector = makeArrayVector<int64_t>({{1}, {2}, {3}, {4}});
  const auto elementVector = makeArrayVector<int64_t>({{9}, {8}, {7}, {6}});
  VectorPtr expected;
...
TEST_F(ArrayConcatTest, singleArray) {
  const auto arrayVector =
      makeArrayVector<int64_t>({{1, 3}, {2, 8}, {3, 9}, {4, 22}});
  VectorPtr expected;
  ...
  testExpression("concat(c0)", {arrayVector}, expected);
}
