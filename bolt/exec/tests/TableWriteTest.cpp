...
struct TestParam {
  ...
  CompressionKind compressionKind() const {
    return static_cast<bytedance::bolt::common::CompressionKind>(
        (value & ((1L << 56) - 1)) >> 48);
  }
...
constexpr bool formatSupportsFilterPushdown(const FileFormat format) {
  switch (format) {
...
class TableWriteTest : public HiveConnectorTestBase {
 protected:
  ...
  std::string partitionNameToPredicate(
    ...
    BOLT_CHECK_EQ(partitionKeyValues.size(), partitionTypes.size());
  ...
  std::string partitionNameToPredicate(
    ...
    std::vector<std::string> conjuncts;
    BOLT_CHECK_EQ(partitionDirNames.size(), partitionTypes_.size());
  ...
  void verifyBucketedFileName(const std::filesystem::path& filePath) {
    if (commitStrategy_ == CommitStrategy::kNoCommit) {
      ...
      if (fileFormat_ == FileFormat::PARQUET) {
        ASSERT_TRUE(RE2::FullMatch(
            filePath.filename().string(),
            ".tmp.bolt.0[0-9]+_0_TaskCursorQuery_[0-9]+_.+\\.parquet$"))
            << filePath.filename().string();
      } else {
        ASSERT_TRUE(RE2::FullMatch(
            filePath.filename().string(),
            ".tmp.bolt.0[0-9]+_0_TaskCursorQuery_[0-9]+_.+"))
            << filePath.filename().string();
      }
    }
  }
  ...
  uint32_t parseBucketId(const std::string& bucketFileName) {
    ...
    if (commitStrategy_ == CommitStrategy::kNoCommit) {
      ...
      BOLT_CHECK(
          RE2::FullMatch(bucketFileName, ".tmp.bolt.(\\d+)_.+", &bucketId));
    }
    return bucketId;
  }
  ...
  std::unique_ptr<core::PartitionFunction> getBucketFunction(
    ...
    BOLT_USER_CHECK_EQ(bucketedByChannels.size(), bucketedBy.size());
  ...
  const std::vector<connector::hive::LocationHandle::TableType> tableTypes_ = {
      // Bolt does not currently support TEMPORARY table type.
...
class PartitionedTableWriterTest
 ...
 public:
  ...
  static std::vector<uint64_t> getTestParams() {
    ...
    std::vector<FileFormat> optionalWriters = {FileFormat::PARQUET};
    for (FileFormat optionalWriter : optionalWriters) {
      if (hasWriterFactory(optionalWriter)) {
        fileFormats.push_back(optionalWriter);
      }
    }
...
class BucketedUnpartitionedTableWriterTest
    : public TableWriteTest,
      public testing::WithParamInterface<uint64_t> {
 public:
  BucketedUnpartitionedTableWriterTest() : TableWriteTest(GetParam()) {}
    ...
    for (bool multiDrivers : multiDriverOptions) {
      for (FileFormat fileFormat : fileFormats) {
        testParams.push_back(TestParam{
            ...
            true,
            multiDrivers,
            bytedance::bolt::common::CompressionKind_ZSTD}
                                 .value);
        testParams.push_back(TestParam{
            fileFormat,
            TestMode::kOnlyBucketed,
...
TEST_P(PartitionedTableWriterTest, multiplePartitions) {
  ...
  auto planBuilder = PlanBuilder();
  auto tableScan = PlanBuilder::TableScanBuilder(planBuilder);
  tableScan.outputType(newOutputType);
  tableScan.isFilterPushdownEnabled(formatSupportsFilterPushdown(fileFormat_));
  planBuilder = tableScan.endTableScan();
...
DEBUG_ONLY_TEST_P(
  ...
  SCOPED_TESTVALUE_SET(
      ...
      std::function<void(memory::MemoryPool*)>([&](memory::MemoryPool* pool) {
        ...
        BOLT_MEM_POOL_CAP_EXCEEDED("Inject write OOM");
      }));
...
DEBUG_ONLY_TEST_F(TableWriterArbitrationTest, tableWriteSpillUseMoreMemory) {
  ...
  SCOPED_TESTVALUE_SET(
      "bytedance::bolt::dwrf::Writer::flushInternal",
      std::function<void(dwrf::Writer*)>(([&](dwrf::Writer* writer) {
