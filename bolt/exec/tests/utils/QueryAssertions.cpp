...
using bytedance::bolt::duckdb::boltTimestampToDuckDB;
using bytedance::bolt::duckdb::duckdbTimestampToBolt;

namespace duckdb {
template <>
DUCKDB_API bytedance::bolt::UnknownValue Value::GetValue() const {
  BOLT_UNREACHABLE();
}
...
::duckdb::Value duckValueAt<TypeKind::TIMESTAMP>(
  ...
  return ::duckdb::Value::TIMESTAMP(
      boltTimestampToDuckDB(vector->as<SimpleVector<T>>()->valueAt(index)));
}
...
::duckdb::Value duckValueAt<TypeKind::ARRAY>(
  ...
  if (size == 0) {
    return ::duckdb::Value::EMPTYLIST(duckdb::fromBoltType(elements->type()));
  }
...
::duckdb::Value duckValueAt<TypeKind::MAP>(
  ...
  auto mapType = ::duckdb::ListType::GetChildType(::duckdb::LogicalType::MAP(
      duckdb::fromBoltType(mapKeys->type()),
      duckdb::fromBoltType(mapValues->type())));
  if (size == 0) {
...
template <>
bolt::variant variantAt<TypeKind::HUGEINT>(
    ::duckdb::DataChunk* dataChunk,
    int32_t row,
    int32_t column) {
  auto unscaledValue =
...
variant variantAt<TypeKind::TIMESTAMP>(
    ...
    int32_t row,
    int32_t column) {
  return variant::timestamp(duckdbTimestampToBolt(
...
template <>
bolt::variant variantAt<TypeKind::HUGEINT>(const ::duckdb::Value& value) {
  auto hugeInt = ::duckdb::HugeIntValue::Get(value);
  return bolt::variant(HugeInt::build(hugeInt.upper, hugeInt.lower));
}

template <>
variant variantAt<TypeKind::TIMESTAMP>(const ::duckdb::Value& value) {
  return variant::timestamp(
      duckdbTimestampToBolt(value.GetValue<::duckdb::timestamp_t>()));
}
...
variant rowVariantAt(const ::duckdb::Value& vector, const TypePtr& rowType) {
  ...
  for (size_t i = 0; i < structValue.size(); ++i) {
    ...
    if (currChild.IsNull()) {
      ...
      auto value = BOLT_DYNAMIC_SCALAR_TYPE_DISPATCH(
...
variant mapVariantAt(const ::duckdb::Value& vector, const TypePtr& mapType) {
  ...
  BOLT_CHECK_EQ(vector.type().id(), ::duckdb::LogicalTypeId::MAP);
  ...
  for (int i = 0; i < valueList.size(); i++) {
    ...
    BOLT_CHECK(!value[0].IsNull());
    variantKey =
        BOLT_DYNAMIC_SCALAR_TYPE_DISPATCH(variantAt, keyType->kind(), value[0]);

    if (value[1].IsNull()) {
      variantValue = nullVariant(valueType);
    } else {
      variantValue = BOLT_DYNAMIC_SCALAR_TYPE_DISPATCH(
...
variant arrayVariantAt(
  ...
  for (int i = 0; i < elementList.size(); i++) {
    ...
    if (elementList[i].IsNull()) {
      ...
      auto variant = BOLT_DYNAMIC_SCALAR_TYPE_DISPATCH(
...
variant variantAt(const VectorPtr& vector, vector_size_t row) {
  ...
  // }
  return BOLT_DYNAMIC_SCALAR_TYPE_DISPATCH(variantAt, typeKind, vector, row);
}
...
struct MaterializedRowEpsilonComparator {
 public:
  ...
  MaterializedRowEpsilonComparator(
    ...
    BOLT_CHECK_GT(numFloatingPointColumns, 0);
    BOLT_CHECK_GE(columns_.size(), numFloatingPointColumns);
  }
...
std::optional<bool> MaterializedRowEpsilonComparator::areEqual(
    const MaterializedRowMultiset& expected,
    const MaterializedRowMultiset& actual) {
  BOLT_CHECK(!expected.empty());
  BOLT_CHECK(!actual.empty());
  BOLT_CHECK(equalTypeKinds(*expected.begin(), *actual.begin()));
...
DuckDBQueryResult DuckDbQueryRunner::execute(const std::string& sql) {
  ::duckdb::Connection con(db_);
  // Changing the default null order of NULLS FIRST used by DuckDB. Bolt uses
  // NULLS LAST.
...
std::tuple<uint32_t, std::vector<bolt::column_index_t>>
findFloatingPointColumns(const MaterializedRow& row) {
  ...
  std::vector<bolt::column_index_t> indices;
...
static bool compareMaterializedRowsLess(
    const MaterializedRowMultiset& less,
    const MaterializedRowMultiset& more) {
  if (less.size() != more.size())
    return false;

  auto lessRow = less.begin();
  auto moreRow = more.begin();
  for (; lessRow != less.end() && moreRow != more.end(); lessRow++, moreRow++) {
    if (lessRow->size() != moreRow->size())
      return false;

    auto lessCell = lessRow->begin();
    auto moreCell = moreRow->begin();
    for (; lessCell != lessRow->end() && moreCell != moreRow->end();
         lessCell++, moreCell++) {
      if (lessCell->kind() != moreCell->kind())
        return false;

      if (moreCell->lessThanWithEpsilon(*lessCell))
        return false;
    }
  }
...
std::shared_ptr<Task> assertQueryBetween(
    const std::shared_ptr<const core::PlanNode>& plan,
    const std::vector<RowVectorPtr>& expectedResultsLess,
    const std::vector<RowVectorPtr>& expectedResultsMore) {
  CursorParameters params;
  params.planNode = plan;
  return assertQueryBetween(params, expectedResultsLess, expectedResultsMore);
}
...
void assertResultsBetween(
    const std::vector<RowVectorPtr>& expectedLess,
    const std::vector<RowVectorPtr>& expectedMore,
    const std::vector<RowVectorPtr>& actual,
    const RowTypePtr& resultType) {
  MaterializedRowMultiset actualRows;
  ...
  MaterializedRowMultiset expectedRowsLess;
  for (auto vector : expectedLess) {
    auto rows = materialize(vector);
    std::copy(
        rows.begin(),
        rows.end(),
        std::inserter(expectedRowsLess, expectedRowsLess.end()));
  }

  MaterializedRowMultiset expectedRowsMore;
  for (auto vector : expectedMore) {
    auto rows = materialize(vector);
    std::copy(
        rows.begin(),
        rows.end(),
        std::inserter(expectedRowsMore, expectedRowsMore.end()));
  }

  if (not compareMaterializedRowsLess(expectedRowsLess, actualRows)) {
    auto message =
        generateUserFriendlyDiff(expectedRowsLess, actualRows, resultType);
    EXPECT_TRUE(false) << "Expected result above lower bound. Diffs are shown:"
                       << message;
  }

  if (not compareMaterializedRowsLess(actualRows, expectedRowsMore)) {
    auto message =
        generateUserFriendlyDiff(actualRows, expectedRowsMore, resultType);
    EXPECT_TRUE(false) << "Expected result below upper bound. Diffs are shown:"
                       << message;
  }
}
