...
std::vector<MachinePageCount> MemoryAllocator::makeSizeClassSizes(
  ...
  BOLT_CHECK_LE(256, largest);
  BOLT_CHECK_EQ(largest, bits::nextPowerOfTwo(largest));
...
MemoryAllocator::SizeMix MemoryAllocator::allocationSize(
  ...
  for (int32_t sizeIndex = sizeClassSizes_.size() - 1; sizeIndex >= 0;
    ...
    BOLT_CHECK_LE(
        classPageSize * numClassPages,
        Allocation::PageRun::kMaxPagesInRun,
        "Allocated {} pages of size class {} for {} pages, "
        "maxNumClassPages {}",
        numClassPages,
        classPageSize,
        numPages,
...
MachinePageCount MemoryAllocator::roundUpToSizeClassSize(
  ...
  BOLT_CHECK_LE(pages, sizes.back());
  ...
  if (reservationCB != nullptr) {
    if (mix.totalPages >= numPagesToFree) {
      ...
      try {
        ...
        BOLT_MEM_LOG_EVERY_MS(WARNING, 1'000)
...
void MemoryAllocator::useHugePages(
    ...
    bool enable) {
#ifdef linux
  if (!FLAGS_bolt_memory_use_hugepages) {
    return;
  }
  ...
  if (rc != 0) {
    BOLT_MEM_LOG(WARNING) << "madvise hugepage errno="
