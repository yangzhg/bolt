...
class LocalExchangeSource : public exec::ExchangeSource {
 public:
  ...
  folly::SemiFuture<Response> request(
    ...
    auto promise = BoltPromise<Response>("LocalExchangeSource::request");
    auto future = promise.getSemiFuture();

    promise_ = std::move(promise);

    auto buffers = OutputBufferManager::getInstance().lock();
    BOLT_CHECK_NOT_NULL(buffers, "invalid OutputBufferManager");
 ...
 private:
  ...
  BoltPromise<Response> promise_{BoltPromise<Response>::makeEmpty()};
