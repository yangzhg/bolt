...
void printCoverageMap(
    ...
    const std::unordered_set<std::string>& boltNames,
    const std::unordered_set<std::string>& boltAggNames,
    const std::unordered_set<std::string>& boltWindowNames,
    const std::string& domain) {
  ...
  // Keep track of cells which contain functions available in Bolt. These cells
  // need to be highlighted using CSS rules.
  TableCellTracker boltCellTracker;

  auto printName = [&](int row,
                       int column,
                       const std::string& name,
                       const std::unordered_set<std::string>& boltNames) {
    if (boltNames.count(name)) {
      boltCellTracker.add(row, column);
  ...
  for (int i = 0; i < numRows; i++) {
    ...
    for (int j = 0; j < numScalarColumns; j++) {
      auto n = i + numRows * j;
      n < scalarCnt
          ? printer.addColumn(printName(i, j, scalarNames[n], boltNames))
    ...
    i < aggCnt ? printer.addColumn(printName(
                     i, numScalarColumns + 1, aggNames[i], boltAggNames))
               : printer.addEmptyColumn();
    ...
    i < windowCnt
        ? printer.addColumn(printName(
              i, numScalarColumns + 3, windowNames[i], boltWindowNames))
        : printer.addEmptyColumn();
  ...
  printTableCss(numScalarColumns, boltCellTracker, std::cout);
...
/// Returns alphabetically sorted list of scalar functions available in Bolt,
...
/// Bolt, excluding compaion functions.
...
/// Returns alphabetically sorted list of window functions available in Bolt,
/// excluding companion functions.
...
/// coverage map showing which of these functions are available in Bolt.
...
void printCoverageMap(
    ...
    const std::vector<std::string>& windowNames,
    const std::string& domain = "") {
  auto boltFunctions = getFunctionSignatures();

  std::unordered_set<std::string> boltNames;
  boltNames.reserve(boltFunctions.size());
  for (const auto& func : boltFunctions) {
    boltNames.emplace(func.first);
  }

  std::unordered_set<std::string> boltAggNames;
  std::unordered_set<std::string> boltWindowNames;
  const auto& boltWindowFunctions = exec::windowFunctions();

  exec::aggregateFunctions().withRLock([&](const auto& boltAggregateFunctions) {
    for (const auto& entry : boltAggregateFunctions) {
      if (!isCompanionFunctionName(entry.first, boltAggregateFunctions)) {
        boltAggNames.emplace(entry.first);
      }
    }
    for (const auto& entry : boltWindowFunctions) {
      if (!isCompanionFunctionName(entry.first, boltAggregateFunctions) &&
          boltAggregateFunctions.count(entry.first) == 0) {
        boltWindowNames.emplace(entry.first);
      }
    }
  });
