...
class ConversionsTest : public testing::Test {
 protected:
  ...
  static void testConversion(
    ...
    if (!expectError) {
      BOLT_CHECK_EQ(input.size(), expectedResult.size());
    }
    ...
    auto cast = [&](TFrom input) -> TTo {
      ...
      if (truncate & legacyCast) {
        result = Converter<toTypeKind, void, TruncateLegacyCastPolicy>::cast(
            input, &nullOutput);
      } else if (!truncate & legacyCast) {
        result = Converter<toTypeKind, void, LegacyCastPolicy>::cast(
            input, &nullOutput);
      } else if (truncate & !legacyCast) {
        result = Converter<toTypeKind, void, TruncateCastPolicy>::cast(
            input, &nullOutput);
      } else {
        result = Converter<toTypeKind, void, DefaultCastPolicy>::cast(
            input, &nullOutput);
      }
