...
ArbitrationParticipant::Config::Config(
  ...
  BOLT_CHECK_GE(slowCapacityGrowRatio, 0);
  BOLT_CHECK_EQ(
  ...
  BOLT_CHECK_GE(minFreeCapacityRatio, 0);
  BOLT_CHECK_LE(minFreeCapacityRatio, 1);
  BOLT_CHECK_EQ(
      minFreeCapacity == 0,
...
ArbitrationParticipant::~ArbitrationParticipant() {
  BOLT_CHECK_NULL(runningOp_);
  BOLT_CHECK(waitOps_.empty());
}
...
uint64_t ArbitrationParticipant::maxGrowCapacity() const {
  ...
  BOLT_CHECK_LE(capacity, maxCapacity_);
...
void ArbitrationParticipant::getGrowTargets(
  ...
  BOLT_CHECK_LE(minGrowBytes, maxGrowBytes);
  BOLT_CHECK_LE(requestBytes, maxGrowBytes);
}
...
void ArbitrationParticipant::finishArbitration(ArbitrationOperation* op) {
  ...
  {
    std::lock_guard<std::mutex> l(stateLock_);
    BOLT_CHECK_EQ(static_cast<void*>(op), static_cast<void*>(runningOp_));
    if (!waitOps_.empty()) {
...
uint64_t ArbitrationParticipant::reclaim(
  ...
  try {
    ...
    BOLT_MEM_LOG(INFO) << "Reclaimed from memory pool " << pool_->name()
                       << " reserved memory " << succinctBytes(reclaimedBytes)
                       << ", capacity " << succinctBytes(reclaimedCapacity);
  } catch (const std::exception& e) {
    BOLT_MEM_LOG(ERROR) << "Failed to reclaim from memory pool "
...
uint64_t ArbitrationParticipant::abortLocked(
  ...
  try {
    BOLT_MEM_LOG(WARNING) << "Memory pool " << pool_->name()
                          << " is being aborted";
    pool_->abort(error);
  } catch (const std::exception& e) {
    BOLT_MEM_LOG(WARNING) << "Failed to abort memory pool " << pool_->toString()
                          << ", error: " << e.what();
  }
  BOLT_MEM_LOG(WARNING) << "Memory pool " << pool_->name() << " aborted";
...
ScopedArbitrationParticipant::ScopedArbitrationParticipant(
    ...
    : ArbitrationParticipant_(std::move(ArbitrationParticipant)),
      pool_(std::move(pool)) {
  BOLT_CHECK_NOT_NULL(ArbitrationParticipant_);
