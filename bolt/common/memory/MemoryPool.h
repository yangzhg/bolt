...
class MemoryPool : public std::enable_shared_from_this<MemoryPool> {
 public:
  ...
  struct Options {
    ...
    std::string poolRegex{};
    int64_t singleAllocationThreshold{0};
    int64_t accumulativeAllocationThreshold{0};
  ...
  virtual void* allocateZeroFilled(
      int64_t numEntries,
      int64_t sizeEach,
      std::optional<uint8_t> alignment = std::nullopt) = 0;
  ...
  virtual std::string treeMemoryUsage(
      bool skipEmptyPool = true,
      bool printTopUsage = true) const = 0;
 ...
 protected:
  ...
  BOLT_FRIEND_TEST(MemoryPoolTest, growFailures);
  BOLT_FRIEND_TEST(MemoryPoolTest, grownonContiguousAllocateFailures);
};
...
class MemoryPoolImpl : public MemoryPool {
 public:
  ...
  std::string treeMemoryUsage(
      bool skipEmptyPool = true,
      bool printTopUsage = true) const override;
 ...
 protected:
  ...
  FOLLY_ALWAYS_INLINE int64_t sizeAlign(int64_t size, uint8_t alignment) const {
  ...
  void incrementReservationNonThreadSafe(MemoryPool* requestor, uint64_t size);
  ...
  FOLLY_ALWAYS_INLINE void sanityCheckLocked() const {
    if (FOLLY_UNLIKELY(
      ...
      BOLT_FAIL("Bad memory usage track state: {}", toStringLocked());
    }
  }
  ...
  MemoryAllocationListener* getListener() {
  ...
  // The number of memory capacity shrinks.
  std::atomic_uint64_t numShrinks_{0};
...
class StlAllocator {
...
#ifndef NDEBUG
    typeMsg_ =
        fmt::format("StlAllocator<{}>", folly::demangle(typeid(T).name()));
#endif
  }

  template <typename U>
  /* implicit */ StlAllocator(const StlAllocator<U>& a)
      : StlAllocator{a.pool} {}

  template <typename PooledAllocator>
  /* implicit */ StlAllocator(PooledAllocator* FOLLY_NONNULL allocator)
      : StlAllocator{*allocator->pool()} {}

  StlAllocator(MemoryPool* FOLLY_NONNULL pool) : StlAllocator{*pool} {}

  T* allocate(size_t n) {
    return static_cast<T*>(pool.allocate(mul(n, sizeof(T))));
  }

  void deallocate(T* p, size_t n) {
    pool.free(p, mul(n, sizeof(T)));
  }
 ...
 private:
  template <typename T1>
  inline T1 mul(const T1& a, const T1& b) {
#ifndef NDEBUG
    return checkedMultiply(a, b, typeMsg_.c_str());
#else
...
struct DefaultAlignment {
  static constexpr size_t value = (alignof(T) > MemoryAllocator::kMinAlignment)
      ? alignof(T)
      : MemoryAllocator::kMinAlignment;
};

template <size_t Alignment>
struct AdjustAlignment {
  static_assert(
      (Alignment & (Alignment - 1)) == 0,
      "Alignment must be a power of two.");

  static constexpr size_t value = (Alignment < MemoryAllocator::kMinAlignment)
      ? MemoryAllocator::kMinAlignment
      : Alignment;
};

// This Allocator will allocate memory from the MemoryPool and align it to
// specified alignment. This will use extra memory for alignment. The extra
// memory size is 8 + sizeof(T) for each allocation. If we can use system
// aligned allocation like posix_memalign or aligned_alloc may be can save
// the extra memory.
template <typename T, uint8_t Alignment = DefaultAlignment<T>::value>
class AlignedStlAllocator {
  static constexpr size_t adjustedAlignment = AdjustAlignment<Alignment>::value;

  static_assert(
      (adjustedAlignment == MemoryAllocator::kMinAlignment) ||
          (adjustedAlignment >= MemoryAllocator::kMinAlignment &&
           adjustedAlignment <= MemoryAllocator::kMaxAlignment &&
           (adjustedAlignment & (adjustedAlignment - 1)) == 0),
      "Alignment check failed, alignmentBytes must be either equal to 16 or a power of two within the range [16, 64]");
 ...
 public:
  ...
  explicit AlignedStlAllocator(MemoryPool& pool) : pool{pool} {
#ifndef NDEBUG
    typeMsg_ = fmt::format(
        "AlignedStlAllocator<{}, {}>",
  ...
  explicit AlignedStlAllocator(MemoryPool* FOLLY_NONNULL pool)
      : AlignedStlAllocator{*pool} {}

  template <typename PooledAllocator>
  explicit AlignedStlAllocator(PooledAllocator* FOLLY_NONNULL allocator)
      : AlignedStlAllocator{allocator->pool()} {}

  template <class U, uint8_t A>
  AlignedStlAllocator(const AlignedStlAllocator<U, A>& a)
      : AlignedStlAllocator{a.pool} {}

  T* allocate(size_t n) {
    return static_cast<T*>(pool.allocate(mul(n, sizeof(T)), adjustedAlignment));
  }

  void deallocate(T* p, size_t n) {
    pool.free(p, mul(n, sizeof(T)), adjustedAlignment);
  }

  template <typename T1, uint8_t A1>
  bool operator==(const AlignedStlAllocator<T1, A1>& rhs) const {
    if constexpr (std::is_same_v<T, T1> && A1 == adjustedAlignment) {
      return &this->pool == &rhs.pool;
    }
    return false;
  }
 ...
 private:
  ...
  template <typename T1>
  inline T1 plus(const T1& a, const T1& b) {
#ifndef NDEBUG
    return checkedPlus(a, b, typeMsg_.c_str());
#else
    return checkedPlus(a, b);
#endif
  }
#ifndef NDEBUG
  std::string typeMsg_;
...
template <>
struct fmt::formatter<bytedance::bolt::memory::MemoryPool::Kind>
    : formatter<std::string> {
  auto format(bytedance::bolt::memory::MemoryPool::Kind s, format_context& ctx)
      const {
    return formatter<std::string>::format(
        bytedance::bolt::memory::MemoryPool::kindString(s), ctx);
  }
};
