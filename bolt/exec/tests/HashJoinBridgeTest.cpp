 ...
 protected:
  ...
  HashJoinBridgeTest()
      : rowType_(ROW({"k1", "k2"}, {BIGINT(), BIGINT()})),
        ...
        jitRowEqVectors_(GetParam().jitRowEqVectors) {}
  ...
  std::shared_ptr<HashJoinBridge> createJoinBridge(bool isMorselDriven) const {
    return std::make_shared<HashJoinBridge>(isMorselDriven);
  }
  ...
  const bool jitRowEqVectors_{false};
...
TEST_P(HashJoinBridgeTest, withoutSpill) {
  // Test both when morsel-driven is disabled and enabled.
  for (const bool isMorselDriven : {false, true}) {
    for (const bool hasNullKeys : {false, true}) {
      ...
      auto joinBridge = createJoinBridge(isMorselDriven);
      // Can't call any other APIs except addBuilder() before start a join
      // bridge first.
      ASSERT_ANY_THROW(
      ...
      // Can't start a bridge without any builders if morsel-driven is disabled.
      // When morsel-driven is enabled, joinBridge should not throw any error
      // when there is no builder.
      if (!isMorselDriven) {
        ASSERT_ANY_THROW(joinBridge->start());
      }

      joinBridge = createJoinBridge(isMorselDriven);
