...
class ApproxPercentileResultVerifier : public ResultVerifier {
 public:
  ...
  void initialize(
    ...
    BOLT_CHECK_LE(allRanges_->size(), numInputs);
  }
  ...
  bool verify(const RowVectorPtr& result) override {
    ...
    // BOLT_CHECK_EQ(ranges->size(), result->size() * percentiles_.size());
    ...
    for (auto i = 0; i < ranges->size(); ++i) {
      if (value->isNullAt(i)) {
        BOLT_CHECK(minPct->isNullAt(i));
        BOLT_CHECK(maxPct->isNullAt(i));
        continue;
      }

      BOLT_CHECK(!minPct->isNullAt(i));
      BOLT_CHECK(!maxPct->isNullAt(i));
      BOLT_CHECK(!pctIndex->isNullAt(i));
 ...
 private:
  ...
  static std::vector<double> extractPercentiles(
    ...
    if (percentileVector->type()->isDouble()) {
      BOLT_CHECK(!percentileVector->isNullAt(0));
  ...
  static std::vector<double> toList(const VectorPtr& vector) {
    BOLT_CHECK(vector->type()->equivalent(*ARRAY(DOUBLE())));

    DecodedVector decoded(*vector);
    auto arrayVector = decoded.base()->as<ArrayVector>();

    BOLT_CHECK(!decoded.isNullAt(0));
    ...
    for (auto i = 0; i < size; ++i) {
      BOLT_CHECK(!elementsVector->isNullAt(offset + i));
