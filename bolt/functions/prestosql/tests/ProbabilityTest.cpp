...
class ProbabilityTest : public functions::test::FunctionBaseTest {
 protected:
  ...
  void poissonCDFTests() {
    ...
    EXPECT_THROW(poissonCDF<ValueType>(kInf, 3), BoltUserError);
  }
...
TEST_F(ProbabilityTest, betaCDF) {
  ...
  BOLT_ASSERT_THROW(betaCDF(0, 3, 0.5), "a must be > 0");
  BOLT_ASSERT_THROW(betaCDF(3, 0, 0.5), "b must be > 0");
  BOLT_ASSERT_THROW(
      betaCDF(3, 5, -0.1), "value must be in the interval [0, 1]");
  BOLT_ASSERT_THROW(betaCDF(3, 5, 1.1), "value must be in the interval [0, 1]");
  BOLT_ASSERT_THROW(betaCDF(kNan, 3, 0.5), "a must be > 0");
  BOLT_ASSERT_THROW(betaCDF(3, kNan, 0.5), "b must be > 0");
  BOLT_ASSERT_THROW(
...
TEST_F(ProbabilityTest, normalCDF) {
  ...
  BOLT_ASSERT_THROW(normal_cdf(0, 0, 0.1985), "standardDeviation must be > 0");
  BOLT_ASSERT_THROW(
...
TEST_F(ProbabilityTest, cauchyCDF) {
  ...
  BOLT_ASSERT_THROW(cauchyCDF(1.0, kNan, 1.0), "scale must be greater than 0");
  BOLT_ASSERT_THROW(cauchyCDF(0, -1, 0), "scale must be greater than 0");
}
...
TEST_F(ProbabilityTest, invBetaCDF) {
  ...
  BOLT_ASSERT_THROW(invBetaCDF(kInf, 3, 0.2), "a must be > 0");
  BOLT_ASSERT_THROW(invBetaCDF(kNan, 3, 0.5), "a must be > 0");
  BOLT_ASSERT_THROW(invBetaCDF(0, 3, 0.5), "a must be > 0");

  BOLT_ASSERT_THROW(invBetaCDF(3, kInf, 0.2), "b must be > 0");
  BOLT_ASSERT_THROW(invBetaCDF(3, kNan, 0.5), "b must be > 0");
  BOLT_ASSERT_THROW(invBetaCDF(3, 0, 0.5), "b must be > 0");

  BOLT_ASSERT_THROW(
      invBetaCDF(3, 3.6, kInf), "p must be in the interval [0, 1]");
  BOLT_ASSERT_THROW(
      invBetaCDF(3, 3.6, kNan), "p must be in the interval [0, 1]");
  BOLT_ASSERT_THROW(invBetaCDF(3, 5, -0.1), "p must be in the interval [0, 1]");
  BOLT_ASSERT_THROW(invBetaCDF(3, 5, 1.1), "p must be in the interval [0, 1]");
}
...
TEST_F(ProbabilityTest, fCDF) {
  ...
  EXPECT_NEAR(
      fCDF(0.4422, kDoubleMin, 0.697).value(), 7.9148959162596482e-306, 1e-14);
  ...
  BOLT_ASSERT_THROW(fCDF(0, 3, 0.5), "numerator df must be greater than 0");
  BOLT_ASSERT_THROW(
      fCDF(kBigIntMin, 5.0, 3.7797), "numerator df must be greater than 0");

  // Test invalid inputs for df2.
  BOLT_ASSERT_THROW(fCDF(3, 0, 0.5), "denominator df must be greater than 0");
  BOLT_ASSERT_THROW(
      fCDF(2.0, kBigIntMin, 3.7797), "denominator df must be greater than 0");

  // Test invalid inputs for value.
  BOLT_ASSERT_THROW(fCDF(3, 5, -0.1), "value must non-negative");
  BOLT_ASSERT_THROW(fCDF(2.0, 5.0, kBigIntMin), "value must non-negative");

  // Test a combination of invalid inputs.
  BOLT_ASSERT_THROW(fCDF(-1.2, 0, -0.1), "value must non-negative");
  BOLT_ASSERT_THROW(fCDF(1, -kInf, -0.1), "value must non-negative");
}
...
TEST_F(ProbabilityTest, laplaceCDF) {
  ...
  BOLT_ASSERT_THROW(laplaceCDF(1.0, 0.0, 0.5), "scale must be greater than 0");
  BOLT_ASSERT_THROW(laplaceCDF(1.0, -1.0, 0.5), "scale must be greater than 0");
}
...
TEST_F(ProbabilityTest, gammaCDF) {
  ...
  // invalid inputs
  BOLT_ASSERT_THROW(gammaCDF(-1.0, 3.0, 6.0), "shape must be greater than 0");
  BOLT_ASSERT_THROW(gammaCDF(2.0, -1.0, 6.0), "scale must be greater than 0");
  BOLT_ASSERT_THROW(
      gammaCDF(2.0, 3.0, -1.0), "value must be greater than, or equal to, 0");
  BOLT_ASSERT_THROW(gammaCDF(kNan, 3.0, 6.0), "shape must be greater than 0");
  BOLT_ASSERT_THROW(gammaCDF(2.0, kNan, 6.0), "scale must be greater than 0");
  BOLT_ASSERT_THROW(
...
TEST_F(ProbabilityTest, weibullCDF) {
  ...
  BOLT_ASSERT_THROW(weibullCDF(0, 3, 0.5), "a must be greater than 0");
  BOLT_ASSERT_THROW(weibullCDF(3, 0, 0.5), "b must be greater than 0");
  BOLT_ASSERT_THROW(weibullCDF(kNan, 3.0, 0.5), "a must be greater than 0");
  BOLT_ASSERT_THROW(weibullCDF(3.0, kNan, 0.5), "b must be greater than 0");
  BOLT_ASSERT_THROW(weibullCDF(-1.0, 1.0, 30.0), "a must be greater than 0");
  BOLT_ASSERT_THROW(weibullCDF(1.0, -1.0, 40.0), "b must be greater than 0");
  BOLT_ASSERT_THROW(
