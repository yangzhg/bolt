...
PlanNodeStats& PlanNodeStats::operator+=(const PlanNodeStats& another) {
  ...
  for (int i = 0; i < 4; i++) {
    rawBytesReads[i] += another.rawBytesReads[i];
    cntReads[i] += another.cntReads[i];
    scanTimeReads[i] += another.scanTimeReads[i];
  }
...
void PlanNodeStats::addTotals(const OperatorStats& stats) {
  ...
  for (int i = 0; i < 4; i++) {
    rawBytesReads[i] += stats.rawBytesReads[i];
    cntReads[i] += stats.cntReads[i];
    scanTimeReads[i] += stats.scanTimeReads[i];
  }
  ...
  spillTotalTime += stats.spillTotalTime;
  sortOutputTime += stats.sortOutputTime;
  sortColToRowTime += stats.sortColToRowTime;
  sortInSortTime += stats.sortInSortTime;
  windowAddInputTime += stats.windowAddInputTime;
  windowComputeWindowFunctionTime += stats.windowComputeWindowFunctionTime;
  windowExtractColumnTime += stats.windowExtractColumnTime;
  windowOutputTime += stats.windowOutputTime;
  windowHasNextPartitionTime += stats.windowHasNextPartitionTime;
  windowLoadFromSpillTime += stats.windowLoadFromSpillTime;
  windowSpillTime += stats.windowSpillTime;
  buildPartitionTime += stats.buildPartitionTime;
}
...
folly::dynamic toPlanStatsJson(const bytedance::bolt::exec::TaskStats& stats) {
  ...
  auto planStats = bytedance::bolt::exec::toPlanStats(stats);
  for (const auto& planStat : planStats) {
    for (const auto& operatorStat : planStat.second.operatorStats) {
      ...
      stat["rawInputBytes<4k"] = operatorStat.second->rawBytesReads[0];
      stat["rawInputBytes<32k"] = operatorStat.second->rawBytesReads[1];
      stat["rawInputBytes<128k"] = operatorStat.second->rawBytesReads[2];
      stat["rawInputBytes>=128k"] = operatorStat.second->rawBytesReads[3];
      stat["cntRead<4k"] = operatorStat.second->cntReads[0];
      stat["cntRead<32k"] = operatorStat.second->cntReads[1];
      stat["cntRead<128k"] = operatorStat.second->cntReads[2];
      stat["cntRead>=128k"] = operatorStat.second->cntReads[3];
      stat["totalTimeRead<4k"] = operatorStat.second->scanTimeReads[0];
      stat["totalTimeRead<32k"] = operatorStat.second->scanTimeReads[1];
      stat["totalTimeRead<128k"] = operatorStat.second->scanTimeReads[2];
      stat["totalTimeRead>=128k"] = operatorStat.second->scanTimeReads[3];
      ...
      stat["spillTotalTime"] = operatorStat.second->spillTotalTime;

      stat["sortOutputTime"] = operatorStat.second->sortOutputTime;
      stat["sortColToRowTime"] = operatorStat.second->sortColToRowTime;
      stat["sortInSortTime"] = operatorStat.second->sortInSortTime;
      stat["windowAddInputTime"] = operatorStat.second->windowAddInputTime;
      stat["windowComputeWindowFunctionTime"] =
          operatorStat.second->windowComputeWindowFunctionTime;
      stat["windowExtractColumnTime"] =
          operatorStat.second->windowExtractColumnTime;
      stat["windowOutputTime"] = operatorStat.second->windowOutputTime;
      stat["windowHasNextPartitionTime"] =
          operatorStat.second->windowHasNextPartitionTime;
      stat["windowLoadFromSpillTime"] =
          operatorStat.second->windowLoadFromSpillTime;
      stat["windowSpillTime"] = operatorStat.second->windowSpillTime;
      stat["buildPartitionTime"] = operatorStat.second->buildPartitionTime;
...
std::vector<uint64_t> getAttributeResultVector(
  ...
  std::vector<uint64_t> attributeResultVector;
  auto planStats = bytedance::bolt::exec::toPlanStats(stats);

  for (const auto& planStat : planStats) {
    for (const auto& operatorStat : planStat.second.operatorStats) {
      if (attribute == "peakMemoryBytes") {
        uint64_t peakMemory = operatorStat.second->peakMemoryBytes;
        attributeResultVector.push_back(peakMemory);
      } else if (attribute == "outputBytes") {
        uint64_t outputBytes = operatorStat.second->outputBytes;
        attributeResultVector.push_back(outputBytes);
      }
    }
  }

  return attributeResultVector;
}
