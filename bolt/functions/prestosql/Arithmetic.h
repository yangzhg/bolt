...
struct CeilFunction {
  ...
  FOLLY_ALWAYS_INLINE bool call(int64_t& result, const TInput value) {
    if constexpr (std::is_integral_v<TInput>) {
      result = value;
    } else if constexpr (std::is_same_v<TInput, float>) {
      result = static_cast<int64_t>(std::ceil(value));
    } else {
...
struct LnFunction {
  ...
  FOLLY_ALWAYS_INLINE void call(double& result, int32_t a) {
    result = std::log(a);
  }

  FOLLY_ALWAYS_INLINE void call(double& result, int64_t a) {
    result = std::log(a);
  }
};
...
struct WidthBucketFunction {
  FOLLY_ALWAYS_INLINE void call(
    ...
    // Benchmark Result:
    // https://github.com/facebookexternal/bolt/pull/1225#discussion_r665621405
    BOLT_USER_CHECK_GT(bucketCount, 0, "bucketCount must be greater than 0");
    BOLT_USER_CHECK(!std::isnan(operand), "operand must not be NaN");
    BOLT_USER_CHECK(std::isfinite(bound1), "first bound must be finite");
    BOLT_USER_CHECK(std::isfinite(bound2), "second bound must be finite");
    BOLT_USER_CHECK_NE(bound1, bound2, "bounds cannot equal each other");
...
struct ToBigintFunction {
  ...
  FOLLY_ALWAYS_INLINE void call(int64_t& result, const arg_type<float>& input) {
    result = int64_t(input);
  }
};
...
struct ToIntegerFunction {
  ...
  FOLLY_ALWAYS_INLINE void call(int32_t& result, const arg_type<float>& input) {
    result = int32_t(input);
  }
};
