...
struct SpillStats {
  ...
  /// The time spent on sorting rows for spilling.
  uint64_t spillConvertTimeUs{0};
  /// All the time spent on for spilling.
  uint64_t spillTotalTimeUs{0};
...
struct SpillReadStats {
  ...
  uint64_t spillDecompressTimeUs{0};
...
/// Updates the time spent on convert rows to rowvector to spill.
void updateGlobalSpillConvertTime(uint64_t timeUs);

/// Updates the time spent on spill.
...
template <>
struct fmt::formatter<bytedance::bolt::common::SpillStats>
    : fmt::formatter<std::string> {
  auto format(
      const bytedance::bolt::common::SpillStats& s,
      format_context& ctx) {
