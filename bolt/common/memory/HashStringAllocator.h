...
#include "bolt/common/memory/StreamArena.h"
 ...
 public:
  ...
  struct Position {
    ...
    int32_t offset() const {
      BOLT_DCHECK_NOT_NULL(header);
      BOLT_DCHECK_NOT_NULL(position);
    ...
    static Position atOffset(Header* header, int32_t offset) {
      BOLT_DCHECK_NOT_NULL(header);
      BOLT_DCHECK_GE(offset, 0);
      BOLT_DCHECK_LE(offset, header->usableSize());
  ...
  uint64_t freeSpace() const {
    ...
    BOLT_CHECK_GE(minFree, 0, "Guaranteed free space cannot be negative");
    return minFree;
  }
