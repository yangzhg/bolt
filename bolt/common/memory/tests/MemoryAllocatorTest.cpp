...
#include "bolt/common/memory/MmapArena.h"
...
TEST_P(MemoryAllocatorTest, allocationClass2) {
  ...
  ASSERT_THROW(allocation->setPool(pool_.get()), BoltRuntimeError);
  ASSERT_TRUE(!allocation->empty());
  allocation->clear();
  ASSERT_TRUE(allocation->empty());
  ASSERT_EQ(allocation->numPages(), 0);
  ...
  ASSERT_THROW(allocation->setPool(pool_.get()), BoltRuntimeError);
  ASSERT_DEATH(allocation.reset(), "");
  ASSERT_THROW(allocation->empty(), BoltRuntimeError);
  allocation->clear();
}
...
TEST_P(MemoryAllocatorTest, allocContiguousGrow) {
  ...
  EXPECT_THROW(
      instance_->growContiguous(100000 * kMinGrow, large), BoltException);
  instance_->freeContiguous(large);
  EXPECT_EQ(
...
TEST_P(MemoryAllocatorTest, StlMemoryAllocator) {
  ...
  EXPECT_TRUE(instance_->checkConsistency());
  {
    ...
    EXPECT_THROW(alloc.deallocate(p, 1ULL << 62), BoltException);
    alloc.deallocate(p, 1);
  }
}

TEST_P(MemoryAllocatorTest, AlignedStlMemoryAllocator) {
  {
    std::vector<double, AlignedStlAllocator<double>> data(
        0, AlignedStlAllocator<double>(*pool_));
    // The contiguous size goes to 2MB, covering malloc, size
    // Allocation from classes and ContiguousAllocation outside size
    // classes.
    constexpr int32_t kNumDoubles = 256 * 1024;
  ...
  EXPECT_TRUE(instance_->checkConsistency());
  {
    AlignedStlAllocator<int64_t> alloc(*pool_);
    EXPECT_THROW(alloc.allocate(1ULL << 62), BoltException);
    auto p = alloc.allocate(1);
    EXPECT_THROW(alloc.deallocate(p, 1ULL << 62), BoltException);
    alloc.deallocate(p, 1);
  ...
  {
    AlignedStlAllocator<int64_t, 4> alloc(*pool_);
    auto p = alloc.allocate(1);
    EXPECT_EQ(reinterpret_cast<uint64_t>(p) % 16, 0);
    alloc.deallocate(p, 1);
  }

  {
    AlignedStlAllocator<int64_t, 8> alloc(*pool_);
    auto p = alloc.allocate(1);
    EXPECT_EQ(reinterpret_cast<uint64_t>(p) % 16, 0);
    alloc.deallocate(p, 1);
  }
  ...
  {
    AlignedStlAllocator<int64_t, 16> alloc(*pool_);
    auto p = alloc.allocate(1);
    EXPECT_EQ(reinterpret_cast<uint64_t>(p) % 16, 0);
    alloc.deallocate(p, 1);
  }
  ...
  {
    AlignedStlAllocator<int64_t, 32> alloc(*pool_);
    auto p = alloc.allocate(1);
    EXPECT_EQ(reinterpret_cast<uint64_t>(p) % 32, 0);
    alloc.deallocate(p, 1);
  }

  {
    AlignedStlAllocator<int64_t, 64> alloc(*pool_);
    auto p = alloc.allocate(1);
    EXPECT_EQ(reinterpret_cast<uint64_t>(p) % 64, 0);
    alloc.deallocate(p, 1);
  }
}

TEST_P(MemoryAllocatorTest, AlignedStlMemoryAllocatorLarge) {
  {
    std::vector<__int128_t, AlignedStlAllocator<__int128_t>> data(
        0, AlignedStlAllocator<__int128_t>(*pool_));
    // The contiguous size goes to 2MB, covering malloc, size
    // Allocation from classes and ContiguousAllocation outside size
    // classes.
    ...
    if (useMmap_) {
      EXPECT_EQ(1024, instance_->numAllocated());
    } else {
      EXPECT_EQ(0, instance_->numAllocated());
    }
  }
  ...
  EXPECT_TRUE(instance_->checkConsistency());
  {
    AlignedStlAllocator<__int128_t> alloc(*pool_);
    EXPECT_THROW(alloc.allocate(1ULL << 62), BoltException);
    auto p = alloc.allocate(1);
    EXPECT_THROW(alloc.deallocate(p, 1ULL << 62), BoltException);
    alloc.deallocate(p, 1);
...
TEST_P(MemoryAllocatorTest, AlignedStlAllocatorWithMap) {
  AlignedStlAllocator<std::pair<const int32_t, double>, 32> allocator(*pool_);
  std::map<
      int32_t,
      double,
      std::less<int32_t>,
      AlignedStlAllocator<std::pair<const int32_t, double>, 32>>
      testMap(allocator);

  for (auto i = 0; i < 10'000; i++) {
    testMap.try_emplace(i, i + 0.05);
  }
  for (auto i = 0; i < 10'000; i++) {
    ASSERT_EQ(1, testMap.count(i));
  }

  testMap.clear();
  for (auto i = 0; i < 10'000; i++) {
    ASSERT_EQ(0, testMap.count(i));
  }

  for (auto i = 10'000; i < 20'000; i++) {
    testMap.try_emplace(i, i + 0.15);
  }
  for (auto i = 10'000; i < 20'000; i++) {
    ASSERT_EQ(1, testMap.count(i));
  }
...
TEST_P(MemoryAllocatorTest, AlignedStlAllocator) {
  // basic test
  {
    AlignedStlAllocator<int> alloc(*pool_);
  ...
  // test with alignment
  {
    AlignedStlAllocator<double> allocDouble(*pool_);
    double* ptr = allocDouble.allocate(1);

    EXPECT_EQ(reinterpret_cast<std::uintptr_t>(ptr) % alignof(double), 0);

    allocDouble.deallocate(ptr, 1);

    AlignedStlAllocator<__int128_t> allocInt128(*pool_);
    __int128_t* ptr128 = allocInt128.allocate(1);

    EXPECT_EQ(
        reinterpret_cast<std::uintptr_t>(ptr128) % alignof(__int128_t), 0);

    allocInt128.deallocate(ptr128, 1);
  }
      ...
      try {
        instance_->allocateNonContiguous(
            nonContAllocPages, small, [](int64_t /* bytes */, bool preAlloc) {
              if (preAlloc) {
                BOLT_FAIL("Fake memory reservation failure");
              }
            });
