...
class MapFromEntriesFunction : public exec::VectorFunction {
 ...
 private:
  VectorPtr applyFlat(
      ...
      context.applyToSelectedNoThrow(rows, [&](vector_size_t row) {
        ...
        for (auto i = 0; i < size; ++i) {
          ...
          if (keyVector->isNullAt(keyIndex)) {
            resetSize(row);
            BOLT_USER_FAIL(kNullKeyErrorMessage);
          }
        }
      });
    }
    ...
    // empty.
    if (throwOnNull || decodedRowVector->size() == 0) {
...
void registerMapFromEntriesFunction(const std::string& name, bool throwOnNull) {
  if (throwOnNull) {
    exec::registerVectorFunction(
        name,
        MapFromEntriesFunction<true>::signatures(),
        std::make_unique<MapFromEntriesFunction<true>>());
  } else {
    exec::registerVectorFunction(
        name,
        MapFromEntriesFunction<false>::signatures(),
        std::make_unique<MapFromEntriesFunction<false>>());
  }
}
} // namespace bytedance::bolt::functions
