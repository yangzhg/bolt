...
#include <expression/fuzzer/FuzzerToolkit.h>
#include <exception>
...
void logRowVector(const RowVectorPtr& rowVector) {
  ...
  VLOG(1) << rowVector->childrenSize() << " vector(s) as input:";
...
fuzzer::ResultOrError ExpressionVerifier::verify(
  ...
  if (resultVector && resultVector->encoding() == VectorEncoding::Simple::ROW) {
    ...
    BOLT_CHECK_EQ(plans.size(), 1);
    commonEvalResult.push_back(resultVector);
    simplifiedEvalResult.resize(1);
  }

  // BoltUserError.
  std::exception_ptr userExceptionCommonPtr;
  std::exception_ptr userExceptionSimplifiedPtr;
  // Other errors that are not expected to appear, e.g., BoltRuntimeError.
  std::exception_ptr otherExceptionCommonPtr;
  std::exception_ptr otherExceptionSimplifiedPtr;
  ...
  try {
    ...
    userExceptionCommonPtr = std::current_exception();
  } catch (...) {
    LOG(ERROR)
        << "Common eval: Exceptions other than BoltUserError are not allowed.";
    persistReproInfoIfNeeded(
        rowVector, columnsToWrapInLazy, copiedResult, sql, complexConstants);
    otherExceptionCommonPtr = std::current_exception();
  }
  ...
  try {
    ...
    userExceptionSimplifiedPtr = std::current_exception();
  } catch (...) {
    LOG(ERROR)
        << "Simplified eval: Exceptions other than BoltUserError are not allowed.";
    persistReproInfoIfNeeded(
        rowVector, columnsToWrapInLazy, copiedResult, sql, complexConstants);
    otherExceptionSimplifiedPtr = std::current_exception();
  }

  fuzzer::ResultOrError result;

  try {
    if (otherExceptionCommonPtr || otherExceptionSimplifiedPtr ||
        userExceptionCommonPtr || userExceptionSimplifiedPtr) {
      result.result = nullptr;
      result.resultMatch = true;

      result.haveUnexpectedException =
          otherExceptionCommonPtr || otherExceptionSimplifiedPtr;

      if (userExceptionCommonPtr || userExceptionSimplifiedPtr) {
        result.haveUserException = true;
        result.userExceptionMatch = fuzzer::compareExceptions(
            userExceptionCommonPtr, userExceptionCommonPtr);
        result.userExceptionPtr =
            result.userExceptionMatch ? userExceptionCommonPtr : nullptr;
      } else {
        result.haveUserException = false;
        result.userExceptionMatch = true;
        result.userExceptionPtr = nullptr;
      }
      ...
      result.userExceptionMatch = true;
      result.userExceptionPtr = nullptr;
      result.haveUnexpectedException = false;

      BOLT_CHECK_EQ(commonEvalResult.size(), plans.size());
      BOLT_CHECK_EQ(simplifiedEvalResult.size(), plans.size());
      for (int i = 0; i < plans.size(); ++i) {
        fuzzer::compareVectors(
            commonEvalResult[i],
