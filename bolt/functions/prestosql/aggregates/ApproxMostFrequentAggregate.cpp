...
#include "bolt/functions/lib/ApproxMostFrequentStreamSummary.h"
...
struct ApproxMostFrequentAggregate : exec::Aggregate {
  ...
  void extractValues(char** groups, int32_t numGroups, VectorPtr* result)
    ...
    if (buckets_ == kMissingArgument) {
      ...
      for (int i = 0; i < numGroups; ++i) {
        BOLT_DCHECK_EQ(value<Accumulator<T>>(groups[i])->summary.size(), 0);
        (*result)->setNull(i, true);
      }
      return;
    }
    BOLT_USER_CHECK_LE(buckets_, std::numeric_limits<int>::max());
 ...
 private:
  ...
  setConstantArgument(const char* name, int64_t& val, int64_t newVal) {
    BOLT_USER_CHECK_GT(newVal, 0, "{} must be positive", name);
  ...
  Accumulator<T>* initAccumulator(char* group) {
    ...
    BOLT_USER_CHECK_LE(capacity_, std::numeric_limits<int>::max());
  ...
  void addIntermediate(
    ...
    rows.applyToSelected([&](auto row) {
      ...
      BOLT_DCHECK_EQ(counts->sizeAt(i), size);
