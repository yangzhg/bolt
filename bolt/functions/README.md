...
There are different ways to define new UDF functions in Bolt.

## Scalar Functions

The most straight forward way to add new UDFs to Bolt is to add them as Scalar function and then declare and register
them in Bolt system. For example all arithmetic functions in Bolt are scalar functions. For example here is how the
`round` function is defined using templates (result first followed by arguments) in `bolt/functions/prestosql/Arithmetic.h`.

```c++
template <typename T>
BOLT_UDF_BEGIN(round)
...
`T` is the cpp type equivalent of Bolt type of the input and output arguments in this case.
...
```c++
BOLT_DECLARE_STATEFUL_VECTOR_FUNCTION(udf_in, InPredicate::create);
```
