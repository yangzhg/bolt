...
class EntropyAggregationTest : public AggregationTestBase {
 protected:
  ...
  void testGroupByAgg(
      ...
      const std::string& boltAggKey,
      const std::string& duckDBAggKey) {
    auto partialAgg = fmt::format("entropy({0})", boltAggKey);
  ...
  // The input of Bolt entropy agg function is value count,
  ...
  // first, and then convert the input data of DuckDB to the input data of Bolt
