...
TEST_F(DecimalArithmeticTest, decimalDivTest) {
  ...
  // Divide and round-up may overflow.
  testDecimalExpr<TypeKind::HUGEINT>(
      {makeFlatVector<int128_t>({1401718507, 1375723975}, DECIMAL(21, 10))},
      "divide(c0, c1)",
      {makeFlatVector<int64_t>({1, 1}, DECIMAL(1, 0)),
       makeFlatVector<int128_t>({71341000000, 72689000000}, DECIMAL(28, 10))});

  auto mul = [](int128_t a) { return a * 10000000000; };

  // Divide and round-up may overflow.
  testDecimalExpr<TypeKind::HUGEINT>(
      {makeFlatVector<int128_t>(
          {mul(1401718506) + 8894464614, mul(1375723974) + 7417078237},
          DECIMAL(38, 20))},
      "divide(c0, c1)",
      {makeFlatVector<int128_t>({1, 1}, DECIMAL(28, 0)),
       makeFlatVector<int128_t>(
           {mul(71341000000), mul(72689000000)}, DECIMAL(28, 20))});
