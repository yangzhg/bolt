...
class SpillTest : public ::testing::TestWithParam<common::CompressionKind>,
 ...
 protected:
  ...
  void spillStateTest(
    ...
    // TODO: find out why there is a situation equal to 0
    ...
    ASSERT_EQ(
        finalStats.toString(),
        fmt::format(
            "spillRuns[{}] spilledInputBytes[{}] spilledBytes[{}] spilledRows[{}] spilledPartitions[{}] spilledFiles[{}] spillTotalTimeUs[{}] spillFillTimeUs[{}] spillSortTime[{}] spillConvertTime[{}] spillSerializationTime[{}] spillWrites[{}] spillFlushTime[{}] spillWriteTime[{}] maxSpillExceededLimitCount[0]",
            ...
            succinctMicros(finalStats.spillTotalTimeUs),
            succinctMicros(finalStats.spillFillTimeUs),
            succinctMicros(finalStats.spillSortTimeUs),
            succinctMicros(finalStats.spillConvertTimeUs),
...
TEST_P(SpillTest, DISABLED_spillState) {
...
TEST_P(SpillTest, DISABLED_spillStateWithSmallTargetFileSize) {
...
TEST_P(SpillTest, spillPartitionSet) {
  for (int iter = 0; iter < 3; ++iter) {
    ...
    while (spillPartitions.size() < numPartitions) {
      ...
      ASSERT_EQ(
          ...
          fmt::format(
              "SPILLED PARTITION[ID:{} FILES:0 SIZE:0B ROWCOUNT:0]",
              id.toString()));
  ...
  std::vector<uint64_t> expectedPartitionRowCount(numPartitions, 0);
  for (int iter = 0; iter < numSpillers; ++iter) {
    ...
    for (int i = 0; i < numPartitions; ++i) {
      ...
      for (const auto& fileInfo : spillFiles) {
        ...
        expectedPartitionRowCount[i] += fileInfo.rowCount;
  ...
  for (int i = 0; i < numPartitions; ++i) {
    RowVectorPtr output;
    {
      ...
      ASSERT_EQ(
          ...
          fmt::format(
              "SPILLED PARTITION[ID:[0,{}] FILES:{} SIZE:{} ROWCOUNT:{}]",
              i,
              expectedPartitionFiles[i],
              succinctBytes(expectedPartitionSizes[i]),
              expectedPartitionRowCount[i]));
