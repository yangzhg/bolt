...
using ::bytedance::bolt::torch::TorchNode;
#endif
#if defined BOLT_HAS_PYTHON && BOLT_HAS_PYTHON == 1
#include "bolt/python/PlanNode.h"
using ::bytedance::bolt::python::PythonNode;
#endif
...
RowTypePtr getAggregationOutputType(
  ...
  for (auto& key : groupingKeys) {
    ...
    BOLT_CHECK(field, "Grouping key must be a field reference");
...
void AggregationNode::addDetails(std::stringstream& stream) const {
  ...
  if (ignoreNullKeys_) {
    stream << " ignore null keys";
  }

#if defined BOLT_HAS_CUDF && BOLT_HAS_CUDF == 1
  CudfEnable::addDetails(stream);
#endif
}
...
PlanNodePtr AggregationNode::create(const folly::dynamic& obj, void* context) {
  ...
  auto planNode = std::make_shared<AggregationNode>(
...
#if defined BOLT_HAS_CUDF && BOLT_HAS_CUDF == 1
  planNode->enableCudfIfDesired(obj);
#endif
...
RowTypePtr getExpandOutputType(
    ...
    std::vector<std::string> names) {
  BOLT_USER_CHECK(!names.empty());
  BOLT_USER_CHECK(!projections.empty());
  BOLT_USER_CHECK_GT(names.size(), 0);
  BOLT_USER_CHECK_GT(projections.size(), 0);

  for (int32_t i = 0; i < projections.size(); i++) {
    BOLT_USER_CHECK_EQ(names.size(), projections[i].size());
  }
...
AbstractJoinNode::AbstractJoinNode(
  ...
  BOLT_CHECK(!leftKeys_.empty(), "JoinNode requires at least one join key");
  BOLT_CHECK_EQ(
      leftKeys_.size(),
      rightKeys_.size(),
  ...
  if (core::isLeftSemiProjectJoin(joinType) ||
    ...
    --numOutputColumms;
    BOLT_CHECK_EQ(outputType_->childAt(numOutputColumms), BOOLEAN());

    // Verify that 'match' column name doesn't match any column from left or
    // right source.
    const auto& name = outputType_->nameOf(numOutputColumms);
    BOLT_CHECK(!leftType->containsChild(name));
    BOLT_CHECK(!rightType->containsChild(name));
  }
...
PlanNodePtr IndexLookupJoinNode::create(
  ...
  BOLT_CHECK_NOT_NULL(lookupSource);

  auto leftKeys = deserializeFields(obj["leftKeys"], context);
  auto rightKeys = deserializeFields(obj["rightKeys"], context);

  BOLT_CHECK_EQ(obj.count("filter"), 0);
...
void addWindowFunction(
  ...
  if (frame.startType == WindowNode::BoundType::kUnboundedFollowing) {
    BOLT_USER_FAIL("Window frame start cannot be UNBOUNDED FOLLOWING");
  }
  if (frame.endType == WindowNode::BoundType::kUnboundedPreceding) {
    BOLT_USER_FAIL("Window frame end cannot be UNBOUNDED PRECEDING");
  }
...
WindowNode::WindowNode(
    ...
    : PlanNode(std::move(id)),
      ...
      followedTopNum_(followedTopNum),
...
MarkDistinctNode::MarkDistinctNode(
  ...
  BOLT_USER_CHECK_GT(markerName_.size(), 0)
  BOLT_USER_CHECK_GT(distinctKeys_.size(), 0);
}
...
TopNRowNumberNode::TopNRowNumberNode(
  ...
  BOLT_USER_CHECK_GT(limit, 0, "Limit must be greater than zero");
...
PartitionedOutputNode::Kind PartitionedOutputNode::stringToKind(
  ...
  if (str == "PARTITIONED") {
    ...
    BOLT_FAIL("Unknown output buffer type: {}", str);
  }
}
...
TopNNode::TopNNode(
    ...
    : PlanNode(id),
      ...
      sources_{source} {
  BOLT_USER_CHECK(!sortingKeys.empty(), "TopN must specify sorting keys");
  BOLT_USER_CHECK_EQ(
      sortingKeys.size(),
      sortingOrders.size(),
...
PlanNodePtr OrderByNode::create(const folly::dynamic& obj, void* context) {
  ...
  std::shared_ptr<OrderByNode> planNode = std::make_shared<OrderByNode>(
      deserializePlanNodeId(obj),
      std::move(sortingKeys),
      std::move(sortingOrders),
      obj["partial"].asBool(),
...
folly::dynamic LocalShuffleNode::serialize() const {
  auto obj = PlanNode::serialize();
  obj["seed"] = seed_;
  return obj;
}

void LocalShuffleNode::addDetails(std::stringstream& stream) const {
  stream << "Shuffle seed: " << seed_;
}

// static
PlanNodePtr LocalShuffleNode::create(const folly::dynamic& obj, void* context) {
  auto source = deserializeSingleSource(obj, context);
  auto seed = obj["seed"].asInt();
  return std::make_shared<LocalShuffleNode>(
      deserializePlanNodeId(obj), seed, std::move(source));
}
...
GeneratorNode::GeneratorNode(
    const PlanNodeId& id,
    std::vector<FieldAccessTypedExprPtr>& replicateCols,
    CallTypedExprPtr& generateFunc,
    const std::vector<std::string>& generateNames,
    const std::optional<std::string>& ordinalityName,
    const PlanNodePtr& source,
    const bool isOuter)
    : PlanNode(id),
      replicateCols_{std::move(replicateCols)},
  ...
  setGeneratorType(generateFunc_->name());

  for (const auto& col : replicateCols_) {
  ...
  if (isExplode()) {
    ...
    for (const auto& generatorInput : generateFunc_->inputs()) {
      if (generatorInput->type()->isArray()) {
        names.emplace_back(generateNames[genIndex++]);
        types.emplace_back(generatorInput->type()->asArray().elementType());
      } else if (generatorInput->type()->isMap()) {
        const auto& mapType = generatorInput->type()->asMap();
        ...
        BOLT_FAIL(
            "Unexpected type of generator Input. Expected ARRAY or MAP, but got {}.",
            generatorInput->type()->toString());
      }
    }
  } else if (isJsonTuple()) {
    for (auto i = 1; i < generateFunc_->inputs().size(); ++i) {
      names.emplace_back(generateNames[genIndex++]);
...
void GeneratorNode::addDetails(std::stringstream& stream) const {
  std::vector<FieldAccessTypedExprPtr> generateFieldAccess;
  for (const auto& input : generateFunc_->inputs()) {
    if (const auto& col =
            std::dynamic_pointer_cast<const FieldAccessTypedExpr>(input)) {
      generateFieldAccess.emplace_back(col);
    }
  }

  if (generateFieldAccess.size()) {
    addFields(stream, generateFieldAccess);
  }
}
...
void PlanNode::toString(
  ...
  if (recursive) {
    for (auto& source : sources()) {
      source->toString(
          stream, detailed, true, indentationSize + 2, withId, addContext);
    }
  }
}
...
void PlanNode::registerSerDe() {
...
#if defined BOLT_HAS_TORCH && BOLT_HAS_TORCH == 1
  registry.Register("TorchNode", TorchNode::create);
#endif
#if defined BOLT_HAS_PYTHON && BOLT_HAS_PYTHON == 1
  registry.Register("PythonNode", PythonNode::create);
#endif
  ...
  registry.Register("LocalShuffleNode", LocalShuffleNode::create);
}
