...
TypePtr resolveTypeImpl(
  ...
  BOLT_CHECK_NOT_NULL(Expressions::getResolverHook());
...
TypedExprPtr Expressions::inferTypes(
  ...
  return inferTypes(
      expr, inputRow, {}, pool, complexConstants, customResolverHook);
}
...
TypedExprPtr Expressions::inferTypes(
  ...
  if (auto fun = std::dynamic_pointer_cast<const CallExpr>(expr)) {
    return createWithImplicitCast(fun, std::move(children), customResolverHook);
  }
  ...
  if (auto constant = std::dynamic_pointer_cast<const ConstantExpr>(expr)) {
    if (constant->type()->kind() == TypeKind::ARRAY) {
      ...
      if (constant->value().isNull()) {
        ...
        constantVector = std::make_shared<ConstantVector<bolt::ComplexType>>(
  ...
  BOLT_FAIL("Unknown expression type: {}", expr->toString());
}
...
TypedExprPtr Expressions::resolveLambdaExpr(
  ...
  BOLT_CHECK_LE(names.size(), lambdaInputTypes.size());
...
bool isLambdaSignature(
    std::shared_ptr<const exec::FunctionSignature> signature,
...
const exec::FunctionSignature* findLambdaSignature(
    const std::vector<std::shared_ptr<const exec::FunctionSignature>>
        signatures,
    const std::shared_ptr<const CallExpr>& callExpr) {
  const exec::FunctionSignature* matchingSignature = nullptr;
  for (const auto& signature : signatures) {
...
const exec::FunctionSignature* findLambdaSignature(
  ...
  static FunctionSignatureMap allSignatures;
  static std::once_flag flag;

  // Register simple functions and vector functions once
  std::call_once(flag, [&]() { allSignatures = getFunctionSignatures(); });
...
TypePtr Expressions::getInputRowType(const TypedExprPtr& expr) {
  ...
  for (auto& input : expr->inputs()) {
    ...
    if (childRowType) {
      BOLT_USER_CHECK(!inputRowType || *inputRowType == *childRowType);
