...
struct BitCountFunction {
  ...
  // Unary function call (calculate max bits)
  template <typename TInput>
  FOLLY_ALWAYS_INLINE bool call(int64_t& result, TInput num) {
  ...
  template <typename TInput>
  FOLLY_ALWAYS_INLINE bool call(int32_t& result, TInput num) {
    constexpr int kMaxBits = sizeof(TInput) * CHAR_BIT;
    auto value = static_cast<uint64_t>(num);
    result = bits::countBits(&value, 0, kMaxBits);
    return true;
  ...
  // Binary function call (with specified bits)
  FOLLY_ALWAYS_INLINE bool
  call(int64_t& result, const int64_t num, const int64_t bits) {
    BOLT_USER_CHECK(
    ...
    BOLT_USER_CHECK(
        ...
        num,
        bits);
    result = bits::countBits(reinterpret_cast<const uint64_t*>(&num), 0, bits);
    return true;
  }
};
...
struct BitwiseArithmeticShiftRightFunction {
  ...
  call(int64_t& result, int64_t number, int64_t shift) {
    BOLT_USER_CHECK_GE(shift, 0, "Shift must be non-negative")
    if (shift >= 63) {
      if (number >= 0) {
        result = 0;
      } else {
...
struct BitwiseLogicalShiftRightFunction {
  FOLLY_ALWAYS_INLINE bool
    ...
    BOLT_USER_CHECK(!(bits <= 1 || bits > 64), "Bits must be between 2 and 64");
    BOLT_USER_CHECK_GE(shift, 0, "Shift must be non-negative");
...
struct BitDaysCountFunction {
  ...
  template <typename TInput>
  FOLLY_ALWAYS_INLINE bool call(int32_t& result, const TInput& binaryDays) {
  ...
  call(int32_t& result, const TInput& binaryDays, int32_t lastNDays) {
    ...
    int skipSize = binaryDays.size() - lastNDays;
    int32_t count = 0, skipCount = 0, index = 0;
    if (UNLIKELY(lastNDays < 1 || lastNDays > binaryDays.size())) {
      BOLT_USER_FAIL(
          "lastNDays {} is larger than input size {} or smaller than 1",
          lastNDays,
          binaryDays.size());
    }
    for (auto element : binaryDays) {
      int32_t countOnes = __builtin_popcountll(element.value_or(0)) - 1;
      if (index <= skipSize - 1) {
        skipCount += countOnes;
      }
      index++;
