...
#include <functions/InlineFlatten.h>
...
inline uint64_t hashOne(DecodedVector& decoded, vector_size_t index) {
  if constexpr (
...
template <TypeKind Kind>
FLATTEN void VectorHasher::hashValues(
  ...
  if (decoded_.isConstantMapping()) {
    ...
    if (mix) {
      ...
      rows.applyToSelected(
          [&](vector_size_t row) { hashVec[row] = kNullHash; });
    }
    rows.applyToSelected(
...
void VectorHasher::hash(
  ...
  if (typeKind_ == TypeKind::UNKNOWN) {
    ...
    BOLT_DYNAMIC_TYPE_DISPATCH(hashValues, typeKind_, rows, mix, result.data());
  }
}
...
void VectorHasher::precompute(const BaseVector& value) {
  ...
  precomputedHash_ =
      BOLT_DYNAMIC_TYPE_DISPATCH(hashOne, typeKind_, decoded_, 0);
}
...
void extendRange(
  ...
  switch (kind) {
    ...
    default:
      BOLT_FAIL("Unsupported VectorHasher typeKind {}", kind);
  }
}
...
uint64_t VectorHasher::enableValueRange(
    ...
    int32_t reservePct) {
  multiplier_ = multiplier;
  BOLT_CHECK_LE(0, reservePct);
  BOLT_CHECK(hasRange_);
