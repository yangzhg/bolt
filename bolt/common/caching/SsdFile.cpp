...
#include "bolt/common/caching/SsdFile.h"
...
CoalesceIoStats SsdFile::load(
  ...
  BOLT_CHECK_EQ(ssdPins.size(), pins.size());
  if (pins.empty()) {
...
bool SsdFile::removeFileEntries(
  ...
  BOLT_SSD_CACHE_LOG(INFO) << "Removed " << entriesAgedOut << " entries from "
                           << fileName_ << ". And erased " << toFree.size()
                           << " regions with " << kMaxErasedSizePct
                           << "% entries removed.";
...
void SsdFile::checkpoint(bool force) {
  ...
  BOLT_SSD_CACHE_LOG(INFO) << "Checkpointing shard " << shardId_
                           << ", force: " << force << " bytesAfterCheckpoint: "
                           << succinctBytes(bytesAfterCheckpoint_)
  ...
  try {
    ...
    const auto checkRc = [&](int32_t rc, const std::string& errMsg) {
      if (rc < 0) {
        BOLT_FAIL("{} with rc {} :{}", errMsg, rc, folly::errnoStr(errno));
      }
      return rc;
    };
...
void SsdFile::readCheckpoint(std::ifstream& state) {
  ...
  BOLT_CHECK_EQ(strncmp(magic, kCheckpointMagic, 4), 0);
  ...
  BOLT_CHECK_EQ(logSize, rc, "Failed to read eviction log");
  ...
  // evicted regions.
  BOLT_CHECK_EQ(scores.size(), tracker_.regionScores().size());
  ...
  BOLT_SSD_CACHE_LOG(INFO) << fmt::format(
