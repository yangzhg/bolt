...
#include "bolt/dwio/dwrf/reader/ReaderBase.h"
...
ReaderBase::ReaderBase(
  ...
  BOLT_CHECK_GE(fileLength_, 4, "File size too small");
...
std::shared_ptr<const Type> ReaderBase::convertType(
  ...
  switch (type.kind()) {
    ...
    case TypeKind::ROW: {
      ...
      // struct as the root type. So the assumption that struct has at least
      // one child doesn't hold.
