...
void recordSilentThrows(Operator& op) {
  auto numThrow = threadNumBoltThrow();
  if (numThrow > 0) {
...
DriverCtx::DriverCtx(
    ...
    : driverId(_driverId),
      ...
      isAbtestControlGroup(folly::Random::oneIn(2)) {}
...
std::optional<common::SpillConfig> DriverCtx::makeSpillConfig(
    int32_t operatorId,
    const std::string& rowBasedSpillMode) const {
  ...
  return common::SpillConfig(
             ...
             CHECK_SWITCH_IN_ABTEST(
                 queryConfig.spillUringEnabled(), isAbtestControlGroup),
...
void BlockingState::setResume(std::shared_ptr<BlockingState> state) {
  BOLT_CHECK(!state->driver_->isOnThread());
  ...
  std::move(state->future_)
      ...
      .thenValue([state](auto&& /* unused */) {
        ...
        BOLT_CHECK(!driver->state().suspended());
        BOLT_CHECK(driver->state().hasBlockingFuture);
...
RowVectorPtr Driver::next(std::shared_ptr<BlockingState>& blockingState) {
  ...
  BOLT_CHECK(
      stop == StopReason::kBlock || stop == StopReason::kAtEnd ||
          ...
          stop == StopReason::kTerminate,
      "Driver::next() failed with stop reason {}",
      stop);
...
void logStatsIfNeeded(const OperatorStats& stats, int64_t lastBatchSize) {
  constexpr int64_t kLogFreq = 1000 * 1000;
  if ((stats.outputPositions / kLogFreq) !=
      (stats.outputPositions - lastBatchSize) / kLogFreq) {
    auto operatorName = fmt::format(
        "{}#{}[pipelineId={}]",
        stats.operatorType,
        stats.operatorId,
        stats.pipelineId);
    LOG(INFO) << "Operator " << operatorName << " stats: "
...
StopReason Driver::runInternal(
  ...
  if (stop != StopReason::kNone) {
    if (stop == StopReason::kTerminate) {
      ...
      // close().
      ctx_->task->setError(std::make_exception_ptr(BoltRuntimeError(
          __FILE__,
          __LINE__,
          __FUNCTION__,
          "",
  ...
  CancelGuard guard(task().get(), &state_, [&](StopReason reason) {
    ...
    if (reason == StopReason::kTerminate) {
      task()->setError(std::make_exception_ptr(BoltRuntimeError(
          __FILE__,
          __LINE__,
          __FUNCTION__,
          "",
            ...
            withDeltaCpuWallTimer(op, &OperatorStats::getOutputTiming, [&]() {
              ...
              if (intermediateResult) {
                ...
                resultBytes = intermediateResult->estimateFlatSize();
                {
                  ...
                  logStatsIfNeeded(*lockedStats, intermediateResult->size());
                }
              }
            });
            pushdownFilters(i);
          ...
          // will come back here after this is again on thread.
          {
            ...
            if (result) {
              ...
              }
              {
                ...
                logStatsIfNeeded(*lockedStats, result->size());
              }
...
void Driver::run(std::shared_ptr<Driver> self) {
  ...
  switch (reason) {
    ...
    case StopReason::kAtEnd:
      return;
    default:
      BOLT_FAIL("Unhandled stop reason");
  }
}
...
Operator* Driver::findOperator(int32_t operatorId) const {
  BOLT_CHECK_LT(operatorId, operators_.size());
...
bool SuspendedSection::leave() {
  leaved_ = true;
  return driver_->task()->leaveSuspended(driver_->state()) == StopReason::kNone;
}

SuspendedSection::~SuspendedSection() {
  if (!leaved_ &&
      driver_->task()->leaveSuspended(driver_->state()) != StopReason::kNone) {
    LOG(WARNING)
...
std::string blockingReasonToString(BlockingReason reason) {
  switch (reason) {
    ...
    case BlockingReason::kYield:
      return "kYield";
    case BlockingReason::kWaitForData:
      return "kWaitForData";
