...
#define BOLT_MEM_POOL_CAP_EXCEEDED(errorMessage)                    \
  _BOLT_THROW(                                                      \
      ::bytedance::bolt::BoltRuntimeError,                          \
      ::bytedance::bolt::error_source::kErrorSourceRuntime.c_str(), \
      ::bytedance::bolt::error_code::kMemCapExceeded.c_str(),       \
      /* isRetriable */ true,                                       \
      "{}",                                                         \
      errorMessage);

#define BOLT_MEM_ARBITRATION_FAILED(errorMessage)                    \
  _BOLT_THROW(                                                       \
      ::bytedance::bolt::BoltRuntimeError,                           \
      ::bytedance::bolt::error_source::kErrorSourceRuntime.c_str(),  \
      ::bytedance::bolt::error_code::kMemArbitrationFailure.c_str(), \
      /* isRetriable */ true,                                        \
      "{}",                                                          \
      errorMessage);

#define BOLT_MEM_POOL_ABORTED(errorMessage)                         \
  _BOLT_THROW(                                                      \
      ::bytedance::bolt::BoltRuntimeError,                          \
      ::bytedance::bolt::error_source::kErrorSourceRuntime.c_str(), \
      ::bytedance::bolt::error_code::kMemAborted.c_str(),           \
      /* isRetriable */ true,                                       \
      "{}",                                                         \
      errorMessage);
...
class MemoryArbitrator {
 public:
  ...
  /// function throws a bolt user exception error.
  ///
...
template <>
struct fmt::formatter<bytedance::bolt::memory::MemoryArbitrator::Stats>
    : formatter<std::string> {
  auto format(
      bytedance::bolt::memory::MemoryArbitrator::Stats s,
