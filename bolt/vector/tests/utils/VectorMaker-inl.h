...
BiasVectorPtr<EvalType<T>> VectorMaker::biasVector(
  ...
  if constexpr (admitsBias<TEvalType>()) {
    ...
    BOLT_UNSUPPORTED("Invalid type for biasing");
  }
}
...
void sequenceEncode(
  ...
  BOLT_CHECK_GT(input.size(), 0, "need at least one element to encode.");
