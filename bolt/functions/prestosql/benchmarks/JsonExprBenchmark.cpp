 ...
 * expression framework and Bolt vectors.
 */
...
class JsonBenchmark : public bolt::functions::test::FunctionBenchmarkBase {
 public:
  JsonBenchmark() : FunctionBenchmarkBase() {
    bolt::functions::prestosql::registerJsonFunctions();
  ...
  bolt::VectorPtr makeJsonData(const std::string& json, int vectorSize) {
    auto jsonVector =
        vectorMaker_.flatVector<bolt::StringView>(vectorSize, JSON());
    for (auto i = 0; i < vectorSize; i++) {
      jsonVector->set(i, bolt::StringView(json));
    }
    return jsonVector;
  }
  ...
  void doRun(
      const int iter,
      bolt::exec::ExprSet& exprSet,
      const bolt::RowVectorPtr& rowVector) {
    uint32_t cnt = 0;
