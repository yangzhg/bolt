...
// BoltRuntimeErrors due to unsupported input values such as unicode input to
...
class BoltException : public std::exception {
 public:
  ...
  BoltException(
      ...
      std::string_view exceptionName = "BoltException");
 ...
 private:
  ...
  explicit BoltException(std::shared_ptr<State const> state) noexcept
      : state_(std::move(state)) {}

  const std::shared_ptr<const State> state_;
};

class BoltUserError : public BoltException {
 public:
  BoltUserError(
      const char* file,
      size_t line,
      ...
      std::string_view errorCode,
      bool isRetriable,
      std::string_view exceptionName = "BoltUserError")
      : BoltException(
            file,
            line,
            function,
...
class BoltRuntimeError final : public BoltException {
 public:
  BoltRuntimeError(
      const char* file,
      size_t line,
      ...
      std::string_view errorCode,
      bool isRetriable,
      std::string_view exceptionName = "BoltRuntimeError")
      : BoltException(
            file,
            line,
            function,
...
/// Returns a reference to a thread level counter of Bolt error throws.
...
struct ExceptionContext {
  using MessageFunction =
      std::string (*)(BoltException::Type exceptionType, void* arg);
  ...
  /// `messageFunc` is null.
  std::string message(BoltException::Type exceptionType) {
...
/// If exceptionPtr represents an std::exception, convert it to BoltUserError
