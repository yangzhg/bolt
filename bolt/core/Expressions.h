...
class InputTypedExpr : public ITypedExpr {
 public:
  explicit InputTypedExpr(TypePtr type)
      : ITypedExpr{std::move(type), TypedExprKind::kInput} {}
...
class ConstantTypedExpr : public ITypedExpr {
 public:
  ...
  ConstantTypedExpr(TypePtr type, variant value)
      : ITypedExpr{std::move(type), TypedExprKind::kConstant},
  ...
  explicit ConstantTypedExpr(const VectorPtr& value)
      : ITypedExpr{value->type(), TypedExprKind::kConstant},
        valueVector_{
...
class CallTypedExpr : public ITypedExpr {
 public:
  ...
  CallTypedExpr(
      ...
      std::string name)
      : ITypedExpr(std::move(type), TypedExprKind::kCall, std::move(inputs)),
        name_(std::move(name)) {}
...
class FieldAccessTypedExpr : public ITypedExpr {
 public:
  ...
  FieldAccessTypedExpr(TypePtr type, std::string name)
      : ITypedExpr(std::move(type), TypedExprKind::kFieldAccess),
  ...
  TypedExprPtr rewriteInputNames(
    ...
    BOLT_CHECK_EQ(1, newInputs.size());
...
class DereferenceTypedExpr : public ITypedExpr {
 public:
  DereferenceTypedExpr(TypePtr type, TypedExprPtr input, uint32_t index)
      : ITypedExpr{std::move(type), TypedExprKind::kDereference, {std::move(input)}},
        index_(index) {
...
class ConcatTypedExpr : public ITypedExpr {
 public:
  ConcatTypedExpr(
      ...
      : ITypedExpr{toType(names, inputs), TypedExprKind::kConcat, inputs} {}
  ...
  TypedExprPtr rewriteInputNames(
    ...
    for (const auto& name : signature_->names()) {
      if (mapping.count(name)) {
        BOLT_USER_FAIL("Ambiguous variable: {}", name);
      }
    }
...
class CastTypedExpr : public ITypedExpr {
 public:
  ...
  CastTypedExpr(
      ...
      bool nullOnFailure)
      : ITypedExpr{type, TypedExprKind::kCast, {input}},
  ...
  CastTypedExpr(
      ...
      const std::vector<TypedExprPtr>& inputs,
      bool nullOnFailure)
      : ITypedExpr{type, TypedExprKind::kCast, inputs},
        nullOnFailure_(nullOnFailure) {
    BOLT_USER_CHECK_EQ(
