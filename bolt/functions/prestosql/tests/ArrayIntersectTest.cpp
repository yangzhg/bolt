...
#include "bolt/vector/tests/TestingDictionaryArrayElementsFunction.h"
...
TEST_F(ArrayIntersectTest, deterministic) {
  ...
  // C1 then C0.
  testExpr(expectedC0C1, "array_intersect(C1, C0)", {c0, c1});
  testExpr(expectedC0C1, "array_intersect(ARRAY[1,4,-2], C0)", {c0});

  auto c2 = makeNullableArrayVector<int32_t>({
      {10, 44, -2, 20, 1},
      {10, 44, -2, 20, 1},
  });

  auto expectedC2C1 = makeNullableArrayVector<int32_t>({
      {-2, 1},
      {-2, 1},
  });
  testExpr(expectedC2C1, "array_intersect(C1, C2)", {c0, c1, c2});
  testExpr(expectedC2C1, "array_intersect(C2, C1)", {c0, c1, c2});

  // regardless C2 as a constant on the left or on the right.
  auto expectedC2C1c = makeNullableArrayVector<int32_t>({
      {1, -2},
      {1, -2},
  });
  testExpr(
      expectedC2C1c,
      "array_intersect(C1, ARRAY[10, 44, -2, 20, 1])",
      {c0, c1, c2});
  testExpr(
      expectedC2C1c,
      "array_intersect(ARRAY[10, 44, -2, 20, 1], C1)",
      {c0, c1, c2});
}
