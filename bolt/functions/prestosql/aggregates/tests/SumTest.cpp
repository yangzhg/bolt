...
TEST_F(SumTest, groupByNan) {
  ...
  auto expectedResult = {makeRowVector({
      makeFlatVector<double>(std::vector{std::nan("")}, DOUBLE()),
  })};

  testAggregations(data, {"c0"}, {}, expectedResult, {}, false);
}
...
TEST_F(SumTest, sumDecimal) {
  // Disable incremental aggregation tests because DecimalAggregate doesn't
  // set StringView::prefix when extracting accumulators, leaving the prefix
  // field undefined that fails the test.
...
TEST_F(SumTest, floatAggregateOverflow) {
  testAggregateOverflow<float, double, double>();
