...
void SelectiveColumnReader::seekTo(int64_t offset, bool readsNullsOnly) {
  ...
  if (readOffset_ < offset) {
    ...
    BOLT_FAIL("Seeking backward on a ColumnReader");
  }
}
...
void SelectiveColumnReader::prepareNulls(
  ...
  } else if (returnReaderNulls_ && !hasBulkPath()) {
    returnReaderNulls_ = false;
    LOG(INFO) << "prepareNulls returnReaderNulls_ from true to false";
  }
...
const uint64_t* SelectiveColumnReader::shouldMoveNulls(RowSet rows) {
  ...
  BOLT_CHECK(resultNulls_ && resultNulls_->as<uint64_t>() == rawResultNulls_);
  BOLT_CHECK_GT(resultNulls_->capacity() * 8, rows.size());
  return moveFrom;
}
...
void SelectiveColumnReader::getFlatValues<int8_t, bool>(
  ...
  BOLT_CHECK_EQ(valueSize_, sizeof(int8_t));
...
void SelectiveColumnReader::compactScalarValues<bool, bool>(
  ...
  for (size_t i = 0; i < numValues_; i++) {
    ...
    BOLT_DCHECK_EQ(outputRows_[i], nextRow);
...
bool SelectiveColumnReader::readsNullsOnly() const {
  ...
  if (filter) {
    ...
    return kind == bolt::common::FilterKind::kIsNull ||
...
void SelectiveColumnReader::addSkippedParentNulls(
  ...
  if (parentNullsRecordedTo_) {
    BOLT_CHECK_EQ(parentNullsRecordedTo_, from);
  }
...
void SelectiveColumnReader::doCastEvaluate(VectorPtr* result) {
  ...
  BOLT_CHECK_NOT_NULL(scanSpec_);
  BOLT_CHECK_NOT_NULL(scanSpec_->getExpressionEvaluator());
  scanSpec_->getExpressionEvaluator()->evaluate(
      castExprSet_.get(), dummySelectivity, *rowVector, convertedResult);
