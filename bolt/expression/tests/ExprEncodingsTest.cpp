 ...
 protected:
  ...
  void fillVectorAndReference(
    ...
    for (auto& option : options) {
      ...
      switch (option.encoding) {
        case VectorEncoding::Simple::FLAT: {
          BOLT_CHECK(!current, "A flat vector must be in a leaf position");
        ...
        case VectorEncoding::Simple::DICTIONARY: {
          BOLT_CHECK(current, "Dictionary must be non-leaf");
          BufferPtr indices;
          BufferPtr nulls;
        ...
        case VectorEncoding::Simple::CONSTANT: {
          BOLT_CHECK(current, "Constant must be non-leaf");
  ...
  void runWithError(const std::string& text) {
    ...
    vector_size_t end = size / 3 * 2;
    {
      ...
      SCOPED_TRACE(text);
      SCOPED_TRACE(fmt::format("[{} - {})", begin, end));
      ASSERT_THROW(exprs.eval(rows, context, result), BoltException);
    }
    ...
    end = size;
    {
      ...
      SCOPED_TRACE(fmt::format("[{} - {})", begin, end));
      ASSERT_THROW(
          exprs.eval(0, 1, false, rows, context, result), BoltException);
    }
  }
