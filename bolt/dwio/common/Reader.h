 ...
 */
// Constant for DR
static const std::string DRFileSuffix = "cf.parquet";
static const std::string DRPartitionID = "spark_partition_id";
...
class RowReader {
 public:
  ...
  /**
   * Skip the next portion of rows.
   * @param size Max number of rows to skip
   * @return number of rows skipped in the file (including any rows filtered out
   *  or deleted in mutation), 0 if there are no more rows to skip.
   */
  virtual uint64_t skip(uint64_t skipSize) {
    BOLT_UNSUPPORTED("skip() is not implemented");
  }
...
class Reader {
 public:
   ...
   * @return file schema
   */
  virtual const bolt::RowTypePtr& rowType() const = 0;
