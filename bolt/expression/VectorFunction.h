...
class ApplyNeverCalled final : public VectorFunction {
  void apply(
    ...
    BOLT_UNREACHABLE("Not expected to be called.")
  }
};
...
struct VectorFunctionMetadata {
  ...
  folly::F14FastMap<std::string, std::string> customInfo;
...
/// deregister stateless VectorFunction to avoid memory leak issues.
...
#define _BOLT_REGISTER_FUNC_NAME(tag) registerVectorFunction_##tag

// Declares a vectorized UDF function given a tag. Goes into the UDF .cpp file.
#define BOLT_DECLARE_VECTOR_FUNCTION(tag, signatures, function) \
  void _BOLT_REGISTER_FUNC_NAME(tag)(const std::string& name) { \
    bytedance::bolt::exec::registerVectorFunction(              \
        (name), (signatures), (function));                      \
  }

#define BOLT_DECLARE_VECTOR_FUNCTION_WITH_METADATA(             \
...
#define BOLT_DECLARE_STATEFUL_VECTOR_FUNCTION(tag, signatures, function) \
  void _BOLT_REGISTER_FUNC_NAME(tag)(const std::string& name) {          \
    bytedance::bolt::exec::registerStatefulVectorFunction(               \
        (name), (signatures), (function));                               \
  }

#define BOLT_DECLARE_STATEFUL_VECTOR_FUNCTION_WITH_METADATA(    \
    tag, signatures, metadata, function)                        \
  void _BOLT_REGISTER_FUNC_NAME(tag)(const std::string& name) { \
    bytedance::bolt::exec::registerStatefulVectorFunction(      \
        (name), (signatures), (function), (metadata));          \
...
#define BOLT_REGISTER_VECTOR_FUNCTION(tag, name)                   \
  {                                                                \
    extern void _BOLT_REGISTER_FUNC_NAME(tag)(const std::string&); \
    _BOLT_REGISTER_FUNC_NAME(tag)(name);                           \
  }
