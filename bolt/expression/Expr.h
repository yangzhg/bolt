...
// TODO:  refine this code: inherit from VectorFunction
struct JitBoltVectorFunction {
  using JitedFuncAddress = int64_t;

  void operator()(const std::vector<VectorPtr>& args, VectorPtr* result);

  JitedFuncAddress jitFunc{0};

  //
  std::function<void(const std::vector<VectorPtr>& args, VectorPtr* result)>
      func;
};
...
class Expr {
...
#ifdef ENABLE_BOLT_EXPR_JIT
  // avoid touching Expr's ctor
  virtual void setJitFunction(JitBoltVectorFunction jitFn) {
    jitVectorFunction_ = jitFn;
  }
 ...
 protected:
  ...
  // retained size of dictionary, save in a variable to avoid recomputation
  uint64_t dictionaryCacheSize_ = 0;
...
#ifdef ENABLE_BOLT_EXPR_JIT
  JitBoltVectorFunction jitVectorFunction_;
#endif
};
...
class ExprSet {
 public:
  ...
  uint64_t& dictionaryCacheTotalSize() {
    return dictionaryCacheTotalSize_;
  }

  bool dictionaryCacheSizeExceedLimit() {
    return dictionaryCacheTotalSize_ > kDictionaryCacheMaxSize;
  }
 ...
 protected:
  ...
  // total retained size of dictionary
  uint64_t dictionaryCacheTotalSize_ = 0;

  static constexpr uint64_t kDictionaryCacheMaxSize = 40 << 20;
...
class SimpleExpressionEvaluator : public core::ExpressionEvaluator {
 public:
  ...
  void evaluate(
      ...
      const RowVector& input,
      VectorPtr& result,
      const std::string& currentSplitStr) override;
