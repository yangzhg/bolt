...
#include "bolt/vector/BiasVector.h"
...
int64_t computeChecksum(
  ...
  if (compressed) {
    BOLT_CHECK_LT(compressedSize, uncompressedSize);
  }
...
void readConstantVector(
  ...
  BOLT_CHECK_EQ(1, children[0]->size());
...
void readDictionaryVector(
  ...
  // Skip 3 * 8 bytes of 'instance id'. Bolt doesn't use 'instance id' for
  // dictionary vectors.
  source->skip(24);
...
void scatterVector(
  ...
  if (scatter != nullptr && scatterSize > 0) {
    ...
    // 'offset'.
    BOLT_CHECK_EQ(vector->size(), scatterSize + offset);

    // The new vector size should cover all 'scatter' destinations and
    // trailing nulls.
    BOLT_CHECK_GE(size, scatter[scatterSize - 1] + 1);
  }
  ...
  switch (vector->encoding()) {
    case VectorEncoding::Simple::DICTIONARY: {
      BOLT_CHECK_EQ(0, offset, "Result offset is not supported yet")
    ...
    default:
      BOLT_FAIL("Unsupported encoding in scatter: {}", vector->encoding());
  }
}
...
void scatterStructNulls(
  ...
  for (auto i = 0; i < row.childrenSize(); ++i) {
    BOLT_CHECK_EQ(row.childAt(i)->size(), row.size());
  }
}
...
class VectorStream {
 public:
  ...
  void appendNulls(
      ...
      int32_t end,
      int32_t numNonNull) {
    BOLT_DCHECK_EQ(numNonNull, bits::countBits(nulls, begin, end));
 ...
 private:
  ...
  /// If false, they are serialized with microsecond precision which is
...
void serializeFlatVector<TypeKind::UNKNOWN>(
    ...
    VectorStream* stream,
    Scratch& scratch) {
  BOLT_CHECK_NOT_NULL(vector->rawNulls());
  for (auto i = 0; i < rows.size(); ++i) {
    BOLT_DCHECK(vector->isNullAt(rows[i]));
    stream->appendNull();
  }
}
...
void estimateConstantSerializedSize(
    ...
    vector_size_t** sizes,
    Scratch& scratch) {
  BOLT_CHECK(vector->encoding() == VectorEncoding::Simple::CONSTANT);
  using T = typename KindToFlatVector<Kind>::WrapperType;
...
void estimateFlatSerializedSize(
  ...
  if (vector->mayHaveNulls()) {
    ...
    BOLT_UNREACHABLE("Non null fixed width case handled before this");
  }
}
...
void flushCompressed(
  ...
  // compress
  if (uncompressedSize <= codec.maxUncompressedLength()) {
    std::unique_ptr<folly::IOBuf> compressed;
    try {
      compressed = codec.compress(iobuf.get());
    ...
    const int32_t& compressedSize = compressed->computeChainDataLength();
...
void PrestoVectorSerde::deserialize(
  ...
  BOLT_CHECK_EQ(
      ...
      0,
      "bytedance presto don't support crc");
#endif
  ...
  if (isChecksumBitSet(pageCodecMarker)) {
    int64_t actualCheckSum = computeChecksum(
  ...
  // when buffer is not compressed, no need codec, return true
  // when buffer is compressed, codec must exist, return codec
  BOLT_CHECK(
      (isCompressedBitSet(pageCodecMarker) ? needCompression(*codec) : true),
      "No codec marker when serialize buffer is compressed.")
  ...
  if (!isCompressedBitSet(pageCodecMarker)) {
    ...
    try {
      uncompress = codec->uncompress(compressBuf.get(), uncompressedSize);
    } catch (const std::exception& e) {
      BOLT_FAIL(
          "got exception in decompression, uncompressedSize: {}, compressedSize: {}, pageCodecMarker: {}, prestoOptions.compressionKind: {}, detail: {}",
          uncompressedSize,
          compressedSize,
          pageCodecMarker,
          prestoOptions.compressionKind,
