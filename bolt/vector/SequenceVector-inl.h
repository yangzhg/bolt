...
SequenceVector<T>::SequenceVector(
  ...
  BOLT_CHECK_GE(length, 0, "Must provide length >= 0");
  BOLT_CHECK(sequenceLengths_ != nullptr, "Sequence Lengths must be not null");
  BOLT_CHECK_EQ(
...
vector_size_t SequenceVector<T>::offsetOfIndex(vector_size_t index) const {
  BOLT_DCHECK_LE(0, index);
  BOLT_DCHECK_LT(index, BaseVector::length_);
