...
#include "bolt/vector/tests/TestingAlwaysThrowsFunction.h"
...
class ExprTest : public testing::Test, public VectorTestBase {
 protected:
  ...
  std::string extractFromErrorContext(
    ...
    BOLT_CHECK(startPos != std::string::npos);
    startPos += strlen(key);
    auto endPos = context.find(".", startPos);
    BOLT_CHECK(endPos != std::string::npos, context);
  ...
  VectorPtr restoreVector(const std::string& path) {
    ...
    BOLT_CHECK(!inputFile.fail(), "Cannot open file: {}", path);
    auto copy = bytedance::bolt::restoreVector(inputFile, pool());
    inputFile.close();
    return copy;
  }

  void verifyDataAndSqlPaths(const BoltException& e, const VectorPtr& data) {
  ...
  ParameterizedExprTest() {
    std::unordered_map<std::string, std::string> configData(
        {{core::QueryConfig::kEnableExpressionEvaluationCache,
          GetParam() ? "true" : "false"}});
    queryCtx_ = bolt::core::QueryCtx::create(
...
class PlusRandomIntegerFunction : public exec::VectorFunction {
 public:
  ...
  void apply(
      ...
      VectorPtr& result) const override {
    BOLT_CHECK_EQ(args.size(), 1);
    BOLT_CHECK_EQ(args[0]->typeKind(), bytedance::bolt::TypeKind::INTEGER);
...
TEST_P(ParameterizedExprTest, lazyLoading) {
  ...
  VectorPtr lazyVector = std::make_shared<LazyVector>(
      ...
      std::make_unique<test::SimpleVectorLoader>([&](RowSet /*rows*/) {
        BOLT_FAIL("This lazy vector is not expected to be loaded");
        return nullptr;
      }));
...
class StatefulVectorFunction : public exec::VectorFunction {
 public:
  ...
  void apply(
      ...
      exec::EvalCtx& context,
      VectorPtr& result) const override {
    BOLT_CHECK_EQ(args.size(), numInputs_);
    ...
    if (!result) {
      ...
      BaseVector::ensureWritable(rows, INTEGER(), context.pool(), result);
      result->copy(numInputs.get(), rows, nullptr, false);
    }
  }
...
class TestingSingleArgDeterministicFunction : public exec::VectorFunction {
 public:
  void apply(
    ...
    result->copy(arg.get(), rows, nullptr, false);
  }
...
TEST_P(ParameterizedExprTest, peelArgs) {
  ...
  BOLT_REGISTER_VECTOR_FUNCTION(udf_testing_constant, "testing_constant");
  BOLT_REGISTER_VECTOR_FUNCTION(udf_testing_dictionary, "testing_dictionary");
...
TEST_P(ParameterizedExprTest, complexNullOutput) {
  BOLT_REGISTER_VECTOR_FUNCTION(udf_null_array, "null_array");
...
TEST_P(ParameterizedExprTest, exceptionContext) {
  ...
  FLAGS_bolt_save_input_on_expression_any_failure_path = "";
  FLAGS_bolt_save_input_on_expression_system_failure_path = "";
  ...
  FLAGS_bolt_save_input_on_expression_any_failure_path = tempDirectory->path;
  FLAGS_bolt_save_input_on_expression_system_failure_path = "";
...
TEST_P(ParameterizedExprTest, constantToSql) {
  ...
  // ASSERT_EQ(
  //     toSql(123'456, INTERVAL_YEAR_MONTH()),
  //     "'123456'::INTERVAL YEAR TO MONTH");
  // ASSERT_EQ(
  //     toSql(variant::null(TypeKind::INTEGER), INTERVAL_YEAR_MONTH()),
...
TEST_P(ParameterizedExprTest, dictionaryResizedInAddNulls) {
  ...
  // are rows.
  // The expression below make sure that we call a resize on a dictionary
...
TEST_F(ExprTest, castExceptionContext) {
  // return null instead of throw exception in bytedance
}
...
TEST_P(ParameterizedExprTest, flatNoNullsFastPath) {
  ...
  ASSERT_EQ(1, exprSet->exprs().size());
  // Due to the fact that the mod function has changed its return value from
  // void to bool, but the function that returns the bool value does not support
  // FlatNoNullsFastPath by default, therefore, it needs to be changed to
  // ASSERT_FALSE, instead of ASSERT_TRUE
  ASSERT_FALSE(exprSet->exprs()[0]->supportsFlatNoNullsFastPath())
      << exprSet->toString();
...
TEST_P(ParameterizedExprTest, applyFunctionNoResult) {
  ...
  assertError(
      ...
      TestingAlwaysThrowsVectorFunction::kBoltErrorMessage);
...
TEST_P(ParameterizedExprTest, maskErrorByNull) {
  ...
  BOLT_ASSERT_THROW(evaluate("(c0 / c1) + 10", data), "division by zero");
  BOLT_ASSERT_THROW(
...
TEST_P(ParameterizedExprTest, cseOverDictionaryAcrossMultipleExpressions) {
  ...
  // correctly, that is, make sure peeling is done before attempting to reuse
  ...
  // 1} where row 0 will be computed and 1 will be reused so row 1 would have
  // wrong result.
  {
...
TEST_P(ParameterizedExprTest, coalesceRowInputTypesAreTheSame) {
...

  {
    ...
    try {
      exec::CoalesceExpr<false> coalesceExpr(
    ...
    auto input = makeRowVector({
        ...
        makeLazy(
            ...
            [](auto& rows) {
              ...
              BOLT_CHECK_EQ(rows.size(), (kSize + 1) / 2);
              for (auto i : rows) {
                BOLT_CHECK(i % 2 == 0);
              }
            }),
