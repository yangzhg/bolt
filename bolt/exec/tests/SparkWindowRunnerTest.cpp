...
DEFINE_string(
    window_fuzzer_repro_path,
    "",
    "Directory path where all plans generated by WindowFuzzer are "
...
static std::string checkAndReturnFilePath(
  ...
  auto path = fmt::format("{}/{}", FLAGS_window_fuzzer_repro_path, fileName);
  if (fs::exists(path)) {
...
static void checkDirForExpectedFiles() {
  LOG(INFO) << "Searching input directory for expected files at "
            << FLAGS_window_fuzzer_repro_path;
...
int main(int argc, char** argv) {
  ...
  if (!FLAGS_window_fuzzer_repro_path.empty()) {
