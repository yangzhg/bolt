...
class VectorTestBase {
 protected:
  ...
  static std::shared_ptr<const RowType> makeRowType(
    ...
    return bolt::test::VectorMaker::rowType(
  ...
  static std::function<bool(vector_size_t /*row*/)> nullEvery(
      int n,
      int startingFrom = 0) {
    return bolt::test::VectorMaker::nullEvery(n, startingFrom);
  }
  ...
  template <typename T>
  FlatVectorPtr<T> makeAllNullFlatVectorWithNullValues(vector_size_t size) {
    return vectorMaker_.allNullFlatVectorWithNullValues<T>(size);
  }
  ...
  BufferPtr makeIndicesInReverse(vector_size_t size) {
    return ::bytedance::bolt::test::makeIndicesInReverse(size, pool());
  }
  ...
  static std::shared_ptr<T> flatten(const VectorPtr& vector) {
    return bolt::test::VectorMaker::flatten<T>(vector);
  }
  ...
  // Convenience function to create a vector of type Map(K, MAP(V, U)).
  // Supports null keys, and values and even null elements.
  // Example:
  //    createMapOfMapsVector<int64_t, int64_t, int64_t>(
  //      {{{1, {{{2, 3}, {4, 5}}}}},
  //       {{6, {{{7, 8}}}}}});
  template <typename K, typename V, typename U>
  VectorPtr createMapOfMapVector(
      std::vector<std::map<
          std::optional<K>,
          std::optional<std::vector<std::pair<V, std::optional<U>>>>>> maps) {
    std::vector<std::optional<K>> keys;
    std::vector<std::optional<std::vector<std::pair<V, std::optional<U>>>>>
        values;
    for (auto& map : maps) {
      for (const auto& [key, value] : map) {
        keys.push_back(key);
    ...
    auto mapValues = makeNullableMapVector<V, U>(values);
    auto mapKeys = makeNullableFlatVector<K>(keys);
    auto size = maps.size();
    ...
    return std::make_shared<MapVector>(
        pool_.get(),
        MAP(CppToType<K>::create(),
            MAP(CppToType<V>::create(), CppToType<U>::create())),
        nullptr,
        size,
        offsets,
        sizes,
  ...
  // Convenience function to create a vector of type Map(K, ROW(V, U)).
  // Supports null keys, and values and even null elements.
  // Example:
  //    createMapOfRowsVector<int64_t, int64_t, int64_t>(
  //      {{{1, {{2, 3}}}, {3, {{4, 5}}}},
  //      {{6, {{7, 8}}}}});
  template <typename K, typename V, typename U>
  VectorPtr createMapOfRowVector(
      std::vector<std::map<
          std::optional<K>,
          std::optional<std::pair<std::optional<V>, std::optional<U>>>>> maps) {
    std::vector<std::optional<K>> keys;
    std::vector<std::optional<V>> firstValues;
    std::vector<std::optional<U>> secondValues;
    for (auto& map : maps) {
      for (const auto& [key, value] : map) {
        keys.push_back(key);
        if (!value.has_value()) {
          firstValues.push_back(std::nullopt);
          secondValues.push_back(std::nullopt);
        } else {
          firstValues.push_back(value->first);
          secondValues.push_back(value->second);
        }
      }
    }

    auto firstVector = makeNullableFlatVector<V>(firstValues);
    auto secondVector = makeNullableFlatVector<U>(secondValues);
    auto mapValues = makeRowVector({firstVector, secondVector});
    auto mapKeys = makeNullableFlatVector<K>(keys);
    auto size = maps.size();
    ...
    return std::make_shared<MapVector>(
        pool_.get(),
        MAP(CppToType<K>::create(),
            ROW({CppToType<V>::create(), CppToType<U>::create()})),
        nullptr,
        size,
        offsets,
        sizes,
  ...
  std::shared_ptr<LazyVector> makeLazyFlatVector(
    ...
    return std::make_shared<LazyVector>(
        ...
        std::make_unique<SimpleVectorLoader>(
            [expectedSize, expectedRowAt, size, valueAt, isNullAt, this](
                RowSet rows) {
              if (expectedSize.has_value()) {
                BOLT_CHECK_EQ(rows.size(), *expectedSize);
              }
              if (expectedRowAt.has_value()) {
                for (auto i = 0; i < rows.size(); i++) {
                  BOLT_CHECK_EQ(rows[i], (*expectedRowAt)(i));
                }
              }
  ...
  VectorPtr wrapInLazyDictionary(VectorPtr vector) {
    return std::make_shared<LazyVector>(
        ...
        vector->type(),
        vector->size(),
        std::make_unique<SimpleVectorLoader>([vector, this](RowSet /*rows*/) {
          auto indices =
