...
template <typename Key, typename Options, typename Value, typename Generator>
class CachedFactory {
 public:
  ...
  std::pair<bool, Value> generate(const Key& key, const Options& options);
...
template <typename Key, typename Options, typename Value, typename Generator>
std::pair<bool, Value> CachedFactory<Key, Options, Value, Generator>::generate(
    const Key& key,
    const Options& options) {
  process::TraceContext trace("CachedFactory::generate");
  if (!cache_) {
    return std::make_pair(false, (*generator_)(key, options));
  }
  ...
  if (pending_.contains(key)) {
    ...
    pending_lock.unlock();
    return generate(key, options); // Regenerate in the edge case.
  } else {
    pending_.insert(key);
    pending_lock.unlock();
    ...
    try {
      generatedValue = (*generator_)(key, options);
...
template <typename Key, typename Options, typename Value, typename Generator>
void CachedFactory<Key, Options, Value, Generator>::retrieveCached(
    const std::vector<Key>& keys,
    std::vector<std::pair<Key, Value>>* cached,
    std::vector<Key>* missing) {
  if (cache_) {
