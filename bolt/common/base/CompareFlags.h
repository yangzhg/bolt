...
struct CompareFlags {
  ...
  enum class NullHandlingMode {
    ...
    */
    ...
    /*
     compare null with null returns null, otherwise behaves like kNullAsValue
    */
    kNullAsSpeciallValue
  };

  NullHandlingMode nullHandlingMode = NullHandlingMode::kNullAsValue;

  // When true, comparison should return left.size - right.size when sizes
  // mismatch.
  bool compareSizeFirst = false;
