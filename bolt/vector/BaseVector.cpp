...
BaseVector::BaseVector(
  ...
  BOLT_CHECK_NOT_NULL(type_, "Vector creation requires a non-null type.");

  if (nulls_) {
    int32_t bytes = byteSize<bool>(length_);
    BOLT_CHECK_GE(nulls_->capacity(), bytes);
...
void BaseVector::addNulls(const uint64_t* bits, const SelectivityVector& rows) {
  ...
  BOLT_CHECK(isNullsWritable());
  BOLT_CHECK_GE(length_, rows.end());
  ensureNulls();
...
void BaseVector::addNulls(const SelectivityVector& nullRows) {
  ...
  BOLT_CHECK(isNullsWritable());
  BOLT_CHECK_GE(length_, nullRows.end());
  ensureNulls();
  auto target = nulls_->asMutable<uint64_t>();
...
void BaseVector::setNulls(const BufferPtr& nulls) {
  if (nulls) {
    BOLT_DCHECK_GE(nulls->size(), bits::nbytes(length_));
    nulls_ = nulls;
...
std::string BaseVector::toPrettyString() const {
  return printVector(*this);
}
std::string BaseVector::toPrettyString(vector_size_t from, vector_size_t size)
    const {
  return printVector(*this, from, size);
}
std::string BaseVector::toPrettyString(const SelectivityVector& rows) const {
  return printVector(*this, rows);
}
...
VectorPtr BaseVector::createConstant(
    ...
    vector_size_t size,
    bolt::memory::MemoryPool* pool) {
  BOLT_CHECK_EQ(type->kind(), value.kind());
  return BOLT_DYNAMIC_SCALAR_TYPE_DISPATCH_ALL(
...
void BaseVector::validate(const VectorValidateOptions& options) const {
  if (nulls_ != nullptr) {
    auto bytes = byteSize<bool>(size());
    BOLT_CHECK_GE(nulls_->size(), bytes);
  }
  if (options.callback) {
...
std::optional<vector_size_t> BaseVector::findDuplicateValue(
  ...
  BOLT_DCHECK_GE(start, 0, "Start index must not be negative");
  BOLT_DCHECK_LT(start, length_, "Start index is too large");
  BOLT_DCHECK_GT(size, 0, "Size must not be negative");
  BOLT_DCHECK_LE(start + size, length_, "Size is too large");
...
std::string printNulls(const BufferPtr& nulls, vector_size_t maxBitsToPrint) {
  BOLT_CHECK_GE(maxBitsToPrint, 0);
...
std::string printIndices(
  ...
  BOLT_CHECK_GE(maxIndicesToPrint, 0);
