...
DEBUG_ONLY_TEST_F(TableScanTest, pendingCoalescedIoWhenTaskFailed) {
  ...
  bytedance::bolt::VectorFuzzer::Options opts;
  opts.vectorSize = 1024;
  bytedance::bolt::VectorFuzzer fuzzer(opts, pool_.get());
  ...
  SCOPED_TESTVALUE_SET(
      "bytedance::bolt::connector::hive::HiveDataSource::next",
      std::function<void(connector::hive::HiveDataSource*)>(
          [&](connector::hive::HiveDataSource* /*unused*/) {
            BOLT_FAIL(errMsg);
          }));
  SCOPED_TESTVALUE_SET(
      "bytedance::bolt::cache::DirectCoalescedLoad::loadData",
...
DEBUG_ONLY_TEST_F(TableScanTest, timeLimitInGetOutput) {
  ...
  SCOPED_TESTVALUE_SET(
      "bytedance::bolt::exec::TableScan::getOutput",
  ...
  size_t numBailed{0};
  SCOPED_TESTVALUE_SET(
      "bytedance::bolt::exec::TableScan::getOutput::bail",
      std::function<void(const TableScan*)>(
...
TEST_F(TableScanTest, subfieldPruningRemainingFilterMapNullChecks) {
  auto mapVector = makeMapVector<int64_t, int64_t>(
      10,
      [](auto) { return 3; },
      [](auto i) { return i % 3; },
  ...
  enum { kNoOutput = 0, kWholeColumn = 1, kSubfieldOnly = 2 };
  for (const auto& remainingFilter : filters) {
    for (int outputColumn = kNoOutput; outputColumn <= kSubfieldOnly;
         ++outputColumn) {
      SCOPED_TRACE(fmt::format(
          "Filter: '{}', Output: {}", remainingFilter, outputColumn));
      std::unordered_map<std::string, std::shared_ptr<connector::ColumnHandle>>
          assignments;
      assignments["a"] = std::make_shared<HiveColumnHandle>(
          "a", HiveColumnHandle::ColumnType::kRegular, BIGINT(), BIGINT());
      ...
      // Calculate expected results based on the filter
      // The map structure: each map has 3 elements with keys [0,1,2] and values
      // [0,1,2] b[0] accesses key 0 (always exists), b[-1] accesses key -1
      // (never exists) The map itself (b) is never null
      std::vector<vector_size_t> expectedRowIndices;
      for (int i = 0; i < 10; ++i) {
        bool shouldInclude = false;
        if (remainingFilter == "b[0] is null") {
          shouldInclude = false; // b[0] always exists (key 0 is always present)
        } else if (remainingFilter == "b[0] is not null") {
          shouldInclude = true; // b[0] always exists
        } else if (remainingFilter == "b[-1] is null") {
          shouldInclude = true; // b[-1] is null (key -1 doesn't exist)
        } else if (remainingFilter == "b[-1] is not null") {
          shouldInclude = false; // b[-1] is always null (key -1 doesn't exist)
        } else if (remainingFilter == "b is null") {
          shouldInclude = false; // map is never null
        } else if (remainingFilter == "b is not null") {
          shouldInclude = true; // map is never null
        }
      ...
      auto rows = result->as<RowVector>();
      ASSERT_TRUE(rows);
      ASSERT_EQ(rows->size(), expectedRowIndices.size());

      // Build expected result vector with only the rows that should pass the
      // filter
      if (expectedRowIndices.size() > 0) {
        auto expectedA = makeFlatVector<int64_t>(
            expectedRowIndices.size(),
            [&](auto idx) { return expectedRowIndices[idx]; });

        RowVectorPtr expected;
        if (outputColumn == kNoOutput) {
          expected = makeRowVector({"a"}, {expectedA});
        } else {
          // Create filtered map vector
          auto expectedMapVector = makeMapVector<int64_t, int64_t>(
              expectedRowIndices.size(),
              [](auto) { return 3; },
              [](auto i) { return i % 3; },
              [](auto i) { return i % 3; });
          ...
          if (outputColumn == kSubfieldOnly) {
            // For subfield-only case, we need to match what the table scan
            // actually returns Check the actual result to determine the
            // structure
            auto actualRows = result->as<RowVector>();
            if (actualRows && actualRows->size() > 0) {
              auto actualMapVector = actualRows->childAt(1)->as<MapVector>();
              if (actualMapVector && actualMapVector->sizeAt(0) == 1) {
                // Table scan returned only the requested subfield
                auto subfieldMapVector = makeMapVector<int64_t, int64_t>(
                    expectedRowIndices.size(),
                    [](auto) { return 1; }, // Only 1 element per map
                    [](auto i) { return 1; }, // Key is always 1
                    [](auto i) { return 1; }); // Value is always 1
                expected =
                    makeRowVector({"a", "b"}, {expectedA, subfieldMapVector});
              } else {
                // Table scan returned full map with size constraint
                auto sizes = allocateIndices(expectedRowIndices.size(), pool());
                auto* rawSizes = sizes->asMutable<vector_size_t>();
                std::fill(rawSizes, rawSizes + expectedRowIndices.size(), 2);
                auto b = std::make_shared<MapVector>(
                    pool(),
                    mapType,
                    nullptr,
                    expectedRowIndices.size(),
                    expectedMapVector->offsets(),
                    sizes,
                    expectedMapVector->mapKeys(),
                    expectedMapVector->mapValues());
                expected = makeRowVector({"a", "b"}, {expectedA, b});
              }
            } else {
              expected =
                  makeRowVector({"a", "b"}, {expectedA, expectedMapVector});
            }
          } else {
            expected =
                makeRowVector({"a", "b"}, {expectedA, expectedMapVector});
        ...
        for (int i = 0; i < static_cast<int>(expectedRowIndices.size()); ++i) {
          ASSERT_TRUE(rows->equalValueAt(expected.get(), i, i))
              << "Row " << i << ": " << rows->toString(i) << " vs "
              << expected->toString(i);
        }
...
TEST_F(TableScanTest, batchSize) {
  ...
  auto numRows = totalInputSize / rowSize; // 2048 rows
  ...
  // test limit the max output batch rows
  {
    auto task =
        AssertQueryBuilder(duckDbQueryRunner_)
            .plan(plan)
            ...
            .config(
                ...
                folly::to<std::string>(totalInputSize * 5))
            .config(
                QueryConfig::kMaxOutputBatchRows, folly::to<std::string>("128"))
            .assertResults("SELECT * FROM tmp");
    const auto opStats = task->taskStats().pipelineStats[0].operatorStats[0];
...
TEST_F(TableScanTest, multipleSplits) {
  ...
  for (const auto& numPrefetchSplit : numPrefetchSplits) {
    ...
    if (numPrefetchSplit != 0) {
      ASSERT_GT(stats.at("preloadSplits").sum, 10);
    } else {
      ASSERT_EQ(stats.count("preloadSplits"), 0);
    }
  }
}
...
TEST_F(TableScanTest, preloadingSplitClose) {
  ...
  ASSERT_GT(stats.at("preloadSplits").sum, 1);
...
TEST_F(TableScanTest, validFileNoData) {
  ...
  auto filePath =
      bytedance::bolt::test::getDataFilePath("data/emptyPresto.dwrf");
  auto split = HiveConnectorSplitBuilder(filePath)
                   .start(0)
...
TEST_F(TableScanTest, addSplitsToFailedTask) {
  ...
  EXPECT_THROW(while (cursor->moveNext()){}, BoltUserError);
...
TEST_F(TableScanTest, dictionaryMemo) {
  ...
  SCOPED_TESTVALUE_SET(
      "bytedance::bolt::exec::Expr::peelEncodings::mayCache",
...
TEST_F(TableScanTest, paimonDeletionVector) {
  ...
  auto plan = PlanBuilder()
                  ...
                  .project({"c1.c0"})
                  .planNode();
  auto deletionFilePath =
      bytedance::bolt::test::getDataFilePath("data/deletionFile");
  auto split1 = HiveConnectorSplitBuilder(file->path).build();
  // delete 3, 5, 9 rows
  split1->customSplitInfo.insert(
      {{connector::hive::KPaimonDeletionFilePath, deletionFilePath},
       {connector::hive::KPaimonDeletionBinOffset, "61"},
       {connector::hive::KPaimonDeletionBinSize, "26"}});
  auto split2 = HiveConnectorSplitBuilder(file->path).build();
  // delete 1, 5, 6, 9, 13, 14 rows
  split2->customSplitInfo.insert(
      {{connector::hive::KPaimonDeletionFilePath, deletionFilePath},
       {connector::hive::KPaimonDeletionBinOffset, "21"},
       {connector::hive::KPaimonDeletionBinSize, "32"}});
  std::vector<int32_t> vec{0, 1, 2, 4, 6, 0, 2, 3, 4};
  auto expected = makeRowVector(
      {makeFlatVector<int32_t>(vec.size(), [&vec](auto i) { return vec[i]; })});
  AssertQueryBuilder(plan).splits({split1, split2}).assertResults(expected);
}

TEST_F(TableScanTest, remainingFilterLazyVector) {
  auto rowType =
      ROW({"device_id",
           "new_active_country",
           "idfa",
           ...
           "new_active_date",
           "app_id"},
          {BIGINT(), VARCHAR(), VARCHAR(), VARCHAR(), VARCHAR(), BIGINT()});
  std::unordered_map<std::string, std::shared_ptr<connector::ColumnHandle>>
      assignments;
  auto filePath =
      bytedance::bolt::test::getDataFilePath("data/filternulls.orc");
  assignments["device_id"] = std::make_shared<HiveColumnHandle>(
      "device_id", HiveColumnHandle::ColumnType::kRegular, BIGINT(), BIGINT());
  assignments["new_active_country"] = std::make_shared<HiveColumnHandle>(
      "new_active_country",
      HiveColumnHandle::ColumnType::kRegular,
      VARCHAR(),
      VARCHAR());
  auto op =
      PlanBuilder(pool_.get())
          .startTableScan()
          .outputType(
              ROW({{"device_id", BIGINT()}, {"new_active_country", VARCHAR()}}))
          .remainingFilter(
              "coalesce(idfa, app_google_aid) NOT IN ('00000000-0000-0000-0000-000000000000', 'Unknown', '') AND NOT(spark_equalnullsafe(new_active_date <= '2023-07-17' and new_active_country IN ('at','be','bg','ch','cy','cz','de','dk','ee','es','fi','fr','gb','gf','gp','gr','hr','hu','ie','is','it','li','lt','lu','lv','mf','mq','mt','nl','no','pl','pt','re','ro','se','si','sk','yt'),TRUE))")
          .dataColumns(rowType)
          .assignments(assignments)
          .endTableScan()
          .planNode();
  auto split = makeHiveConnectorSplit(filePath, 0, 9589);
  auto result = AssertQueryBuilder(op).split(split).copyResults(pool());
  auto rows = result->as<RowVector>();
  ASSERT_TRUE(rows);
  ASSERT_EQ(rows->childrenSize(), 2);
  auto country = rows->childAt(1)->asFlatVector<StringView>();
  ASSERT_TRUE(country);
  int expectedNulls = 386;
  int nullCount = 0;
  for (int i = 0; i < country->size(); ++i) {
    nullCount += country->isNullAt(i);
  }
  ASSERT_EQ(expectedNulls, nullCount);
}
...
TEST_F(TableScanTest, hugeStripe) {
  ...
  auto cursor = TaskCursor::create(params);
  auto path = bytedance::bolt::test::getDataFilePath("data/many-nulls.dwrf");
...
TEST_F(TableScanTest, orcDecimalFilterChangePrecision) {
  auto outputRowType = ROW({"id", "amount"}, {BIGINT(), DECIMAL(5, 2)});
  auto inputRowType = ROW({"id", "amount"}, {BIGINT(), DECIMAL(5, 2)});

  auto filePath = bytedance::bolt::test::getDataFilePath(
      "data/testDecimalFilterChangePrecision.orc");
  auto op = PlanBuilder(pool_.get())
                .startTableScan()
                .outputType(outputRowType)
                .remainingFilter("amount=('5.00'::decimal(5,2))")
                .dataColumns(inputRowType)
                .endTableScan()
                .planNode();
  auto split =
      makeHiveConnectorSplits(filePath, 1, dwio::common::FileFormat::ORC)[0];
  auto result = AssertQueryBuilder(op).split(split).copyResults(pool());
  auto rows = result->as<RowVector>();
...
TEST_F(TableScanTest, orcDecimalFilter) {
  auto rowType = ROW({"id", "amount"}, {BIGINT(), DECIMAL(5, 2)});
  auto data_vec = makeVectors(1, 1, rowType);

  auto filePath =
      bytedance::bolt::test::getDataFilePath("data/testDecimalFilter.orc");
  auto op = PlanBuilder(pool_.get())
                .startTableScan()
                .outputType(rowType)
                .remainingFilter("amount=('6.67'::decimal(5,2))")
                .dataColumns(rowType)
                .endTableScan()
                .planNode();
  auto split =
