...
TEST_F(TopNRowNumberTest, planNodeValidation) {
  ...
  //   BOLT_ASSERT_THROW(
  //       plan({"a", "b"}, {}), "Number of sorting keys must be greater than
  //       zero");
...
TEST_F(TopNRowNumberTest, withoutSortingKeys) {
  auto limit = 10;
  core::PlanNodeId topNRowNumberId;
  auto data = split(
      makeRowVector(
          {"a", "b", "c", "d", "e"},
          {
              makeFlatVector<int64_t>(limit, [](auto row) { return row; }),
              makeFlatVector<int64_t>(limit, [](auto row) { return row; }),
              makeFlatVector<int64_t>(limit, [](auto row) { return row; }),
              makeFlatVector<int64_t>(limit, [](auto row) { return row; }),
              makeFlatVector<int64_t>(limit, [](auto row) { return row; }),
  ...
  auto plan = PlanBuilder()
                  .values(data)
                  .topNRowNumber({"a", "b"}, {}, limit, true)
                  .capturePlanNodeId(topNRowNumberId)
                  .planNode();

  createDuckDbTable(data);
  auto sql = fmt::format(
      "SELECT * FROM (SELECT *, row_number() over (partition by a,b) as rn FROM tmp) "
      " WHERE rn <= {}",
      limit);
  assertQuery(plan, sql);
}
