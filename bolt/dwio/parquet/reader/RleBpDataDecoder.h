...
// encoded.
class RleBpDataDecoder : public bytedance::bolt::parquet::RleBpDecoder {
 public:
  using super = bytedance::bolt::parquet::RleBpDecoder;
 ...
 private:
    ...
    for (;;) {
      if (remainingValues_) {
        ...
        if (!numInRun) {
          ...
          BOLT_CHECK(!numAdvanced, "Would advance past end of RLEv1 run");
        } else if (repeating_) {
  ...
  static inline uint64_t safeLoadBits(
    ...
    BOLT_DCHECK_GE(7, bitOffset);
    BOLT_DCHECK_GE(56, bitWidth);
    if (ptr < lastSafeWord) {
      return *reinterpret_cast<const uint64_t*>(ptr) >> bitOffset;
    }
    const int32_t byteWidth = bits::roundUp(bitOffset + bitWidth, 8) / 8;
