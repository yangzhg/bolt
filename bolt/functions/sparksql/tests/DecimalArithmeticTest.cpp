...
class DecimalArithmeticTest : public SparkFunctionBaseTest {
 ...
 protected:
  ...
  const std::string kThirtyFive9s = std::string(35, '9');
  const std::string kThirtySix9s = std::string(36, '9');
  const std::string kThirtyEight9s = std::string(38, '9');
}; // namespace
...
TEST_F(DecimalArithmeticTest, multiply) {
  ...
  // out_precision == 38, very large values, trimming of scale (requires
  // convert to 256). should cause overflow.
  testDecimalExpr<TypeKind::HUGEINT>(
  ...
  auto ratio = makeNullableFlatVector<int128_t>(
      {std::nullopt, HugeInt::parse("10000000000000000000000000")},
      DECIMAL(38, 25));
  auto cost = makeNullableFlatVector<int128_t>(
      {HugeInt::parse("533600000000000000000000000000"),
       HugeInt::parse("533600000000000000000000000000")},
      DECIMAL(38, 25));
  testDecimalExpr<TypeKind::HUGEINT>(
      makeNullableFlatVector<int128_t>(
          {std::nullopt, 5336000000000000L}, DECIMAL(38, 11)),
      "c0 * c1",
      {ratio, cost});
}
...
TEST_F(DecimalArithmeticTest, decimalDivTest) {
  ...
  //     = spark.sql("select a / -6.0 from decimals_test") df.printSchema()
...
TEST_F(DecimalArithmeticTest, denyPrecisionLoss) {
  ...
  testArithmeticFunction(
      ...
      {makeConstant<int128_t>(201, 3, DECIMAL(20, 5)),
       makeConstant<int128_t>(301, 3, DECIMAL(20, 5))},
      makeConstant<int128_t>(60501, 3, DECIMAL(38, 10)));
