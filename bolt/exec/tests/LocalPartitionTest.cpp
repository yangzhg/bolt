...
TEST_F(LocalPartitionTest, blockingOnLocalExchangeQueue) {
  ...
  struct {
    ...
    std::string debugString() const {
      ...
      {smallInput,
       0,
       false}, // Small input will not make LocalPartition blocked.
      {largeInput, 1, false}, // Large input will make LocalPartition blocked.
      // morsel-driven tests removed here because morsel-driven
      // will not be triggered here for "localExchange->singleAgg" case.
  };
  ...
  for (const auto& test : testSettings) {
    ...
    auto task = AssertQueryBuilder(duckDbQueryRunner_)
                    ...
                    // Turn on morsel-driven for this task.
                    .config(
                        core::QueryConfig::kEnableMorselDriven,
                        test.isMorselDrivenEnabled)
...
TEST_F(LocalPartitionTest, producerError) {
  ...
  ASSERT_THROW(
      while (cursor->moveNext()) { ; }, BoltException);
