...
#include "bolt/dwio/common/ErrorTolerance.h"
#include "bolt/dwio/common/FlatMapHelper.h"
...
class RowReaderOptions {
 private:
  ...
  std::shared_ptr<bolt::common::ScanSpec> scanSpec_ = nullptr;
  std::shared_ptr<bolt::common::MetadataFilter> metadataFilter_;
  ...
  std::function<void(
      bytedance::bolt::dwio::common::flatmap::FlatMapKeySelectionStats)>
  ...
  /// Dictionary filtering allows skipping row groups by checking their
  /// dictionaries
 ...
 public:
  ...
  const std::shared_ptr<bolt::common::ScanSpec>& getScanSpec() const {
    return scanSpec_;
  }

  void setScanSpec(std::shared_ptr<bolt::common::ScanSpec> scanSpec) {
    scanSpec_ = std::move(scanSpec);
  }

  const std::shared_ptr<bolt::common::MetadataFilter>& getMetadataFilter()
      const {
    return metadataFilter_;
  }

  void setMetadataFilter(
      std::shared_ptr<bolt::common::MetadataFilter> metadataFilter) {
  ...
  bool getAppendParquetRowNumberAndFileName() const {
    return appendParquetRowNumberAndFileName_;
  }
  ...
  const std::function<
      void(bytedance::bolt::dwio::common::flatmap::FlatMapKeySelectionStats)>
  ...
  /// Sets whether dictionary filtering should be enabled.
  /// @param enableDictionaryFilter true to enable, false to disable
  void setEnableDictionaryFilter(bool enableDictionaryFilter) {
    enableDictionaryFilter_ = enableDictionaryFilter;
  }

  /// Returns whether dictionary filtering is enabled.
  ...
  int32_t getDecodeRepDefPageCount() const {
  ...
  int32_t getParquetRepDefMemoryLimit() const {
  ...
  bool getUseColumnNamesForColumnMapping() const {
  ...
  std::string toString() const {
    ...
    ss << "preloadStripe=" << std::boolalpha << preloadStripe << ", ";
    ss << "projectSelectedType=" << projectSelectedType << ", ";
    ss << "returnFlatVector_=" << returnFlatVector_ << ", ";
    ss << "errorTolerance_=" << &errorTolerance_
       << ", "; // Assuming object address is sufficient
    ss << "selector_=" << (selector_ ? "set" : "null") << ", ";
    ss << "scanSpec_=" << (scanSpec_ ? "set" : "null") << ", ";
    ss << "metadataFilter_=" << (metadataFilter_ ? "set" : "null") << ", ";

    ss << "flatmapNodeIdAsStruct_={";
    for (const auto& [k, v] : flatmapNodeIdAsStruct_) {
      ss << k << ":[";
      for (const auto& s : v)
        ss << s << ",";
      if (!v.empty())
    ...
    if (!flatmapNodeIdAsStruct_.empty())
      ss.seekp(-1, std::ios_base::end);
    ss << "}, ";
    ...
    ss << "keySelectionCallback_=" << (keySelectionCallback_ ? "set" : "null")
       << ", ";
    ss << "blockedOnIoCallback_=" << (blockedOnIoCallback_ ? "set" : "null")
       << ", ";
    ss << "decodingTimeMsCallback_="
       << (decodingTimeMsCallback_ ? "set" : "null") << ", ";
    ss << "eagerFirstStripeLoad=" << eagerFirstStripeLoad << ", ";
    ss << "skipRows_=" << skipRows_ << ", ";
    ss << "timestampPrecision_=" << static_cast<int>(timestampPrecision_)
       << ", ";
...
class ReaderOptions : public io::ReaderOptions {
 ...
 public:
   ...
   * Get the file schema.
   */
  const std::shared_ptr<const bolt::RowType>& getFileSchema() const {
    return fileSchema;
  }
  ...
  bool useNestedColumnNamesForColumnMapping() const {
    if (useNestedColumnNamesForColumnMapping_) {
      BOLT_CHECK(!useColumnNamesForColumnMapping_);
    }
