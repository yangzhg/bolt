...
class HashProbe : public Operator {
 ...
 private:
  ...
  // output.
  bool joinIncludesMissesFromLeft(core::JoinType joinType) {
    return ((isLeftJoin(joinType) || isFullJoin(joinType)) && includingMiss_) ||
    ...
    finishIteration(TOnMiss onMiss, bool endOfData, size_t freeOutputRows) {
      if (endOfData) {
        if (!currentRowPassed && currentRow != -1) {
          ...
          BOLT_CHECK_GE(freeOutputRows, 0);
          onMiss(currentRow);
          freeOutputRows--;
        }
  ...
  void setupSpillRestorForRangePartition(
      const std::optional<SpillPartitionId>& restoredSpillPartitionId);

  void prepareMatchFlag(vector_size_t size, RowVectorPtr& matchFlag);

  void updateAndSpillProbeMatchFlags(
      RowVectorPtr& matchFlag,
      vector_size_t size,
      bool doSpill);

  void mergeAndSpillProbeMatchFlags();

  bool reuseSpillReader_{false};
  bool probeRangePartition_{false};
  bool includingMiss_{true};

  std::unique_ptr<Spiller> matchFlagSpiller_;
  std::unique_ptr<UnorderedStreamReader<BatchStream>> spillMatchFlagReader_;
