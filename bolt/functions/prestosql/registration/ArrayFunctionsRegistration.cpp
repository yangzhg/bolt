...
void registerInternalArrayFunctions() {
  ...
  BOLT_REGISTER_VECTOR_FUNCTION(udf_$internal$contains, "$internal$contains");
}

void registerArrayFunctions(const std::string& prefix) {
  registerArrayConstructor(prefix + "array_constructor");
  BOLT_REGISTER_VECTOR_FUNCTION(udf_all_match, prefix + "all_match");
  BOLT_REGISTER_VECTOR_FUNCTION(udf_any_match, prefix + "any_match");
  BOLT_REGISTER_VECTOR_FUNCTION(udf_none_match, prefix + "none_match");

  BOLT_REGISTER_VECTOR_FUNCTION(udf_find_first, prefix + "find_first");
  BOLT_REGISTER_VECTOR_FUNCTION(
      udf_find_first_index, prefix + "find_first_index");

  BOLT_REGISTER_VECTOR_FUNCTION(udf_array_distinct, prefix + "array_distinct");
  BOLT_REGISTER_VECTOR_FUNCTION(
      udf_array_duplicates, prefix + "array_duplicates");
  BOLT_REGISTER_VECTOR_FUNCTION(
      udf_array_intersect, prefix + "array_intersect");
  BOLT_REGISTER_VECTOR_FUNCTION(udf_array_contains, prefix + "contains");
  ...
  exec::registerExpressionRewrite([prefix](const auto& expr) {
    return rewriteArrayContains2In(prefix, expr);
  });

  BOLT_REGISTER_VECTOR_FUNCTION(udf_array_sum, prefix + "array_sum");
  exec::registerStatefulVectorFunction(
      prefix + "repeat", repeatSignatures(), makeRepeat);
  BOLT_REGISTER_VECTOR_FUNCTION(udf_sequence, prefix + "sequence");
  ...
  exec::registerStatefulVectorFunction(
      prefix + "sort_array",
      sparksql::sortArraySignatures(),
      sparksql::makeSortArray);
}
}; // namespace bytedance::bolt::functions
