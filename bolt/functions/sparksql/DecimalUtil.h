 ...
 public:
  ...
  divideWithRoundUp(R& r, A a, B b, uint8_t aRescale, bool& overflow) {
    ...
    if (bitsRequiredAfterScaling < sizeof(R) * 8) {
      ...
      overflow = __builtin_mul_overflow(
          ...
          R(bolt::DecimalUtil::getPowersOfTen(aRescale)),
          &unsignedDividendRescaled);
      ...
      int256_t aLargeScaledUp =
          aLarge * bolt::DecimalUtil::getPowersOfTen(aRescale);
      int256_t bLarge = b;
  ...
  inline static InputType isDivisiable10(const InputType& input) {
    if constexpr (std::is_same_v<InputType, int128_t>) {
      ...
      // value % 10 = (upper * 2^64 + lower) % 10
      //            = (upper * (2^64/10 * 10 + 2^64%10) + lower) % 10
      //            = (upper * (2^64%10) + lower) % 10
      //            = (upper * 6 + lower) % 10
      //            = ((upper % 10) * 6 + lower % 10) % 10
      uint64_t lower = value, upper = (uint64_t)(value >> 64);
      if (upper == 0) {
        return (value % 10) == 0;
      }
      return ((6 * (upper % 10) + lower % 10) % 10) == 0;
    } else {
      return (input % 10) == 0;
    }
  }
  ...
  template <typename InputType>
  inline static InputType div10(const InputType& input) {
    if constexpr (std::is_same_v<InputType, int128_t>) {
      int128_t value = input;
      bool negative = (input < 0);
      if (negative) {
        if (value == INT128_MIN) {
          return INT128_MIN / 10;
        } else {
          value = -value;
        }
      }
      ...
      // value = upper * 2^64 + lower
      // let upper = a * 10 + b, lower = c * 10 + d, which b < 10 and d < 10
      // value / 10 = ((a * 10 + b) * 2^64 + (c * 10 + d)) / 10
      //            = a * 2^64 + c + (b * 2^64 + d) / 10
      //            = a * 2^64 + c + (b * (2^64/10 * 10 + 2^64 % 10) + d) / 10
      //            = a * 2^64 + c + b * 2^64/10 + (b * 6 + d) / 10

      // 2^64 / 10
      uint64_t twoPower64Div10 = 1844674407370955161ULL;
      uint64_t newLower0 = (upper % 10) * twoPower64Div10;
      // this add is never overflow
      uint64_t newLower1 = lower / 10 + (6 * (upper % 10) + lower % 10) / 10;
      uint64_t newUpper = upper / 10;
      int128_t result = (((int128_t)newUpper) << 64) + (int128_t)newLower0 +
          (int128_t)newLower1;
  ...
  inline static void stripTrailingZeros(InputType& input, uint8_t& scale) {
    while (std::abs<int64_t>(input) >= 10L && scale > 0) {
      ...
      if (!isDivisiable10(input)) { // to be optimized
        break;
      }
      input = div10(input);
      // todo : checkScale
