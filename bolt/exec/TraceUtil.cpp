...
std::string findLastPathNode(const std::string& path) {
  ...
  BOLT_CHECK(!pathNodes.empty(), "No valid path nodes found from {}", path);
...
void createTraceDirectory(const std::string& traceDir) {
  try {
    std::unordered_map<std::string, std::string> emptyConfigStrs;
    auto emptyConfig =
        std::make_shared<config::ConfigBase>(std::move(emptyConfigStrs));
    const auto fs = filesystems::getFileSystem(traceDir, emptyConfig);
