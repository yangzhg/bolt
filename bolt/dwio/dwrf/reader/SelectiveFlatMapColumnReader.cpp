...
#include "bolt/dwio/dwrf/reader/SelectiveFlatMapColumnReader.h"

#include "bolt/dwio/common/FlatMapHelper.h"
#include "bolt/dwio/dwrf/reader/SelectiveDwrfReader.h"
#include "bolt/dwio/dwrf/reader/SelectiveStructColumnReader.h"
...
std::vector<KeyNode<T>> getKeyNodes(
  ...
  auto streams = stripe.visitStreamsOfNode(
      dataValueType->id(), [&](const StreamInformation& stream) {
        ...
        BOLT_CHECK(inMap, "In map stream is required");
...
class SelectiveFlatMapReader : public SelectiveStructColumnReaderBase {
 ...
 private:
  ...
  void fillValues(
    ...
    BOLT_CHECK_EQ(fill, totalChildValues);
  ...
  static std::vector<BufferPtr> vectorStrings(BaseVector& vector) {
    if (vector.encoding() == VectorEncoding::Simple::CONSTANT) {
      ...
      BOLT_FAIL("String value is is neither flat, dictionary  nor constant");
    }
  }
  ...
  bool fastPath(
    ...
    for (auto i = 0; i < children_.size(); ++i) {
      ...
      if (child->encoding() == VectorEncoding::Simple::DICTIONARY) {
        ...
        BOLT_FAIL("Flat map columns must be flat or single level dictionaries");
      }
  ...
  switch (kind) {
    ...
    case TypeKind::VARCHAR:
      return createReader<StringView>(
          columnReaderOptions, requestedType, fileType, params, scanSpec);
    default:
      BOLT_UNSUPPORTED("Not supported key type: {}", kind);
  }
}
