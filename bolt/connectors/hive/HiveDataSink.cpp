...
HiveSortingColumn::HiveSortingColumn(
  ...
  BOLT_USER_CHECK(!sortColumn_.empty(), "hive sort column must be set");

  if (FOLLY_UNLIKELY(
          (sortOrder_.isAscending() && !sortOrder_.isNullsFirst()) ||
          (!sortOrder_.isAscending() && sortOrder_.isNullsFirst()))) {
    BOLT_USER_FAIL("Bad hive sort order: {}", toString());
  }
}
...
void HiveBucketProperty::validate() const {
  BOLT_USER_CHECK_GT(bucketCount_, 0, "Hive bucket count can't be zero");
  BOLT_USER_CHECK(!bucketedBy_.empty(), "Hive bucket columns must be set");
  BOLT_USER_CHECK_EQ(
      bucketedBy_.size(),
      bucketTypes_.size(),
...
std::string HiveDataSink::stateString(State state) {
  switch (state) {
    ...
    case State::kAborted:
      return "ABORTED";
    default:
      BOLT_UNREACHABLE("BAD STATE: {}", static_cast<int>(state));
  }
}

void HiveDataSink::computePartitionAndBucketIds(const RowVectorPtr& input) {
  BOLT_CHECK(isPartitioned() || isBucketed());
  if (isPartitioned()) {
...
void HiveDataSink::checkStateTransition(State oldState, State newState) {
  ...
  BOLT_FAIL("Unexpected state transition from {} to {}", oldState, newState);
}
...
uint32_t HiveDataSink::appendWriter(const HiveWriterId& id) {
  ...
  BOLT_CHECK_EQ(writers_.size(), writerInfo_.size());
  BOLT_CHECK_EQ(writerIndexMap_.size(), writerInfo_.size());
...
HiveWriterId HiveDataSink::getWriterId(size_t row) const {
  ...
  if (isPartitioned()) {
    BOLT_CHECK_LT(partitionIds_[row], std::numeric_limits<uint32_t>::max());
...
void HiveDataSink::splitInputRowsAndEnsureWriters() {
  ...
  if (isBucketed() && isPartitioned()) {
    BOLT_CHECK_EQ(bucketIds_.size(), partitionIds_.size());
  }
  ...
  for (auto row = 0; row < numRows; ++row) {
    ...
    BOLT_DCHECK_LT(index, partitionSizes_.size());
    BOLT_DCHECK_EQ(partitionSizes_.size(), partitionRows_.size());
    BOLT_DCHECK_EQ(partitionRows_.size(), rawPartitionRows_.size());
  ...
  for (uint32_t i = 0; i < partitionSizes_.size(); ++i) {
    if (partitionSizes_[i] != 0) {
      BOLT_CHECK_NOT_NULL(partitionRows_[i]);
...
std::pair<std::string, std::string> HiveDataSink::getWriterFileNames(
  ...
  BOLT_CHECK(!targetFileName.empty());
  const std::string writeFileName = isCommitRequired()
      ? fmt::format(".tmp.bolt.{}_{}", targetFileName, makeUuid())
      : targetFileName;
  if (generateFileName &&
  ...
  if (insertTableHandle_->isExistingTable()) {
    if (insertTableHandle_->isPartitioned()) {
      ...
      if (hiveConfig_->immutablePartitions()) {
        BOLT_USER_FAIL("Unpartitioned Hive tables are immutable.");
      }
...
bool HiveDataSink::WriterReclaimer::reclaimableBytes(
  ...
  BOLT_CHECK_EQ(pool.name(), writerInfo_->writerPool->name());
  reclaimableBytes = 0;
