...
class MultiFragmentTest : public HiveConnectorTestBase {
 protected:
  ...
  static exec::Consumer noopConsumer() {
    return [](RowVectorPtr, ContinueFuture*, unsigned int) {
...
TEST_F(MultiFragmentTest, morselDrivenEarlyCompletion) {
  ...
  //  "Exchange->LocalPartition" and "LocalExchange->HashJoin(probe)"
  //  2. Number of drivers in task2 and task3 with morsel-driven pipelines
  //  enabled differ from maxDrivers of 4.
  //  3. Verify all tasks finish successfully without hanging.
  for (auto enableMorselDriven : {"false", "true"}) {
    configSettings_[core::QueryConfig::kEnableMorselDriven] =
        enableMorselDriven;
    ...
    for (int i = 0; i < 2; ++i) {
      ...
      auto joinPlan =
          makeJoinOverExchangePlan(asRowType(plan->outputType()), buildData);
      ...
      // When morselDrivenEnabled is enabled, the number of pipeline is 1 more
      // than when it is disabled in this task, to ensure
      // "Exchange->HashJoin(probe)" is split into two pipelines,
      // "Exchange->LocalPartition" and "LocalExchange->HashJoin(probe)".
      if (strcmp(enableMorselDriven, "true") == 0) {
        ASSERT_EQ(task->numPipelines(), 3);
      } else {
        ASSERT_EQ(task->numPipelines(), 2);
      }
    ...
    if (strcmp(enableMorselDriven, "true") == 0) {
      // When morselDrivenEnabled is enabled, the number of pipeline is 1 more
      // than when it is disabled in this task. Hence, there are 4 more finished
      // drivers.
      ASSERT_EQ(tasks[1]->numFinishedDrivers(), 9);
      ASSERT_EQ(tasks[2]->numFinishedDrivers(), 9);
    } else {
      ASSERT_EQ(tasks[1]->numFinishedDrivers(), 5);
      ASSERT_EQ(tasks[2]->numFinishedDrivers(), 5);
    }
  }
...
TEST_F(MultiFragmentTest, morselDrivenEarlyCompletion2) {
  ...
  //  "Exchange->LocalPartition" and
  //  "LocalExchange->HashJoin(probe)->HashJoin(probe)"
  //  2. Number of drivers in task2 and task3 with morsel-driven pipelines
  //  enabled differ from maxDrivers of 4.
  //  3. Verify all tasks finish successfully without hanging.
  for (auto enableMorselDriven : {"false", "true"}) {
    ...
    for (int i = 0; i < 2; ++i) {
      ...
      RowVectorPtr buildData2;
      if (i == 0) {
        buildData2 = makeRowVector(ROW({"u_c0"}, {BIGINT()}), 0);
      } else {
        buildData2 = makeRowVector(
            {"u_c0"}, {makeFlatVector<int64_t>({1, 2, 3, 4, 5, 6})});
      }

      auto joinPlan = makeSequentialJoinsOverExchangePlan(
          asRowType(data->type()), buildData, buildData2);
    ...
    if (strcmp(enableMorselDriven, "true") == 0) {
      ...
      // than when it is disabled in this task. Hence, there are 4 more finished
      // drivers.
      ASSERT_EQ(tasks[1]->numFinishedDrivers(), 10);
      ASSERT_EQ(tasks[2]->numFinishedDrivers(), 10);
    } else {
      ASSERT_EQ(tasks[1]->numFinishedDrivers(), 6);
      ASSERT_EQ(tasks[2]->numFinishedDrivers(), 6);
    }
  }
...
DEBUG_ONLY_TEST_F(
  ...
  SCOPED_TESTVALUE_SET(
      "bytedance::bolt::exec::Task::setError",
  ...
  SCOPED_TESTVALUE_SET(
      "bytedance::bolt::exec::Task::terminate",
      std::function<void(Task*)>(([&](Task* task) {
        if (task->taskId() != kRootTaskId) {
          return;
        }
  ...
  auto rootTask = makeTask(
      ...
      [](RowVectorPtr /*unused*/,
         ContinueFuture* /*unused*/,
         int partitionId /*unused*/) -> BlockingReason {
        return BlockingReason::kNotBlocked;
      },
      kRootMemoryLimit);
  rootTask->start(1);
...
TEST_F(
  ...
  // taskTerminateWithProblematicRemainingRemoteSplits to test MorselDriven
  // execution and verify:
  //  1. In case plan is
  //    -- HashJoin
  //        -- Values (Probe)
  //        -- Exchange (Build)
  //    , our localPlanner does not make incorrect plan translation (inserting
  //    localPartition to the build side) that might cause execution error
  //    later.
  for (auto enableMorselDriven : {"false", "true"}) {
    configSettings_[core::QueryConfig::kEnableMorselDriven] =
        enableMorselDriven;
...
DEBUG_ONLY_TEST_F(MultiFragmentTest, mergeWithEarlyTermination) {
  ...
  SCOPED_TESTVALUE_SET(
      "bytedance::bolt::exec::Merge::isBlocked",
...
class DataFetcher {
 public:
  ...
  ContinueFuture fetch() {
    auto [p, f] = makeBoltContinuePromiseContract("DataFetcher");
    promise_ = std::move(p);
...
DEBUG_ONLY_TEST_F(MultiFragmentTest, exchangeStatsOnFailure) {
  ...
  SCOPED_TESTVALUE_SET(
      ...
      std::function<void(void* data)>([&](void* data) {
        ...
        if (numPages > 10) {
          BOLT_FAIL("Forced failure after {} pages", numPages);
        }
      }));
  ...
  ASSERT_TRUE(test::waitForTaskFailure(task.get(), 30'000'000));
