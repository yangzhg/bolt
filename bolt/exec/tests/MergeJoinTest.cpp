...
class MergeJoinTest : public HiveConnectorTestBase {
 protected:
  ...
  class MySimpleVectorLoader : public VectorLoader {
   public:
    ...
    void loadInternal(
      ...
      BOLT_CHECK_GE(batchId_, *count_, "Lazy vectors loaded out of order.");
      BOLT_CHECK(!loaded_, "Trying to load a lazy vector twice.");
      *result = loader_(rows);
      loaded_ = true;
    }
  ...
  void testJoin1(
    ...
    // Use very very small output batch size(one by one).
    assertQuery(
        makeCursorParameters(plan, 1),
        "SELECT t.c0, t.c1, u.c1 FROM t FULL JOIN u ON t.c0 = u.c0");
  ...
  auto right = makeRowVector(
      {"c", "d"},
      {
          makeNullableFlatVector<std::string>(
              {"homepage_hot_draw",
               "homepage_hot_draw",
               "homepage_hot_live_cell_click",
               "homepage_hot_live_cell_click",
               "homepage_hot_video_head_click"}),
          makeNullableFlatVector<int32_t>(
              {std::nullopt, 0, std::nullopt, 0, 8}),
      });
  ...
  std::vector<VectorPtr> rightKeys = {
      ...
      makeNullableFlatVector<int32_t>(
          {std::nullopt,
           ...
           std::nullopt}),
      makeNullableFlatVector<int32_t>(
          {std::nullopt, std::nullopt, 13, std::nullopt, std::nullopt}),
  };
...
TEST_F(MergeJoinTest, outputAfterRightMatch) {
  ...
  // nonNullRow
  std::vector<VectorPtr> leftKeys = {
      makeNullableFlatVector<int32_t>({1, 2, 3, 4, 4}),
  };
  std::vector<VectorPtr> rightKeys = {
      makeNullableFlatVector<int32_t>({1, 2, 3, 4, 4}),
      makeNullableFlatVector<int32_t>({4, 4, 4, 4, 4}),
      makeNullableFlatVector<int32_t>(
          {4, 4, std::nullopt, std::nullopt, std::nullopt}),
      makeNullableFlatVector<int32_t>(
          {std::nullopt, std::nullopt, 18, std::nullopt, std::nullopt}),
  };
  testJoin1(leftKeys, rightKeys);
  testJoin1(rightKeys, leftKeys);
}
TEST_F(MergeJoinTest, outputAfterRightMatchInBatch) {
  ...
  std::vector<VectorPtr> leftKeys = {
      makeNullableFlatVector<int32_t>({1, 2, 3, 4, 4}),
      makeNullableFlatVector<int32_t>({6, 6, 8, 9, 10}),
  };
  std::vector<VectorPtr> rightKeys = {
      makeNullableFlatVector<int32_t>({4, 4, std::nullopt, std::nullopt, 5}),
      makeNullableFlatVector<int32_t>(
          {6, 6, std::nullopt, std::nullopt, std::nullopt}),
      makeNullableFlatVector<int32_t>(
          {std::nullopt, std::nullopt, 13, std::nullopt, std::nullopt}),
  };
  testJoin1(leftKeys, rightKeys);
  ...
  std::vector<VectorPtr> rightKeys = {
      ...
      makeNullableFlatVector<int32_t>(
          {std::nullopt,
           ...
           std::nullopt}),
      makeNullableFlatVector<int32_t>(
          {std::nullopt, std::nullopt, 5, std::nullopt, std::nullopt}),
  };
  testJoin1(leftKeys, rightKeys);
  testJoin1(rightKeys, leftKeys);
}
...
TEST_F(MergeJoinTest, outputAfterRightMoveNext) {
  ...
  std::vector<VectorPtr> leftKeys = {
      makeNullableFlatVector<int32_t>({1, 2, 3, 4, 5}),
      makeNullableFlatVector<int32_t>({6, 7, 8, 9, 11}),
  };
  std::vector<VectorPtr> rightKeys = {
      makeNullableFlatVector<int32_t>(
          {0, std::nullopt, std::nullopt, std::nullopt, 5}),
      makeNullableFlatVector<int32_t>(
          {10,
           std::nullopt,
           std::nullopt,
...
DEBUG_ONLY_TEST_F(MergeJoinTest, failureOnRightSide) {
  ...
  SCOPED_TESTVALUE_SET(
      "bytedance::bolt::exec::MergeSource::next",
  ...
  SCOPED_TESTVALUE_SET(
      "bytedance::bolt::exec::MergeSource::enqueue",
      std::function<void(const MergeJoinSource*)>([&](const MergeJoinSource*) {
