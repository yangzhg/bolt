...
TEST_F(HashStringAllocatorTest, stlAllocatorOverflow) {
  ...
  BOLT_ASSERT_THROW(alloc.allocate(1ULL << 62), "integer overflow");
  AlignedStlAllocator<int64_t, 16> alignedAlloc(allocator_.get());
  BOLT_ASSERT_THROW(alignedAlloc.allocate(1ULL << 62), "integer overflow");
}
...
TEST_F(HashStringAllocatorTest, fastRowStringCmp) {
  allocator_->allocate(HashStringAllocator::kMinAlloc * 4);

  std::vector<std::string> rawStrs;
  std::srand(std::time(nullptr));
  for (auto i = 0; i < 1024; ++i) {
    int rnd = std::rand();
    std::string s(rnd % 64, 'c');
    rawStrs.emplace_back("pre_" + s + std::to_string(i));
  }
  {
    rawStrs.emplace_back("32006141091834900\0\0\0");
    rawStrs.emplace_back("3200614109183\0");
    rawStrs.emplace_back("32006141091\0");
    rawStrs.emplace_back("3200\0");
    rawStrs.emplace_back("aaaaccc");
    rawStrs.emplace_back("bbbbddd");
    rawStrs.emplace_back("aa");
    rawStrs.emplace_back("bb");
    rawStrs.emplace_back("");
    rawStrs.emplace_back("\0\0");
    rawStrs.emplace_back("\0");
  }

  for (auto i = 0; i < rawStrs.size(); ++i) {
    for (auto j = 0; j < rawStrs.size(); ++j) {
      ...
      auto slowRes = HashStringAllocator::contiguousString(left, leftStorage)
                         .compare(HashStringAllocator::contiguousString(
                             right, rightStorage));

      ASSERT_TRUE(fastRes * slowRes > 0 || (fastRes == 0 && slowRes == 0));
    }
  }
}
...
TEST_F(HashStringAllocatorTest, multipleFreeAncCheckCumulativeBytes) {
  ...
  // After all blocks execute clear(), the allocated bytes should be equal to 0.
  ASSERT_EQ(allocator_->cumulativeBytes(), 0);
}

TEST_F(HashStringAllocatorTest, greaterThanMaxAlloc) {
  ASSERT_NO_THROW(allocator_->toString());

  const auto kBigSize = HashStringAllocator::kMaxAlloc * 4;
  const auto kTinySize = 123;
  auto h1 = allocate(kBigSize);
  ASSERT_EQ(h1->toString(), "size: " + std::to_string(kBigSize));
  // Note that `checkConsistency()` does not include the bytes of external
  // allocations.
  ASSERT_EQ(allocator_->checkConsistency(), 0);
  ASSERT_EQ(kBigSize, allocator_->cumulativeBytes());

  auto h2 = allocate(kTinySize);
  ASSERT_EQ(h2->toString(), "size: " + std::to_string(kTinySize));
  // allocator_->checkConsistency() == 123
  ASSERT_EQ(allocator_->checkConsistency(), kTinySize);

  auto h3 = allocate(kBigSize);
  ASSERT_EQ(h3->toString(), "size: " + std::to_string(kBigSize));

  ASSERT_EQ(
      allocator_->checkConsistency() + 2 * kBigSize,
      allocator_->cumulativeBytes());

  allocator_->free(h3);
  ASSERT_EQ(
      allocator_->checkConsistency() + kBigSize, allocator_->cumulativeBytes());
  ...
  allocator_->free(h1);
  ASSERT_EQ(allocator_->checkConsistency(), allocator_->cumulativeBytes());
