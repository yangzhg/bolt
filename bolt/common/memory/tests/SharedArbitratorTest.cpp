...
class FakeMemoryOperator : public Operator {
 public:
  ...
  void reclaim(uint64_t targetBytes, memory::MemoryReclaimer::Stats& stats)
    ...
    BOLT_CHECK(!driver->state().isOnThread() || driver->state().suspended());
 ...
 private:
  void clear() {
    ...
    allocations_.clear();
    BOLT_CHECK_EQ(totalBytes_.load(), 0);
  }
...
DEBUG_ONLY_TEST_P(
  ...
  SCOPED_TESTVALUE_SET(
      "bytedance::bolt::exec::Task::leaveSuspended",
...
DEBUG_ONLY_TEST_P(
  ...
  SCOPED_TESTVALUE_SET(
      "bytedance::bolt::exec::Operator::MemoryReclaimer::reclaim",
...
TEST_P(
  ...
  FLAGS_bolt_suppress_memory_capacity_exceeding_error_message = true;
