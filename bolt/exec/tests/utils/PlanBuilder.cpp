...
typename TypeTraits<ToKind>::NativeType cast(const variant& v) {
  return util::Converter<ToKind, void, util::DefaultCastPolicy>::cast(
      v.value<FromKind>(), nullptr);
}
} // namespace
...
PlanBuilder& PlanBuilder::tpchTableScan(
  ...
  for (const auto& columnName : columnNames) {
    ...
    outputTypes.emplace_back(resolveTpchColumn(
        table, columnName, connectorId == kBoltTpchConnectorId));
  }
...
PlanBuilder& PlanBuilder::values(
    ...
    bool parallelizable,
    size_t repeatTimes) {
  BOLT_CHECK_NULL(planNode_, "Values must be the source node");
...
PlanBuilder& PlanBuilder::exchange(const RowTypePtr& outputType) {
  BOLT_CHECK_NULL(planNode_, "Exchange must be the source node");
  planNode_ =
...
PlanBuilder& PlanBuilder::mergeExchange(
    const RowTypePtr& outputType,
    const std::vector<std::string>& keys) {
  BOLT_CHECK_NULL(planNode_, "MergeExchange must be the source node");
...
PlanBuilder& PlanBuilder::project(const std::vector<std::string>& projections) {
  BOLT_CHECK_NOT_NULL(planNode_, "Project cannot be the source node");
...
PlanBuilder& PlanBuilder::filter(const std::string& filter) {
  BOLT_CHECK_NOT_NULL(planNode_, "Filter cannot be the source node");
...
core::PlanNodePtr PlanBuilder::createIntermediateOrFinalAggregation(
  ...
  if (useGPU) {
    ...
    aggregationNode->setUseCudf();
#else
    BOLT_FAIL("useGPU is true, but it is disabled.");
#endif
  }
...
PlanBuilder& PlanBuilder::intermediateAggregation(const bool useGPU) {
  ...
  planNode_ = createIntermediateOrFinalAggregation(step, aggNode, useGPU);
  return *this;
}

PlanBuilder& PlanBuilder::finalAggregation(const bool useGPU) {
  const auto* aggNode = findPartialAggregation(planNode_.get());
...
PlanBuilder& PlanBuilder::aggregation(
  ...
  if (useGPU) {
#if defined BOLT_HAS_CUDF && BOLT_HAS_CUDF == 1
    auto aggregationNode = std::static_pointer_cast<core::AggregationNode>(
        std::const_pointer_cast<core::PlanNode>(planNode_));
    aggregationNode->setUseCudf();
#else
    BOLT_FAIL("useGPU is true, but it is disabled.");
#endif
...
PlanBuilder& PlanBuilder::expand(
  ...
  BOLT_CHECK(!projections.empty(), "projections must not be empty.");
  ...
  for (auto i = 0; i < numRows; i++) {
    ...
    BOLT_CHECK_EQ(numColumns, projections[i].size())
    for (auto j = 0; j < numColumns; j++) {
      ...
      if (i == 0) {
        ...
        if (typedExpression->type()->equivalent(*expectedType)) {
          ...
          BOLT_CHECK_NOT_NULL(constantExpr);
          BOLT_CHECK(constantExpr->value().isNull());
...
PlanBuilder& PlanBuilder::localMerge(
    const std::vector<std::string>& keys,
    std::vector<core::PlanNodePtr> sources) {
  BOLT_CHECK_NULL(planNode_, "localMerge() must be the first call");
  BOLT_CHECK_GE(sources.size(), 1, "localMerge() requires at least one source");
...
PlanBuilder& PlanBuilder::orderBy(
    ...
    bool isPartial,
    bool useGPU) {
  BOLT_CHECK_NOT_NULL(planNode_, "OrderBy cannot be the source node");
  auto [sortingKeys, sortingOrders] =
  ...
  if (useGPU) {
#if defined BOLT_HAS_CUDF && BOLT_HAS_CUDF == 1
    auto orderByNode = std::static_pointer_cast<core::OrderByNode>(
        std::const_pointer_cast<core::PlanNode>(planNode_));
    orderByNode->setUseCudf();
    BOLT_CHECK(std::static_pointer_cast<const core::OrderByNode>(planNode_)
                   ->useCudf());
#else
    BOLT_FAIL("useGPU is true, but it is disabled.");
#endif
...
PlanBuilder& PlanBuilder::localShuffle(int64_t seed) {
  planNode_ = std::make_shared<core::LocalShuffleNode>(
      nextPlanNodeId(), seed, planNode_);
  return *this;
}
...
PlanBuilder& PlanBuilder::topN(
    ...
    int32_t count,
    bool isPartial) {
  BOLT_CHECK_NOT_NULL(planNode_, "TopN cannot be the source node");
  auto [sortingKeys, sortingOrders] =
      parseOrderByClauses(keys, planNode_->outputType(), pool_);
...
PlanBuilder& PlanBuilder::localPartition(
    const std::vector<std::string>& keys,
    const std::vector<core::PlanNodePtr>& sources) {
  BOLT_CHECK_NULL(planNode_, "localPartition() must be the first call");
...
PlanBuilder& PlanBuilder::localPartitionByBucket(
    const std::shared_ptr<connector::hive::HiveBucketProperty>&
        bucketProperty) {
  BOLT_CHECK_NOT_NULL(planNode_, "LocalPartition cannot be the source node");
...
PlanBuilder& PlanBuilder::mergeJoin(
    ...
    const std::vector<std::string>& outputLayout,
    core::JoinType joinType) {
  BOLT_CHECK_NOT_NULL(planNode_, "MergeJoin cannot be the source node");
  BOLT_CHECK_EQ(leftKeys.size(), rightKeys.size());
...
PlanBuilder& PlanBuilder::nestedLoopJoin(
    ...
    const std::vector<std::string>& outputLayout,
    core::JoinType joinType) {
  BOLT_CHECK_NOT_NULL(planNode_, "NestedLoopJoin cannot be the source node");
...
PlanBuilder& PlanBuilder::indexLookupJoin(
    ...
    const std::vector<std::string>& outputLayout,
    core::JoinType joinType) {
  BOLT_CHECK_NOT_NULL(planNode_, "indexLookupJoin cannot be the source node");
...
PlanBuilder& PlanBuilder::unnest(
  ...
  BOLT_CHECK_NOT_NULL(planNode_, "Unnest cannot be the source node");
  std::vector<std::shared_ptr<const core::FieldAccessTypedExpr>>
      replicateFields;
...
PlanBuilder& PlanBuilder::window(
    ...
    bool inputSorted,
    int32_t top_n) {
  BOLT_CHECK_NOT_NULL(planNode_, "Window cannot be the source node");
  BOLT_CHECK_GT(
  ...
  bytedance::bolt::duckdb::ParseOptions options;
  options.parseIntegerAsBigint = options_.parseIntegerAsBigint;
...
PlanBuilder& PlanBuilder::window(
    const std::vector<std::string>& windowFunctions,
    int32_t top_n) {
  return window(windowFunctions, false, top_n);
}
...
PlanBuilder& PlanBuilder::topNRowNumber(
  ...
  BOLT_CHECK_NOT_NULL(planNode_, "TopNRowNumber cannot be the source node");
...
PlanBuilder& PlanBuilder::markDistinct(
  ...
  BOLT_CHECK_NOT_NULL(planNode_, "MarkDistinct cannot be the source node");
...
#if defined BOLT_HAS_TORCH && BOLT_HAS_TORCH == 1
PlanBuilder& PlanBuilder::torch(
    std::string script,
    const RowTypePtr& outputType) {
  planNode_ = std::make_shared<::bytedance::bolt::torch::TorchNode>(
      nextPlanNodeId(),
      std::move(planNode_),
      std::move(script),
      std::move(outputType));
...
#if defined BOLT_HAS_PYTHON && BOLT_HAS_PYTHON == 1
PlanBuilder& PlanBuilder::python(
    const std::string functionName,
    pybind11::function function,
    pybind11::list args,
    pybind11::dict kwargs,
    const RowTypePtr& outputType) {
  planNode_ = std::make_shared<::bolt::python::PythonNode>(
      nextPlanNodeId(),
      std::move(planNode_),
...
std::vector<core::TypedExprPtr> PlanBuilder::exprs(
  ...
  for (auto& expr : expressions) {
    ...
    if (auto field = dynamic_cast<const core::FieldAccessTypedExpr*>(
      ...
      BOLT_FAIL("Expected field name or constant: {}", expr);
    }
  }
