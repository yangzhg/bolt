...
class ParquetTestBase : public testing::Test, public test::VectorTestBase {
 protected:
  ...
  std::unique_ptr<bytedance::bolt::parquet::ParquetReader> createReader(
      const std::string& path,
      const dwio::common::ReaderOptions& opts) {
    auto input = std::make_unique<dwio::common::BufferedInput>(
        std::make_shared<LocalReadFile>(path), opts.getMemoryPool());
    return std::make_unique<bytedance::bolt::parquet::ParquetReader>(
        std::move(input), opts);
  }
  ...
  void assertReadWithReaderAndFilters(
    ...
    for (auto&& [column, filter] : filters) {
      scanSpec->getOrCreateChild(column)->setFilter(std::move(filter));
    }
  ...
  std::unique_ptr<bytedance::bolt::parquet::Writer> createWriter(
      ...
      std::function<
          std::unique_ptr<bytedance::bolt::parquet::DefaultFlushPolicy>()>
          flushPolicy,
      const RowTypePtr& rowType,
