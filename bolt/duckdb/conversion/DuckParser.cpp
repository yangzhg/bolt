...
// Convert duckDB operator name to Bolt function. Some expression types such as
// coalesce and subscript need special treatment because
// `ExpressionTypeToOperator` returns an empty string.
std::string duckOperatorToBolt(ExpressionType type) {
  switch (type) {
...
// This function returns a full Bolt function name with the registered prefix.
...
std::shared_ptr<const core::IExpr> parseConstantExpr(
  ...
  return std::make_shared<const core::ConstantExpr>(
      toBoltType(value.type()), duckValueToVariant(value), getAlias(expr));
}
...
std::shared_ptr<const core::IExpr> parseOperatorExpr(
    ...
    if (areAllChildrenConstant(operExpr)) {
      ...
      for (const auto& child : operExpr.children) {
        if (auto constantExpr =
          ...
          if (!value.IsNull()) {
            valueType = toBoltType(value.type());
          }
        } else {
          BOLT_UNREACHABLE();
        }
    ...
    for (auto i = 0; i < numValues; i++) {
      ...
      if (const auto castExpr = dynamic_cast<CastExpression*>(valueExpr)) {
        if (castExpr->child->GetExpressionType() ==
          ...
          valueType = toBoltType(castExpr->cast_type);
          continue;
        }
      }
      ...
      BOLT_UNSUPPORTED("IN list values need to be constant");
    }
  ...
  if (expr.GetExpressionType() == ExpressionType::STRUCT_EXTRACT) {
    ...
    if (auto constantExpr =
      ...
      BOLT_UNSUPPORTED("STRUCT_EXTRACT field name must be constant");
    }
  }
  ...
  return callExpr(
      duckOperatorToBolt(expr.GetExpressionType()),
      std::move(params),
      getAlias(expr),
      options);
}
...
std::shared_ptr<const core::IExpr> parseCastExpr(
  ...
  // We may need to expand toBoltType in the future to support
  // Map and Array and Struct properly.
  auto targetType = toBoltType(castExpr.cast_type);
  BOLT_CHECK(!params.empty());
  ...
  switch (expr.GetExpressionClass()) {
    ...
    default:
      throw std::invalid_argument(
          "Unsupported expression type for DuckDB -> bolt conversion: " +
...
parseExpression(const std::string& exprString) {
  ...
  try {
    ...
    BOLT_FAIL("Cannot parse expression: {}. {}", exprString, e.what());
  }
}
...
std::vector<std::shared_ptr<const core::IExpr>> parseMultipleExpressions(
  ...
  BOLT_CHECK_GT(parsedExpressions.size(), 0);
...
bool isAscending(::duckdb::OrderType orderType, const std::string& exprString) {
  switch (orderType) {
    ...
    default:
      BOLT_FAIL("Cannot parse ORDER BY clause: {}", exprString)
  }
}
