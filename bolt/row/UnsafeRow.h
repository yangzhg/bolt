...
#define SCALAR_TRAIT(BoltKind)                                              \
  template <>                                                               \
  struct ScalarTraits<TypeKind::BoltKind> {                                 \
...
class UnsafeRow {
 public:
  ...
  void writeOffsetPointer(size_t pos, size_t size) {
    BOLT_CHECK_LE(variableLengthOffset_, UINT32_MAX);
    BOLT_CHECK_LE(size, UINT32_MAX);
 ...
 private:
  ...
  void setVariableLengthOffset(size_t offset) {
    BOLT_CHECK_GE(offset, variableLengthOffset_);
...
FOLLY_ALWAYS_INLINE size_t serializedSizeInBytes(const TypePtr& type) {
  ...
  return BOLT_DYNAMIC_SCALAR_TYPE_DISPATCH(serializedSizeInBytes, type->kind());
}
