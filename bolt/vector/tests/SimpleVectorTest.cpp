...
#include "bolt/vector/tests/SimpleVectorTestHelper.h"
...
TEST_F(SimpleVectorNonParameterizedTest, isAscii) {
  for (auto encoding : kAsciiEncodings) {
    ...
    EXPECT_THROW(vector->isAscii(empty).has_value(), BoltException);
  }
}
...
TEST_F(SimpleVectorNonParameterizedTest, isAsciiIndex) {
  for (auto encoding : kAsciiEncodings) {
    ...
    EXPECT_THROW(vector->isAscii(-1).has_value(), BoltException);
  }
}
...
BOLT_TYPED_TEST_SUITE(SimpleVectorTypedTest, SimpleTypes);
...
BOLT_TYPED_TEST_SUITE(SimpleVectorBinaryTypedTest, SimpleTypes);
...
BOLT_TYPED_TEST_SUITE(SimpleVectorUnaryTypedTest, SimpleTypes);
...
BOLT_TYPED_TEST_SUITE(SimpleVectorCompareTest, NumericTypes);
  ...
  switch (outVector->encoding()) {
    ...
    case VectorEncoding::Simple::SEQUENCE:
      return dynamic_cast<const SequenceVector<T>*>(outVector)
          ->loadSIMDValueBufferAt(byteOffset);
    default:
      BOLT_FAIL("Invalid vector encoding - {}", outVector->encoding());
  }
}
...
BOLT_TYPED_TEST_SUITE(SimpleVectorSimdTypedTest, SimpleTypes);
