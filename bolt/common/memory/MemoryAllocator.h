...
struct Stats {
  ...
  void recordAllocate(int64_t bytes, int32_t count, Op op) {
    if (FLAGS_bolt_time_allocations) {
      auto index = sizeIndex(bytes);
      bolt::ClockTimer timer(sizes[index].allocateClocks);
      op();
...
template <>
struct fmt::formatter<bytedance::bolt::memory::MemoryAllocator::InjectedFailure>
    : fmt::formatter<int> {
  auto format(
      bytedance::bolt::memory::MemoryAllocator::InjectedFailure s,
