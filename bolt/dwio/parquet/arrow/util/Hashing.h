...
#include <xxhash.h>
namespace bytedance::bolt::parquet::arrow::internal {
 ...
 public:
  ...
  std::pair<Entry*, bool> Lookup(hash_t h, CmpFunc&& cmp_func) {
    auto p = Lookup<DoCompare, CmpFunc>(
        h, entries_, capacity_mask_, std::forward<CmpFunc>(cmp_func));
 ...
 protected:
  ...
  std::pair<uint64_t, bool> Lookup(
    ...
    while (true) {
      ...
      if (CompareEntry<CKind, CmpFunc>(
              h, entry, std::forward<CmpFunc>(cmp_func))) {
        // Found
        return {index, true};
      }
  ...
  Status Upsize(uint64_t new_capacity) {
    ...
    ARROW_ASSIGN_OR_RAISE(
        auto previous, entries_builder_.FinishWithLength(capacity_));
    // Allocate new buffer
    ...
    for (uint64_t i = 0; i < capacity_; i++) {
      ...
      if (entry) {
        ...
        auto p = Lookup<NoCompare>(
            entry.h, entries_, new_mask, [](const Payload*) { return false; });
  ...
  Entry* entries_;
  ::arrow::TypedBufferBuilder<Entry> entries_builder_;
};
...
class ScalarMemoTable : public MemoTable {
 public:
  ...
  int32_t size() const override {
    return static_cast<int32_t>(hash_table_.size()) +
  ...
  void CopyValues(Scalar* out_data) const {
...
class SmallScalarMemoTable : public MemoTable {
 public:
  ...
  int32_t GetNull() const {
    return value_to_index_[cardinality];
  }
  ...
  // (which is also 1 + the largest memo index)
  int32_t size() const override {
    return static_cast<int32_t>(index_to_value_.size());
  }
...
class BinaryMemoTable : public MemoTable {
 public:
  ...
  Status GetOrInsert(
    ...
    if (p.second) {
      ...
      // Insert string value
      RETURN_NOT_OK(
          binary_builder_.Append(static_cast<const char*>(data), length));
      // Insert hash entry
      RETURN_NOT_OK(hash_table_.Insert(
          const_cast<HashTableEntry*>(p.first), h, {memo_index}));
  ...
  Status GetOrInsert(
    ...
    return GetOrInsert(
        value.data(),
        static_cast<builder_offset_type>(value.length()),
        std::forward<Func1>(on_found),
        std::forward<Func2>(on_not_found),
        out_memo_index);
  }
  ...
  int32_t size() const override {
    return static_cast<int32_t>(
        hash_table_.size() + (GetNull() != kKeyNotFound));
  }

  int64_t values_size() const {
    return binary_builder_.value_data_length();
  }
  ...
  void CopyOffsets(int32_t start, Offset* out_data) const {
    ...
    for (int32_t i = start; i < size(); ++i) {
      ...
      assert(
          static_cast<builder_offset_type>(cast_offset) ==
    ...
    *out_data =
        static_cast<Offset>(binary_builder_.value_data_length() - delta);
  }
  ...
  void CopyValues(int32_t start, int64_t out_size, uint8_t* out_data) const {
    ...
    const auto length =
        binary_builder_.value_data_length() - static_cast<size_t>(offset);
  ...
  void CopyValues(uint8_t* out_data) const {
    CopyValues(0, -1, out_data);
  }
  ...
  void VisitValues(int32_t start, VisitFunc&& visit) const {
    for (int32_t i = start; i < size(); ++i) {
      auto sv = binary_builder_.GetView(i);
 ...
 protected:
  ...
  Lookup(hash_t h, const void* data, builder_offset_type length) const {
    auto cmp_func = [&](const Payload* payload) {
      auto lhs = binary_builder_.GetView(payload->memo_index);
  ...
  *null_bitmap = nullptr;
  ...
  if (null_index != kKeyNotFound && null_index >= start_offset) {
    ...
    *null_count = 1;
    ARROW_ASSIGN_OR_RAISE(
        *null_bitmap,
        ::arrow::internal::BitmapAllButOne(pool, dict_length, null_index));
  }
