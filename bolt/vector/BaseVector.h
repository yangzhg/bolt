...
class BaseVector {
 public:
  ...
  bolt::memory::MemoryPool* pool() const {
    return pool_;
  }

  virtual bool isNullAt(vector_size_t idx) const {
    BOLT_DCHECK_GE(idx, 0, "Index must not be negative");
    BOLT_DCHECK_LT(idx, length_, "Index is too large");
  ...
  std::optional<uint64_t> getNullCount(vector_size_t begin, vector_size_t end)
      const {
    if (rawNulls_) {
      BOLT_DCHECK_GE(begin, 0);
      BOLT_DCHECK_GE(end, 0);
      BOLT_DCHECK_LE(end, length_);
      BOLT_DCHECK_LE(begin, end);
      return bits::countNulls(rawNulls_, begin, end);
    }
    return std::nullopt;
  }
  ...
  virtual vector_size_t baseSize() const {
    return length_;
  }

  virtual vector_size_t itemSize() const {
    return length_;
  }
  ...
  virtual const VectorPtr& valueVector() const {
    BOLT_UNSUPPORTED("Vector is not a wrapper");
  }
  ...
  virtual const BufferPtr& values() const {
    BOLT_UNSUPPORTED("Only flat vectors have a values buffer");
  }
  ...
  /// Returns the byte size of memory that is used by 'this'.
  virtual uint64_t usedSize() const {
    return nulls_ ? nulls_->size() : 0;
  }

  /// Returns an estimate after exporting to Arrow.
  ///
  /// Note that if the vector is a lazy vector that has not been loaded, it will
  /// be loaded first before estimation. If it is a flatVector of StringView,
  /// it will calculate the size of each StringView per row.
  virtual uint64_t estimateExportArrowSize() const {
    return nulls_ ? nulls_->size() : 0;
  }
  ...
  std::string toPrettyString(vector_size_t from, vector_size_t size = 10) const;
  std::string toPrettyString(const SelectivityVector& rows) const;
  ...
  bool containingLazyAndWrapped() {
    return containsLazyAndIsWrapped_;
  }
    ...
    switch (flags.nullHandlingMode) {
      case CompareFlags::NullHandlingMode::kNullAsIndeterminate:
        if (flags.equalsOnly) {
          ...
          BOLT_USER_FAIL("Ordering nulls is not supported");
        }
      case CompareFlags::NullHandlingMode::kNullAsValue:
        ...
        return thisNull ? 1 : -1;
      case CompareFlags::NullHandlingMode::kNullAsSpeciallValue:
        if (thisNull && otherNull) {
          return kIndeterminate;
        }
...
template <>
class FormatValue<bytedance::bolt::VectorEncoding::Simple> {
 public:
  explicit FormatValue(const bytedance::bolt::VectorEncoding::Simple& type)
      : type_(type) {}

  template <typename FormatCallback>
  void format(FormatArg& arg, FormatCallback& cb) const {
    return format_value::formatString(
        bytedance::bolt::VectorEncoding::mapSimpleToName(type_), arg, cb);
  }

 private:
  bytedance::bolt::VectorEncoding::Simple type_;
};

} // namespace folly

template <>
struct fmt::formatter<bytedance::bolt::VectorEncoding::Simple> {
  ...
  auto format(
      const bytedance::bolt::VectorEncoding::Simple& x,
      FormatContext& ctx) const {
    return fmt::format_to(
        ctx.out(), "{}", bytedance::bolt::VectorEncoding::mapSimpleToName(x));
  }
};
