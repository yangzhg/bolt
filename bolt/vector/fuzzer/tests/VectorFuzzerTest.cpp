...
class VectorFuzzerTest : public testing::Test {
 public:
  ...
  static void printGeneratedData(const VectorPtr& vector) {
    if (vector->isLazy()) {
      auto* vectorNotLazy = vector->loadedVector();
      std::cout << printVector(*vectorNotLazy) << std::endl;
    } else {
...
TEST_F(VectorFuzzerTest, specialValues) {
  VectorFuzzer::Options opts;
  opts.nullRatio = 0.1;
  opts.enableDuplicates = true;
  VectorFuzzer fuzzer(opts, pool());
  VectorPtr vector;

  const uint8_t kNumVectorsEachType = 5;
  const std::vector<TypePtr> testedTypes = {
      TINYINT(), SMALLINT(), INTEGER(), BIGINT(), REAL(), DOUBLE(), BOOLEAN()};
  std::vector<TypePtr> types;
  types.reserve(testedTypes.size() * kNumVectorsEachType);
  for (const auto& type : testedTypes) {
    types.insert(types.end(), kNumVectorsEachType, type);
  }
...
TEST_F(VectorFuzzerTest, enhancedStringFuzzer) {
  VectorFuzzer::Options opts;
  opts.nullRatio = 0.5;
  opts.enableStringIncrementalGeneration = true;
  opts.enableDuplicates = true;
  opts.stringVariableLength = true;
  opts.charEncodings = {
      ASCII, UNICODE_CASE_SENSITIVE, MATHEMATICAL_SYMBOLS, ALL_OTHERS};
  VectorFuzzer fuzzer(opts, pool());

  std::vector<TypePtr> types(10, VARCHAR());
  VectorPtr vector;
...
TEST_F(VectorFuzzerTest, enhancedDecimalFuzzer) {
  VectorFuzzer::Options opts;
  opts.nullRatio = 0.1;
  opts.vectorSize = 50;
  opts.enableDuplicates = true;
  ...
  const uint8_t kNumVectorsEachType = 5;
  const std::vector<TypePtr> testedTypes = {
      fuzzer.randShortDecimalType(), fuzzer.randLongDecimalType()};
  std::vector<TypePtr> types;
  types.reserve(testedTypes.size() * kNumVectorsEachType);
  for (const auto& type : testedTypes) {
    types.insert(types.end(), kNumVectorsEachType, type);
...
TEST_F(VectorFuzzerTest, enhancedTimestampFuzzer) {
  // Standard format.
  {
    VectorFuzzer::Options opts;
    opts.nullRatio = 0.5;
    opts.enableDuplicates = true;
    opts.timestampFormat = std::vector<VectorFuzzer::Options::TimestampFormat>{
        VectorFuzzer::Options::TimestampFormat::kStandard};
    VectorFuzzer fuzzer(opts, pool());

    std::vector<TypePtr> types(10, TIMESTAMP());
    VectorPtr vector;
  ...
  // Special format.
  {
    ...
    opts.enableDuplicates = true;
    opts.timestampFormat = std::vector<VectorFuzzer::Options::TimestampFormat>{
        VectorFuzzer::Options::TimestampFormat::kSpecial};
    VectorFuzzer fuzzer(opts, pool());
...
TEST_F(VectorFuzzerTest, containerHasNulls) {
  ...
  opts.enableStringIncrementalGeneration = false;
