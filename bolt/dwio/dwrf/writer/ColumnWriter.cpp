...
#include "bolt/dwio/dwrf/writer/DictionaryEncodingUtils.h"
#include "bolt/dwio/dwrf/writer/EntropyEncodingSelector.h"
#include "bolt/dwio/dwrf/writer/FlatMapColumnWriter.h"
      ...
      [&, this](size_t start, size_t end, size_t strideIndex) {
        ...
        if (writeInDictionaryStream) {
          ...
          sortedStrideDictKeyIndexBuffer.reserve(strideDictKeyCount);
          {
            ...
            errorGuard.dismiss();
            inDictWriter.close();
            BOLT_CHECK_EQ(strideDictSize, strideDictKeyCount);
          }
...
uint64_t FloatColumnWriter<T>::write(
  ...
  if (slice->encoding() == VectorEncoding::Simple::FLAT) {
    auto flatVector = slice->asFlatVector<T>();
    BOLT_CHECK_NOT_NULL(flatVector, "unexpected vector type");
    writeNulls(slice, ranges);
    auto nulls = slice->rawNulls();
