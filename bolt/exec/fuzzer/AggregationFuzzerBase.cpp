...
bool AggregationFuzzerBase::addSignature(
  ...
  if (!signature->variables().empty()) {
    ...
    for (const auto& arg : signature->argumentTypes()) {
      ...
      BOLT_CHECK_NOT_NULL(resolvedType);
...
AggregationFuzzerBase::PlanWithSplits AggregationFuzzerBase::deserialize(
  ...
  auto plan = bolt::ISerializable::deserialize<core::PlanNode>(
      obj["plan"], pool_.get());
...
bolt::fuzzer::ResultOrError AggregationFuzzerBase::execute(
  ...
  } catch (BoltUserError& e) {
    // NOTE: bolt user exception is accepted as it is caused by the invalid
    // fuzzer test inputs.
...
AggregationFuzzerBase::computeReferenceResultsAsVector(
    const core::PlanNodePtr& plan,
    const std::vector<RowVectorPtr>& input) {
  BOLT_CHECK(referenceQueryRunner_->supportsBoltVectorResults());
...
void AggregationFuzzerBase::compare(
  ...
  if (expected.userExceptionPtr || actual.userExceptionPtr) {
    // Throws in case exceptions are not compatible.
    bolt::fuzzer::compareExceptions(
        expected.userExceptionPtr, actual.userExceptionPtr);
    return;
  }
  ...
  BOLT_CHECK_NOT_NULL(expected.result);
  BOLT_CHECK_NOT_NULL(actual.result);
...
void persistReproInfo(
    ...
    const std::string& basePathStr) {
  fs::path basePath(basePathStr);
  std::string absoluteBasePathStr = fs::absolute(basePath).string();

  if (!common::generateFileDirectory(absoluteBasePathStr.c_str())) {
    return;
  }
...
std::vector<std::string> retrieveWindowFunctionName(
  ...
  BOLT_CHECK_NOT_NULL(windowNode);
