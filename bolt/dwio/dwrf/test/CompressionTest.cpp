...
using bytedance::bolt::BoltException;
  ...
  for (const auto& testData : testSettings) {
    ...
    WriterContext context{
        ...
        memoryManager()->addRootPool(
            "oomOnCompression",
            kind_ == bytedance::bolt::common::CompressionKind_NONE ? 3L << 20
                                                                   : 6L << 20)};
    context.initBuffer();
    ...
    if (testData.oomOnNextCall) {
      ...
      BOLT_ASSERT_THROW(compressStream->flush(), "Exceeded memory pool cap");
    }
    if (kind_ != bytedance::bolt::common::CompressionKind_NONE) {
...
TEST(CompressionOptionsTest, testCompressionOptions) {
  auto options = getDwrfOrcCompressionOptions(
      bytedance::bolt::common::CompressionKind_ZLIB, 256, 4, 7);
  ...
  options = getDwrfOrcCompressionOptions(
      bytedance::bolt::common::CompressionKind_ZSTD, 256, 4, 7);
