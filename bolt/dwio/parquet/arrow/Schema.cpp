...
#include "bolt/dwio/parquet/arrow/SchemaInternal.h"
...
using bytedance::bolt::parquet::arrow::format::SchemaElement;
...
std::string ColumnDescriptor::ToString() const {
  ...
  if (physical_type() ==
      ::bytedance::bolt::parquet::arrow::Type::FIXED_LEN_BYTE_ARRAY) {
    ss << "  length: " << type_length() << "," << std::endl;
  }

  if (converted_type() ==
      ::bytedance::bolt::parquet::arrow::ConvertedType::DECIMAL) {
