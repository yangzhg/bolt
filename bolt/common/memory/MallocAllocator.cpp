...
MallocAllocator::~MallocAllocator() {
  ...
  if (FLAGS_bolt_memory_leak_check_enabled) {
    BOLT_CHECK(
...
bool MallocAllocator::allocateContiguousImpl(
  ...
  if (numContiguousCollateralPages > 0) {
    ...
    if (::munmap(allocation.data(), allocation.maxSize()) < 0) {
      BOLT_MEM_LOG(ERROR) << "munmap got " << folly::errnoStr(errno) << "for "
...
void MallocAllocator::freeContiguousImpl(ContiguousAllocation& allocation) {
  ...
  if (::munmap(allocation.data(), allocation.maxSize()) < 0) {
    BOLT_MEM_LOG(ERROR) << "Error for munmap(" << allocation.data() << ", "
                        << succinctBytes(bytes) << "): '"
...
void* MallocAllocator::allocateBytesWithoutRetry(
  ...
  if (FOLLY_UNLIKELY(result == nullptr)) {
    BOLT_MEM_LOG(ERROR) << "Failed to allocateBytes " << succinctBytes(bytes)
...
void* MallocAllocator::allocateZeroFilledWithoutRetry(uint64_t bytes) {
  ...
  if (FOLLY_UNLIKELY(result == nullptr)) {
    BOLT_MEM_LOG(ERROR) << "Failed to allocateZeroFilled "
                        << succinctBytes(bytes);
  }
  return result;
}
...
void* MallocAllocator::reallocateBytes(
    ...
    uint64_t bytes,
    uint16_t alignment) {
  BOLT_CHECK(p != nullptr, "reallocateBytes got nullptr");
  if (FOLLY_UNLIKELY(!incrementUsage(bytes))) {
    auto errorMsg = fmt::format(
        "Failed to reallocateBytes {}: Exceeded memory allocator "
        "limit of {}",
        succinctBytes(bytes),
        succinctBytes(capacity_));
    BOLT_MEM_LOG_EVERY_MS(WARNING, 1000) << errorMsg;
  ...
  if (FOLLY_UNLIKELY(!isAlignmentValid(bytes, alignment))) {
    decrementUsage(bytes);
    BOLT_FAIL(
  ...
  void* result = ::rallocx(p, bytes, MALLOCX_ALIGN(alignment));
  if (FOLLY_UNLIKELY(result == nullptr)) {
    BOLT_MEM_LOG(ERROR) << "Failed to reallocateBytes " << succinctBytes(bytes)
                        << " with " << alignment << " alignment";
  }
  return result;
}
