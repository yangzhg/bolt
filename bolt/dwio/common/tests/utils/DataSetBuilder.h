...
class DataSetBuilder {
 public:
  ...
  DataSetBuilder& withUniqueStringsForField(
      const bytedance::bolt::common::Subfield& field);
  ...
  DataSetBuilder& withReapeatingValuesForField(
      ...
      int32_t lastRow,
      T value) {
    BOLT_CHECK_LT(batchIndex, batches_->size());
    ...
    BOLT_CHECK_LT(firstRow, numRows);
    BOLT_CHECK_LT(firstRow, lastRow);
    BOLT_CHECK_LE(lastRow, numRows);
