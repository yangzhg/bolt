...
#include "bolt/dwio/common/FilterNode.h"
...
class ColumnSelector {
 public:
   ...
   */
  explicit ColumnSelector(
      const std::shared_ptr<const bolt::RowType>& schema,
  ...
  explicit ColumnSelector(
      const std::shared_ptr<const bolt::RowType>& schema,
      const std::shared_ptr<const bolt::RowType>& contentSchema,
      MetricsLogPtr log = nullptr,
      bool fileColumnNamesReadAsLowerCase = false)
  ...
  const FilterTypePtr& getNode(size_t id) const {
    BOLT_CHECK(inRange(id), "node is out of range");
    return nodes_[id];
  }
  ...
  const std::shared_ptr<const bolt::Type>& getDataType(
      const uint64_t node) const {
  ...
  std::shared_ptr<const bolt::RowType> buildSelectedReordered() const;

  /// Builds a column filter out of filter tree
  ...
  const std::shared_ptr<const bolt::RowType>& getSchema() const {
    return schema_;
  }
  ...
  static ColumnSelector apply(
      ...
      const std::shared_ptr<const bolt::RowType>& fileSchema,
 ...
 private:
  ...
  static void copy(
      FilterTypePtr&,
      const std::shared_ptr<const bolt::Type>& diskType,
      const FilterTypePtr& origin,
      bool useColumnNamesForColumnMapping);

  void setUseColumnNamesForColumnMapping(bool flag) {
  ...
  FilterTypePtr buildNode(
      ...
      const std::shared_ptr<const bolt::Type>& contentType,
      bool inContent);
  ...
  const std::shared_ptr<const bolt::RowType> schema_;
