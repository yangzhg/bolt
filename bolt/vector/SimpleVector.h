...
// This class abstracts over various Columnar Storage Formats such that Bolt
...
class SimpleVector : public BaseVector {
 public:
  ...
  void resize(vector_size_t size, bool setNotNull = true) override {
    BOLT_CHECK(false, "Can only resize flat vectors.");
  }
  ...
  std::string valueToString(T value) const {
    if constexpr (std::is_same_v<T, bool>) {
      ...
      if (type()->isDecimal()) {
        ...
        return bolt::to<std::string>(value);
      }
    } else {
      return bolt::to<std::string>(value);
    }
 ...
 protected:
  ...
  std::optional<V> getMetaDataValue(
    ...
    return value == metaData.end()
        ? std::nullopt
        : std::optional<V>(bolt::to<V>(value->second));
  }
