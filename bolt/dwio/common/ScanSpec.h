...
class ScanSpec {
 public:
  ...
  // Returns true if the column data needs to be read from the file rather than
  // using a constant (e.g. partition column or missing column).
  ...
  void setRuntimeStatistics(dwio::common::RuntimeStatistics* statis) {
    statis_ = statis;
  }

  dwio::common::RuntimeStatistics* getRuntimeStatistics() const {
    return statis_;
  }
