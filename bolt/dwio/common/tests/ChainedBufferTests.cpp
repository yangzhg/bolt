...
TEST_F(ChainedBufferTests, testApplyRange) {
  ...
  BOLT_ASSERT_THROW(buf.applyRange(2, 1, fn), "(2 vs. 1)");
  BOLT_ASSERT_THROW(buf.applyRange(1, 65, fn), "(65 vs. 64)");
...
TEST_F(ChainedBufferTests, testTrailingZeros) {
  ...
  BOLT_ASSERT_THROW(ChainedBuffer<int32_t>::trailingZeros(0), "(0 vs. 0)");
}
