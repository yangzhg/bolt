...
enum class BlockingReason {
  ...
  kYield,
  /// Some operators can be blocked due to the operator's input being
  /// unavailable. But the the task should not stop when the operator is
  /// blocked.
  /// This state is only use for memory source operators
  /// (e.g. MemorySource in Blint) when data is not ready.
  kWaitForData,
...
struct DriverFactory {
  ...
  /// Total number of drivers dynamically created under the morsel-driven mode
  uint32_t numMorselDrivenDrivers;

  uint32_t numFinishedPushBasedDriverSchedulers;
  ...
  bool morselDrivenEnabled_{false};

  void enableMorselDriven() {
    morselDrivenEnabled_ = true;
  }

  /// [morsel-driven] Check whether we can enable morsel-driven for a pipeline
  /// estimated=true is used during query planning before maxDrivers is
  /// determined
  bool isMorselDriven(bool estimated = false) {
    // If morsel driven is disabled in the QueryConfig, or current pipeline uses
    // group execution, return false.
    if (!morselDrivenEnabled_ || groupedExecution) {
      return false;
    }

    // If the pipeline does not have localExchange, then it cannot be morsel
    // driven
    auto exchangeId = needsLocalExchange();
    if (!exchangeId.has_value()) {
      return false;
    }

    for (auto node : planNodes) {
      // For final or single agg, morsel-driven is not allowed, because we need
      // to guarantee to start one driver for each partition.
      if (auto hashAggNode =
              std::dynamic_pointer_cast<const core::AggregationNode>(node)) {
        if (hashAggNode->step() == core::AggregationNode::Step::kFinal ||
            hashAggNode->step() == core::AggregationNode::Step::kSingle) {
          return false;
        }
      }
      // For window operators, morsel-driven is not allowed, because we need to
      // guarantee to start one driver for each partition.
      else if (
          auto windowNode =
    ...
    if (this->consumerNode != nullptr) {
      // If the end of the pipeline is a localPartition, then it cannot be
      // morsel-driven. This is to avoid make pipeline like
      // "hashBuild->localPartition" in stage 2 of TPC-DS Q2 morsel-driven,
      // which might cause error "BoltRuntimeError: !noMoreProducers_
      // addProducer called after noMoreProducers".
      if (auto localPartitionNode =
              std::dynamic_pointer_cast<const core::LocalPartitionNode>(
                  this->consumerNode)) {
        return false;
      }
      // For HashBuild, morsel-driven is allowed, but might cause performance
      // regression, Thus, we turn off morsel-driven for hashbuild for now.
      else if (
          auto hashJoinNode =
...
/// Note: Suspended driver may be killed during suspended, caller can
/// use `leave()` to exit the suspended section, and return `true` if
/// the driver is still alive.
class SuspendedSection {
 public:
  explicit SuspendedSection(Driver* driver);

  // leave suspended section and return whether the driver is still alive
  bool leave();
...
class DriverThreadContext {
 public:
  ...
  const DriverCtx* driverCtx() const {
    BOLT_CHECK_NOT_NULL(driverCtx_);
    return driverCtx_;
  }
...
template <>
struct fmt::formatter<bytedance::bolt::exec::StopReason>
    : formatter<std::string> {
  auto format(const bytedance::bolt::exec::StopReason s, format_context& ctx) {
    return formatter<std::string>::format(
        bytedance::bolt::exec::stopReasonString(s), ctx);
  }
};

template <>
struct fmt::formatter<bytedance::bolt::exec::BlockingReason>
    : fmt::formatter<std::string> {
  auto format(
      const bytedance::bolt::exec::BlockingReason e,
      format_context& ctx) {
    return formatter<std::string>::format(
        bytedance::bolt::exec::blockingReasonToString(e), ctx);
  }
};
