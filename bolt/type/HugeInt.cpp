...
int128_t HugeInt::parse(const std::string& str) {
  ...
  BOLT_CHECK(!str.empty(), "Empty string cannot be converted to int128_t.");
  ...
  for (; idx < str.size(); ++idx) {
    ...
    if ((result > max / 10)) {
      BOLT_FAIL(fmt::format("{} is out of range of int128_t", str));
    }
...
string to_string(bytedance::bolt::int128_t x) {
  if (x == 0) {
    return "0";
  }
  string ans;
