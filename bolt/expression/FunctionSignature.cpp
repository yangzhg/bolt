...
void validateBaseTypeAndCollectTypeParams(
  ...
  if (!variables.count(arg.baseName())) {
    ...
    if (typeName == "ANY") {
      BOLT_USER_CHECK(!isReturnType, "Type 'Any' cannot appear in return type");
    ...
    if (!isPositiveInteger(typeName) &&
      ...
      BOLT_USER_CHECK(hasType(typeName), "Type doesn't exist: '{}'", typeName);
    }
...
FunctionSignaturePtr FunctionSignatureBuilder::build() {
  BOLT_CHECK(returnType_.has_value());
...
AggregateFunctionSignatureBuilder::build() {
  BOLT_CHECK(returnType_.has_value());
  BOLT_CHECK(intermediateType_.has_value());
