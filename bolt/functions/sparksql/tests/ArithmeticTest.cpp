...
TEST_F(ArithmeticTest, log) {
  static const auto log = [&](std::optional<double> a,
                              std::optional<double> b) {
    return evaluateOnce<double>("log(c0, c1)", a, b);
  };

  static const auto log_value = [&](std::optional<double> a,
                                    std::optional<double> b) {
    return evaluateOnce<double>("log(c0, c1)", a, b).value_or(-1);
  };

  static const auto ln = [&](std::optional<double> a) {
    return evaluateOnce<double>("ln(c0)", a);
  };

  EXPECT_EQ(log(0, 5), std::nullopt);
  EXPECT_EQ(log(4, 0), std::nullopt);
  EXPECT_EQ(log(0, 0), std::nullopt);
  EXPECT_EQ(log(0, 0), std::nullopt);
  EXPECT_EQ(log(-1, 0), std::nullopt);
  EXPECT_EQ(log_value(2, 4), 2);
  EXPECT_EQ(log_value(5, 25), 2);
  EXPECT_EQ(log_value(1, 5), kInf);
  EXPECT_TRUE(std::isnan(log(1, 1).value_or(0)));
  EXPECT_EQ(ln(-1), std::nullopt);
  EXPECT_EQ(ln(1), 0.0);
}
