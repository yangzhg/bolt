...
struct ArrayJoinFunction {
  ...
  template <typename C>
  void writeValue(out_type<bolt::Varchar>& result, const C& value) {
    result +=
  ...
  void writeOutput(
      ...
      const arg_type<bolt::Varchar>& delim,
      const C& value,
      bool& firstNonNull) {
    if (!firstNonNull) {
  ...
  void createOutputString(
      out_type<bolt::Varchar>& result,
      const arg_type<bolt::Array<T>>& inputArray,
      const arg_type<bolt::Varchar>& delim,
  ...
  FOLLY_ALWAYS_INLINE bool call(
      ...
      const arg_type<bolt::Array<T>>& inputArray,
      const arg_type<bolt::Varchar>& delim,
      const arg_type<bolt::Varchar>& nullReplacement) {
...
struct CombinationsFunction {
  ...
  void appendEntryFromCombination(
      out_type<bolt::Array<bolt::Array<T>>>& result,
      const arg_type<bolt::Array<T>>& array,
...
struct ArrayHasDuplicatesFunction {
  ...
  FOLLY_ALWAYS_INLINE void callNullFree(
      bool& out,
      const null_free_arg_type<bolt::Array<T>>& inputArray) {
...
struct ArrayFrequencyFunction {
  ...
  FOLLY_ALWAYS_INLINE void call(
      out_type<bolt::Map<T, int>>& out,
      arg_type<bolt::Array<T>> inputArray) {
...
struct ArrayConcatFunction {
  ...
  static constexpr int32_t kMinArity = 1;
...
inline void checkIndexArrayTrim(int64_t size, int64_t arraySize) {
  if (size < 0) {
    BOLT_USER_FAIL("size must not be negative: {}", size);
  }
...
struct ArrayNGramsFunction {
  ...
  void call(Out& out, const In& input, int32_t n) {
    BOLT_USER_CHECK_GT(n, 0, "N must be greater than zero.");
