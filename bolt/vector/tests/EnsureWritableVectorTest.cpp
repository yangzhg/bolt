...
struct VectorPointersBase {
  ...
  void assertUnique(const VectorPtr& vector) {
    ASSERT_TRUE(vector.use_count() == 1);
...
TEST_F(EnsureWritableVectorTest, array) {
  for (auto canCopyAll : {true, false}) {
    vector_size_t size = 1'000;
    ...
    result->copy(b.get(), oddRows, nullptr, canCopyAll);
    ...
    assertEqualOffsetsOrSizes(
        a->offsets(), offsetsCopy, a->size(), isNullAt(a));
...
TEST_F(EnsureWritableVectorTest, map) {
  for (auto canCopyAll : {true, false}) {
    ...
    for (vector_size_t i = 0; i < a->mapKeys()->size(); i++) {
      ASSERT_TRUE(a->mapKeys()->equalValueAt(keysCopy.get(), i, i))
          << "at " << i;
    }
...
TEST_F(EnsureWritableVectorTest, allNullArray) {
  for (auto canCopyAll : {true, false}) {
    ...
    result->copy(a.get(), oddRows, nullptr, canCopyAll);
...
TEST_F(EnsureWritableVectorTest, testSizeExceed) {
  SelectivityVector rows(8);

  VectorPtr result;
  BaseVector::ensureWritable(rows, BOOLEAN(), pool_.get(), result);
  ASSERT_TRUE(result);
  ASSERT_EQ(rows.size(), result->size());
  ASSERT_EQ(VectorEncoding::Simple::FLAT, result->encoding());

  auto flatResult = result->asFlatVector<bool>();
  auto values = flatResult->mutableValues(4096 * 8);
  ASSERT_EQ(flatResult->size(), 8);
  ASSERT_EQ(values->size(), BaseVector::byteSize<bool>(4096 * 8));
  BaseVector::ensureWritable(rows, BOOLEAN(), pool_.get(), result);
}
