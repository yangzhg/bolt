...
struct Separators {
  ...
  static constexpr char unicodeCaret = '^';

  bool isSeparator(char32_t c) const {
    return (
        c == w_closeBracket || c == w_dot || c == w_openBracket ||
        c == w_quote || c == w_wildCard);
  }

  static constexpr char32_t w_backSlash = U'\\';
  static constexpr char32_t w_closeBracket = U']';
  static constexpr char32_t w_dot = U'.';
  static constexpr char32_t w_openBracket = U'[';
  static constexpr char32_t w_quote = U'\"';
  static constexpr char32_t w_wildCard = U'*';
};
  ...
  class NestedField final : public PathElement {
   public:
    explicit NestedField(const std::string& name) : name_(name) {
      BOLT_USER_CHECK(!name.empty(), "NestedFields must have non-empty names.");
    }
...
namespace std {
template <>
struct hash<::bytedance::bolt::common::Subfield> {
  size_t operator()(const ::bytedance::bolt::common::Subfield& subfield) const {
...
template <>
struct fmt::formatter<::bytedance::bolt::common::Subfield>
    : formatter<std::string> {
  auto format(const ::bytedance::bolt::common::Subfield& s, format_context& ctx)
      const {
...
template <>
struct fmt::formatter<::bytedance::bolt::common::SubfieldKind>
    : formatter<int> {
  auto format(
      const ::bytedance::bolt::common::SubfieldKind& s,
