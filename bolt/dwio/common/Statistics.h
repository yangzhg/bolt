...
struct KeyInfo {
 public:
  ...
  std::string toString() const {
    ...
    BOLT_UNREACHABLE("Illegal null key info");
  }
...
class IntegerColumnStatistics : public virtual ColumnStatistics {
 public:
  IntegerColumnStatistics(
      ...
      std::unique_ptr<BlockSplitBloomFilter>&& blockBloom = nullptr)
      : ColumnStatistics(valueCount, hasNull, rawSize, size),
        min_(min),
        max_(max),
        sum_(sum),
        blockBloom_(std::move(blockBloom)) {}
  ...
  const std::unique_ptr<BlockSplitBloomFilter>& getBlockBloomFilter() {
    return blockBloom_;
  }
 ...
 protected:
  ...
  std::unique_ptr<BlockSplitBloomFilter> blockBloom_;
};
 ...
 public:
  StringColumnStatistics(
      ...
      std::vector<std::pair<
          ...
          std::unique_ptr<NGramBloomFilter>>> token_bloom_filters)
      : ColumnStatistics(valueCount, hasNull, rawSize, size),
        min_(min),
        max_(max),
        length_(length),
        blockBloom_(std::move(blockBloom)),
        token_bloom_filters_(std::move(token_bloom_filters)) {}
  ...
  getTokenBloomFilters() const {
    return token_bloom_filters_;
  }
...
struct RuntimeStatistics {
  ...
  void merge(const RuntimeStatistics& other) {
    ...
    totalStrides += other.totalStrides;
