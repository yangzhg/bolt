...
class SpillerTest : public exec::test::RowContainerTestBase {
 ...
 protected:
  ...
  void testSortedSpillWithUring(
    ...
    setupSpiller(2'000'000, 0, makeError, 0, false, true);
    ...
    BOLT_ASSERT_THROW(spiller_->spill(0, nullptr), "Unexpected spiller type");
    BOLT_ASSERT_THROW(
    ...
    // finalized.
    BOLT_ASSERT_THROW(spiller_->spill(), "Spiller has been finalize");
    BOLT_ASSERT_THROW(spiller_->spill(0, nullptr), "Spiller has been finalize");
    BOLT_ASSERT_THROW(spiller_->spill(RowContainerIterator{}), "");

    verifySortedSpillData(&spillPartition, outputBatchSize, true);
  ...
  void setupSpillData(
    ...
    if (!numRowsPerPartition.empty()) {
      ...
      BOLT_CHECK_EQ(totalCount, numRows);
    }
  ...
  void setupSpiller(
    ...
    spillConfig_.spillUringEnabled = spillUringEnabled;
    ...
    if (type_ == Spiller::Type::kHashJoinProbe) {
      ...
      spiller_ = std::make_unique<Spiller>(
        ...
        type_ == Spiller::Type::kOrderByOutput) {
      spiller_ = std::make_unique<Spiller>(
          type_, rowContainer_.get(), rowType_, &spillConfig_);
    } else {
      BOLT_CHECK_EQ(type_, Spiller::Type::kHashJoinBuild);
      spiller_ = std::make_unique<Spiller>(
          type_,
          rowContainer_.get(),
          rowType_,
  ...
  void verifySortedSpillData(
    ...
    auto merge = spillPartition->createOrderedReader(pool(), spillUringEnabled);
    ASSERT_TRUE(merge != nullptr);
    ASSERT_TRUE(
        spillPartition->createOrderedReader(pool(), spillUringEnabled) ==
        nullptr);
  ...
  void splitByPartition(
    ...
    for (int partition = 0; partition < numPartitions_; ++partition) {
      if (numPartitionInputs_[partition] == 0) {
        ...
        inputsByPartition[partition].push_back(wrapAndCombineDict(
  ...
  void testNonSortedSpillWithUring(
      int numSpillers,
      int numBatchRows,
    ...
    for (int iter = 0; iter < numSpillers; ++iter) {
      ...
      setupSpillData(
          ...
          nullptr,
          {});
      setupSpiller(targetFileSize, 0, false, maxSpillRunRows, false, true);
    ...
    verifyNonSortedSpillData(
        std::move(spillers), spillPartitionNumSet, inputsByPartition, true);
  ...
  void verifyNonSortedSpillData(
    ...
    for (auto& spiller : spillers) {
      ...
      BOLT_ASSERT_THROW(spiller->spill(), "Spiller has been finalized");
    }
  ...
  void verifyNonSortedSpillData(
    ...
    BOLT_ASSERT_THROW(spiller_->spill(), "Spiller has been finalized");
    BOLT_ASSERT_THROW(spiller_->spill(RowContainerIterator{}), "");
    ASSERT_EQ(spillPartitionSet.size(), spillPartitionNumSet.size());
...
TEST_P(AllTypes, nonSortedSpillFunctionsWithUring) {
  struct {
    uint64_t maxSpillRunRows;
  ...
  for (const auto& testData : testSettings) {
    if (type_ == Spiller::Type::kOrderByInput ||
      ...
      sortSpillData();
      setupSpiller(100'000, 0, false, testData.maxSpillRunRows, false, true);
      {
      ...
      verifySortedSpillData(spillPartitionSet.begin()->second.get(), 0, true);
      continue;
    }
    testNonSortedSpillWithUring(2, 5'000, 3, 1, testData.maxSpillRunRows);
    testNonSortedSpillWithUring(
        2, 5'000, 3, 1'000'000'000, testData.maxSpillRunRows);
    // Empty case.
    testNonSortedSpillWithUring(1, 5'000, 0, 1, testData.maxSpillRunRows);
  }
}
...
TEST_P(NoHashJoin, spilFewWithUring) {
  // Test with distinct sort keys.
  testSortedSpillWithUring(10, 1);
  testSortedSpillWithUring(10, 1, false, false);
  testSortedSpillWithUring(10, 1, true);
  testSortedSpillWithUring(10, 1, true, false);
  // Test with duplicate sort keys.
  testSortedSpillWithUring(10, 10);
  testSortedSpillWithUring(10, 10, false, false);
  testSortedSpillWithUring(10, 10, true);
  testSortedSpillWithUring(10, 10, true, false);
}

TEST_P(NoHashJoin, spilMostWithUring) {
  // Test with distinct sort keys.
  testSortedSpillWithUring(60, 1);
  testSortedSpillWithUring(60, 1, false, false);
  testSortedSpillWithUring(60, 1, true);
  testSortedSpillWithUring(60, 1, true, false);
  // Test with duplicate sort keys.
  testSortedSpillWithUring(60, 10);
  testSortedSpillWithUring(60, 10, false, false);
  testSortedSpillWithUring(60, 10, true);
  testSortedSpillWithUring(60, 10, true, false);
}

TEST_P(NoHashJoin, spillAllWithUring) {
  // Test with distinct sort keys.
  testSortedSpillWithUring(100, 1);
  testSortedSpillWithUring(100, 1, false, false);
  testSortedSpillWithUring(100, 1, true);
  testSortedSpillWithUring(100, 1, true, false);
  // Test with duplicate sort keys.
  testSortedSpillWithUring(100, 10);
  testSortedSpillWithUring(100, 10, false, false);
  testSortedSpillWithUring(100, 10, true);
  testSortedSpillWithUring(100, 10, true, false);
}

TEST_P(NoHashJoin, errorWithUring) {
  testSortedSpillWithUring(100, 1, 0, true);
}
...
TEST_P(HashJoinBuildOnly, spillPartitionWithUring) {
  ...
  setupSpiller(100'000, 0, false, 0, false, true);
  spiller_->spill();
  rowContainer_->clear();
  spiller_->spill();
  verifyNonSortedSpillData(allPartitionNumSet(), vectorsByPartition, true);
  BOLT_ASSERT_THROW(spiller_->spill(), "Spiller has been finalized");
  BOLT_ASSERT_THROW(spiller_->spill(RowContainerIterator{}), "");
}

TEST_P(HashJoinBuildOnly, writeBufferSizeWithUring) {
  ...
  for (const auto writeBufferSize : writeBufferSizes) {
    ...
    setupSpillData(rowType_, numKeys_, 1'000, 1, nullptr, {});
    setupSpiller(4'000'000'000, writeBufferSize, false, 0, false, true);
    spiller_->spill();
    ASSERT_TRUE(spiller_->isAllSpilled());
    ...
    SpillPartitionNumSet spillPartitionSet(
        spillPartitionNums.begin(), spillPartitionNums.end());
    verifyNonSortedSpillData(
        spillPartitionSet, expectedVectorByPartition, true);

    const auto stats = spiller_->stats();
    if (writeBufferSize > 0) {
      // With disk write buffering, all the input merged into one disk write
      // per partition.
      ASSERT_EQ(stats.spillWrites, numNonEmptySpilledPartitions);
    } else {
      // By disable write buffering, then each spill input causes a disk
      // write.
...
TEST_P(AggregationOutputOnly, basicWithUring) {
  ...
  struct {
    ...
    std::string debugString() const {
      return fmt::format(
          "spillRowOffset {}, maxSpillRunRows {}, spillUringEnabled {}",
          spillRowOffset,
          maxSpillRunRows,
          spillUringEnabled);
    }
  ...
  for (const auto& testData : testSettings) {
    ...
    auto merge =
        spillPartition.createOrderedReader(pool(), testData.spillUringEnabled);
...
TEST_P(OrderByOutputOnly, basicWithUring) {
  const int numRows = 5'000;
  struct {
    int numSpillRows;
    bool spillUringEnabled = true;
    std::string debugString() const {
      return fmt::format(
          "numSpillRows {} spillUringEnabled {}",
          numSpillRows,
          spillUringEnabled);
    }
  ...
  for (const auto& testData : testSettings) {
    ...
    sortSpillData();
    // NOTE: target file size is ignored by aggregation output spiller type.
    setupSpiller(0, 1'000'000, false, 0, false, testData.spillUringEnabled);
    RowContainerIterator rowIter;
    std::vector<char*> rows(numRows);
    int numListedRows{0};
    numListedRows =
...
class RowBasedSpillTest : public SpillerTest,
                          public testing::WithParamInterface<TestParam> {
 public:
  RowBasedSpillTest() : SpillerTest(GetParam()) {}
...
TEST_P(RowBasedSpillTest, withUring) {
  const int numRows = 5'000;

  setupSpillData(rowType_, numKeys_, numRows, 0);
  sortSpillData();
  // NOTE: target file size is ignored by aggregation output spiller type.
  setupSpiller(0, 1'000'000, false, 0, true, true);
  RowContainerIterator rowIter;
  std::vector<char*> rows(numRows);
  int numListedRows{0};
  ...
  auto spillPartition = spiller_->finishSpill();
  ASSERT_TRUE(spiller_->finalized());
  std::vector<std::unique_ptr<RowBasedSpillReadFile>> readerFiles;
  readerFiles.reserve(spillPartition.numFiles());
  for (auto& fileInfo : spillPartition.files()) {
    readerFiles.push_back(
        RowBasedSpillReadFile::create(fileInfo, pool(), true));
  }
  ...
  while (readerFiles[0]->nextBatch(readRows)) {
    for (auto row : readRows) {
      ...
      for (auto i = 0; i < rowContainer_->columns().size(); ++i) {
        ...
        auto readColumn = spillPartition.files()[0].rowInfo->rowColumns[i];
        ASSERT_EQ(writeColumn.offset(), readColumn.offset());
        ASSERT_EQ(writeColumn.nullByte(), readColumn.nullByte());
        ASSERT_EQ(writeColumn.nullMask(), readColumn.nullMask());
        auto type = rowType_->childAt(i);
        if (type->isFixedWidth()) {
          ASSERT_EQ(
              rowContainer_->compare(row, writeRow, i, CompareFlags()), 0);
        } else {
          bool isNull = RowContainer::isNullAt(row, readColumn);
          bool isOtherNull = RowContainer::isNullAt(writeRow, writeColumn);
          ASSERT_EQ(isNull, isOtherNull);
          ...
          if (type->kind() == TypeKind::VARCHAR ||
            ...
            if (sv.isInline() ||
              ...
              s.resize(sv.size());
              stream->readBytes(s.data(), sv.size());
            }
            ASSERT_EQ(s, readSV.getString());
          } else {
            const std::string_view& sv = *reinterpret_cast<std::string_view*>(
                writeRow + writeColumn.offset());
            const std::string_view& readSV =
                *reinterpret_cast<std::string_view*>(row + readColumn.offset());
            std::string s;
            auto stream = HashStringAllocator::prepareRead(
                HashStringAllocator::headerOf(sv.data()));
            s.resize(sv.size());
...
TEST_P(MaxSpillRunTest, basicWithUring) {
  struct {
    uint64_t maxSpillRunRows;
