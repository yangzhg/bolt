...
void DecodedVector::makeIndices(
    ...
    int32_t numLevels) {
  if (rows) {
    BOLT_CHECK_LE(rows->end(), vector.size());
  }
...
DecodedVector::DictionaryWrapping DecodedVector::dictionaryWrapping(
  ...
  BOLT_CHECK(!isIdentityMapping_);
  BOLT_CHECK(!isConstantMapping_);
  BOLT_CHECK_LE(size, size_);
  ...
  if (hasExtraNulls_) {
    ...
    if (isIdentityMapping_) {
      ...
      if (rows != nullptr) {
        ...
        // end but not greater.
        BOLT_CHECK_LE(rows->end(), size_);
      }
      auto baseSize = baseVector_->size();
      applyToRows(rows, [&](auto i) {
        BOLT_DCHECK_LT(indices_[i], baseSize);
