...
bool ByteInputStream::atEnd() const {
  ...
  BOLT_CHECK(current_ >= ranges_.data() && current_ <= &ranges_.back());
...
std::streampos ByteInputStream::tellp() const {
  ...
  BOLT_FAIL("ByteInputStream 'current_' is not in 'ranges_'.");
}
...
void ByteInputStream::next(bool throwIfPastEnd) {
  BOLT_CHECK(current_ >= &ranges_[0]);
  size_t position = current_ - &ranges_[0];
  BOLT_CHECK_LT(position, ranges_.size());
  if (position == ranges_.size() - 1) {
    if (throwIfPastEnd) {
      BOLT_FAIL("Reading past end of ByteInputStream");
    }
    return;
  }
  ++current_;
...
std::streampos ByteOutputStream::tellp() const {
  ...
  BOLT_FAIL("ByteOutputStream 'current_' is not in 'ranges_'.");
}
...
void ByteOutputStream::extend(int32_t bytes) {
  ...
  BOLT_CHECK_GT(allocatedBytes_, 0);
  if (isBits_) {
...
std::unique_ptr<ByteInputStream> ByteOutputStream::inputStream() const {
  BOLT_CHECK(!ranges_.empty());
  updateEnd();
  auto rangeCopy = ranges_;
  rangeCopy.back().size = lastRangeEnd_;
  return std::make_unique<ByteInputStream>(std::move(rangeCopy));
}
