...
TEST_F(HiveDataSinkTest, basic) {
  ...
  ASSERT_EQ(
      stats.toString(),
      "numWrittenBytes 0B numWrittenFiles 0 spillRuns[0] spilledInputBytes[0B] spilledBytes[0B] spilledRows[0] spilledPartitions[0] spilledFiles[0] spillTotalTimeUs[0us] spillFillTimeUs[0us] spillSortTime[0us] spillConvertTime[0us] spillSerializationTime[0us] spillWrites[0] spillFlushTime[0us] spillWriteTime[0us] maxSpillExceededLimitCount[0]");
...
TEST_F(HiveDataSinkTest, close) {
  for (bool empty : {true, false}) {
    ...
    BOLT_ASSERT_THROW(dataSink->close(), "Hive data sink is not running");
    BOLT_ASSERT_THROW(dataSink->abort(), "Hive data sink is not running");
...
DEBUG_ONLY_TEST_F(HiveDataSinkTest, sortWriterFailureTest) {
  ...
  SCOPED_TESTVALUE_SET(
      ...
      std::function<void(memory::MemoryPool*)>(
          [&](memory::MemoryPool* pool) { BOLT_FAIL("inject failure"); }));

  BOLT_ASSERT_THROW(dataSink->close(), "inject failure");
}
} // namespace
} // namespace bytedance::bolt::connector::hive
