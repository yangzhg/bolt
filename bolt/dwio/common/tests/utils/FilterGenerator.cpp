...
#include "bolt/dwio/common/tests/utils/FilterGenerator.h"
  ...
  for (int i = 1; i < path.size(); ++i) {
    switch (type->kind()) {
      case TypeKind::ROW:
        ...
        break;
      case TypeKind::ARRAY:
        BOLT_CHECK(dynamic_cast<const Subfield::AllSubscripts*>(path[i].get()));
...
std::unique_ptr<Filter> ColumnStats<bool>::makeRangeFilter(
  ...
  if (values_.empty()) {
    return std::make_unique<bolt::common::IsNull>();
  }
  bool value = valueAtPct(filterSpec.startPct + filterSpec.selectPct);
  return std::make_unique<bolt::common::BoolValue>(
...
std::unique_ptr<Filter> ColumnStats<float>::makeRangeFilter(
  ...
  if (lowerUnbounded && upperUnbounded) {
    return std::make_unique<bolt::common::IsNotNull>();
  }

  return std::make_unique<bolt::common::FloatRange>(
      lower,
      lowerUnbounded,
      false,
      upper,
...
std::unique_ptr<Filter> ColumnStats<double>::makeRangeFilter(
  ...
  if (values_.empty()) {
    if (filterSpec.allowNulls_) {
      ...
      return std::make_unique<bolt::common::DoubleRange>(
...
std::unique_ptr<Filter> ColumnStats<int128_t>::makeRangeFilter(
  ...
  return std::make_unique<bolt::common::HugeintRange>(
...
std::unique_ptr<Filter> ColumnStats<StringView>::makeRangeFilter(
  ...
  if (filterSpec.selectPct == 0) {
    return std::make_unique<bolt::common::BytesRange>(
        std::string(lower),
        false,
        true,
        std::string(upper),
...
std::unique_ptr<Filter> ColumnStats<StringView>::makeRandomFilter(
  ...
  if (upperIndex - lowerIndex < 1000 && ++counter_ % 10 <= 3) {
    ...
    if (counter_ % 2 == 0 && filterSpec.selectPct != 100.0) {
      return std::make_unique<bolt::common::NegatedBytesValues>(
          inRange, filterSpec.selectPct < 75);
    }
    return std::make_unique<bolt::common::BytesValues>(
        inRange, filterSpec.selectPct > 25);
  }

  // sometimes create a negated filter instead
  if (counter_ % 4 == 1 && filterSpec.selectPct < 100.0) {
    return std::make_unique<bolt::common::NegatedBytesRange>(
        std::string(lower),
        false,
        false,
        std::string(upper),
...
std::unique_ptr<Filter> ColumnStats<Timestamp>::makeRangeFilter(
  ...
  return std::make_unique<bolt::common::TimestampRange>(
...
std::unique_ptr<Filter> ColumnStats<Timestamp>::makeRowGroupSkipRangeFilter(
  ...
  return std::make_unique<bolt::common::TimestampRange>(max, max, false);
}
