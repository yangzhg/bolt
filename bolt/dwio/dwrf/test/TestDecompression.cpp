...
TEST_F(DecompressionTest, testLzoLong) {
  ...
  bzero(buffer, BOLT_ARRAY_SIZE(buffer));
  // header
  buffer[0] = 190;
  buffer[1] = 3;
...
TEST_F(DecompressionTest, testLz4Long) {
  ...
  memset(buffer, 255, BOLT_ARRAY_SIZE(buffer));
  // header
  buffer[0] = 38;
  buffer[1] = 3;
  buffer[2] = 0;
...
TEST_F(DecompressionTest, testLiteralBlocks) {
  ...
  std::unique_ptr<SeekableInputStream> result = createTestDecompressor(
      CompressionKind_ZLIB,
      std::unique_ptr<SeekableInputStream>(
          new SeekableArrayInputStream(buffer, BOLT_ARRAY_SIZE(buffer), 5)),
      5);
  EXPECT_EQ(
...
TEST_F(DecompressionTest, testInflateSequence) {
  ...
  std::unique_ptr<SeekableInputStream> result = createTestDecompressor(
      CompressionKind_ZLIB,
      std::unique_ptr<SeekableInputStream>(
          new SeekableArrayInputStream(buffer, BOLT_ARRAY_SIZE(buffer), 3)),
      1000);
  const void* ptr;
  int32_t length;
...
TEST_F(DecompressionTest, testDelayedSkip) {
  ...
  ASSERT_THROW(result->Next(&data, &length), BoltException);
}
...
class TestSeek : public ::testing::Test {
 public:
  ...
  void runTest(Codec& codec, CompressionKind kind) {
    ...
    for (size_t i = 0; i < BOLT_ARRAY_SIZE(arr); ++i) {
      auto pos = arr[i];
...
class TestingSeekableInputStream : public SeekableInputStream {
 public:
  ...
  void BackUp(int32_t count) override {
    BOLT_CHECK_LE(count, lastSize_);
    position_ -= count;
  }

  bool SkipInt64(int64_t count) override {
    BOLT_CHECK_LE(count + position_, length_);
    position_ += count;
    return true;
  }
