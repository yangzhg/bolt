...
struct DoublerGenerator {
  int operator()(const int& value, const Options&) {
    ++generated_;
    return value * 2;
  }
...
TEST(CachedFactoryTest, clearCache) {
  ...
  EXPECT_EQ(factory.generate(1, ops), cacheMiss(2));
}
...
TEST(CachedFactoryTest, basicExceptionHandling) {
  ...
  CachedFactory<int, Options, int, DoublerWithExceptionsGenerator> factory(
      std::make_unique<SimpleLRUCache<int, int>>(1000), std::move(generator));
  struct Options ops;
  auto val1 = factory.generate(1, ops);
  ...
  EXPECT_EQ(*generated, 1);
  try {
    auto val2 = factory.generate(3, ops);
    FAIL() << "Factory generation should have failed";
  } catch (const std::invalid_argument& e) {
    // Expected.
  }
  val1 = factory.generate(4, ops);
  EXPECT_EQ(getCachedValue(val1), 8);
  EXPECT_EQ(*generated, 2);
  val1 = factory.generate(1, ops);
  EXPECT_EQ(getCachedValue(val1), 2);
  EXPECT_EQ(*generated, 2);
}
...
TEST(CachedFactoryTest, multiThreadedGeneration) {
  ...
  for (int i = 0; i < requestsPerValue; i++) {
    for (int j = 0; j < numValues; j++) {
      pool.add([&, j]() {
        auto value = factory.generate(j, ops);
...
TEST(CachedFactoryTest, multiThreadedGenerationAgain) {
  ...
  for (int i = 0; i < requestsPerValue; i++) {
    for (int j = 0; j < numValues; j++) {
      pool.add([&factory, &latch, &cachedValues, i, j, ops]() {
        cachedValues[i * numValues + j] = factory.generate(j, ops);
        latch.count_down();
      });
    }
  }
