...
TEST_F(ExprCompilerTest, concatArrayBigIntFlattening) {
  // Verify that array concat is flattened only if all its inputs are of the
  // same type.
  auto rowType =
      ROW({"array1", "array2", "bigint"},
  ...
  expression = concatCall(
      {field("array1"),
       concatCall(
           {field("array2"), concatCall({field("array2"), field("bigint")})})});
  ASSERT_EQ(
      "concat(array1, array2, concat(array2, bigint))",
      compile(expression)->toString());
}

TEST_F(ExprCompilerTest, concatOneArrayBigIntFlattening) {
  // Verify that array concat is flattened only if all its inputs are of the
  // same type.
  auto rowType =
      ROW({"array1", "array2", "bigint"},
  ...
  auto expression = concatCall({field("array1")});
  ASSERT_EQ("concat(array1)", compile(expression)->toString());
}
