...
class InputByteStreamTest : public ByteStreamTest,
 ...
 protected:
  ...
  std::unique_ptr<ByteInputStream> createStream(
    ...
    if (GetParam()) {
      return std::make_unique<ByteInputStream>(std::move(byteRanges));
    } else {
      const auto filePath =
...
TEST_P(InputByteStreamTest, inputStream) {
  ...
  if (GetParam()) {
    BOLT_ASSERT_THROW(
        byteStream->skip(1), "Reading past end of ByteInputStream");
  } else {
    BOLT_ASSERT_THROW(
        byteStream->skip(1),
...
TEST_P(InputByteStreamTest, skipNegativeSize) {
  constexpr int32_t kBufferSize = 4096;
  uint8_t buffer[kBufferSize];
  auto byteStream = std::make_unique<ByteInputStream>(
...
TEST_P(InputByteStreamTest, view) {
  SCOPED_TRACE(fmt::format("ByteInputStream: {}", GetParam()));
...
TEST_P(InputByteStreamTest, skip) {
  ...
  if (GetParam()) {
    BOLT_ASSERT_THROW(
        byteStream->skip(kBufferSize), "Reading past end of ByteInputStream");
    ASSERT_EQ(byteStream->remainingSize(), 0);
    ASSERT_TRUE(byteStream->atEnd());
  } else {
    BOLT_ASSERT_THROW(
        byteStream->skip(kBufferSize),
        "Skip past the end of FileInputStream: 4096");
    ASSERT_EQ(byteStream->remainingSize(), kBufferSize - 1);
    ASSERT_FALSE(byteStream->atEnd());
  }
}
