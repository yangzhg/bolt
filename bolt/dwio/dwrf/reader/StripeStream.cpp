...
#include "bolt/common/base/BitSet.h"
#include "bolt/dwio/common/exception/Exception.h"
#include "bolt/dwio/dwrf/common/DecoderUtil.h"
#include "bolt/dwio/dwrf/common/wrap/coded-stream-wrapper.h"
...
void findProjectedNodes(
  ...
  switch (actual.type()->kind()) {
    case TypeKind::ROW: {
      if (matchStructsByName) {
        const auto& expectedRow = expected.type()->asRow();
        const auto& actualRow = actual.type()->asRow();
        for (uint64_t i = 0; i < expected.size(); ++i) {
          const auto& childName = expectedRow.nameOf(i);
          if (!actualRow.containsChild(childName)) {
            continue;
          }
          findProjectedNodes(
              projectedNodes,
              *expected.childAt(i),
              *actual.childByName(childName),
...
StripeStreamsBase::getIntDictionaryInitializerForNode(
  ...
  stripeDictionaryCache_->registerIntDictionary(
      ...
      [dictReader = createDirectDecoder</* isSigned = */ true>(
       ...
       dictionaryWidth,
       dictionarySize](bolt::memory::MemoryPool* pool) mutable {
        return BOLT_WIDTH_DISPATCH(
...
void StripeStreamsImpl::loadStreams() {
  ...
  const bool matchStructsByName = opts_.getUseColumnNamesForColumnMapping();
  findProjectedNodes(
      projectedNodes,
      *expected,
      *actual,
      [&](uint32_t node) { return selector_.shouldReadNode(node); },
      matchStructsByName);
