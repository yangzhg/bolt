...
class PARQUET_EXPORT PageWriter {
 public:
  ...
  // Return accurate uncompressed bytes written (including header size)
  virtual int64_t Close(bool has_dictionary, bool fallback) = 0;

  // Return uncompressed bytes written
  // including header size which is imprecise for BufferedPageWriter
  virtual int64_t WriteDataPage(std::unique_ptr<DataPage>) = 0;

  // Return inaccurate uncompressed bytes written
  // including header size which is imprecise for BufferedPageWriter
  virtual int64_t WriteDictionaryPage(std::unique_ptr<DictionaryPage>) = 0;
