...
RowNumber::RowNumber(
  ...
  const bool enableJitRowEqVectors = false; // support it later
  if (numKeys > 0) {
    ...
    lookup_ =
        std::make_unique<HashLookup>(table_->hashers(), enableJitRowEqVectors);
...
RowVectorPtr RowNumber::getOutputForSinglePartition() {
  ...
  if (limit_) {
    BOLT_CHECK_LT(numTotalInput_, limit_.value());
    numOutput =
...
void RowNumber::setupHashTableSpiller() {
  ...
  hashTableSpiller_ = std::make_unique<Spiller>(
      ...
      false);
  hashTableSpiller_->setSpillConfig(&spillConfig);
}
...
void RowNumber::setupInputSpiller() {
  ...
  inputSpiller_ = std::make_unique<Spiller>(
      ...
      &spillConfig,
      spillConfig.maxFileSize);
  inputSpiller_->setSpillConfig(&spillConfig);
...
void RowNumber::spill() {
  ...
  BOLT_CHECK_NULL(hashTableSpiller_);
  BOLT_CHECK_NULL(inputSpiller_);
...
void RowNumber::spillInput(
  ...
  for (int32_t partition = 0; partition < numSpillInputs.size(); ++partition) {
    ...
    inputSpiller_->spill(
        partition,
        wrapAndCombineDict(numInputs, partitionIndices[partition], input));
  }
}
