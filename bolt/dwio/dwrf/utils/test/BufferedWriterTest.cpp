...
#include "bolt/dwio/dwrf/utils/BufferedWriter.h"
...
TEST_P(BufferedWriterTest, flushFailureTest) {
  ...
  BOLT_ASSERT_THROW(writer->flush(), "flushFailureTest");
  writer->close();
}
...
TEST_P(BufferedWriterTest, closeTest) {
  ...
  BOLT_ASSERT_THROW(writer->close(), "");
  BOLT_ASSERT_THROW(writer->abort(), "");
  BOLT_ASSERT_THROW(writer->add('a'), "");
  BOLT_ASSERT_THROW(writer->flush(), "");
}
