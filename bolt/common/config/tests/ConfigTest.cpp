...
TEST_F(ConfigTest, immutableConfig) {
  ...
  BOLT_ASSERT_THROW(config->set(TestConfig::kInt32Entry, 100), "Cannot set");
  BOLT_ASSERT_THROW(
      config->set(TestConfig::kInt32Entry.key, "100"), "Cannot set");
  BOLT_ASSERT_THROW(config->unset(TestConfig::kInt32Entry), "Cannot unset");
  BOLT_ASSERT_THROW(config->reset(), "Cannot reset");
