...
TEST_F(GenericWriterTest, castToDifferentTypesNotSupported) {
  ...
  EXPECT_THROW(writer.current().castTo<Array<int64_t>>(), BoltException);
...
TEST_F(GenericWriterTest, arrayIntCast) {
  ...
  ASSERT_NO_THROW(current.tryCastTo<double>());
  ASSERT_TRUE(current.tryCastTo<double>() == nullptr);
  ASSERT_THROW(current.tryCastTo<Array<Any>>(), BoltUserError);
}
...
TEST_F(GenericWriterTest, row) {
  ...
  *rowWriter->get_writer_at<1>().tryCastTo<int64_t>() = 3 * 2 + 1;
  ...
  writer.setOffset(0);
  auto& current = writer.current();
  ASSERT_THROW(current.castTo<Row<Any>>(), BoltUserError);

  // Casting to DynamicRow after casting to Row<Any, ...> is not allowed.
  ASSERT_THROW(current.castTo<DynamicRow>(), BoltUserError);
  ASSERT_THROW(current.tryCastTo<DynamicRow>(), BoltUserError);
}
...
TEST_F(GenericWriterTest, dynamicRow) {
  ...
  *rowWriter->get_writer_at(0).tryCastTo<int64_t>() = 4 * 2;
  ...
  writer.setOffset(0);
  auto& current = writer.current();
  ASSERT_THROW((current.castTo<Row<Any, Any>>()), BoltUserError);
  ASSERT_THROW((current.tryCastTo<Row<Any, Any>>()), BoltUserError);
  ...
  ASSERT_THROW(typedCurrent.get_writer_at(2), BoltUserError);
  ASSERT_THROW(typedCurrent.set_null_at(2), BoltUserError);
...
TEST_F(GenericWriterTest, copyFromGeneric) {
  bytedance::bolt::functions::prestosql::registerGeneralFunctions();
  registerFunction<
      SubscriptSimpleFunc,
      Generic<T1>,
      Array<Generic<T1>>,
