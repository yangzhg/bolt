...
std::string SubstraitParser::mapToBoltFunction(
    const std::string& substraitFunction) const {
  auto it = substraitBoltFunctionMap_.find(substraitFunction);
  if (it != substraitBoltFunctionMap_.end()) {
    return it->second;
  }

  // If not finding the mapping from Substrait function name to Bolt function
