...
class QueryCtx : public std::enable_shared_from_this<QueryCtx> {
 public:
  ...
  const std::any& customContext() const {
  ...
  bool testingUnderArbitration() const {
    ...
    BOLT_CHECK(underArbitration_ || arbitrationPromises_.empty());
...
class ExecCtx {
 public:
  ...
  std::unique_ptr<SelectivityVector> getSelectivityVector(int32_t size) {
    BOLT_CHECK(exprEvalCacheEnabled_ || selectivityVectorPool_.empty());
  ...
  std::unique_ptr<DecodedVector> getDecodedVector() {
    BOLT_CHECK(exprEvalCacheEnabled_ || decodedVectorPool_.empty());
