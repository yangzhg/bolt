...
struct AsciiFunction {
  ...
  FOLLY_ALWAYS_INLINE void call(int32_t& result, const arg_type<Varchar>& s) {
    ...
    int size;
    result =
        stringCore::utf8proc_codepoint(s.data(), s.data() + s.size(), &size);
  }
...
struct SpaceFunction {
  BOLT_DEFINE_FUNCTION_TYPES(T);
  FOLLY_ALWAYS_INLINE void call(out_type<Varchar>& result, const int32_t& n) {
    if (n <= 0) {
      result.setEmpty();
      return;
    }
...
struct LocateFunction {
  ...
  FOLLY_ALWAYS_INLINE void call(
      ...
      const arg_type<Varchar>& string) {
    callAscii(result, subString, string, 1);
  }
  ...
  FOLLY_ALWAYS_INLINE void call(
    ...
    if (count > 0) {
      index = stringImpl::stringPosition<true, true>(
          std::string_view(str), std::string_view(delim), count);
    } else {
  ...
  FOLLY_ALWAYS_INLINE void call(
    ...
    if constexpr (leftTrim) {
      while (resultBegin < srcStr.end()) {
        int charLen = stringCore::utf8proc_char_length(resultBegin);
    ...
    if constexpr (rightTrim) {
      ...
      while (curPos >= resultBegin) {
        if (stringCore::utf8proc_char_first_byte(curPos)) {
...
/// left and right base function
/// left/right(string, length) -> string
/// Returns the leftmost/rightmost length characters from the string
/// Return an empty string if length is less or equal than 0
template <typename T, bool isRight>
struct LeftRightFunctionImp {
  BOLT_DEFINE_FUNCTION_TYPES(T);
...
template <typename T>
struct LeftFunction : public LeftRightFunctionImp<T, false> {};

template <typename T>
struct RightFunction : public LeftRightFunctionImp<T, true> {};
...
struct TranslateFunction {
  ...
  folly::F14FastMap<std::string, std::string> buildUnicodeDictionary(
    ...
    while (i < match.size()) {
      ...
      if (j < replace.size()) {
        int replaceCharLength =
            stringCore::utf8proc_char_length(replace.data() + j);
        replaceChar = std::string(replace.data() + j, replaceCharLength);
        j += replaceCharLength;
      }
      int matchCharLength = stringCore::utf8proc_char_length(match.data() + i);
  ...
  FOLLY_ALWAYS_INLINE void call(
    ...
    int k = 0;
    while (k < input.size()) {
      int inputCharLength = stringCore::utf8proc_char_length(input.data() + k);
...
struct ConvFunction {
  ...
  // note the resultSize may be larger than the actual size of the result.
  static std::pair<uint64_t, int32_t> getUnsignedValueAndResultSize(
    ...
    int32_t resultSize =
        (int32_t)std::ceil(std::log(unsignedValue) / std::log(toBase)) + 1;
  ...
  static void toChars(
    ...
    BOLT_CHECK(toStatus.ec == std::errc());
...
///
/// uuid(seed) -> varchar
/// returns a random UUID as a string with the given seed

struct UUIDFunction {
  static constexpr uint8_t kUUIDLength = 36;

 public:
  void initializeGenerator(uint64_t seed) {
    generator_ = std::mt19937_64{};
    generator_.seed(seed);
  }
  void generateUuid(exec::StringWriter<false>& result) {
    result.resize(kUUIDLength);
    bolt::makeUuid(result.data(), generator_);
  }
...
template <typename T>
struct UUIDWithSeedFunction : public UUIDFunction {
  BOLT_DEFINE_FUNCTION_TYPES(T);
  ...
  void initialize(
    ...
    BOLT_CHECK_NOT_NULL(seedInput, "seed argument must be constant");
    auto partitionId = config.sparkPartitionId();
    uint64_t seed = seedInput ? *seedInput : 0;
    initializeGenerator(seed + partitionId);
  }
...
template <typename T>
struct UUIDWithoutSeedFunction : public UUIDFunction {
  BOLT_DEFINE_FUNCTION_TYPES(T);
  ...
  FOLLY_ALWAYS_INLINE void initialize(
    ...
    initializeGenerator(rd());
  }

  FOLLY_ALWAYS_INLINE void call(exec::StringWriter<false>& result) {
    generateUuid(result);
  }
};

/// toTitle|InitCap function
/// toTitle(str) -> varchar
/// toInitCap(str) -> varchar
/// returns the given string with the first letter of each word capitalized
template <typename T>
struct ToTitleFunction {
  BOLT_DEFINE_FUNCTION_TYPES(T);
  FOLLY_ALWAYS_INLINE void call(
      out_type<Varchar>& result,
      const arg_type<Varchar>& str) {
    result.append(stringCore::toTitle(str.data(), str.size()));
  }
};
...
/// Bolt Presto LevenshteinDistanceFunction.
template <typename T>
