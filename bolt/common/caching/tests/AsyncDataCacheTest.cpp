...
#include "bolt/common/base/Semaphore.h"
...
using bytedance::bolt::memory::MemoryAllocator;
...
class TestingCoalescedLoad : public CoalescedLoad {
 public:
  ...
  std::vector<CachePin> loadData(bool /*isPrefetch*/) override {
    ...
    BOLT_CHECK(!injectError_, "Testing error");
    return pins;
  }
...
void AsyncDataCacheTest::loadOne(
  ...
  for (;;) {
    ...
    if (entry->isShared()) {
      ...
      checkContents(*entry);
      BOLT_CHECK(!injectError, "Testing error");
      return;
    }
...
TEST_F(AsyncDataCacheTest, pin) {
  ...
  CachePin otherPin;
  EXPECT_THROW(otherPin = pin, BoltException);
...
TEST_F(AsyncDataCacheTest, shrinkCache) {
  ...
  for (const auto& testData : testSettings) {
    ...
    BOLT_ASSERT_THROW(cache_->shrink(0), "");
...
DEBUG_ONLY_TEST_F(AsyncDataCacheTest, shrinkWithSsdWrite) {
  ...
  SCOPED_TESTVALUE_SET(
      "bytedance::bolt::cache::SsdCache::write",
