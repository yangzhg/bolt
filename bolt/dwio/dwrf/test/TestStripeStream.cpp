...
#include "bolt/dwio/dwrf/writer/WriterBase.h"
...
using bytedance::bolt::RowType;
using bytedance::bolt::type::fbhive::HiveTypeParser;

class RecordingInputStream : public bytedance::bolt::InMemoryReadFile {
 public:
...
TEST_F(StripeStreamTest, readEncryptedStreams) {
  ...
  *stripeFooter->add_encryptiongroups() = "";
  ...
  auto readerBase = std::make_shared<ReaderBase>(
      *readerPool,
      std::make_unique<BufferedInput>(
          std::make_shared<bytedance::bolt::InMemoryReadFile>(std::string()),
          *readerPool),
      std::make_unique<PostScript>(std::move(ps)),
      footer,
      nullptr,
...
class TestStripeStreams : public StripeStreamsBase {
 public:
  ...
  const bytedance::bolt::dwio::common::ColumnSelector& getColumnSelector()
      const override {
    BOLT_UNSUPPORTED();
  }

  const bytedance::bolt::dwio::common::RowReaderOptions& getRowReaderOptions()
      const override {
    BOLT_UNSUPPORTED();
  }
...
TEST_F(StripeStreamTest, shareDictionary) {
  ...
  bytedance::bolt::memory::AllocationPool allocPool(pool_.get());
  StreamLabels labels(allocPool);
  std::vector<std::function<bytedance::bolt::BufferPtr()>> dictInits{};
  dictInits.push_back(
