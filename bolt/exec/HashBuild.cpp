...
HashBuild::HashBuild(
    ...
    : Operator(
      ...
      supportRangePartitionSkewed_(
          driverCtx->queryConfig().isHashJoinSkewedPartitionEnabled()),
      skewFileSizeRatioThreshold_(
          driverCtx->queryConfig().skewFileSizeRatioThreshold()),
      skewRowCountRatioThreshold_(
...
void HashBuild::setupTable() {
  ...
  if (joinNode_->isRightJoin() || joinNode_->isFullJoin() ||
    ...
    table_ = HashTable<false>::createForJoin(
        std::move(keyHashers),
        dependentTypes,
        isDREnabled_ ? false : true, // allowDuplicates
        true, // hasProbedFlag
...
void HashBuild::setupSpiller(
  ...
  if (canUseRowBasedSpill) {
    *const_cast<common::RowBasedSpillMode*>(&spillConfig.rowBasedSpillMode) =
        common::strToRowBasedSpillMode(
            operatorCtx_->driverCtx()->queryConfig().rowBasedSpillMode());
    canUseRowBasedSpill =
        spillConfig.rowBasedSpillMode != common::RowBasedSpillMode::DISABLE;
    if (canUseRowBasedSpill) {
      rowFormatInfo_ = std::make_shared<RowFormatInfo>(
          table_->rows(),
          spillConfig.rowBasedSpillMode ==
              common::RowBasedSpillMode::COMPRESSION);

      LOG(INFO) << name() << " HashBuild Setup rowBasedSpillMode "
                << operatorCtx_->driverCtx()->queryConfig().rowBasedSpillMode()
                << " for " << operatorCtx_->toString();
    }
  }
  ...
  if (spillPartition == nullptr) {
    ...
    offsetTojoinBits_[spillConfig.startPartitionBit] =
        spillConfig.joinPartitionBits;
  } else {
    LOG(INFO) << name() << " HashBuild Setup reader to read spilled input from "
              << spillPartition->toString()
              << ", memory pool: " << pool()->name();

    restoringRangePartition_ = spillPartition->id().isSubRangePartiton();
    spillInputReader_ = spillPartition->createUnorderedReader(
        pool(), spillConfig.spillUringEnabled, canUseRowBasedSpill);

    const auto bitOffset = spillPartition->id().partitionBitOffset();
    auto search = offsetTojoinBits_.find(bitOffset);
    BOLT_CHECK(search != offsetTojoinBits_.end());

    const auto partitionBits = search->second;
    const auto startBit = bitOffset + partitionBits;

    if (hasBeenCalculated_.find(startBit) == hasBeenCalculated_.end()) {
      calculateJoinBits<true>(64 - startBit, maxPartitionRowCount);
      offsetTojoinBits_.emplace(startBit, spillConfig.joinRepartitionBits);
      hasBeenCalculated_.insert(startBit);
    }
    ...
    if (spillConfig.exceedJoinSpillLevelLimit(offsetTojoinBits_)) {
...
void HashBuild::setupFilterForAntiJoins(
  ...
  BOLT_DCHECK_EQ(exprs.exprs().size(), 1);
...
void HashBuild::addSpilledRowInput(std::vector<char*>& rows, uint32_t size) {
  ...
  ensureInputFits(input_, {&rows, size});
  auto spillStep = 1024;
  auto container = table_->rows();
  for (auto id = 0; id < rows.size(); id += spillStep) {
    auto step = std::min<int>(spillStep, rows.size() - id);
    activeRows_.resize(step);
    activeRows_.setAll();
    for (auto j = id; j < id + step; ++j) {
      container->copySerializedRow(rows[j], rowFormatInfo_.get());
    }
    spillRowBasedInput();
  }
}
...
void HashBuild::addInput(RowVectorPtr input) {
  ...
  if (!ensureInputFits(input, {nullptr, 0})) {
  ...
  if (dropDuplicates_ && !abandonBuildNoDupHash_) {
    ...
    if (abandonEarly) {
      ...
      // RowContainer.
      addRuntimeStat("earlyAbandonDeduplicateInHashBuild", RuntimeCounter(1));
...
bool HashBuild::ensureInputFits(RowVectorPtr& input, SpilledRows spilledRows) {
  ...
  if (!reserveMemory(input, spilledRows)) {
    if (isFirstSpill_) {
      isFirstSpill_ = false;
      if (calculateJoinBits<false>()) {
        spiller_.reset();
        setupSpiller();
      }
      spiller_->setMemoryUsedTriggered(pool()->currentBytes());
      auto* spillConf = const_cast<common::SpillConfig*>(spillConfig());
...
bool HashBuild::reserveMemory(
  ...
  bool rowBasedSpill = spilledRows.rows != nullptr;

  auto inputBytes =
      rowBasedSpill ? spilledRows.size : input->estimateFlatSize();
  auto inputNum = rowBasedSpill ? spilledRows.rows->size() : input->size();

  const auto minReservationBytes =
      currentUsage * spillConfig_->minSpillableReservationPct / 100;
  const auto availableReservationBytes = pool()->availableReservation();
  const auto tableIncrementBytes = table_->hashTableSizeIncrease(inputNum);
  const int64_t flatBytes = inputBytes;
  const auto rowContainerIncrementBytes = numRows == 0
      ? flatBytes * 2
      : rows->sizeIncrement(inputNum, outOfLineBytes > 0 ? flatBytes * 2 : 0);
...
void HashBuild::spillRowBasedInput() {
  ...
  if (!spillEnabled() || spiller_ == nullptr || !spiller_->isAnySpilled() ||
      !activeRows_.hasSelections() || spiller_->container()->numRows() == 0) {
    return;
  }
  spiller_->spill();
  table_->clear();
...
void HashBuild::spillPartition(
  ...
  if (isInputFromSpill()) {
    spiller_->spill(partition, wrapAndCombineDict(size, indices, input));
  } else {
    spiller_->spill(
        partition,
        wrapAndCombineDict(
            size, indices, tableType_, spillChildVectors_, input->pool()));
  }
}

bool HashBuild::requestSpill(RowVectorPtr& input) {
  BOLT_CHECK_GT(numSpillRows_, 0);
  BOLT_CHECK_GT(numSpillBytes_, 0);
...
bool HashBuild::finishHashBuild() {
  ...
  for (auto* build : otherBuilds) {
    std::unique_ptr<Spiller> buildSpiller;
    {
      ...
      BOLT_CHECK_NOT_NULL(build->table_);
  ...
  if (spiller_ != nullptr) {
    if (spillThreshold_.has_value()) {
      ...
      auto usedMemBytes = memoryUsedForFirstSpill_.value();
      auto bytesForHashTable = bits::nextPowerOfTwo(maxRestoreRowCount) * 8;
      if (usedMemBytes > bytesForHashTable) {
        maxRestoreRowCount =
      ...
      maxRestoreRowCount =
          std::min(maxRestoreRowCount, (uint64_t)maxHashTableBucketCount_);
      spiller_->setMaxRowsInMemory(maxRestoreRowCount);
    }
  ...
  if (operatorCtx_->driverCtx()->queryConfig().morselDrivenEnabled() &&
      operatorCtx_->driverCtx()
          ->queryConfig()
          .hashProbeFinishEarlyOnEmptyBuild() &&
      skipProbeOnEmptyBuild() && table_->numDistinct() == 0) {
    auto sources = joinNode_->sources();
    // note: The while loop here will not fall into an infinite loop unless the
    // plan itself is cyclic.
    while (!sources.empty()) {
      // Find the probe side node
      auto probeSourceNode = sources[0];
      if (auto probeSourceLocalPartitionNode =
              std::dynamic_pointer_cast<const core::LocalPartitionNode>(
                  probeSourceNode)) {
        auto localExchangeQueues =
            this->operatorCtx_->task()->getLocalExchangeQueues(
                this->operatorCtx_->driverCtx()->splitGroupId,
                probeSourceLocalPartitionNode->id());
        bool ifAbortSucceeded = false;
        for (auto localExchangeQueue : localExchangeQueues) {
          // Note: when morsel-driven is off or the pipeline is not
          // morsel-driven, localExchangeQueue->setAbort() will not take effect.
          ifAbortSucceeded |= localExchangeQueue->setAbort();
        }
        if (ifAbortSucceeded) {
          VLOG(1)
              << "[morsel-driven] HashBuild found empty table and can skip probe. Notify probe-side driver creator in localExchangeQueue. task="
              << this->operatorCtx_->taskId()
              << " pipeline=" << this->operatorCtx_->driverCtx()->pipelineId
              << " driver=" << this->operatorCtx_->driverCtx()->driverId;
        }
        // Break, since we already find the LocalPartition
        break;
      } else {
        VLOG(1)
            << "[morsel-driven] HashBuild found empty table and can skip probe. probe-side current source="
            << probeSourceNode
            << ", not LocalPartition, continue to check the source of current source.";
        // Go down to the source of the source
        sources = probeSourceNode->sources();
      }
    }
  }
...
void HashBuild::recordSpillReadStats() {
...
void HashBuild::setupSpillInput(HashJoinBridge::SpillInput spillInput) {
  ...
  uint64_t maxPartitionRowCount = 0;
  // when kMaxSpillLevel is been set to 0, exceedJoinSpillLevelLimit will return
  // true, therefore will not create spiller_ when repartition.
  // when spiller_ is nullptr, spillInputReader_ will read spilled data from
  // spilled file and use HashBuild::addInput(RowVectorPtr input) to add input
  // data, but spillInput(input) in addInput(RowVectorPtr input) will not
  // trigger spill again (OOM in end).
  if (spiller_ != nullptr) {
    auto maxPartitionRowCount = spiller_->maxPartitionRowCount();
  }
  table_.reset();
  spiller_.reset();
  ...
  setupTable();
  setupSpiller(spillInput.spillPartition.get(), maxPartitionRowCount);
  intermediateStateCleared_ = false;
  numInputRows_ = 0;
...
void HashBuild::processSpillInput() {
  checkRunning();
  {
    ...
    } else { // read row based spill input
      std::vector<char*> rows;
      while (auto size = spillInputReader_->nextBatch(rows)) {
        if (size == 0) {
          break;
        }
        addSpilledRowInput(rows, size);
        if (!isRunning()) {
          return;
        }
      }
  ...
  noMoreInputInternal();
  restoringRangePartition_ = false;
}
...
void HashBuild::addRuntimeStats() {
  ...
  if (spiller_ != nullptr && spiller_->isAnySpilled()) {
    lockedStats->addRuntimeStat(
        "maxSpillLevel",
        RuntimeCounter(spillConfig()->joinSpillLevel(offsetTojoinBits_)));
  }
}
  ...
  switch (state_) {
    ...
    case State::kWaitForSpill:
      if (!future_.valid()) {
        ...
        DeltaCpuWallTimer timer{[this](const CpuWallTiming& timing) {
          auto selfDelta =
              operatorCtx_->driver()->processLazyTiming(*this, timing);
          this->stats().wlock()->addInputTiming.add(selfDelta);
        }};
        addInput(std::move(input_));
      }
    ...
    case State::kWaitForProbe:
      ...
      break;
    default:
      BOLT_UNREACHABLE("Unexpected state: {}", stateName(state_));
      break;
  }
  if (future_.valid()) {
    BOLT_CHECK(!isRunning() && !isFinished());
...
void HashBuild::checkRunning() const {
  BOLT_CHECK(isRunning(), stateName(state_));
}
...
void HashBuild::checkStateTransition(State state) {
  ...
  switch (state) {
    case State::kRunning:
      if (!spillEnabled()) {
        BOLT_CHECK_EQ(state_, State::kWaitForBuild);
      } else {
        BOLT_CHECK_NE(state_, State::kFinish);
      }
      break;
    case State::kWaitForBuild:
      [[fallthrough]];
    ...
    case State::kFinish:
      ...
      break;
    default:
      BOLT_UNREACHABLE(stateName(state_));
      break;
  }
}
...
void HashBuild::reclaim(
  ...
  if (nonReclaimableState()) {
    ...
    LOG(WARNING) << name() << " can't reclaim from hash build operator, state_["
  ...
  BOLT_CHECK(task->pauseRequested());
  const std::vector<Operator*> operators =
      task->findPeerOperators(operatorCtx_->driverCtx()->pipelineId, this);

  if (isFirstSpill_ && operators.size() == 1) {
    isFirstSpill_ = false;
    LOG(INFO) << name() << " calculateJoinBits triggered by " << __FUNCTION__;
    if (calculateJoinBits<false>()) {
      spiller_.reset();
      setupSpiller();
    }
  ...
  for (auto* op : operators) {
    ...
    BOLT_CHECK_NOT_NULL(buildOp);
    BOLT_CHECK(buildOp->canReclaim());
...
bool HashBuild::calculateJoinBits(
  ...
  if constexpr (!isRepartition) {
    operatorCtx_->traverseOpToGetRowCount(totalRowCnt, processedRowCnt);
    spillThreshold_ = rowsInMem;
    memoryUsedForFirstSpill_ = pool()->currentBytes();

    if (totalRowCnt <= 0 || processedRowCnt <= 0) {
      LOG(INFO) << name() << " DO NOTHING!!!  totalRowCnt=" << totalRowCnt
                << ", processedRowCnt=" << processedRowCnt;
      return false;
    }

    int32_t maxPartitionRowCnt =
        std::min((uint64_t)maxHashTableBucketCount_, processedRowCnt);
    numPartitions =
        (totalRowCnt + maxPartitionRowCnt) / (maxPartitionRowCnt + 1);
  } else {
    if (maxPartitionRowCount > 0 && spillThreshold_.has_value()) {
      totalRowCnt = maxPartitionRowCount;
      processedRowCnt = spillThreshold_.value();
  ...
  uint8_t estimatedJoinBits =
      sizeof(unsigned int) * CHAR_BIT - __builtin_clz(numPartitions);

  uint32_t estimatedPartitions = 1 << estimatedJoinBits;
  if ((estimatedPartitions < spillConfig_->spillPartitionsAdaptiveThreshold &&
       numPartitions >= estimatedPartitions / 4 * 3) ||
      estimatedPartitions == numPartitions) {
    ++estimatedJoinBits;
  }

  LOG(INFO) << name() << " totalRowCnt = " << totalRowCnt
            << ", processedRowCnt = " << processedRowCnt
            << ", maxHashTableSize = " << maxHashTableBucketCount_
            << ", rowsInMem = " << rowsInMem
            << ", numPartitions = " << numPartitions
            << ", estimatedJoinBits = " << (int)estimatedJoinBits
            << ", poolreservedBytes = "
            << succinctBytes(pool()->reservedBytes())
            << ", poolCurrenrBytes = " << succinctBytes(pool()->currentBytes());

  auto* spillConf = const_cast<common::SpillConfig*>(spillConfig());
  if constexpr (!isRepartition) {
    if (estimatedJoinBits <= spillConf->joinPartitionBits) {
      return false;
    }
    spillConf->joinPartitionBits = std::min(estimatedJoinBits, maxjoinBits);
    LOG(INFO) << name() << ": " << __FUNCTION__
              << " Finally, spillConf.joinPartitionBits = "
              << (uint32_t)spillConf->joinPartitionBits;
    useAdaptiveHashBuild_ = 1;
  } else {
    if (estimatedJoinBits <= spillConf->joinRepartitionBits) {
      return false;
    }
    spillConf->joinRepartitionBits = std::min(estimatedJoinBits, maxjoinBits);
    LOG(INFO) << name() << ": " << __FUNCTION__
              << " Finally, spillConf.joinRepartitionBits = "
              << (uint32_t)spillConf->joinRepartitionBits;
  }
  return true;
}

void HashBuild::recordHashBuildSpillStats() {
  common::HashBuildSpillStats stats{spillLevel_, useAdaptiveHashBuild_};
  Operator::recordHashBuildSpillStats(stats);
}
