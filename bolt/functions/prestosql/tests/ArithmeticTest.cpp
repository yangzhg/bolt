...
TEST_F(ArithmeticTest, log2) {
  ...
  EXPECT_DOUBLE_EQ(log2(89.48219084520171).value(), 6.483528674069538);
  EXPECT_DOUBLE_EQ(log2(89.48219084520173).value(), 6.483528674069539);
  EXPECT_DOUBLE_EQ(log2(51.73126085458665).value(), 5.692964449902672);
  EXPECT_DOUBLE_EQ(log2(51.73126085458666).value(), 5.692964449902672);
}
...
TEST_F(ArithmeticTest, widthBucket) {
  ...
  BOLT_ASSERT_THROW(widthBucket(kNan, 0, 4, 10), "operand must not be NaN");
  BOLT_ASSERT_THROW(
...
TEST_F(ArithmeticTest, toBase) {
  ...
  ASSERT_THROW(to_base(1, 37), bolt::BoltUserError);
}
