...
VectorPtr BatchMaker::createVector<TypeKind::BIGINT>(
  ...
  if (type->isShortDecimal()) {
    ...
    auto powersOfTen = DecimalUtil::getPowersOfTen(precision);
    return createScalar<int64_t>(
        size,
        gen,
        [&gen, powersOfTen]() { return Random::rand64(gen) % powersOfTen; },
        pool,
        isNullAt,
        type);
  }
...
VectorPtr createRows(
  ...
  for (size_t i = 0; i < rowType.size(); ++i) {
    ...
    children[i] = BOLT_DYNAMIC_TYPE_DISPATCH(
...
VectorPtr BatchMaker::createVector<TypeKind::ARRAY>(
  ...
  auto elements = BOLT_DYNAMIC_TYPE_DISPATCH(
      BatchMaker::createVector,
      keyType->kind(),
      keyType,
      childSize,
...
VectorPtr createMapKeys(
  ...
  switch (keyType->kind()) {
    ...
    default:
      BOLT_CHECK(false, "Not supported key type: {}", keyType->kind());
      return nullptr;
  }
}
...
VectorPtr BatchMaker::createVector<TypeKind::MAP>(
  ...
  auto values = BOLT_DYNAMIC_TYPE_DISPATCH(
      BatchMaker::createVector,
      valType->kind(),
      valType,
      childSize,
