...
std::string formatDecimal(uint8_t scale, int128_t unscaledValue) {
  ...
  int128_t integralPart = unscaledValue / DecimalUtil::getPowersOfTen(scale);

  std::string fractionString;
  if (isFraction) {
    auto fraction =
        std::to_string(unscaledValue % DecimalUtil::getPowersOfTen(scale));
    fractionString += ".";
...
int32_t DecimalUtil::stringSize(int32_t precision, int32_t scale) {
  auto strSize = precision + 1;
  if (scale > 0) {
    ++strSize; // A dot.
    if (scale > 7) {
      strSize += 4; // Scientific Exponent
    }
  }
  if (precision == scale) {
...
Status DecimalUtil::parseHugeInt(
  ...
  if (decimalComponents.fractionalDigits.size() > 0) {
    ...
    bool overflow =
        __builtin_mul_overflow(out, DecimalUtil::getPowersOfTen(length), &out);
    if (overflow) {
      return Status::UserError("Value too large.");
    }
