...
class MallocAllocator : public MemoryAllocator {
 public:
  ...
  void registerCache(const std::shared_ptr<Cache>& cache) override {
    BOLT_CHECK_NULL(cache_);
    BOLT_CHECK_NOT_NULL(cache);
    BOLT_CHECK(cache->allocator() == this);
    cache_ = cache;
  }
 ...
 private:
  ...
  void* reallocateBytes(void* p, uint64_t bytes, uint16_t alignment) override;
  ...
  inline bool incrementUsageWithReservationFunc(
      ...
      uint32_t increment,
      std::mutex& lock) {
    BOLT_CHECK_LT(increment, reservationByteLimit_);
  ...
  inline bool incrementUsageWithoutReservation(int64_t bytes) {
    BOLT_CHECK_GE(bytes, reservationByteLimit_);
  ...
  inline void decrementUsageWithReservationFunc(
      ...
      uint32_t decrement,
      std::mutex& lock) {
    BOLT_CHECK_LT(decrement, reservationByteLimit_);
    std::lock_guard<std::mutex> l(lock);
    counter += decrement;
    ...
    BOLT_CHECK_LT(counter, 2 * reservationByteLimit_);
  }
