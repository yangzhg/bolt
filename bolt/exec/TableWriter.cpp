...
TableWriter::TableWriter(
  ...
  if (tableWriteNode->outputType()->size() == 1) {
    BOLT_USER_CHECK_NULL(tableWriteNode->aggregationNode());
  } else {
    BOLT_USER_CHECK(tableWriteNode->outputType()->equivalent(
...
void TableWriter::updateStats(const connector::DataSink::Stats& stats) {
  {
    ...
    if (!closed_) {
      ...
      BOLT_CHECK_EQ(stats.numWrittenFiles, 0);
      BOLT_CHECK(stats.spillStats.empty());
      return;
    }
...
void TableWriter::setConnectorMemoryReclaimer() {
  BOLT_CHECK_NOT_NULL(connectorPool_);
...
RowVectorPtr TableWriteTraits::createAggregationStatsOutput(
  ...
  BOLT_CHECK_GT(aggregationOutput->childrenSize(), 0);
...
int64_t TableWriteTraits::getRowCount(const RowVectorPtr& output) {
  ...
  BOLT_CHECK_NOT_NULL(rowCountVector);
  int64_t rowCount{0};
