...
void getFreePortsImpl(int numPorts, int* ports) {
  ...
  for (int i = 0; i < numPorts; i++) {
    ...
    BOLT_CHECK_NE(sock, -1, "Error while creating socket: {}", errno);
    sockets[i] = sock;
    ...
    BOLT_CHECK_NE(result, -1, "Error while binding socket: {}", errno);

    result = getsockname(sock, reinterpret_cast<sockaddr*>(&addr), &len);
    BOLT_CHECK_NE(result, -1, "Error on getsockname: {}", errno);
