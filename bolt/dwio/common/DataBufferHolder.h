...
class DataBufferHolder {
 public:
  DataBufferHolder(
    ...
    BOLT_CHECK_GT(initialSize_, 0);
    BOLT_CHECK_LE(initialSize_, maxSize_);
    BOLT_CHECK_GE(growRatio_, MIN_PAGE_GROW_RATIO);
  }
  ...
  void truncate(size_t newSize) {
    BOLT_CHECK_LE(newSize, size_);
    BOLT_CHECK_NULL(sink_, "Only non sink buffers can be truncated");
