...
class PlanBuilder {
 public:
  ...
  /// @param connectorId The connector id to use for the table scan.
  ...
  PlanBuilder& finalAggregation(const bool useGPU = false);
  ...
  /// aggregation.
  PlanBuilder& intermediateAggregation(const bool useGPU = false);
  ...
  PlanBuilder& aggregation(
      ...
      bool useGPU = false) {
    return aggregation(
        groupingKeys, {}, aggregates, masks, step, ignoreNullKeys, useGPU);
  }
  ...
  /// Add a LocalShuffleNode to shuffle input rows.
  /// @param seed Seed for the shuffle.

  PlanBuilder& localShuffle(int64_t seed);
  ...
  /// Add a torch node following the current node.
  /// The torch node will take the current node output and give it to the
  /// the last function in the input pytorch `script`.
  PlanBuilder& torch(std::string script, const RowTypePtr& outputType);
#endif
