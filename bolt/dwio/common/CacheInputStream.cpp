...
using bolt::cache::ScanTracker;
using bolt::cache::TrackingId;
using bolt::memory::MemoryAllocator;
...
CacheInputStream::CacheInputStream(
    ...
    : bufferedInput_(bufferedInput),
      ...
      columnCacheBlackList_(columnCacheBlackList),
...
void CacheInputStream::setRemainingBytes(uint64_t remainingBytes) {
  BOLT_CHECK_GE(region_.length, position_ + remainingBytes);
...
void CacheInputStream::loadSync(Region region) {
  ...
  do {
    ...
    if (std::find(
            columnCacheBlackList_.begin(),
            columnCacheBlackList_.end(),
            trackingId_.id()) != columnCacheBlackList_.end()) {
      setNoRetention();
    }
    ...
    pin_.clear();
    pin_ = cache_->findOrCreate(key, region.length, &wait, cacheable_);
    // LOG(INFO) << "findOrCreate for key in Memory,fileNum is " << key.fileNum
    //           << " offset is:" << key.offset
    //           << "   region.length:" << region.length << " end";

    if (pin_.empty()) {
      // LOG(INFO) << "cache of fileNum_:" << fileNum_
      //           << " region.offset:" << region.offset << " from memory is
      //           null";
      BOLT_CHECK(wait.valid());
    ...
    if (entry->isExclusive()) {
      ...
      //           << " is exclusizve, cacheable:" << cacheable_;
      ...
      if (cacheable_ && loadFromSsd(region, *entry)) {
        LOG(INFO) << "cache of fileNum_:" << fileNum_
                  << " region.offset:" << region.offset
                  << " has already get from ssd";
        return;
      }
...
bool CacheInputStream::loadFromSsd(
  ...
  try {
    ...
    LOG(INFO) << "get cache from SSD" << ssdPin.toString();
