...
class TestArbitrator : public MemoryArbitrator {
 public:
  ...
  bool growCapacity(MemoryPool* memoryPool, uint64_t requestBytes) override {
    ...
    return memoryPool->grow(requestBytes, requestBytes);
  }
...
DEBUG_ONLY_TEST_F(ArbitrationParticipantTest, reclaimLock) {
  ...
  SCOPED_TESTVALUE_SET(
      "bytedance::bolt::memory::ArbitrationParticipant::abortLocked",
      std::function<void(ArbitrationParticipant*)>(
          ([&](ArbitrationParticipant* /*unused*/) {
...
DEBUG_ONLY_TEST_F(ArbitrationParticipantTest, abortedCheck) {
  ...
  abortThread2.join();
  ASSERT_TRUE(scopedParticipant->aborted());
  BOLT_ASSERT_THROW(task->allocate(MB), "test abort1");
}
...
DEBUG_ONLY_TEST_F(ArbitrationParticipantTest, concurrentAbort) {
  ...
  SCOPED_TESTVALUE_SET(
      ...
      std::function<void(ArbitrationParticipant*)>(
          ([&](ArbitrationParticipant* /*unused*/) {
            BOLT_FAIL("reclaim abort message");
          })));
  ...
  BOLT_ASSERT_THROW(std::rethrow_exception(task->abortError()), "test abort 2");
}
...
TEST_F(ArbitrationParticipantTest, arbitrationOperation) {
  ...
  BOLT_ASSERT_THROW(op.stats(), "(init vs. finished)");
  ...
  ASSERT_EQ(op.minGrowBytes(), 0);
  ASSERT_FALSE(op.hasTimeout());
  BOLT_ASSERT_THROW(op.setGrowTargets(), "");
  ASSERT_EQ(op.requestBytes(), requestBytes);
  ASSERT_EQ(op.maxGrowBytes(), 0);
  ASSERT_EQ(op.minGrowBytes(), 0);
  ...
  BOLT_ASSERT_THROW(op.finish(), "");
  op.start();
  ASSERT_EQ(op.state(), ArbitrationOperation::State::kRunning);
  BOLT_ASSERT_THROW(op.stats(), "(running vs. finished)");
  op.setGrowTargets();
  ASSERT_EQ(op.requestBytes(), requestBytes);
  ...
  BOLT_ASSERT_THROW(op.recordGlobalArbitrationStartTime(), "");
  BOLT_ASSERT_THROW(op.stats(), "(running vs. finished)");
  std::this_thread::sleep_for(std::chrono::milliseconds(200)); // NOLINT
