 ...
 public:
  ...
  FLATTEN void toIntermediate(
    ...
    if (arrayInput->mayHaveNulls()) {
      ...
      copy->copy(arrayInput.get(), rows, nullptr, false);
...
void registerSetAgg(const std::string& name) {
  ...
  exec::registerAggregateFunction(
      ...
      [name](
        ...
        switch (typeKind) {
          case TypeKind::BOOLEAN:
            return std::make_unique<SetAggAggregate<bool, ignoreNulls>>(
                resultType);
          case TypeKind::TINYINT:
            return std::make_unique<SetAggAggregate<int8_t, ignoreNulls>>(
                resultType);
          case TypeKind::SMALLINT:
            return std::make_unique<SetAggAggregate<int16_t, ignoreNulls>>(
                resultType);
          case TypeKind::INTEGER:
            return std::make_unique<SetAggAggregate<int32_t, ignoreNulls>>(
                resultType);
          case TypeKind::BIGINT:
            return std::make_unique<SetAggAggregate<int64_t, ignoreNulls>>(
                resultType);
          case TypeKind::HUGEINT:
            return std::make_unique<SetAggAggregate<int128_t, ignoreNulls>>(
                resultType);
          case TypeKind::REAL:
            return std::make_unique<SetAggAggregate<float, ignoreNulls>>(
                resultType);
          case TypeKind::DOUBLE:
            return std::make_unique<SetAggAggregate<double, ignoreNulls>>(
                resultType);
          case TypeKind::TIMESTAMP:
            return std::make_unique<SetAggAggregate<Timestamp, ignoreNulls>>(
                resultType);
          case TypeKind::VARBINARY:
            [[fallthrough]];
          case TypeKind::VARCHAR:
            return std::make_unique<SetAggAggregate<StringView, ignoreNulls>>(
                resultType);
          case TypeKind::ARRAY:
            [[fallthrough]];
          case TypeKind::MAP:
          ...
          case TypeKind::UNKNOWN:
            [[fallthrough]];
          case TypeKind::ROW:
            return std::make_unique<SetAggAggregate<ComplexType, ignoreNulls>>(
                resultType, throwOnNestedNulls);
          default:
            BOLT_UNREACHABLE("Unexpected type {}", mapTypeKindToName(typeKind));
        }
      },
      /*registerCompanionFunctions*/ true);
}

void registerSetUnion(const std::string& name) {
  std::vector<std::shared_ptr<exec::AggregateFunctionSignature>> signatures = {
      exec::AggregateFunctionSignatureBuilder()
          .typeVariable("T")
          .returnType("array(T)")
...
void registerSetAggAggregate(const std::string& prefix) {
  registerSetAgg<false>(prefix + kSetAgg);
}
