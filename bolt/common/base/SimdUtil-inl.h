...
#if XSIMD_WITH_NEON
  inline static int toBitMask(
      xsimd::batch_bool<T, A> mask,
      const xsimd::neon&) noexcept {
    constexpr int8x16_t kShift{
        -7, -6, -5, -4, -3, -2, -1, 0, -7, -6, -5, -4, -3, -2, -1, 0};

    uint8x16_t vmask = vshlq_u8(vandq_u8(mask, vdupq_n_u8(0x80)), kShift);
...
struct BitMask<T, A, 2> {
  ...
  inline static int toBitMask(
      xsimd::batch_bool<T, A> mask,
      const xsimd::neon&) noexcept {
    uint8x16_t mask_tmp = reinterpret_cast<uint8x16_t>(mask.data);

    // == _mm_movemask_epi8
    constexpr int8x16_t kShift{
        -7, -6, -5, -4, -3, -2, -1, 0, -7, -6, -5, -4, -3, -2, -1, 0};
    uint8x16_t vmask = vshlq_u8(vandq_u8(mask_tmp, vdupq_n_u8(0x80)), kShift);
    auto tmp =
        (vaddv_u8(vget_high_u8(vmask)) << 8) | vaddv_u8(vget_low_u8(vmask));

    return bits::extractBits<uint32_t>(tmp, 0xAAAA);
  }
#endif
...
#if XSIMD_WITH_NEON64
  // genericToBitMask does not work on ARM
  inline static int toBitMask(
      xsimd::batch_bool<T, A> mask,
      const xsimd::neon&) noexcept {
    // _mm_movemask_ps in ARM NEON
    // https://developer.arm.com/documentation/dui0472/m/Using-NEON-Support/NEON-intrinsics-for-shifts-by-signed-variable
    // shift sign bit: 10...00 x 4 => 00...01 x 4
    uint32x4_t tmp = vshrq_n_u32(mask.data, 31);

    // shift & reduction add
    // 0001, 0001, 0001, 0001 =>  0001, 0010, 0100, 1000
    // 0001 + 0010 + 100 + 1000
    constexpr int32x4_t shift{0, 1, 2, 3};
...
struct BitMask<T, A, 8> {
  ...
  inline static int toBitMask(
      ...
      const xsimd::neon&) noexcept {
    uint64x2_t input =
        vreinterpretq_u64_f64(reinterpret_cast<float64x2_t>(mask.data));
    uint64x2_t high_bits = vshrq_n_u64(input, 63);
    return (
        int)(vgetq_lane_u64(high_bits, 0) | (vgetq_lane_u64(high_bits, 1) << 1));
  }
#endif
