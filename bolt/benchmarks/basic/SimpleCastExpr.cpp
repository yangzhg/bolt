...
class SimpleCastBenchmark : public functions::test::FunctionBenchmarkBase {
 public:
  ...
  run(const TypePtr& inputType, const TypePtr& outputType, size_t batchSize) {
    ...
    auto castInput =
        std::make_shared<bytedance::bolt::core::FieldAccessTypedExpr>(
            inputType, std::string(kColName));
    std::vector<bytedance::bolt::core::TypedExprPtr> expr{
        std::make_shared<bytedance::bolt::core::CastTypedExpr>(
