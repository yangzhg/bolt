...
///  `bolt/docs/develop/scalar-functions.rst`
...
void register2() {
  ...
  // `bolt/functions/lib/RegistrationHelpers.h` provides helper functions to
...
//
// Bolt supports two string types: VARBINARY and VARCHAR. Both types will be
...
//
// In order to define these types, use the BOLT_DEFINE_FUNCTION_TYPES(T) macro:
template <typename T>
...
// `bolt/examples/OpaqueType.cpp` for a more thorough example of Opaque type
