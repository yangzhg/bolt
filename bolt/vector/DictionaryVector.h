...
class DictionaryVector : public SimpleVector<T> {
 public:
  ...
  const T valueAt(vector_size_t idx) const override {
    BOLT_DCHECK(initialized_);
    BOLT_DCHECK(!isNullAt(idx), "found null value at: {}", idx);
    auto innerIndex = getDictionaryIndex(idx);
  ...
  vector_size_t baseSize() const override {
    return dictionaryValues_->baseSize();
  }

  vector_size_t itemSize() const override {
    return dictionaryValues_->itemSize() * 1.0 / dictionaryValues_->size() *
        BaseVector::length_;
  }
  ...
  uint64_t usedSize() const override {
    uint64_t avgValueSize = dictionaryValues_->usedSize() /
        std::max<vector_size_t>(dictionaryValues_->size(), 1);
    return BaseVector::usedSize() + avgValueSize * BaseVector::length_ +
        indices_->size();
  }
  ...
  uint64_t estimateExportArrowSize() const override {
    uint64_t avgValueSize = dictionaryValues_->estimateExportArrowSize() /
        std::max<vector_size_t>(dictionaryValues_->size(), 1);
    return BaseVector::estimateExportArrowSize() +
        avgValueSize * BaseVector::length_;
  }
...
#include "bolt/vector/DictionaryVector-inl.h"
