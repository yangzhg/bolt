...
// error messages.
//
// @throws BOLT_USER_FAIL If the pattern is found to use unsupported features.
...
void ensureRegexIsConstantAndCompatible(
  ...
  if (!patternVector || !patternVector->isConstantEncoding()) {
    BOLT_USER_FAIL("{} requires a constant pattern.", functionName);
  }
...
struct RegexpReplaceFunction {
  ...
  void call(
    ...
    BOLT_USER_CHECK_GE(position, 1, "regexp_replace requires a position >= 1");
    ...
    while (numCodePoints < position - 1 && utf8Position <= stringInput.size()) {
      int charLength = stringCore::utf8proc_char_length(
