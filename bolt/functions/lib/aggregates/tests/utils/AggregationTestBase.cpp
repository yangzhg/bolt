...
using bytedance::bolt::exec::Spiller;
using bytedance::bolt::exec::test::AssertQueryBuilder;
using bytedance::bolt::exec::test::CursorParameters;
...
void enableAbandonPartialAggregation(AssertQueryBuilder& queryBuilder) {
  queryBuilder.config(core::QueryConfig::kAbandonPartialAggregationMinRows, "1")
      ...
      .config(core::QueryConfig::kPartialAggregationSpillMaxPct, "0")
      .maxDrivers(1);
}
...
std::string getExtractFunctionNameWithSuffix(
  ...
  BOLT_CHECK(signatures.has_value());
  ...
  BOLT_UNREACHABLE("Could not find a function bound to argTypes.");
}
...
std::tuple<std::string, std::string, std::string> getCompanionAggregates(
    ...
    const std::vector<TypePtr>& argTypes) {
  BOLT_CHECK_EQ(companionFunctions.partial.size(), 1);
  auto partialAggregate = fmt::format(
      "{}({})", companionFunctions.partial.front().functionName, aggregateArgs);

  BOLT_CHECK_EQ(companionFunctions.merge.size(), 1);
  ...
  if (companionFunctions.extract.size() == 1) {
    ...
    BOLT_CHECK_GT(companionFunctions.extract.size(), 1);
...
void AggregationTestBase::testAggregationsImpl(
  ...
  if (!groupingKeys.empty() && allowInputShuffle_) {
    {
      SCOPED_TRACE("Run partial + final with column based spilling");
      PlanBuilder builder(pool());
      makeSource(builder);
    ...
    {
      SCOPED_TRACE("Run partial + final with row based spilling");
      PlanBuilder builder(pool());
      makeSource(builder);
    ...
    {
      SCOPED_TRACE(
          "Run partial + final with row based spilling + bypass hash table");
      PlanBuilder builder(pool());
      makeSource(builder);
      ...
      queryBuilder.configs(config)
          ...
          .config(core::QueryConfig::kRowBasedSpillMode, "compression")
          .config(core::QueryConfig::kSpilledAggregationBypassHTRatio, "0.2")
          .spillDirectory(spillDirectory->path)
          .maxDrivers(4);
...
std::pair<TypePtr, TypePtr> getResultTypes(
  ...
  if (!signatures.has_value()) {
    BOLT_FAIL("Aggregate {} not registered", name);
  }
  for (auto& signature : signatures.value()) {
    ...
    if (binder.tryBind()) {
      ...
      BOLT_CHECK(finalType, "failed to resolve final type for {}", name);
      return {intermediateType, finalType};
    }
  }
  BOLT_FAIL("Could not infer intermediate type for aggregate {}", name);
}
} // namespace
...
VectorPtr AggregationTestBase::testStreaming(
    ...
    const std::unordered_map<std::string, std::string>& config) {
  BOLT_CHECK(!rawInput1.empty());
  BOLT_CHECK(!rawInput2.empty());
...
std::unique_ptr<exec::Aggregate> createAggregateFunction(
  ...
  BOLT_CHECK(intermediateType->equivalent(
      *func->intermediateType(functionName, inputTypes)));
  BOLT_CHECK(finalType->equivalent(*func->resultType()));
...
void AggregationTestBase::testIncrementalAggregation(
  ...
  for (int i = 0; i < aggregationNode.aggregates().size(); ++i) {
    ...
    bolt::test::assertEqualVectors(intermediateResult1, intermediateResult2);
    ...
    bolt::test::assertEqualVectors(result1, result2);
  }
}
...
VectorPtr AggregationTestBase::testStreaming(
  ...
  func->initializeNewGroups(groups.data(), std::vector<vector_size_t>(1, 0));
  if (testGlobal) {
    func->addSingleGroupRawInput(
  ...
  auto func2 =
      createAggregateFunction(functionName, rawInputTypes, allocator, config);
  func2->initializeNewGroups(groups.data(), std::vector<vector_size_t>(1, 0));
  if (testGlobal) {
...
void AggregationTestBase::testFailingAggregations(
    ...
    const std::unordered_map<std::string, std::string>& config) {
  {
    ...
    queryBuilder.configs(config);
    BOLT_ASSERT_THROW(queryBuilder.copyResults(pool()), expectedMessage);
  }
