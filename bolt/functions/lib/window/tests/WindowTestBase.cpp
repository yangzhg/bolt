...
void WindowTestBase::rangeFrameTestImpl(
  ...
  auto size = 25;
  // For frames with k RANGE PRECEDING/FOLLOWING, Bolt requires the application
  ...
  auto computeOffset = [&](const RangeFrameBound& frame) -> int64_t {
    ...
    BOLT_CHECK(frame.value.has_value());
  ...
  // Bolt frames require the column bounds.
  auto boltFrameBound = [](const RangeFrameBound& frame,
  ...
  if (unorderedColumns) {
    ...
    boltFrame = "range between " + boltFrameBound(startBound, "c1") + " and " +
        boltFrameBound(endBound, "c0");
    ...
    boltFrame = "range between " + boltFrameBound(startBound, "c2") + " and " +
        boltFrameBound(endBound, "c3");
  ...
  std::string boltFunction =
      fmt::format("{} over ({} {})", function, overClause, boltFrame);
