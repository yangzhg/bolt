...
VectorSerde* getVectorSerde() {
  ...
  BOLT_CHECK_NOT_NULL(serde, "Vector serde is not registered.");
  return serde;
}
...
void registerVectorSerde(std::unique_ptr<VectorSerde> serdeToRegister) {
  ...
  BOLT_CHECK_NULL(serde, "Vector serde is already registered.");
...
RowVectorPtr IOBufToRowVector(
  ...
  auto byteStream = std::make_unique<ByteInputStream>(std::move(ranges));
