...
#include "bolt/common/base/WindowStat.h"
...
class Operator : public BaseRuntimeStatWriter {
 public:
  ...
  virtual void setRuntimeMetric(
      OperatorMetricKey key,
      OperatorMetricValue value);

  virtual OperatorMetricValue getRuntimeMetric(
      OperatorMetricKey key,
      OperatorMetricValue defaultReturnValue = "");

  virtual std::string toString() const;

  // an identifier for the operator using for logging
  ...
  const int32_t getConcurrency() const {
    return operatorCtx_->getConcurrency();
  }
  ...
  /// [api compatible] Returns the operator context of this operator.
 ...
 protected:
  ...
  void recordSpillReadStats(const common::SpillReadStats& spillReadStats);
  void recordSortStats(const common::SortStats& sortStats);
  void recordGroupingSetStats(const common::AggregationStats& stats);

  void recordWindowStats(const common::WindowStats& windowStats);

  /// Invoked to record hash build spill stats in operator stats.
  ...
  std::unordered_map<OperatorMetricKey, OperatorMetricValue> runtimeMetric_;
...
class SourceOperator : public Operator {
 public:
  ...
  void addInput(RowVectorPtr /* unused */) override {
    BOLT_FAIL("SourceOperator does not support addInput()");
  }

  virtual void resetBlockingReason() {
    BOLT_FAIL("SourceOperator does not support resetBlockingReason()");
  }

  void noMoreInput() override {
    BOLT_FAIL("SourceOperator does not support noMoreInput()");
  }
};
} // namespace bytedance::bolt::exec
