...
void AsyncDataCacheEntry::setExclusiveToShared(bool cacheable) {
  BOLT_CHECK(isExclusive());
  numPins_ = 1;
  ...
  if (cacheable && (ssdFile_ == nullptr) &&
      (shard_->cache()->ssdCache() != nullptr)) {
...
void AsyncDataCacheEntry::release() {
  ...
  if (numPins_ == kExclusive) {
    ...
    BOLT_CHECK_LE(1, oldPins, "pin count goes negative");
  }
}
...
CachePin CacheShard::findOrCreate(
  ...
  AsyncDataCacheEntry* entryToInit = nullptr;
  {
    ...
    if (it != entryMap_.end()) {
      ...
      BOLT_CACHE_LOG_EVERY_MS(WARNING, 1'000)
    ...
    BOLT_CHECK_EQ(entryToInit->size_, 0);
    entryToInit->size_ = size;
    entryToInit->isFirstUse_ = true;
  }
  return initEntry(key, entryToInit, cacheable);
}
...
bool CoalescedLoad::loadOrFuture(folly::SemiFuture<bool>* wait) {
  {
    ...
    BOLT_CHECK_EQ(State::kPlanned, state_);
  ...
  try {
    ...
    for (const auto& pin : pins) {
      ...
      BOLT_CHECK(entry->key().fileNum.hasValue());
      BOLT_CHECK(entry->isExclusive());
      entry->setExclusiveToShared(entry->getCacheable());
    }
    ...
    LOG(ERROR) << "CoalescedLoad " << (uint64_t)this << " failed: " << e.what();
    throw;
  }
  return true;
}
...
void CoalescedLoad::setEndState(State expectState, State tState, State fState) {
  std::lock_guard<std::mutex> l(mutex_);
  if (state_ == expectState) {
    state_ = tState;
  } else {
    state_ = fState;
  }
...
uint64_t CacheShard::evict(
  ...
  int32_t evictSaveableSkipped = 0;
  {
    ...
    while (++counter <= size) {
      ...
      if (candidate->numPins_ == 0 &&
        ...
        // split cacheable_ -> cacheEntry(candidate) cacheable_ -> （can skip
        // cache_->saveToSsd() method）->Compliant with soft affinity strategy
        if (skipSsdSaveable && candidate->cacheable_ &&
            candidate->ssdSaveable_ && !evictAllUnpinned) {
        ...
        if (pagesToAcquire > 0) {
          ...
          BOLT_CHECK(candidate->data().empty());
        } else {
  ...
  if (evictSaveableSkipped) {
    ...
    if (ssdCache->startWrite()) {
      ...
      BOLT_SSD_CACHE_LOG(INFO) << "Start save for old saveable, skipped "
...
void CacheShard::appendSsdSaveable(std::vector<CachePin>& pins) {
  ...
  BOLT_CHECK(cache_->ssdCache()->writeInProgress());
  for (auto& entry : entries_) {
...
bool CacheShard::removeFileEntries(
  ...
  if (filesToRemove.empty()) {
    BOLT_CACHE_LOG(INFO) << "Removed 0 AsyncDataCache entry.";
    return true;
  }
  ...
  BOLT_CACHE_LOG(INFO) << "Removed " << toFree.size()
...
CachePin AsyncDataCache::findOrCreate(
  ...
  return shards_[shard]->findOrCreate(key, size, wait, cacheable);
}
...
uint64_t AsyncDataCache::shrink(uint64_t targetBytes) {
  ...
  LOG(INFO) << "Try to shrink cache to free up "
            << bolt::succinctBytes(targetBytes) << "  memory";
  ...
  LOG(INFO) << "Freed " << bolt::succinctBytes(evictedBytes)
            << " cache memory, spent " << bolt::succinctMicros(shrinkTimeUs)
            << "\n"
            << toString();
  return evictedBytes;
}
...
void AsyncDataCache::backoff(int32_t counter) {
  ...
  BOLT_CACHE_LOG_EVERY_MS(INFO, 1'000)
...
void AsyncDataCache::possibleSsdSave(uint64_t bytes) {
  ...
  if (memory::AllocationTraits::numPages(ssdSaveable_) >
    ...
    LOG(INFO) << "possibleSsdSave:" << ssdSaveable_ << "--"
              << memory::AllocationTraits::kPageSize << "--"
              << ssdSaveable_ / memory::AllocationTraits::kPageSize << "--"
              << std::max<int32_t>(kMinSavePages, cachedPages_ / 8);
    saveToSsd();
  }
}
...
void AsyncDataCache::saveToSsd() {
  ...
  BOLT_CHECK(ssdCache_->writeInProgress());
  ssdSaveable_ = 0;
  for (auto& shard : shards_) {
    shard->appendSsdSaveable(pins);
  }
  LOG(INFO) << "begin write to SSD, pins:" << pins.capacity();
...
CoalesceIoStats readPins(
  ...
  return coalesceIo<CachePin, folly::Range<char*>>(
      ...
      [&](const CachePin& pin, std::vector<folly::Range<char*>>& ranges) {
        ...
        BOLT_CHECK_EQ(offsetInRuns, size);
      },
