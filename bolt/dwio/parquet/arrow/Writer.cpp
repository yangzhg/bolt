...
#include "bolt/dwio/parquet/arrow/ArrowSchema.h"
...
#include "bolt/dwio/parquet/arrow/PathInternal.h"
...
namespace bytedance::bolt::parquet::arrow::arrow {
...
class FileWriterImpl : public FileWriter {
 public:
  ...
  Status WriteRecordBatch(const RecordBatch& batch) override {
    ...
    const int64_t parquet_block_size = this->properties().parquet_block_size();
    ...
    auto shouldFlush = [max_row_group_length, parquet_block_size](
                           RowGroupWriter* rowGroupWriter,
                           int64_t numRowsWritten,
                           int64_t numRows) {
      DCHECK(rowGroupWriter != nullptr);
      // No data in RowGroup, not flush
      if (numRowsWritten <= 0) {
        return false;
      }
      ...
      // Only consider whether the number of rows reaches the threshold
      if (parquet_block_size <= 0) {
        return false;
      }
      auto bytesWritten = rowGroupWriter->total_compressed_bytes_all();
      auto bytesPerRow = bytesWritten / numRowsWritten;
      return numRows * bytesPerRow + bytesWritten > parquet_block_size;
    };

    int64_t offset = 0;
    while (offset < batch.num_rows()) {
      auto numRowsWritten = row_group_writer_->num_rows();
      const int64_t batch_size = std::min(
          max_row_group_length - numRowsWritten, batch.num_rows() - offset);
      // Flush current row group if it is full.
      if (shouldFlush(row_group_writer_, numRowsWritten, batch_size)) {
        RETURN_NOT_OK(NewBufferedRowGroup());
      }
  ...
  const int64_t getWrittenBytesPerRow() const override {
    if (row_group_writer_ != nullptr) {
      auto rows = row_group_writer_->num_rows();
      if (rows > 0) {
        return row_group_writer_->total_compressed_bytes_written() / rows;
      }
    }
    return 0;
  }

  const int64_t totalCompressedBytesAll() const override {
    if (row_group_writer_ != nullptr) {
      return row_group_writer_->total_compressed_bytes_all();
    }
    return 0;
  }
...
Status WriteFileMetaData(
  ...
  PARQUET_CATCH_NOT_OK(::bytedance::bolt::parquet::arrow::WriteFileMetaData(
      file_metadata, sink));
  return Status::OK();
}
...
Status WriteMetaDataFile(
    const FileMetaData& file_metadata,
    ::arrow::io::OutputStream* sink) {
  PARQUET_CATCH_NOT_OK(::bytedance::bolt::parquet::arrow::WriteMetaDataFile(
      file_metadata, sink));
  return Status::OK();
}
