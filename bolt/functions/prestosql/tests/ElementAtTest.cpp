...
// This default behavior is modified to match presto/spark
// #1 - start indices at 1.
// #2 - allow out of bounds access for arrays.
...
TEST_F(ElementAtTest, allFlavors2) {
  ...
  // #2
  EXPECT_EQ(elementAtSimple("C0[4]", {arrayVector}), std::nullopt);
  EXPECT_EQ(elementAtSimple("C0[5]", {arrayVector}), std::nullopt);
  ...
  // #3
  EXPECT_EQ(elementAtSimple("C0[-1]", {arrayVector}), std::nullopt);
  EXPECT_EQ(elementAtSimple("C0[-4]", {arrayVector}), std::nullopt);
...
TEST_F(ElementAtTest, allFlavors3) {
  ...
  exec::registerVectorFunction(
      "__f2",
      SubscriptImpl<false, true, false, false, true>::signatures(),
      std::make_unique<SubscriptImpl<false, true, false, false, true>>(false));
...
TEST_F(ElementAtTest, allFlavors4) {
  ...
  exec::registerVectorFunction(
      "__f3",
      SubscriptImpl<true, false, true, false, true>::signatures(),
      std::make_unique<SubscriptImpl<true, false, true, false, true>>(false));
...
TEST_F(ElementAtTest, testCachingOptimzation) {
  ...
  auto verfyCachedContent = [&]() {
    auto& cachedMapTyped =
        *static_cast<
             bytedance::bolt::functions::LookupTable<TypeKind::BIGINT>*>(
