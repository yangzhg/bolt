 ...
 public:
...
#endif
  {
    ...
    if (__builtin_mul_overflow(
            a, DecimalUtil::getPowersOfTen(aRescale), &aRescaled) ||
        __builtin_mul_overflow(
            b, DecimalUtil::getPowersOfTen(bRescale), &bRescaled)) {
      BOLT_ARITHMETIC_ERROR("Decimal overflow: {} + {}", a, b);
    }
 ...
 public:
  ...
  apply(R& r, const A& a, const B& b, uint8_t aRescale, uint8_t bRescale) {
    r = checkedMultiply<R>(
        ...
        R(DecimalUtil::getPowersOfTen(aRescale + bRescale)));
    DecimalUtil::valueInRange(r);
  }
  ...
  inline static void apply(
    ...
    if (!decimalParam.has_value()) {
      ...
      DecimalUtil::divideWithRoundUp<R, A, int128_t>(
          r, a, DecimalUtil::getPowersOfTen(aRescale), false, 0, 0);
      return;
    }
    // RoundN function.
    ...
    auto reScaleFactor =
        DecimalUtil::getPowersOfTen(aRescale - decimalParam.value());
    DecimalUtil::divideWithRoundUp<R, A, int128_t>(
...
std::shared_ptr<exec::VectorFunction> createDecimalUnary(
  ...
  if (inputArgs.size() == 2) {
    ...
    BOLT_CHECK_EQ(inputArgs[1].type->kind(), TypeKind::INTEGER);
...
std::shared_ptr<exec::VectorFunction> createDecimalBetweenFunction(
  ...
  if (aType->isShortDecimal()) {
    BOLT_CHECK(bType->isShortDecimal());
    BOLT_CHECK(cType->isShortDecimal());
    // Arguments are short decimals.
    return std::make_shared<DecimalBetweenFunction<int64_t>>();
  } else {
    BOLT_CHECK(bType->isLongDecimal());
    BOLT_CHECK(cType->isLongDecimal());
