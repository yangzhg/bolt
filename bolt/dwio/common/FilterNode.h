...
using ColumnFilter = std::vector<bolt::dwio::common::FilterNode>;
...
class FilterType {
 private:
  ...
  std::shared_ptr<const bolt::Type> requestType_;
  // data type in the filter tree node
  std::shared_ptr<const bolt::Type> fileType_;
 ...
 public:
  ...
  inline const std::shared_ptr<const bolt::Type>& getRequestType() const {
    return requestType_;
  }

  inline const std::shared_ptr<const bolt::Type>& getDataType() const {
    return fileType_;
  }

  inline void setDataType(const std::shared_ptr<const bolt::Type>& fileType) {
    fileType_ = fileType;
  }
