...
class DriverTest : public OperatorTestBase {
 ...
 public:
  ...
  void registerForWakeup(ContinueFuture* future) {
    ...
    auto [promise, semiFuture] = makeBoltContinuePromiseContract("wakeup");
...
class TestingPauser : public Operator {
 public:
  ...
  BlockingReason isBlocked(ContinueFuture* future) override {
    BOLT_CHECK(!operatorCtx_->driver()->state().suspended());
    if (future_.valid()) {
 ...
 public:
  ...
  std::unique_ptr<Operator> toOperator(
    ...
    if (auto throwNode = std::dynamic_pointer_cast<const ThrowNode>(node)) {
      BOLT_CHECK_LT(driversCreated, maxDrivers_, "Too many drivers");
      ++driversCreated;
...
TEST_F(DriverTest, driverCreationThrow) {
  ...
  BOLT_ASSERT_THROW(cursor->moveNext(), "Too many drivers");
  EXPECT_EQ(TaskState::kFailed, task->state());
}

TEST_F(DriverTest, nonBoltOperatorException) {
...
DEBUG_ONLY_TEST_F(DriverTest, driverSuspensionCalledFromOffThread) {
  ...
  BOLT_ASSERT_THROW(driver->task()->enterSuspended(driver->state()), "");
  BOLT_ASSERT_THROW(driver->task()->leaveSuspended(driver->state()), "");
}
...
struct ThrowRuntimeExceptionFunction {
  ...
  void call(TResult& out, const TInput& in) {
    BOLT_CHECK(false, "Throwing exception");
  }
};
} // namespace
