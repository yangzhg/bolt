...
Result HiveTypeParser::parseType() {
  Token nt = nextToken();
  BOLT_CHECK(!nt.isEOS(), "Unexpected end of stream parsing type!!!");
  if (nt.isValidType() && nt.isPrimitiveType()) {
    ...
    switch (nt.typeKind()) {
      ...
      case bolt::TypeKind::MAP: {
        ...
        return Result{
            bolt::MAP(resultList.typelist.at(0), resultList.typelist.at(1))};
      }
      case bolt::TypeKind::ARRAY: {
        BOLT_CHECK(
            resultList.typelist.size() == 1,
            "wrong param count for array type def");
        return Result{bolt::ARRAY(resultList.typelist.at(0))};
      }
      default:
        BOLT_FAIL("unsupported kind: " + std::to_string((int)nt.typeKind()));
    }
  } else {
...
Token HiveTypeParser::eatToken(TokenType tokenType, bool ignorePredefined) {
  ...
  BOLT_FAIL("Unexpected token " + token.remaining.toString());
}
...
TokenAndRemaining HiveTypeParser::nextToken(
  ...
  BOLT_FAIL("Bad Token at " + sp.toString());
}
