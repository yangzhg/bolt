...
void mergeOverlap(
  ...
  BOLT_DCHECK_LE(limA, startC);
  BOLT_DCHECK_LE(startC + lenA, startB);
  uint32_t a = startA;
  uint32_t b = startB;
  uint32_t c = startC;
  while (a < limA && b < limB) {
...
CompressResult generalCompress(
  ...
  for (uint8_t level = 0; level < currentNumLevels; ++level) {
    ...
    if ((currentItemCount < targetItemCount) ||
      ...
      BOLT_DCHECK_GE(rawBeg, outLevels[level]);
  ...
  BOLT_DCHECK_EQ(outLevels[currentNumLevels] - outLevels[0], currentItemCount);
...
uint32_t KllSketch<T, A, C>::insertPosition() {
  if (levels_[0] == 0) {
    ...
    // current level.
    BOLT_DCHECK_EQ(levels_[level], rawBeg + halfAdjPop);
...
int KllSketch<T, A, C>::findLevelToCompact() const {
  for (int level = 0;; ++level) {
    BOLT_DCHECK_LT(level + 1, levels_.size());
...
void KllSketch<T, A, C>::addEmptyTopLevelToCompletelyFullSketch() {
  ...
  BOLT_DCHECK_EQ(levels_[0], 0);
  BOLT_DCHECK_EQ(items_.size(), curTotalCap);

  const uint32_t deltaCap = detail::levelCapacity(k_, numLevels() + 1, 0);
  shiftItems(deltaCap);
  BOLT_DCHECK_EQ(levels_.back(), curTotalCap + deltaCap);
...
void KllSketch<T, A, C>::compact() {
  ...
  BOLT_DCHECK_LE(k, items_.size());
  items_.resize(k);
  BOLT_DCHECK_EQ(items_.size(), levels_.back());
}
...
void KllSketch<T, A, C>::estimateQuantiles(
    ...
    T* out) const {
  BOLT_USER_CHECK_GT(n_, 0, "estimateQuantiles called on empty sketch");
...
void KllSketch<T, A, C>::mergeViews(const folly::Range<const View*>& others) {
  ...
  if (tmpNumItems > getNumRetained()) {
    ...
    auto result = detail::generalCompress<T, C>(
        ...
        isLevelZeroSorted_,
        randomBit_);
    BOLT_DCHECK_LE(result.finalNumLevels, ub);
  ...
  n_ = newN;
  BOLT_DCHECK_EQ(detail::sumSampleWeights(numLevels(), levels_.data()), n_);
}
...
void KllSketch<T, A, C>::serialize(char* out) const {
  ...
  BOLT_DCHECK_EQ(i, serializedByteSize());
}
...
void KllSketch<T, A, C>::View::deserialize(const char* data) {
  ...
  BOLT_CHECK_EQ(version, detail::kVersion, "Unsupported version: {}", version);
...
KllSketch<T, A, C> KllSketch<T, A, C>::fromRepeatedValue(
  ...
  BOLT_CHECK_LE(numLevels, detail::kMaxLevel);
  KllSketch<T, A, C> ans(allocator, seed);
  ans.k_ = k;
  ans.n_ = count;
