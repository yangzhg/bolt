...
SpillStats::SpillStats(
    ...
    uint64_t _spillConvertTimeUs,
    uint64_t _spillTotalTimeUs,
    ...
    : spillRuns(_spillRuns),
      ...
      spillConvertTimeUs(_spillConvertTimeUs),
      spillTotalTimeUs(_spillTotalTimeUs),
...
SpillStats& SpillStats::operator+=(const SpillStats& other) {
  ...
  spillConvertTimeUs += other.spillConvertTimeUs;
  spillTotalTimeUs += other.spillTotalTimeUs;
...
SpillStats SpillStats::operator-(const SpillStats& other) const {
  ...
  result.spillConvertTimeUs = spillConvertTimeUs - other.spillConvertTimeUs;
  result.spillTotalTimeUs = spillTotalTimeUs - other.spillTotalTimeUs;
...
#define UPDATE_COUNTER(counter)           \
  ...
  UPDATE_COUNTER(spillConvertTimeUs);
  UPDATE_COUNTER(spillTotalTimeUs);
...
bool SpillStats::operator==(const SpillStats& other) const {
  return std::tie(
      ...
      std::tie(
             ...
             other.spillConvertTimeUs,
             other.spillTotalTimeUs,
...
std::string SpillStats::toString() const {
  return fmt::format(
      "spillRuns[{}] spilledInputBytes[{}] spilledBytes[{}] spilledRows[{}] spilledPartitions[{}] spilledFiles[{}] spillTotalTimeUs[{}] spillFillTimeUs[{}] spillSortTime[{}] spillConvertTime[{}] spillSerializationTime[{}] spillWrites[{}] spillFlushTime[{}] spillWriteTime[{}] maxSpillExceededLimitCount[{}]",
      spillRuns,
      ...
      spilledPartitions,
      spilledFiles,
      succinctMicros(spillTotalTimeUs),
      succinctMicros(spillFillTimeUs),
      succinctMicros(spillSortTimeUs),
      succinctMicros(spillConvertTimeUs),
...
void updateGlobalSpillConvertTime(uint64_t timeUs) {
  RECORD_HISTOGRAM_METRIC_VALUE(kMetricSpillConvertTimeMs, timeUs / 1'000);
  localSpillStats().wlock()->spillConvertTimeUs += timeUs;
}

void updateGlobalSpillTotalTime(uint64_t timeUs) {
  RECORD_HISTOGRAM_METRIC_VALUE(kMetricSpillTotalTimeMs, timeUs / 1'000);
  localSpillStats().wlock()->spillTotalTimeUs += timeUs;
}
