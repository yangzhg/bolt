...
class SimpleFunctionAdapter : public VectorFunction {
  ...
  using func_return_type = typename FUNC::return_type;
 ...
 public:
  ...
  template <
    ...
    if constexpr (isVariadicType<arg_at<POSITION>>::value) {
      if constexpr (std::is_same_v<
                        typename arg_at<POSITION>::underlying_type,
                        func_return_type>) {
      ...
      return nullptr;
    } else if constexpr (std::is_same_v<arg_at<POSITION>, func_return_type>) {
      using type =
  ...
  void apply(
    ...
    if (reuseStringsFromArg >= 0) {
      BOLT_CHECK_LT(reuseStringsFromArg, args.size());
