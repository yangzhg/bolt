...
int64_t RowGroupWriter::total_compressed_bytes_all() const {
...
class RowGroupSerializer : public RowGroupWriter::Contents {
 public:
  ...
  int64_t total_bytes_written() const override {
    ...
    int64_t total_bytes_written = total_bytes_written_;
    for (size_t i = 0; i < column_writers_.size(); i++) {
      if (column_writers_[i]) {
  ...
  int64_t total_compressed_bytes_written() const override {
    ...
    int64_t total_compressed_bytes_written = total_compressed_bytes_written_;
    for (size_t i = 0; i < column_writers_.size(); i++) {
      if (column_writers_[i]) {
  ...
  int64_t total_compressed_bytes_all() const override {
    if (closed_ || !buffered_row_group_) {
      // After closed, there are no more bytes in the buffer and encoder, then
      // `total_compressed_bytes_written` has same value with
      // `total_compressed_bytes_all`
      return total_compressed_bytes_written();
    }
    int64_t total_compressed_bytes_all = 0;
    for (size_t i = 0; i < column_writers_.size(); i++) {
      if (column_writers_[i]) {
        total_compressed_bytes_all +=
            column_writers_[i]->total_compressed_bytes_all();
      }
    }
