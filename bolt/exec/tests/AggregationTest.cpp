...
void checkRowBasedSpillStats(PlanNodeStats& stats, bool isRowBased) {
  if (isRowBased) {
    ASSERT_EQ(stats.customStats["spillConvertTime"].sum, 0);
  }
}

class AggregationTest : public OperatorTestBase,
                        public WithGPUParamInterface<> {
 protected:
  ...
  static void TearDownTestCase() {
    FLAGS_bolt_testing_enable_arbitration = false;
  ...
  void testSingleKey(
    ...
    if (!distinct) {
      ...
      if (useGPU) {
        // NOTE: The current GPU operator does not support custom aggregate
        // functions such as sumnonpod().
        // Testing with sumnonpod is not valid although it passes.
        // That is because it does not modify values such as
        // NonPODInt64::constructed, NonPODInt64::destructed.
        aggregates.push_back("sum(1)");
      } else {
        aggregates.push_back("sumnonpod(1)");
      }
    }
  ...
  void makeModeTestKeys(
    ...
    for (auto count = 0; count < numRows; ++count) {
      if (count % 1000 == 0) {
        const auto batchSize = std::min(1000, numRows - count);
        rowVector =
            BaseVector::create<RowVector>(rowType, batchSize, pool_.get());
...
TEST_P(AggregationTest, missingFunctionOrSignature) {
...
TEST_P(AggregationTest, missingLambdaFunction) {
  auto data = makeRowVector({
      makeFlatVector<int64_t>({1, 2, 3}),
  });
...
TEST_P(AggregationTest, singleBigintKey) {
  auto vectors = makeVectors(rowType_, 10, 100);
  createDuckDbTable(vectors);
  testSingleKey<int64_t>(vectors, "c0", false, false, GetParam().useGPU);
  testSingleKey<int64_t>(vectors, "c0", true, false, GetParam().useGPU);
}

TEST_P(AggregationTest, singleBigintKeyDistinct) {
  auto vectors = makeVectors(rowType_, 10, 100);
  createDuckDbTable(vectors);
  testSingleKey<int64_t>(vectors, "c0", false, true, GetParam().useGPU);
  testSingleKey<int64_t>(vectors, "c0", true, true, GetParam().useGPU);
}

TEST_P(AggregationTest, singleStringKey) {
  auto vectors = makeVectors(rowType_, 10, 100);
  createDuckDbTable(vectors);
  testSingleKey<StringView>(vectors, "c6", false, false, GetParam().useGPU);
  testSingleKey<StringView>(vectors, "c6", true, false, GetParam().useGPU);
}

TEST_P(AggregationTest, singleStringKeyDistinct) {
  auto vectors = makeVectors(rowType_, 10, 100);
  createDuckDbTable(vectors);
  testSingleKey<StringView>(vectors, "c6", false, true, GetParam().useGPU);
  testSingleKey<StringView>(vectors, "c6", true, true, GetParam().useGPU);
}

TEST_P(AggregationTest, multiKey) {
  auto vectors = makeVectors(rowType_, 10, 100);
  createDuckDbTable(vectors);
  testMultiKey(vectors, false, false, GetParam().useGPU);
  testMultiKey(vectors, true, false, GetParam().useGPU);
}

TEST_P(AggregationTest, multiKeyDistinct) {
  auto vectors = makeVectors(rowType_, 10, 100);
  createDuckDbTable(vectors);
  testMultiKey(vectors, false, true, GetParam().useGPU);
  testMultiKey(vectors, true, true, GetParam().useGPU);
}

TEST_P(AggregationTest, aggregateOfNulls) {
...
TEST_P(AggregationTest, setNull) {
...
TEST_P(AggregationTest, rangeToDistinct) {
  ...
  RowVectorPtr output = AssertQueryBuilder(op).copyResults(pool_.get());

  std::cout << output->toString() << std::endl;
}

TEST_P(AggregationTest, allKeyTypes) {
...
TEST_P(AggregationTest, partialAggregationMemoryLimit) {
  if (GetParam().useGPU) {
    GTEST_SKIP()
        << "GPU Aggregation does not support spilling and statistics\n";
  }
...
TEST_P(AggregationTest, partialDistinctWithAbandon) {
  auto vectors = {
...
TEST_P(AggregationTest, toIntermediate) {
  ...
  task =
      AssertQueryBuilder(duckDbQueryRunner_)
          ...
          .plan(PlanBuilder()
                    .values(vectors)
                    .partialAggregation(
                        {"c0"}, {"count(c0)"}, {}, GetParam().useGPU)
                    .finalAggregation(GetParam().useGPU)
                    .planNode())
          .assertResults("SELECT distinct c0, count(c0) FROM tmp group by c0");
  ...
  task = AssertQueryBuilder(duckDbQueryRunner_)
             ...
             .plan(PlanBuilder()
                       .values(vectors)
                       .partialAggregation(
                           {"c0"}, {"avg(c0)"}, {}, GetParam().useGPU)
                       .finalAggregation(GetParam().useGPU)
                       .planNode())
             .assertResults("SELECT distinct c0, avg(c0) FROM tmp group by c0");
}

TEST_P(AggregationTest, toIntermediateWithFilter) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support filter\n";
  }
  ...
  std::vector<VectorPtr> c2 = {
      ...
      // 3rd batch will not produce any new distinct values.
      makeFlatVector<bool>(50, [](auto row) { return true; }),
      // 4th batch will not produce 10 new distinct values.
      makeFlatVector<bool>(200, [](auto row) { return false; })};

  std::vector<RowVectorPtr> vectors;
  for (int i = 0; i < c0.size(); i++) {
    vectors.push_back(makeRowVector({c0[i], c1[i], c2[i]}));
  }
  ...
  task = AssertQueryBuilder(duckDbQueryRunner_)
             ...
             .plan(PlanBuilder()
                       .values(vectors)
                       .partialAggregation(
                           {"c0"}, {"count(c1)"}, {}, GetParam().useGPU)
                       .finalAggregation(GetParam().useGPU)
                       .planNode())
             .assertResults("SELECT c0, count(c1) FROM tmp group by c0");
  ...
  task = AssertQueryBuilder(duckDbQueryRunner_)
             ...
             .plan(PlanBuilder()
                       .values(vectors)
                       .partialAggregation(
                           {"c0"}, {"sum(c1)"}, {"c2"}, GetParam().useGPU)
                       .finalAggregation(GetParam().useGPU)
                       .planNode())
             .assertResults(
                 "SELECT c0, sum(c1) FILTER (WHERE c2) FROM tmp group by c0");
  ...
  task = AssertQueryBuilder(duckDbQueryRunner_)
             ...
             .plan(PlanBuilder()
                       .values(vectors)
                       .partialAggregation(
                           {"c0"}, {"avg(c1)"}, {"c2"}, GetParam().useGPU)
                       .finalAggregation(GetParam().useGPU)
                       .planNode())
             .assertResults(
                 "SELECT c0, avg(c1) FILTER (WHERE c2) FROM tmp group by c0");
  ...
  task = AssertQueryBuilder(duckDbQueryRunner_)
             ...
             .plan(PlanBuilder()
                       .values(vectors)
                       .partialAggregation(
                           {"c0"}, {"count(c1)"}, {"c2"}, GetParam().useGPU)
                       .finalAggregation(GetParam().useGPU)
                       .planNode())
             .assertResults(
                 "SELECT c0, count(c1) FILTER (WHERE c2) FROM tmp group by c0");
  ...
  task = AssertQueryBuilder(duckDbQueryRunner_)
             ...
             .plan(PlanBuilder()
                       .values(vectors)
                       .partialAggregation(
                           {"c0"}, {"min(c1)"}, {"c2"}, GetParam().useGPU)
                       .finalAggregation(GetParam().useGPU)
                       .planNode())
             .assertResults(
                 "SELECT c0, min(c1) FILTER (WHERE c2) FROM tmp group by c0");
  ...
  task = AssertQueryBuilder(duckDbQueryRunner_)
             ...
             .plan(PlanBuilder()
                       .values(vectors)
                       .partialAggregation(
                           {"c0"}, {"max(c1)"}, {"c2"}, GetParam().useGPU)
                       .finalAggregation(GetParam().useGPU)
                       .planNode())
             .assertResults(
                 "SELECT c0, max(c1) FILTER (WHERE c2) FROM tmp group by c0");
}

TEST_P(AggregationTest, largeValueRangeArray) {
  ...
  auto op =
      PlanBuilder()
          .values({vectors})
          .partialAggregation({"c0"}, {"array_agg(c1)"}, {}, GetParam().useGPU)
...
TEST_P(AggregationTest, partialAggregationMemoryLimitIncrease) {
  if (GetParam().useGPU) {
...
TEST_P(AggregationTest, partialAggregationMaybeReservationReleaseCheck) {
  auto vectors = {
      makeRowVector({makeFlatVector<int32_t>(
          100, [](auto row) { return row; }, nullEvery(5))}),
      makeRowVector({makeFlatVector<int32_t>(
...
TEST_P(AggregationTest, rowBasedspillWithMemoryLimit) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support spilling\n";
  }
  ...
  constexpr int64_t kMaxBytes = 1LL << 30; // 1GB
  rng_.seed(1);
  rowType_ = ROW({"c0", "c1", "c2"}, {INTEGER(), VARCHAR(), VARCHAR()});
  auto batches = makeVectors(rowType_, 100, 5);

  core::PlanNodeId aggrNodeId;
  const auto plan = PlanBuilder()
                        .values(batches)
                        .singleAggregation({"c0"}, {"max(c1)", "min(c2)"}, {})
                        .capturePlanNodeId(aggrNodeId)
                        .planNode();
  const auto expectedResults =
      AssertQueryBuilder(plan).copyResults(pool_.get());
...
TEST_P(AggregationTest, DISABLED_spillWithNonSpillingPartition) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support spilling\n";
  }
...
TEST_P(AggregationTest, spillAll) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support spilling\n";
  }
...
TEST_P(AggregationTest, DISABLED_memoryAllocations) {
...
TEST_P(AggregationTest, optimizedDistinctAggregationPath) {
  // NOTE: Unlike CPU operator, GPU Aggregation does not have optimized distinct
  // aggregation path

  // Create test data with a known number of distinct values
  auto largeBatch = makeRowVector({
      makeFlatVector<StringView>(
          5000,
          [](auto row) {
            // Ensure we have exactly 1000 distinct values (0-999)
            return StringView::makeInline(fmt::format("str_{}", row % 1000));
          }),
  });

  createDuckDbTable({largeBatch});

  // Test with the standard SQL comparison
  assertQuery(
      PlanBuilder()
          .values({largeBatch})
          .singleAggregation({"c0"}, {}, {}, GetParam().useGPU)
          .planNode(),
      "SELECT c0 FROM tmp GROUP BY c0");

  // Test with multiple batches
  std::vector<RowVectorPtr> multiBatch;
  for (int i = 0; i < 3; i++) {
    multiBatch.push_back(makeRowVector({
        makeFlatVector<StringView>(
            2000,
            [](auto row) {
              return StringView::makeInline(fmt::format("str_{}", row % 1000));
  ...
  auto intBatch = makeRowVector({
      makeFlatVector<int32_t>(5000, [](auto row) { return row % 1000; }),
  });

  createDuckDbTable({intBatch});
  assertQuery(
      PlanBuilder()
          .values({intBatch})
          .singleAggregation({"c0"}, {}, {}, GetParam().useGPU)
...
TEST_P(AggregationTest, compareDistinctVsSumAggPerformance) {
  // Create test data with sufficient size to measure performance differences
  auto vectors = makeVectors(rowType_, 10'000, 10);
  createDuckDbTable(vectors);

  // Capture plan node IDs for stats collection
  core::PlanNodeId distinctNodeId;
  core::PlanNodeId sumNodeId;

  // Test distinct aggregation (simple group by)
  auto distinctPlan = PlanBuilder()
                          .values(vectors)
                          .aggregation(
                              {"c0"},
                              {},
  ...
  std::shared_ptr<Task> distinctTask;

  auto distinctResult =
      AssertQueryBuilder(distinctPlan).copyResults(pool(), distinctTask);
  auto distinctStats = toPlanStats(distinctTask->taskStats());

  // Test sum aggregation
  auto sumPlan = PlanBuilder()
                     .values(vectors)
                     .aggregation(
                         {"c0"},
                         {},
  ...
  std::shared_ptr<Task> sumTask;
  auto sumResult = AssertQueryBuilder(sumPlan).copyResults(pool(), sumTask);
  auto sumStats = toPlanStats(sumTask->taskStats());

  // Compare processing times
  auto distinctTime = distinctStats.at("0").cpuWallTiming.wallNanos;
  auto sumTime = sumStats.at("0").cpuWallTiming.wallNanos;

  LOG(INFO) << "Distinct aggregation time: " << distinctTime << "ns";
  LOG(INFO) << "Sum aggregation time: " << sumTime << "ns";
}

TEST_P(AggregationTest, groupingSets) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support grouping set";
  }
  ...
  plan = PlanBuilder()
             ...
             .groupId({"k1", "k2"}, {{"k1"}, {"k2"}}, {})
             .singleAggregation(
                 {"k1", "k2", "group_id"}, {}, {}, GetParam().useGPU)
             .project({"k1", "k2"})
             .planNode();
...
TEST_P(AggregationTest, groupingSetsOutput) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support grouping set";
  }
...
TEST_P(AggregationTest, groupingSetsSameKey) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support grouping set";
  }
...
TEST_P(AggregationTest, groupingSetsEmptyInput) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support grouping set";
  }
  ...
  plan = PlanBuilder()
             ...
             .partialAggregation(
                 ...
                 {},
                 GetParam().useGPU)
             .intermediateAggregation(GetParam().useGPU)
             .finalAggregation(GetParam().useGPU)
             .project({"count_c2"})
             .planNode();
  ...
  plan = PlanBuilder()
             ...
             .filter("c1 < 0")
             .groupId({"c1"}, {{"c1"}, {}}, {})
             .partialAggregation({"c1", "group_id"}, {}, {}, GetParam().useGPU)
             .finalAggregation(GetParam().useGPU)
             .project({"c1"})
             .planNode();
  ...
  plan = PlanBuilder()
             ...
             .filter("c1 < 0")
             .groupId({"c1"}, {{}, {}}, {})
             .singleAggregation({"c1", "group_id"}, {}, {}, GetParam().useGPU)
             .planNode();
...
TEST_P(AggregationTest, outputBatchSizeCheckWithSpill) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support spilling\n";
  }
...
TEST_P(AggregationTest, spillDuringOutputProcessing) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support spilling\n";
  }
...
TEST_P(AggregationTest, outputBatchSizeCheckWithoutSpill) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support spilling\n";
  }
...
DEBUG_ONLY_TEST_P(AggregationTest, minSpillableMemoryReservation) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support spilling\n";
  }
  ...
  for (int32_t minSpillableReservationPct : {5, 50, 100}) {
    ...
    SCOPED_TESTVALUE_SET(
        "bytedance::bolt::exec::GroupingSet::addInputForActiveRows",
...
TEST_P(AggregationTest, distinctWithSpilling) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support spilling\n";
  }
...
TEST_P(AggregationTest, distinctSpillWithMemoryLimit) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support spilling\n";
  }
  ...
  for (const auto& testData : testSettings) {
    ...
    queryCtx->testingOverrideMemoryPool(
        memory::memoryManager()->addRootPool(queryCtx->queryId(), kMaxBytes));
    auto results = AssertQueryBuilder(PlanBuilder()
                                          .values(batches)
                                          .singleAggregation({"c0"}, {}, {})
                                          .planNode())
                       .queryCtx(queryCtx)
                       .copyResults(pool_.get());
    auto task = AssertQueryBuilder(PlanBuilder()
                                       .values(batches)
                                       .singleAggregation({"c0"}, {}, {})
                                       .planNode())
                    .queryCtx(queryCtx)
    ...
    if (testData.expectSpill) {
      ...
      ASSERT_GT(stats.runtimeStats[Operator::kSpillWrites].sum, 0);
      ...
      ASSERT_EQ(stats.runtimeStats[Operator::kSpillWrites].sum, 0);
    ...
    ASSERT_EQ(
        stats.runtimeStats[Operator::kSpillWrites].count,
        stats.runtimeStats["spillWriteTime"].count);
    OperatorTestBase::deleteTaskAndCheckSpillDirectory(task);
  }
}

TEST_P(AggregationTest, spillingForAggrsWithSorting) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support spilling\n";
  }
...
TEST_P(AggregationTest, preGroupedAggregationWithSpilling) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support spilling\n";
  }
...
TEST_P(AggregationTest, adaptiveOutputBatchRows) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support output batch rows";
  }
  ...
  auto plan = PlanBuilder()
                  .values(vectors)
                  .singleAggregation({"k0"}, {"sum(c0)"}, {}, GetParam().useGPU)
                  .planNode();
...
DEBUG_ONLY_TEST_P(AggregationTest, reclaimDuringReserve) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support spilling\n";
  }
...
DEBUG_ONLY_TEST_P(AggregationTest, reclaimDuringAllocation) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support spilling\n";
  }
...
DEBUG_ONLY_TEST_P(AggregationTest, reclaimDuringOutputProcessing) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support spilling\n";
  }
...
DEBUG_ONLY_TEST_P(AggregationTest, reclaimDuringNonReclaimableSection) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support spilling\n";
  }
...
DEBUG_ONLY_TEST_P(AggregationTest, reclaimWithEmptyAggregationTable) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support spilling\n";
  }
...
void abortPool(memory::MemoryPool* pool) {
  try {
    BOLT_FAIL("Memory pool manually aborted");
...
DEBUG_ONLY_TEST_P(AggregationTest, abortDuringOutputProcessing) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support spilling\n";
  }
...
DEBUG_ONLY_TEST_P(AggregationTest, abortDuringInputgProcessing) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support spilling\n";
  }
...
TEST_P(AggregationTest, noAggregationsNoGroupingKeys) {
  auto data = makeRowVector({
  ...
  auto plan = PlanBuilder()
                  .values({data})
                  .partialAggregation({}, {}, {}, GetParam().useGPU)
                  .finalAggregation()
                  .planNode();
...
TEST_P(AggregationTest, distinctHang) {
  ...
  auto plan = PlanBuilder()
                  ...
                  .partialAggregation({"c0", "c1"}, {}, {}, GetParam().useGPU)
                  .capturePlanNodeId(aggNodeId)
                  .planNode();
...
TEST_P(AggregationTest, memoryPoolAllocationAtInit) {
  auto data = makeRowVector({
...
DEBUG_ONLY_TEST_P(AggregationTest, reclaimEmptyInput) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support spilling\n";
  }
...
DEBUG_ONLY_TEST_P(AggregationTest, reclaimEmptyOutput) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support spilling\n";
  }
...
TEST_P(AggregationTest, maxSpillBytes) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support spilling\n";
  }
...
DEBUG_ONLY_TEST_P(AggregationTest, reclaimFromAggregation) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support spilling\n";
  }
...
DEBUG_ONLY_TEST_P(AggregationTest, reclaimFromDistinctAggregation) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support spilling\n";
  }
...
DEBUG_ONLY_TEST_P(AggregationTest, reclaimFromAggregationOnNoMoreInput) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support spilling\n";
  }
...
DEBUG_ONLY_TEST_P(AggregationTest, reclaimFromAggregationDuringOutput) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support spilling\n";
  }
...
TEST_P(AggregationTest, reclaimFromCompletedAggregation) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support spilling\n";
  }
...
TEST_P(AggregationTest, ignoreNullKeys) {
  // Some keys are null.
  auto data = makeRowVector({
...
TEST_P(AggregationTest, mutliKeysWithStringSpill) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support spilling\n";
  }

  std::vector<std::string> strs{
      "aaaaaaaaaaaaa", "aaaaaaaaaaaaa", "bb", "cccccccccccc"};
  std::vector<RowVectorPtr> data;

  constexpr size_t BATCH_NUM = 20;
  constexpr size_t BATCH_SIZE = 1000;
  for (auto i = 0; i < BATCH_NUM; i++) {
    auto batch = makeRowVector({
        makeFlatVector<int32_t>(
            BATCH_SIZE,
            [&i](auto row) -> int32_t { return i * BATCH_SIZE + row; },
            nullptr),
        makeFlatVector<StringView>(
            BATCH_SIZE,
            [&strs](auto row) {
              auto x = StringView(strs[row % strs.size()]);
              return x;
            }),
        makeFlatVector<int64_t>(
            BATCH_SIZE,
            [&i](auto row) -> int64_t { return i * 10 + (row + 1) * 10; }),
    });
...
TEST_P(AggregationTest, partialSpillWithMemoryLimit) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support spilling\n";
  }

  constexpr int32_t kNumDistinct = 2;
  constexpr int64_t kMaxBytes = 1LL << 30; // 1GB
  rng_.seed(1);
  rowType_ = ROW({"c0", "c1", "c2"}, {INTEGER(), INTEGER(), INTEGER()});
  auto batches = makeVectors(rowType_, 100, 5);
  ...
  for (const auto& testData : testSettings) {
    ...
    auto task =
        AssertQueryBuilder(plan)
            .spillDirectory(spillDirectory->path)
            .config(QueryConfig::kSpillEnabled, "true")
            // make sure aggregation ratio threshold reached
            ...
            .config(
                QueryConfig::kAggregationSpillMemoryThreshold,
                std::to_string(testData.aggregationMemLimit))
            .config(QueryConfig::kAbandonPartialAggregationMinRows, 50)
            .assertResults(expectedResults);
...
TEST_P(AggregationTest, partialDistinctSpillWithMemoryLimit) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support spilling\n";
  }
...
TEST_P(AggregationTest, rowBasedSpillNull) {
  if (GetParam().useGPU) {
    GTEST_SKIP() << "GPU Aggregation does not support spilling\n";
  }
  ...
  const auto plan =
      PlanBuilder()
          .values(batches)
          .project({"c0", "c1", "c2", "null as c3"})
          .partialAggregation({"c3", "c0"}, {"count(1)", "sum(c1)"})
          .capturePlanNodeId(partialAggrNodeId)
          .finalAggregation()
          .planNode();
  const auto expectedResults =
