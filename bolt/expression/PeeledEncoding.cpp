...
SelectivityVector* PeeledEncoding::translateToInnerRows(
  ...
  BOLT_CHECK(wrapEncoding_ != VectorEncoding::Simple::FLAT);
  ...
  bolt::translateToInnerRows(outerRows, indices, flatNulls, *newRows);
...
inline void setPeeled(
  ...
  BOLT_DCHECK(peeled.size() > fieldIndex);
...
bool PeeledEncoding::peelInternal(
  ...
  if (firstPeeled == -1) {
    ...
    if (decodedVector.isConstantMapping()) {
      ...
      BOLT_CHECK(peeledVectors.size() == 1);
      auto innerIdx = decodedVector.index(rows.begin());
      BOLT_CHECK(peeledVectors.back()->isNullAt(innerIdx));
      wrapEncoding_ = VectorEncoding::Simple::CONSTANT;
