...
// Union two integer arrays with null.
TEST_F(ArrayUnionTest, nullArrayUdf) {
  const auto array1 = makeNullableArrayVector<int64_t>({
      {{1, std::nullopt, 3, 4}},
      {7, 8, 9},
      {{10, std::nullopt, std::nullopt}},
  ...
  testExpression("array_union_udf(c0, c1)", {array1, array2}, expected);
}

TEST_F(ArrayUnionTest, nanArrayUdf) {
  const auto array1 = makeNullableArrayVector<double>({
      {1.3, std::nullopt, 3.5, std::numeric_limits<double>::quiet_NaN()},
  });
  const auto array2 = makeNullableArrayVector<double>({
      {std::nullopt, 3.5, 10.4, std::numeric_limits<double>::quiet_NaN(), 5.7},
  });

  VectorPtr expected;
  expected = makeNullableArrayVector<double>({
      {std::nullopt,
       1.3,
       3.5,
       5.7,
