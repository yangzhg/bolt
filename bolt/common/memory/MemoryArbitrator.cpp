...
namespace {
#define MEM_POOL_CAP_EXCEEDED_NOOP(errorMessage, requestPool) \
...
class NoopArbitrator : public MemoryArbitrator {
 public:
  ...
  bool growCapacity(MemoryPool* pool, uint64_t size) override {
    MEM_POOL_CAP_EXCEEDED_NOOP(
        fmt::format(
            "Exceeded memory pool capacity."
            ...
            pool->name(),
            succinctBytes(size),
            succinctBytes(pool->maxCapacity()),
...
uint64_t MemoryReclaimer::run(
    ...
    Stats& stats) {
  BOLT_CHECK(underMemoryArbitration());
  ...
  BOLT_CHECK_GE(reclaimedBytes, 0);
...
bool MemoryReclaimer::reclaimableBytes(
  ...
  BOLT_CHECK(reclaimable || reclaimableBytes == 0);
  return reclaimable;
}
...
uint64_t MemoryReclaimer::reclaim(
  ...
  for (const auto& candidate : candidates) {
    BOLT_CHECK_GT(candidate.reclaimableBytes, 0);
...
ScopedReclaimedBytesRecorder::ScopedReclaimedBytesRecorder(
  ...
  BOLT_CHECK_NOT_NULL(reclaimedBytes_);
  BOLT_CHECK_EQ(*reclaimedBytes_, 0);
}
