 ...
 public:
  ...
  void apply(
    ...
    if constexpr (isIntersect) {
      if (!constantSet_.has_value() && left->itemSize() < right->itemSize()) {
        left = args[1].get();
        right = args[0].get();
      }
    }
    ...
    if (baseLeftArray->elements()->size() >
        std::max<vector_size_t>(baseLeftArray->size(), 4096)) {
      BaseVector::flattenVector(newElements);
    }
...
SetWithNull<T> validateConstantVectorAndGenerateSet(
  ...
  auto constantVector = baseVector->as<ConstantVector<bolt::ComplexType>>();
  auto constantArray = constantVector->as<ConstantVector<bolt::ComplexType>>();
  BOLT_CHECK_NOT_NULL(constantArray, "wrong constant type found");
  BOLT_CHECK_NOT_NULL(constantVector, "wrong constant type found");
  auto arrayVecPtr = constantVector->valueVector()->as<ArrayVector>();
  BOLT_CHECK_NOT_NULL(arrayVecPtr, "wrong array literal type");
...
std::shared_ptr<exec::VectorFunction> createArrayIntersect(
  ...
  return BOLT_DYNAMIC_SCALAR_TEMPLATE_TYPE_DISPATCH(
