 ...
 public:
  DecimalRoundFunction(
    ...
    auto rescaleFactor = [&](int32_t rescale) {
      ...
      return DecimalUtil::getPowersOfTen(
          std::min(rescale, (int32_t)LongDecimalType::kMaxPrecision));
    };
    if (scale_ < 0) {
...
TypePtr DecimalRoundCallToSpecialForm::resolveType(
  ...
  BOLT_FAIL("Decimal round function does not support type resolution.");
}
