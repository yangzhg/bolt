...
class EvalCtx {
 public:
  EvalCtx(
      ...
      const bool acceptNullableChild = false);
  ...
  // If exceptionPtr is known to be a BoltException use setBoltExceptionError
  // instead.
  ...
  // for sure that exception_ptr is a bolt exception.
  void setBoltExceptionError(
      vector_size_t index,
  ...
  void applyToSelectedNoThrow(const SelectivityVector& rows, Callable func) {
    rows.template applyToSelected([&](auto row) INLINE_LAMBDA {
      try {
        ...
        setBoltExceptionError(row, std::current_exception());
  ...
  void moveOrCopyResult(
    ...
    if (resultShouldBePreserved(result, rows)) {
      ...
      result->copy(localResult.get(), rows, nullptr, false);
    } else {
      result = localResult;
    }
  }
  ...
  StringView currentSplitStr() {
    return StringView(currentSplitStr_.c_str(), currentSplitStr_.length());
  }

  void setNegatedRelation(const bool isNegated) {
    isNegatedRelation_ = isNegated;
  }

  bool isNegatedRelation() const {
    return isNegatedRelation_;
  }
...
class LocalSelectivityVector {
 public:
  ...
  SelectivityVector& operator*() {
    BOLT_DCHECK_NOT_NULL(vector_, "get(size) must be called.");
    return *vector_;
  }
...
class LocalDecodedVector {
 public:
  ...
  DecodedVector& operator*() {
    BOLT_DCHECK_NOT_NULL(vector_, "get() must be called.");
    return *vector_;
  }
...
class ScopeNegatedRelationSetter {
 public:
  ScopeNegatedRelationSetter(EvalCtx& evalCtx, bool isNegated)
      : evalCtx_(evalCtx), oldIsNegated_(evalCtx_.isNegatedRelation()) {
    if (!oldIsNegated_) {
      evalCtx_.setNegatedRelation(isNegated);
    }
  }

  ~ScopeNegatedRelationSetter() {
    evalCtx_.setNegatedRelation(oldIsNegated_);
  }

 private:
  EvalCtx& evalCtx_;
  const bool oldIsNegated_;
};
