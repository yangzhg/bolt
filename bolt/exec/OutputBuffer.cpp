...
void ArbitraryBuffer::enqueue(std::unique_ptr<SerializedPage> page) {
  BOLT_CHECK_NOT_NULL(page, "Unexpected null page");
  BOLT_CHECK(!hasNoMoreData(), "Arbitrary buffer has set no more data marker");
...
std::vector<std::shared_ptr<SerializedPage>> ArbitraryBuffer::getPages(
    uint64_t maxBytes) {
  BOLT_CHECK_GT(maxBytes, 0, "maxBytes can't be zero");
...
void DestinationBuffer::Stats::recordEnqueue(const SerializedPage& data) {
  ...
  BOLT_CHECK(numRows.has_value(), "SerializedPage's numRows must be valid");
...
void DestinationBuffer::Stats::recordAcknowledge(const SerializedPage& data) {
  ...
  bytesBuffered -= size;
  BOLT_DCHECK_GE(bytesBuffered, 0, "bytesBuffered must be non-negative");
  rowsBuffered -= numRows.value();
  BOLT_DCHECK_GE(rowsBuffered, 0, "rowsBuffered must be non-negative");
  --pagesBuffered;
  BOLT_DCHECK_GE(pagesBuffered, 0, "pagesBuffered must be non-negative");
  bytesSent += size;
...
std::vector<std::unique_ptr<folly::IOBuf>> DestinationBuffer::getData(
  ...
  for (auto i = sequence - sequence_; i < data_.size(); ++i) {
    ...
    if (data_[i] == nullptr) {
      BOLT_CHECK_EQ(i, data_.size() - 1, "null marker found in the middle");
...
void DestinationBuffer::finish() {
  BOLT_CHECK_NULL(notify_, "notify must be cleared before finish");
  BOLT_CHECK(data_.empty(), "data must be fetched before finish");
  stats_.finished = true;
}

void DestinationBuffer::maybeLoadData(ArbitraryBuffer* buffer) {
  BOLT_CHECK(!buffer->empty() || buffer->hasNoMoreData());
  if (notify_ == nullptr) {
    return;
  }
  ...
  BOLT_CHECK_GT(notifyMaxBytes_, 0);
...
OutputBuffer::OutputBuffer(
    ...
    : task_(std::move(task)),
      ...
      numPushBasedDriverSchedulers_(numDrivers) {
...
void OutputBuffer::updateOutputBuffers(int numBuffers, bool noMoreBuffers) {
  if (isPartitioned()) {
    BOLT_CHECK_EQ(buffers_.size(), numBuffers);
    BOLT_CHECK(noMoreBuffers);
    noMoreBuffers_ = true;
    return;
  }
...
void OutputBuffer::updateNumDrivers(uint32_t newNumDrivers) {
  bool isNoMoreDrivers{false};
  {
    ...
    if (is_morsel_driven_ ? numFinishedPushBasedDriverSchedulers_ ==
                    numPushBasedDriverSchedulers_ &&
                numDrivers_ == numFinished_
                          : numDrivers_ == numFinished_) {
      isNoMoreDrivers = true;
      LOG(ERROR)
          << "OutputBuffer::updateNumDrivers numPushBasedDriverSchedulers_="
          << numPushBasedDriverSchedulers_
...
void OutputBuffer::updateStatsWithFreedPagesLocked(
  ...
  bufferedBytes_ -= pageBytes;
  BOLT_CHECK_GE(bufferedBytes_, 0);
  bufferedPages_ -= numPages;
  BOLT_CHECK_GE(bufferedPages_, 0);
}
  ...
  bool blocked = false;
  {
    ...
    switch (kind_) {
      ...
      case PartitionedOutputNode::Kind::kPartitioned:
        ...
        break;
      default:
        BOLT_UNREACHABLE(PartitionedOutputNode::kindString(kind_));
    }
...
void OutputBuffer::enqueueBroadcastOutputLocked(
  ...
  BOLT_DCHECK(isBroadcast());
  BOLT_CHECK_NULL(arbitraryBuffer_);
  BOLT_DCHECK(dataAvailableCbs.empty());
...
void OutputBuffer::enqueueArbitraryOutputLocked(
    std::unique_ptr<SerializedPage> data,
    std::vector<DataAvailable>& dataAvailableCbs) {
  BOLT_DCHECK(isArbitrary());
  BOLT_DCHECK_NOT_NULL(arbitraryBuffer_);
  BOLT_DCHECK(dataAvailableCbs.empty());
  BOLT_CHECK(!arbitraryBuffer_->hasNoMoreData());

  arbitraryBuffer_->enqueue(std::move(data));
  BOLT_CHECK_LT(nextArbitraryLoadBufferIndex_, buffers_.size());
...
void OutputBuffer::checkIfDone(bool oneDriverFinished) {
  std::vector<DataAvailable> finished;
  {
    ...
    if (is_morsel_driven_) {
      BOLT_CHECK_LE(
          numFinishedPushBasedDriverSchedulers_, numPushBasedDriverSchedulers_);
    } else {
      BOLT_CHECK_LE(
          numFinished_,
          numDrivers_,
    ...
    // If the driver is dynamically created, we need to make sure 1) all drivers
    // finished and 2) all driver schedulers finished. otherwise, we only need
    // to make sure 1) all drivers finished.
    atEnd_ = is_morsel_driven_ ? numFinishedPushBasedDriverSchedulers_ ==
                numPushBasedDriverSchedulers_ &&
...
void OutputBuffer::updateAfterAcknowledgeLocked(
  ...
  if (freedPages == 0) {
    BOLT_CHECK_EQ(freedBytes, 0);
    return;
  }
  BOLT_CHECK_GT(freedBytes, 0);
...
bool OutputBuffer::deleteResults(int destination) {
  ...
  DataAvailable dataAvailable;
  {
    ...
    buffer->finish();
    BOLT_CHECK_LT(destination, finishedBufferStats_.size());
...
OutputBuffer::Stats OutputBuffer::stats() {
  ...
  BOLT_CHECK_EQ(buffers_.size(), finishedBufferStats_.size());
