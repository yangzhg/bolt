 ...
 private:
  ...
  void initHeader() {
    if (!getVlqInt(valuesPerBlock_) || !getVlqInt(miniBlocksPerBlock_) ||
      ...
      BOLT_FAIL("initHeader EOF");
    }

    BOLT_CHECK_GT(valuesPerBlock_, 0, "cannot have zero value per block");
    BOLT_CHECK_EQ(
        valuesPerBlock_ % 128,
        0,
  ...
  void initBlock() {
    BOLT_DCHECK_GT(totalValuesRemaining_, 0, "initBlock called at EOF");

    if (!getZigZagVlqInt(minDelta_)) {
      BOLT_FAIL("initBlock EOF")
    }
