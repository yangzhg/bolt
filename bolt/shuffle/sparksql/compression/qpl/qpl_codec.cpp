 ...
 private:
  ...
  qpl_job* getJobCodecPtr() {
    if (!swJob) {
      ...
      // Job initialization
      if (auto status = qpl_init_job(qpl_path_software, swJob);
          status != QPL_STS_OK)
        throw GlutenException(
 ...
 public:
  ...
  arrow::Result<int64_t> Compress(
    ...
    if (QplJobHWPool::GetInstance().IsJobPoolReady()) {
      res = hwCodec_->doCompressData(
          input, input_len, output_buffer, output_buffer_len);
    }
    if (res == HardwareCodecDeflateQpl::RET_ERROR) {
      return swCodec_->doCompressData(
          input, input_len, output_buffer, output_buffer_len);
    }
    return res;
  }
  ...
  int64_t MaxCompressedLen(
    ...
    /// Aligned with ZLIB
    return (
        (input_len) + ((input_len) >> 12) + ((input_len) >> 14) +
        ((input_len) >> 25) + 13);
  }
...
bool SupportsCodec(const std::string& codec) {
  if (std::any_of(
          qpl_supported_codec.begin(),
          qpl_supported_codec.end(),
          [&](const auto& qat_codec) { return qat_codec == codec; })) {
    return true;
  }
  return false;
}
...
std::unique_ptr<arrow::util::Codec> MakeQplGZipCodec(int compressionLevel) {
  ...
  return std::unique_ptr<arrow::util::Codec>(
      new QplGzipCodec(qplCompressionLevel));
}
