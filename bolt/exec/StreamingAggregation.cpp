...
void StreamingAggregation::close() {
  ...
  Operator::recordGroupingSetStats(stats_);
  Operator::close();
}
...
RowVectorPtr StreamingAggregation::getOutput() {
  ...
  // hold on accept new input if groups_ is too large to avoid large memory
  // usage and too many copy
  if (numGroups_ < groupNumberThreshold_) {
    auto numInput = input_->size();
