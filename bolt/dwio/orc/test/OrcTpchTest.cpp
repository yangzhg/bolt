...
class OrcTpchTest : public testing::Test {
 protected:
  ...
  static void saveTpchTablesAsParquet() {
    ...
    for (const auto& table : tpch::tables) {
      ...
      plan = PlanBuilder()
                 .values({rows})
                 .tableWrite(tableDirectory, dwio::common::FileFormat::ORC)
                 .planNode();
