...
class BufferedInput {
 public:
  ...
  // Internal API, do not use outside Bolt.
 ...
 protected:
  ...
  static int adjustedReadPct(const cache::TrackingData& trackingData) {
    ...
    BOLT_CHECK_LE(0, pct, "Bad read percentage: {}", pct);
  ...
  static void moveCoalesced(
    ...
    BOLT_CHECK_EQ(k, numOldPrefetch + numMoved);
    prefetch.resize(k);
    BOLT_CHECK_EQ(l + numMoved, noPrefetch.size());
    noPrefetch.resize(l);
  }
 ...
 private:
  ...
  std::vector<bolt::common::Region> regions_;
  ...
  folly::Range<char*> allocate(const bolt::common::Region& region) {
  ...
  bool tryMerge(
      bolt::common::Region& first,
      const bolt::common::Region& second);
};
