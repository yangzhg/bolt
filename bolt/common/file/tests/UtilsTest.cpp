...
#include "bolt/common/file/tests/TestUtils.h"

using namespace ::testing;
using namespace ::bytedance::bolt;
using namespace ::bytedance::bolt::file::utils;
using namespace ::bytedance::bolt::tests::utils;
...
auto getReader(
  ...
  return [buf = std::move(content), chained](uint64_t offset, uint64_t size) {
    if (offset + size > buf.size()) {
      BOLT_FAIL("read is too big.");
    }
    if (size == 0) {
      BOLT_FAIL("empty read not allowed");
    }
    if (chained) {
