...
inline DataBatchIteratorPtr getBatchIteratorPtr(
  ...
  BOLT_NYI("Unknown data type " + type->toString());
  return nullptr;
}
} // namespace
...
struct UnsafeRowPrimitiveBatchDeserializer {
  ...
  /**
   * @param data
   * @return the value in bolt::StringView
   */
...
struct UnsafeRowDeserializer {
  ...
  static VectorPtr createFlatUnknownVector(
    ...
    for (int32_t i = 0; i < size; ++i) {
      BOLT_CHECK(iterator->isNull(i), "UNKNOWN type supports only NULL values");
    }
  ...
  static VectorPtr convertToVectors(
    ...
    if (type->isPrimitiveType()) {
      ...
      BOLT_NYI("Unsupported data iterators type");
    }
  }
