...
void duplicateJoinKeyCheck(
  ...
  BOLT_USER_CHECK_EQ(lookupKeyNames.size(), keys.size());
}
} // namespace
...
void IndexLookupJoin::initialize() {
  Operator::initialize();
  BOLT_CHECK_NOT_NULL(joinNode_);
  BOLT_CHECK_NULL(indexSource_);
...
void IndexLookupJoin::initLookupInput() {
  BOLT_CHECK_NULL(lookupInputType_);
  BOLT_CHECK(lookupInputChannels_.empty());
  ...
  SCOPE_EXIT {
    BOLT_CHECK_GE(lookupInputNames.size(), numKeys_ + lookupConditions_.size());
    BOLT_CHECK_EQ(lookupInputNames.size(), lookupInputChannels_.size());
    lookupInputType_ =
  ...
  for (auto keyIdx = 0; keyIdx < numKeys_; ++keyIdx) {
    ...
    BOLT_CHECK_EQ(probeKeyColumnNames.count(lookupInputNames.back()), 0);
  ...
  for (const auto& lookupCondition : lookupConditions_) {
    ...
    for (auto& input : lookupConditionExpr->distinctFields()) {
      ...
      if (probeIndexOpt.has_value()) {
        ...
        // columns.
        BOLT_CHECK_EQ(probeKeyColumnNames.count(columnName), 0);
...
void IndexLookupJoin::initLookupOutput() {
  BOLT_CHECK_NULL(lookupOutputType_);
  ...
  for (auto i = 0; i < outputType_->size(); ++i) {
    ...
    BOLT_CHECK(outputType_->childAt(i)->equivalent(*lookupOutputTypes.back()));
  }
...
RowVectorPtr IndexLookupJoin::getOutput() {
  ...
  if (lookupResultIter_ == nullptr) {
    BOLT_CHECK(!lookupFuture_.valid());
    prepareLookupInput();
    lookup();
  }

  BOLT_CHECK_NOT_NULL(lookupResultIter_);
...
void IndexLookupJoin::lookup() {
  BOLT_CHECK_NOT_NULL(indexSource_);
  BOLT_CHECK_NOT_NULL(input_);
  BOLT_CHECK_NOT_NULL(lookupInput_);
  BOLT_CHECK_NULL(lookupResultIter_);
  BOLT_CHECK_EQ(lookupInput_->size(), input_->size());
...
RowVectorPtr IndexLookupJoin::getOutputFromLookupResult() {
  ...
  BOLT_CHECK_NOT_NULL(lookupResult_);
...
RowVectorPtr IndexLookupJoin::produceOutputForInnerJoin() {
  BOLT_CHECK_EQ(joinType_, core::JoinType::kInner);
  BOLT_CHECK_NOT_NULL(lookupResult_);
  BOLT_CHECK_LE(nextOutputResultRow_, lookupResult_->size());
...
RowVectorPtr IndexLookupJoin::produceOutputForLeftJoin() {
  BOLT_CHECK_EQ(joinType_, core::JoinType::kLeft);
  BOLT_CHECK_NOT_NULL(lookupResult_);
  BOLT_CHECK_LE(nextOutputResultRow_, lookupResult_->size());
  BOLT_CHECK_NOT_NULL(rawLookupInputHitIndices_);

  prepareOutputRowMappings(outputBatchSize_);
  BOLT_CHECK_NOT_NULL(rawLookupOutputNulls_);
  ...
  for (; numOutputRows < outputBatchSize_ &&
    ...
    BOLT_CHECK_GE(numMissedInputRows, -1);
...
RowVectorPtr IndexLookupJoin::produceRemainingOutputForLeftJoin() {
  BOLT_CHECK_EQ(joinType_, core::JoinType::kLeft);
  BOLT_CHECK_NULL(lookupResult_);
  BOLT_CHECK(hasRemainingOutputForLeftJoin());
  BOLT_CHECK_NULL(rawLookupInputHitIndices_);
