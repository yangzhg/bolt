...
class InPredicate : public exec::VectorFunction {
 public:
  ...
  static std::shared_ptr<exec::VectorFunction> create(
    ...
    BOLT_CHECK_GE(inputArgs.size(), 2);
    ...
    BOLT_CHECK_EQ(inListType->kind(), TypeKind::ARRAY);
    BOLT_CHECK_EQ(2, inputArgs.size());
    ...
    BOLT_CHECK_EQ(values->typeKind(), TypeKind::ARRAY);
    ...
    try {
      BOLT_USER_CHECK_GT(size, 0, "IN list must not be empty");
    } catch (...) {
 ...
 private:
  ...
  void applyTyped(
    ...
    BOLT_CHECK(filter_, "IN predicate supports only constant IN list");
    ...
    BOLT_CHECK_EQ(arg->encoding(), VectorEncoding::Simple::FLAT);
