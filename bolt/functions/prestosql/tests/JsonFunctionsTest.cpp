...
TEST_F(JsonFunctionsTest, jsonParse) {
  ...
  auto useSonic = queryCtx_->queryConfig().enableSonicJsonParse();

  if (useSonic) {
    BOLT_ASSERT_THROW(jsonParse(R"({"k1":})"), "Parse: JSON has invalid chars");
    BOLT_ASSERT_THROW(jsonParse(R"({:"k1"})"), "Parse: JSON has invalid chars");
    BOLT_ASSERT_THROW(
        jsonParse(R"(not_json)"), "Parse: JSON has invalid chars");
    BOLT_ASSERT_THROW(jsonParse("[1"), "Parse: JSON has invalid chars");
    BOLT_ASSERT_THROW(jsonParse(""), "Parse: JSON has invalid chars");
  } else {
    BOLT_ASSERT_THROW(
    ...
    BOLT_ASSERT_THROW(
        jsonParse(R"({:"k1"})"), "The JSON document has an improper structure");
    BOLT_ASSERT_THROW(
        jsonParse(R"(not_json)"), "Problem while parsing an atom");
    BOLT_ASSERT_THROW(
        jsonParse("[1"),
        "JSON document ended early in the middle of an object or array");
    BOLT_ASSERT_THROW(jsonParse(""), "no JSON found");
  }
  ...
  bolt::test::assertEqualVectors(expectedVector, resultVector);
  ...
  if (useSonic) {
    BOLT_ASSERT_THROW(
        evaluate("json_parse(c0)", data), "Parse: JSON has invalid chars");
  } else {
    BOLT_ASSERT_THROW(
...
true, true, true, true, true, true, true, true, true, true, true])",
  ...
  if (!queryCtx_->queryConfig().enableSonicJsonArrayContains()) {
...
TEST_F(JsonFunctionsTest, invalidPath) {
  BOLT_ASSERT_THROW(jsonSize(R"([0,1,2])", ""), "Invalid JSON path");
  BOLT_ASSERT_THROW(jsonSize(R"([0,1,2])", "$[]"), "Invalid JSON path");
  BOLT_ASSERT_THROW(jsonSize(R"([0,1,2])", "$[-1]"), "Invalid JSON path");
  BOLT_ASSERT_THROW(jsonSize(R"({"k1":"v1"})", "$k1"), "Invalid JSON path");
  BOLT_ASSERT_THROW(jsonSize(R"({"k1":"v1"})", "$.k1."), "Invalid JSON path");
  BOLT_ASSERT_THROW(jsonSize(R"({"k1":"v1"})", "$.k1]"), "Invalid JSON path");
  BOLT_ASSERT_THROW(jsonSize(R"({"k1":"v1)", "$.k1]"), "Invalid JSON path");
}

TEST_F(JsonFunctionsTest, jsonToMap) {
  auto test_json_to_map =
      [&](StringView json,
          bool expect_exception = true,
          std::vector<std::pair<StringView, std::optional<StringView>>>
              key_values = {}) {
        auto flat_vector = makeNullableFlatVector<StringView>({json});
        if (expect_exception) {
          ASSERT_THROW(
              evaluate<MapVector>(
                  "json_to_map(c0)", makeRowVector({flat_vector})),
              std::exception);
          return;
        }
        auto actual = evaluate<MapVector>(
            "json_to_map(c0)", makeRowVector({flat_vector}));
        auto expected = makeMapVector<StringView, StringView>({key_values});
        ::bytedance::bolt::test::assertEqualVectors(expected, actual);
      };

  test_json_to_map("{\"abc\":1}", false, {{"abc", "1"}});
  test_json_to_map(
      "{\"abc\":1, \"test\":\"case\"}",
      false,
      {{"abc", "1"}, {"test", "case"}});
  test_json_to_map("{\"a\":[]}", false, {{"a", "[]"}});

  test_json_to_map("ab");
  test_json_to_map("\"ab\"");
  test_json_to_map("{1}");
  test_json_to_map("{1:1}");
...
TEST_F(JsonFunctionsTest, jsonExtract) {
  ...
  EXPECT_EQ(
      "{\"a\":1,\"b\":2}", jsonExtract("{\"x\":{\"a\":1,\"b\":2}}", "$.x"));
  ...
  BOLT_ASSERT_THROW(jsonExtract("{\"\":\"\"}", ""), "Invalid JSON path");

  if (!queryCtx_->queryConfig().enableSonicJsonExtract()) {
    // Json pointer does not support wild card
    EXPECT_EQ(
  ...
  // work in Bolt yet. Figure out how to add support for these.
  BOLT_ASSERT_THROW(jsonExtract(kJson, "$..price"), "Invalid JSON path");
  BOLT_ASSERT_THROW(
      jsonExtract(kJson, "$.store.book[?(@.price < 10)].title"),
      "Invalid JSON path");
  BOLT_ASSERT_THROW(jsonExtract(kJson, "max($..price)"), "Invalid JSON path");
  BOLT_ASSERT_THROW(
