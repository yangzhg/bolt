  ...
  for (const auto* aggregate : aggregates) {
    for (auto i = 0; i < aggregate->inputs.size(); ++i) {
      ...
      if (input != kConstantChannel) {
        ...
        BOLT_USER_CHECK_NOT_NULL(aggregate->constantInputs[i]);
      }
    }

    for (auto sortingKey : aggregate->sortingKeys) {
      BOLT_USER_CHECK_NE(sortingKey, kConstantChannel);
...
void SortedAggregations::sortSingleGroup(
  ...
  MetaRowsSorterWraper<BufferRows>::MetaCodegenSort(
      groupRows, inputData_.get(), sorter_, sortingSpec);
#else
  boost::sort::pdqsort(
      groupRows.begin(),
      groupRows.end(),
...
vector_size_t SortedAggregations::extractSingleGroup(
  ...
  BOLT_CHECK_EQ(numInputs, inputVectors.size());
  ...
  for (const auto& [sortingSpec, aggregates] : aggregates_) {
    ...
    for (auto* group : groups) {
      ...
      for (const auto& aggregate : aggregates) {
        ...
        // TODO Process group rows in batches to avoid creating very large
        // input vectors.
        const auto numRows =
