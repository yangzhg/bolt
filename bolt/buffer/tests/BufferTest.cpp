...
static_assert(Buffer::is_pod_like_v<bolt::Range<const char*>>, "");
...
#ifndef NDEBUG
  EXPECT_THROW(buf->as<char>(), BoltException);
#endif
  buf->as<void>();
...
TEST_F(BufferTest, testAllocateSizeOverflow) {
  EXPECT_THROW(
      AlignedBuffer::allocate<int64_t>(1ull << 62, pool_.get()), BoltException);
  auto buf = AlignedBuffer::allocate<int64_t>(8, pool_.get());
  EXPECT_THROW(
      AlignedBuffer::reallocate<int64_t>(&buf, 1ull << 62), BoltException);
}
