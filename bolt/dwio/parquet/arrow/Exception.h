...
#ifdef END_PARQUET_CATCH_EXCEPTIONS
#undef END_PARQUET_CATCH_EXCEPTIONS
#endif

#define END_PARQUET_CATCH_EXCEPTIONS                                           \
  }                                                                            \
  catch (const ::bytedance::bolt::parquet::arrow::ParquetStatusException& e) { \
    return e.status();                                                         \
  }                                                                            \
  catch (const ::bytedance::bolt::parquet::arrow::ParquetException& e) {       \
...
#ifdef PARQUET_IGNORE_NOT_OK
#undef PARQUET_IGNORE_NOT_OK
#endif
...
#ifdef PARQUET_THROW_NOT_OK
#undef PARQUET_THROW_NOT_OK
#endif
...
#define PARQUET_THROW_NOT_OK(s)                                        \
  do {                                                                 \
    ::arrow::Status _s = ::arrow::internal::GenericToStatus(s);        \
    if (!_s.ok()) {                                                    \
      throw ::bytedance::bolt::parquet::arrow::ParquetStatusException( \
          std::move(_s));                                              \
    }                                                                  \
  } while (0)
