...
Status returnIf(bool cond) {
  BOLT_RETURN_IF(cond, Status::Invalid("error"));
  return Status::OK();
}

Status returnNotOk(Status s) {
  BOLT_RETURN_NOT_OK(s);
...
TEST(StatusTest, macros) {
  ...
  BOLT_CHECK_OK(Status::OK()); // does not throw.

  bool didThrow = false;
  try {
    BOLT_CHECK_OK(Status::Invalid("invalid"));
  } catch (const BoltRuntimeError& e) {
    didThrow = true;
  }
  ASSERT_TRUE(didThrow) << "BOLT_CHECK_OK did not throw";
}
