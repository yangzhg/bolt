...
#include "bolt/dwio/common/Closeable.h"
...
class FileSink : public Closeable {
 public:
  struct Options {
    ...
    /// such as S3.
    std::shared_ptr<const config::ConfigBase> connectorProperties{nullptr};
    ...
    const filesystems::FileOptions* fileOptions{nullptr};
  };
...
class WriteFileSink final : public FileSink {
 public:
  ...
  // TODO: Hack to make Alpha writer work with Bolt.  To be removed after Alpha
...
#define BOLT_REGISTER_DATA_SINK_METHOD_DEFINITION(class, function)        \
  void class ::registerFactory() {                                        \
    bytedance::bolt::dwio::common::FileSink::registerFactory((function)); \
  }
