...
#include "bolt/common/hyperloglog/DenseHll.h"

#include <exception>
#include <sstream>
#include "bolt/common/base/IOUtils.h"
#include "bolt/common/hyperloglog/BiasCorrection.h"
#include "bolt/common/hyperloglog/HllUtils.h"
...
void DenseHll::initialize(int8_t indexBitLength) {
  BOLT_CHECK_GE(indexBitLength, 4, "indexBitLength must be in [4, 16] range");
  BOLT_CHECK_LE(indexBitLength, 16, "indexBitLength must be in [4, 16] range");
...
DenseHllView deserialize(const char* serialized) {
  ...
  BOLT_CHECK_EQ(kPrestoDenseV2, version);
