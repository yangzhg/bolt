...
class QatZipCodec : public arrow::util::Codec {
 protected:
  explicit QatZipCodec(int compressionLevel)
  ...
  arrow::Result<int64_t> Decompress(
    ...
    if (ret == QZ_OK) {
      ...
      return arrow::Status::IOError(
          "QAT decompression failure: params is invalid");
    } else if (ret == QZ_FAIL) {
      return arrow::Status::IOError(
          "QAT decompression failure: Function did not succeed");
    } else {
      return arrow::Status::IOError(
          "QAT decompression failure with error:", ret);
    }
  }
  ...
  arrow::Result<int64_t> Compress(
    ...
    if (ret == QZ_OK) {
      ...
      return arrow::Status::IOError(
          "QAT compression failure: params is invalid");
    } else if (ret == QZ_FAIL) {
      return arrow::Status::IOError(
          "QAT compression failure: function did not succeed");
    } else {
  ...
  arrow::Result<std::shared_ptr<arrow::util::Compressor>> MakeCompressor()
    ...
    return arrow::Status::NotImplemented(
        "Streaming compression unsupported with QAT");
  }

  arrow::Result<std::shared_ptr<arrow::util::Decompressor>> MakeDecompressor()
      override {
    return arrow::Status::NotImplemented(
        "Streaming decompression unsupported with QAT");
  }
...
class QatGZipCodec final : public QatZipCodec {
 public:
  QatGZipCodec(QzPollingMode_T pollingMode, int compressionLevel)
      : QatZipCodec(compressionLevel) {
...
bool supportsCodec(const std::string& codec) {
  return !codec.empty() &&
      std::any_of(
          kQatSupportedCodec.begin(),
          kQatSupportedCodec.end(),
          [&](const auto& qatCodec) { return qatCodec == codec; });
}

std::unique_ptr<arrow::util::Codec> makeQatGZipCodec(
    QzPollingMode_T pollingMode,
    int compressionLevel) {
  return std::unique_ptr<arrow::util::Codec>(
      new QatGZipCodec(pollingMode, compressionLevel));
}
...
class QatDevice {
 public:
  ...
  static std::shared_ptr<QatDevice> getInstance() {
    std::call_once(
        initQat_, []() { instance_ = std::make_shared<QatDevice>(); });
    return instance_;
  }
...
class QatZstdCodec final : public arrow::util::Codec {
 public:
  explicit QatZstdCodec(int compressionLevel)
      : compressionLevel_(compressionLevel) {}
  ...
  arrow::Status initCCtx() {
    ...
    if (qatDevice_->deviceInitialized()) {
      ...
      ZSTD_registerSequenceProducer(
          zc_, sequenceProducerState_, qatSequenceProducer);
...
std::unique_ptr<arrow::util::Codec> makeQatZstdCodec(int compressionLevel) {
  return std::unique_ptr<arrow::util::Codec>(
      new QatZstdCodec(compressionLevel));
}
