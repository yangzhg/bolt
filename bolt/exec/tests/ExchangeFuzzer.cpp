  ...
  bool runOne(const std::vector<RowVectorPtr>& vectors, Params params) {
    ...
    try {
      ...
      if (FLAGS_enable_oom_injection) {
        ...
        if (const auto* boltException =
                dynamic_cast<const BoltRuntimeError*>(&e)) {
          const bool isInjectedOomException = boltException->errorCode() ==
                  bytedance::bolt::error_code::kMemCapExceeded &&
              boltException->message() == ScopedOOMInjector::kErrorMessage;
          const bool isTaskAbortedException = boltException->errorCode() ==
                  bytedance::bolt::error_code::kInvalidState &&
              boltException->message() == "Aborted for external error";
          ...
          if (!waitForTaskCompletion(task.get(), 1'000'000)) {
            if (task->state() == TaskState::kFailed) {
              try {
                ...
                if (const auto* boltException =
                        dynamic_cast<const BoltRuntimeError*>(&taskException)) {
                  if (boltException->errorCode() ==
                          bytedance::bolt::error_code::kMemCapExceeded &&
