...
////  remove varchar from list of varchar containing empty string
TEST_F(ArrayRemoveTest, stringArrayContainEmpty) {
  // ['true', '', '', 'true']
  const auto arrayVector =
      makeNullableArrayVector<std::string>({{"true", "", "", "true"}});
  const auto elementVector = makeNullableFlatVector<std::string>({""});
  const auto expected =
      makeNullableArrayVector<std::string>({{"true", "true"}});
  testExpression(
      "array_remove(c0, c1)", {arrayVector, elementVector}, expected);
