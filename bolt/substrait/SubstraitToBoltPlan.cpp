...
core::PlanNodePtr SubstraitBoltPlanConverter::toBoltPlan(
  ...
  for (const auto& grouping : aggRel.groupings()) {
    for (const auto& groupingExpr : grouping.grouping_expressions()) {
      ...
      boltGroupingExprs.emplace_back(
          exprConverter_->toBoltExpr(groupingExpr.selection(), inputType));
    }
  }
  ...
  for (const auto& measure : aggRel.measures()) {
    ...
    auto funcName = substraitParser_->findBoltFunction(
...
core::PlanNodePtr SubstraitBoltPlanConverter::toBoltPlan(
  ...
  if (topNFlag) {
    ...
    BOLT_CHECK_EQ(fetchRel.offset(), 0);
...
core::PlanNodePtr SubstraitBoltPlanConverter::toBoltPlan(
  ...
  if (rel.has_read()) {
    ...
    auto planNode = toBoltPlan(rel.read(), splitInfo);
...
core::PlanNodePtr SubstraitBoltPlanConverter::toBoltPlan(
  ...
  exprConverter_ =
      std::make_shared<SubstraitBoltExprConverter>(pool_, functionMap_);
