...
class AggregateCompanionFunctionBase : public Aggregate {
 public:
  ...
  char* serializeAccumulator(char* group, char* dst) const override final;

  char* deserializeAccumulator(char* group, char* src) const override final;
  ...
  bool isSpillExtract() const override final;

  void setSpillExtract(bool spillExtract) override final;
...
struct AggregateCompanionAdapter {
  ...
  class MergeFunction : public AggregateCompanionFunctionBase {
   public:
    ...
    // merge function's toIntermediate just copy args to result
    void toIntermediate(
        const SelectivityVector& rows,
        std::vector<VectorPtr>& args,
