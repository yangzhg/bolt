...
class RegexFunctionsTest : public test::SparkFunctionBaseTest {
 public:
  ...
  std::shared_ptr<bytedance::bolt::SimpleVector<bytedance::bolt::StringView>>
  ...
  std::shared_ptr<bytedance::bolt::FlatVector<bytedance::bolt::StringView>>
  convertOutput(const std::vector<std::string>& output, size_t repeatCount) {
    std::vector<std::optional<bytedance::bolt::StringView>> repeatedOutput(
        output.size() * repeatCount);

    using StringView = bytedance::bolt::StringView;
...
TEST_F(RegexFunctionsTest, javaRegexIncompatibilities) {
  ...
  EXPECT_THROW(rlike("[]", R"([a[b]])"), BoltUserError);
  // Character class intersection not supported; parsed as [a&\[b]\].
  EXPECT_THROW(rlike("&]", R"([a&&[b]])"), BoltUserError);
  // Character class difference not supported; parsed as [\w&\[\^b]\].
  EXPECT_THROW(rlike("^]", R"([\w&&[^b]])"), BoltUserError);
  // Unsupported character classes.
  EXPECT_THROW(rlike(" ", "\\h"), BoltUserError);
  EXPECT_THROW(rlike(" ", "\\H"), BoltUserError);
  EXPECT_THROW(rlike(" ", "\\V"), BoltUserError);
  EXPECT_THROW(rlike(" ", "\\uffff"), BoltUserError);
  EXPECT_THROW(rlike(" ", "\\e"), BoltUserError);
  EXPECT_THROW(rlike(" ", "\\c1"), BoltUserError);
  EXPECT_THROW(rlike(" ", "\\G"), BoltUserError);
  EXPECT_THROW(rlike(" ", "\\Z"), BoltUserError);
  EXPECT_THROW(rlike(" ", "\\R"), BoltUserError);
  // Backreferences not supported.
  EXPECT_THROW(rlike("00", R"((\d)\1)"), BoltUserError);
  // Possessive quantifiers not supported.
  EXPECT_THROW(rlike(" ", " ?+"), BoltUserError);
  EXPECT_THROW(rlike(" ", " *+"), BoltUserError);
  EXPECT_THROW(rlike(" ", " ++"), BoltUserError);
  EXPECT_THROW(rlike(" ", " {1}+"), BoltUserError);
  // Possessive quantifiers not supported.
  EXPECT_THROW(rlike(" ", " ?+"), BoltUserError);
  EXPECT_THROW(rlike(" ", " *+"), BoltUserError);
  EXPECT_THROW(rlike(" ", " ++"), BoltUserError);
...
TEST_F(RegexFunctionsTest, blockUnsupportedEdgeCases) {
  ...
  EXPECT_THROW(rlike("", "[a[b]]"), BoltUserError);
}
...
TEST_F(RegexFunctionsTest, regexBadJavaPattern) {
  EXPECT_THROW(testRegexpReplace("[]", "[a[b]]", ""), BoltUserError);
  EXPECT_THROW(testRegexpReplace("[]", "[a&&[b]]", ""), BoltUserError);
  EXPECT_THROW(testRegexpReplace("[]", "[a&&[^b]]", ""), BoltUserError);
}
...
TEST_F(RegexFunctionsTest, regexpReplaceNegativePosition) {
  EXPECT_THROW(testRegexpReplace("abc", "a", "", {-1}), BoltUserError);
}

TEST_F(RegexFunctionsTest, regexpReplaceZeroPosition) {
  EXPECT_THROW(testRegexpReplace("abc", "a", "", {0}), BoltUserError);
}
...
TEST_F(RegexFunctionsTest, regexpReplaceCacheLimitTest) {
  ...
  EXPECT_THROW(
      testingRegexpReplaceRows(strings, patterns, replaces), BoltUserError);
}
