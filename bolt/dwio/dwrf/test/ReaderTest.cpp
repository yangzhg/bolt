...
#include "bolt/dwio/common/ExecutorBarrier.h"
...
TEST_F(TestReader, testFlatmapAsStructRequiringKeyList) {
  ...
  RowReaderOptions rowReaderOpts;
  EXPECT_THROW(
      rowReaderOpts.setFlatmapNodeIdsAsStruct(emptyKeys), BoltException);
}
...
TEST_F(TestReader, testMismatchSchemaIncompatible) {
  ...
  for (auto& t : types) {
    ...
    EXPECT_THROW(
        ColumnSelector cs(reqType, rowType), bytedance::bolt::BoltUserError);
  }
}
