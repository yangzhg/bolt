...
#include "bolt/common/memory/Scratch.h"
...
class ByteInputStream {
 ...
 public:
  ...
  virtual void readBytes(uint8_t* bytes, int32_t size);
  ...
  virtual std::string_view nextView(int32_t size);
...
class ByteOutputStream {
 public:
  ...
  void setRange(ByteRange range, int32_t lastWrittenPosition) {
    ...
    ranges_[0] = range;
    current_ = ranges_.data();
    BOLT_CHECK_GE(ranges_.back().size, lastWrittenPosition);
