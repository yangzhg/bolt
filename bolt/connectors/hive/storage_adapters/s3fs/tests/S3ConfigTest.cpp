...
TEST(S3ConfigTest, defaultConfig) {
  ...
  ASSERT_EQ(s3Config.iamRoleSessionName(), "bolt-session");
...
TEST(S3ConfigTest, overrideConfig) {
  std::unordered_map<std::string, std::string> configFromFile = {
      ...
      {S3Config::baseConfigKey(S3Config::Keys::kIamRoleSessionName), "bolt"},
  ...
  ASSERT_EQ(s3Config.iamRoleSessionName(), "bolt");
