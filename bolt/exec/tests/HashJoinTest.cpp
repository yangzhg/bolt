...
class HashJoinBuilder {
 public:
  ...
  HashJoinBuilder& planNode(core::PlanNodePtr planNode) {
    BOLT_CHECK_NULL(planNode_);
    planNode_ = planNode;
    return *this;
  }

  HashJoinBuilder& keyTypes(const std::vector<TypePtr>& keyTypes) {
    BOLT_CHECK_NULL(probeType_);
    BOLT_CHECK_NULL(buildType_);
  ...
  HashJoinBuilder& probeVectors(int32_t vectorSize, int32_t numVectors) {
    BOLT_CHECK_NOT_NULL(probeType_);
    BOLT_CHECK(probeVectors_.empty());
  ...
  HashJoinBuilder& buildVectors(int32_t vectorSize, int32_t numVectors) {
    BOLT_CHECK_NOT_NULL(buildType_);
    BOLT_CHECK(buildVectors_.empty());
 ...
 private:
  ...
  std::vector<RowVectorPtr> makeVectors(
      ...
      bool shuffle = true) {
    BOLT_CHECK_GE(vectorSize, 0);
    BOLT_CHECK_GT(numVectors, 0);
  ...
  void runTest(const core::PlanNodePtr& planNode) {
    ...
    if (injectSpill_) {
      if (maxSpillLevel_.has_value()) {
        ...
        bool doSkewedPartitionTest =
            configs_.count(
                core::QueryConfig::kHashJoinSkewedPartitionEnabled) == 0;
        runTest(planNode, true, 0, 100);
        runTest(planNode, true, 2, 100);
        if (doSkewedPartitionTest) {
          runTest(planNode, true, 2, 100, true);
        }
      }
    }
  }
  ...
  void runTest(
      ...
      bool enableSkewedPartitionTest = false) {
    ...
    if (numDrivers_ > 1) {
      config(core::QueryConfig::kHashJoinSkewedPartitionEnabled, "false");
    }
    int32_t spillPct{0};
    if (injectSpill) {
      ...
      config(
          core::QueryConfig::kHashJoinSkewedPartitionEnabled,
    ...
    if (injectSpill) {
      if (checkSpillStats_) {
        ...
        if (maxSpillLevel != -1 && !enableSkewedPartitionTest) {
 ...
 protected:
  ...
  static core::JoinType flipJoinType(core::JoinType joinType) {
    switch (joinType) {
      ...
      default:
        BOLT_FAIL("Cannot flip join type: {}", core::joinTypeName(joinType));
    }
  }
...
DEBUG_ONLY_TEST_P(
  ...
  SCOPED_TESTVALUE_SET(
      "bytedance::bolt::exec::SpillOperatorGroup::runSpill",
      std::function<void(Operator*)>([&](Operator* op) {
...
DEBUG_ONLY_TEST_P(
    MultiThreadedHashJoinTest,
    parallelJoinBuildCheckMoreThan255Drivers) {
  std::atomic<bool> isParallelBuild{false};
  SCOPED_TESTVALUE_SET(
      "bytedance::bolt::exec::HashTable::parallelJoinBuild",
      std::function<void(void*)>([&](void*) { isParallelBuild = true; }));
...
TEST_P(MultiThreadedHashJoinTest, fullJoinWithNoMatchRangePartition) {
  ...
  std::vector<RowVectorPtr> probeVectors1 = mergeBatches(
      makeBatches(
          3,
          [&](int32_t /*unused*/) {
            return makeRowVector({
  ...
  // Right side keys are [-3, -2, -1, 0, 1, 2, 3].
  std::vector<RowVectorPtr> buildVectors1 =
      makeBatches(3, [&](int32_t /*unused*/) {
        return makeRowVector({
            makeFlatVector<int32_t>(
                123, [](auto row) { return -3 + row % 7; }, nullEvery(11)),
  ...
  HashJoinBuilder(*pool_, duckDbQueryRunner_, driverExecutor_.get())
      .numDrivers(1)
      .probeKeys({"c0"})
      .probeVectors(std::move(probeVectors1))
      .buildKeys({"u_c0"})
      .buildVectors(std::move(buildVectors1))
      .buildFilter("c0 < 0")
      .buildProjections({"c0 AS u_c0", "c1 AS u_c1"})
      .joinType(core::JoinType::kLeft)
      .joinOutputLayout({"c1"})
      .referenceQuery(
          "SELECT t.c1 FROM t LEFT OUTER JOIN (SELECT * FROM u WHERE c0 < 0) u ON t.c0 = u.c0")
      .run();
}
...
TEST_F(HashJoinTest, dynamicFiltersAppliedToPreloadedSplits) {
  ...
  HashJoinBuilder(*pool_, duckDbQueryRunner_, driverExecutor_.get())
      ...
      .verifier([&](const std::shared_ptr<Task>& task, bool /*hasSpill*/) {
        ...
        ASSERT_LT(1, getStatSum(probeScanId, "preloadSplits"));
      })
      .run();
}
...
TEST_F(HashJoinTest, semiJoinDeduplicateInfinityLoopWhenInsert) {
  // According to the requirements of the VectorHasher::cardinality function
  // called in HashTable<ignoreNullKeys>::analyze(), the number of data rows
  // must be greater than 10k, so that the HashTable<ignoreNullKeys>::analyze()
  // function can return true.
  const int vectorSize = 110, batches = 1100;
  ...
  // According to the requirements of the VectorHasher::cardinality function
  // called in HashTable<ignoreNullKeys>::analyze(), the difference between the
  // max and min values ​​of the data must be greater than
  // VectorHasher::kMaxRange.
  auto buildVectors = makeBatches(batches, [&](int32_t batch) {
    if (batch < batches - 1) {
      return makeRowVector({
          makeFlatVector<int64_t>(
              vectorSize, [&](vector_size_t row) { return batch * 100 + row; }),
          makeFlatVector<int64_t>(
              vectorSize, [&](vector_size_t row) { return rand(); }),
      });
    } else {
      // Ensure `VectorHasher::max_ - VectorHasher::min_` greater than
      // VectorHasher::kMaxRange
      return makeRowVector({
          makeFlatVector<int64_t>(
              vectorSize,
...
TEST_F(HashJoinTest, spillPartitionBitsOverlap) {
  auto builder =
      HashJoinBuilder(*pool_, duckDbQueryRunner_, driverExecutor_.get())
          ...
          .checkSpillStats(false)
          .maxSpillLevel(0);
  BOLT_ASSERT_THROW(builder.run(), "vs. 8");
}
...
DEBUG_ONLY_TEST_F(HashJoinTest, minSpillableMemoryReservation) {
  ...
  for (int32_t minSpillableReservationPct : {5, 50, 100}) {
    ...
    SCOPED_TESTVALUE_SET(
        "bytedance::bolt::exec::HashBuild::addInput",
  ...
  HashJoinBuilder(*pool_, duckDbQueryRunner_, driverExecutor_.get())
      ...
      .verifier([&](const std::shared_ptr<Task>& task, bool /*unused*/) {
        auto joinStats = task->taskStats()
                             ...
                             .operatorStats.back()
                             .runtimeStats;
        // kSpillNumPartitionBits default value change from 2 to 3
...
DEBUG_ONLY_TEST_F(HashJoinTest, arbitrationTriggeredByEnsureJoinTableFit) {
  std::atomic<bool> injectOnce{true};
  SCOPED_TESTVALUE_SET(
      "bytedance::bolt::exec::HashBuild::ensureTableFits",
...
DEBUG_ONLY_TEST_F(HashJoinTest, skewPartitionSpill) {
  constexpr int64_t kMaxBytes = 1LL << 30; // 1GB
  VectorFuzzer fuzzer({.vectorSize = 1000}, pool());
  const int32_t numBuildVectors = 30;
  std::vector<RowVectorPtr> buildVectors;
  buildVectors.push_back(fuzzer.fuzzRow(buildType_));
  for (int32_t i = 1; i < numBuildVectors; ++i) {
    auto vector = fuzzer.fuzzRow(buildType_);
    // Build the build vector with the same join key to make build side skewed
    vector->childAt(0) = makeFlatVector<int32_t>(
        vector->size(), [](auto /*unused*/) { return 1; });
    buildVectors.push_back(std::move(vector));
  }
  ...
  } testSettings[] = {{0, true, true}};
  /*, {{0, true, true}, {0, false, false}, {0, false, false}};*/
  for (const auto& testData : testSettings) {
    SCOPED_TRACE(testData.debugString());
    ...
    std::thread taskThread([&]() {
      HashJoinBuilder(*pool_, duckDbQueryRunner_, driverExecutor_.get())
          ...
          .config(core::QueryConfig::kMaxSpillFileSize, std::to_string(1000))
          .verifier([&](const std::shared_ptr<Task>& task, bool /*unused*/) {
            const auto statsPair = taskSpilledStats(*task);
            if (testData.expectedReclaimable) {
              ASSERT_GT(statsPair.first.spilledBytes, 0);
...
TEST_F(HashJoinTest, duplicateLazyNotLoadedProbeVector) {
  // VectorMaker vectorMaker{pool_};
  auto lazyVectorA = vectorMaker_.lazyFlatVector<int32_t>(
      100,
      [](auto row) { return 1 + row % 2; },
      [](auto row) { return row > 50; });
  auto flatVectorA = makeFlatVector<int32_t>(
      100,
      [](auto row) { return 1 + row % 2; },
      [](auto row) { return row > 50; });

  auto probeVectors =
      vectorMaker_.rowVector({flatVectorA, lazyVectorA, lazyVectorA});

  auto probeVectorsFlat = makeRowVector({
      makeFlatVector<int32_t>(
          100,
          [](auto row) { return 1 + row % 2; },
          [](auto row) { return row > 50; }),
  ...
  createDuckDbTable("t", {probeVectorsFlat});
  createDuckDbTable("u", {buildVectors});
  ...
  HashJoinBuilder(*pool_, duckDbQueryRunner_, driverExecutor_.get())
      .planNode(std::move(plan))
      .config(
          core::QueryConfig::kPreferredOutputBatchRows, std::to_string(1000))
      .referenceQuery(
          "SELECT t.c0, t.c1, t.c2, u.u_c1 FROM t inner join u on t.c0 = u.u_c0")
      .injectSpill(false)
      .run();
}
