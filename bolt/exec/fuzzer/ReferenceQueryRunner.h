...
class ReferenceQueryRunner {
 public:
  ...
  /// Converts Bolt plan into SQL accepted by the reference database.
  ...
  virtual std::multiset<std::vector<bolt::variant>> execute(
      const std::string& sql,
      const std::vector<RowVectorPtr>& input,
      const RowTypePtr& resultType) = 0;

  /// Returns true if 'executeVector' can be called to get results as Bolt
  /// Vector.
  virtual bool supportsBoltVectorResults() const {
    return false;
  }

  /// Similar to 'execute' but returns results in RowVector format.
  /// Caller should ensure 'supportsBoltVectorResults' returns true.
