...
void FunctionBaseTest::testEncodings(
  ...
  auto testDictionary = [&](vector_size_t dictionarySize,
    ...
    bolt::test::assertEqualVectors(expectedResult, evaluate(expr, encodedRow));
  };
  ...
  auto testConstant = [&](vector_size_t row) {
    ...
    bolt::test::assertEqualVectors(expectedResult, evaluate(expr, constantRow));
  };
