...
class Buffer {
 public:
  ...
  const T* as() const {
    ...
    BOLT_DCHECK((std::is_same_v<T, void>) || podType_ == is_pod_like_v<T>);
 ...
 protected:
  ...
  virtual void freeToPool() {
    BOLT_FAIL("Buffer::freeToPool() must be overridden by concrete buffers");
  }

  virtual void copyFrom(const Buffer* other, size_t bytes) {
    BOLT_CHECK(!isView());
    BOLT_CHECK_GE(capacity_, bytes);
    BOLT_CHECK(podType_);
...
class AlignedBuffer : public Buffer {
 public:
  ...
  static void reallocate(
    ...
    bolt::memory::MemoryPool* pool = old->pool();
    if (!is_pod_like_v<T>) {
 ...
 protected:
  ...
  void fillNewMemory(
    ...
    BOLT_CHECK_LE(newBytes, capacity());
    if (newBytes <= oldBytes) {
      return;
    }
    if (initValue) {
      BOLT_DCHECK(newBytes % sizeof(RawT) == 0);
      BOLT_DCHECK(oldBytes % sizeof(RawT) == 0);
 ...
 protected:
  ...
  void checkEndGuardImpl() const override {
    if (*reinterpret_cast<uint64_t*>(data_ + capacity_) != kEndGuard) {
      BOLT_FAIL("Write past Buffer capacity() {}", capacity_);
    }
  }
...
class NonPODAlignedBuffer : public Buffer {
 public:
  ...
  void setSize(size_t size) override {
    size_t old = size_;
    BOLT_CHECK_EQ(old % sizeof(T), 0);
    BOLT_CHECK_EQ(size % sizeof(T), 0);
    Buffer::setSize(size);
 ...
 protected:
  NonPODAlignedBuffer(bolt::memory::MemoryPool* pool, size_t capacity)
      : Buffer(
            pool,
            reinterpret_cast<uint8_t*>(this) + sizeof(*this),
            capacity,
  ...
  void copyFrom(const Buffer* other, size_t bytes) override {
    BOLT_CHECK(!isView());
    BOLT_CHECK_GE(size_, bytes);
    BOLT_DCHECK(
        dynamic_cast<const NonPODAlignedBuffer<T>*>(other) != nullptr,
        "Types don't match");
    BOLT_CHECK_EQ(bytes % sizeof(T), 0);
  ...
  void fillNewMemory(
    ...
    BOLT_CHECK_EQ(newBytes % sizeof(T), 0);
    BOLT_CHECK_EQ(oldBytes % sizeof(T), 0);
