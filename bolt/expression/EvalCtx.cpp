...
EvalCtx::EvalCtx(
    ...
    const bool acceptNullableChild)
    : EvalCtx(
          execCtx,
          exprSet,
          row,
...
EvalCtx::EvalCtx(
    ...
    : execCtx_(execCtx),
      ...
      currentSplitStr_(currentSplitStr) {
  // TODO Change the API to replace raw pointers with non-const references.
  // Sanity check inputs to prevent crashes.
  BOLT_CHECK_NOT_NULL(execCtx);
  BOLT_CHECK_NOT_NULL(exprSet);
  BOLT_CHECK_NOT_NULL(row);

  inputFlatNoNulls_ = true;
  for (const auto& child : row->children()) {
    BOLT_CHECK(acceptNullableChild || child != nullptr);
    if (acceptNullableChild && !child) {
      continue;
    }
...
auto throwError(const std::exception_ptr& exceptionPtr) {
  std::rethrow_exception(toBoltException(exceptionPtr));
}
} // namespace

void EvalCtx::setStatus(vector_size_t index, Status status) {
  BOLT_CHECK(!status.ok(), "Status must be an error");

  if (status.isUserError()) {
    setBoltExceptionError(
        index,
...
void EvalCtx::setErrors(
  ...
  auto boltException = toBoltException(exceptionPtr);
  rows.applyToSelected(
      [&](auto row) { addError(row, boltException, errors_); });
}
