...
#define BOLT_HAS_SYMBOLIZER 1
#else
#define BOLT_HAS_SYMBOLIZER 0
#endif
...
const std::vector<std::string>& StackTrace::toStrVector(
    bool demangleSym) const {
  folly::call_once(bt_vector_flag_, [&] {
    ...
    for (auto ptr : bt_pointers_) {
      ...
      bt_vector_.push_back(fmt::format(
          "# {:<2d} {}",
          frame++,
          demangleSym ? demangle(framename.data()) : framename));
    }
  });
  return bt_vector_;
}

const std::string& StackTrace::toString(bool demangleSym) const {
  folly::call_once(bt_flag_, [&] {
    const auto& vec = toStrVector(demangleSym);
    size_t needed = 0;
