...
#include <bolt/tpch/gen/dbgen/include/tpch_constants.hpp>
#include "bolt/tpch/gen/DBGenIterator.h"
...
RowTypePtr getTableSchemaWithPrefix(Table table) {
  switch (table) {
    case Table::TBL_PART: {
...
RowTypePtr getTableSchemaWithoutPrefix(Table table) {
  switch (table) {
    case Table::TBL_PART: {
      static RowTypePtr type = ROW(
          {
...
RowTypePtr getTableSchema(Table table, bool usePrefixWithColumnName) {
  if (usePrefixWithColumnName) {
    return getTableSchemaWithPrefix(table);
  } else {
    return getTableSchemaWithoutPrefix(table);
  }
}

TypePtr resolveTpchColumn(
    Table table,
    const std::string& columnName,
    bool usePrefixInColumnName) {
  return getTableSchema(table, usePrefixInColumnName)->findChild(columnName);
}
...
RowVectorPtr genTpchOrders(
  ...
  auto ordersRowType = getTableSchema(Table::TBL_ORDERS, usePrefixInColumnName);
...
RowVectorPtr genTpchLineItem(
  ...
  auto lineItemRowType =
      getTableSchema(Table::TBL_LINEITEM, usePrefixInColumnName);
...
RowVectorPtr genTpchPart(
    ...
    bool usePrefixInColumnName) {
  // Create schema and allocate vectors.
  auto partRowType = getTableSchema(Table::TBL_PART, usePrefixInColumnName);
  size_t vectorSize =
...
RowVectorPtr genTpchSupplier(
    ...
    bool usePrefixInColumnName) {
  // Create schema and allocate vectors.
  auto supplierRowType =
      getTableSchema(Table::TBL_SUPPLIER, usePrefixInColumnName);
  size_t vectorSize = getVectorSize(
...
RowVectorPtr genTpchPartSupp(
    ...
    bool usePrefixInColumnName) {
  // Create schema and allocate vectors.
  auto partSuppRowType =
      getTableSchema(Table::TBL_PARTSUPP, usePrefixInColumnName);
  size_t vectorSize = getVectorSize(
  ...
  BOLT_CHECK_EQ(partSuppCount, vectorSize);
  return std::make_shared<RowVector>(
      pool,
      partSuppRowType,
      BufferPtr(nullptr),
...
RowVectorPtr genTpchCustomer(
    ...
    bool usePrefixInColumnName) {
  // Create schema and allocate vectors.
  auto customerRowType =
      getTableSchema(Table::TBL_CUSTOMER, usePrefixInColumnName);
  size_t vectorSize = getVectorSize(
...
RowVectorPtr genTpchNation(
    ...
    bool usePrefixInColumnName) {
  // Create schema and allocate vectors.
  auto nationRowType = getTableSchema(Table::TBL_NATION, usePrefixInColumnName);
  size_t vectorSize = getVectorSize(
...
RowVectorPtr genTpchRegion(
    ...
    bool usePrefixInColumnName) {
  // Create schema and allocate vectors.
  auto regionRowType = getTableSchema(Table::TBL_REGION, usePrefixInColumnName);
  size_t vectorSize = getVectorSize(
...
std::string getQuery(int query) {
  if (query <= 0 || query > TPCH_QUERIES_COUNT) {
    BOLT_FAIL("Out of range TPC-H query number {}", query);
  }
