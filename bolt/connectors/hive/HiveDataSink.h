  ...
  static std::string updateModeToString(UpdateMode updateMode) {
    switch (updateMode) {
      ...
      case UpdateMode::kAppend:
        return "APPEND";
      default:
        BOLT_UNSUPPORTED("Unsupported update mode.");
    }
  }
...
class HiveDataSink : public DataSink {
 ...
 private:
  ...
  class WriterReclaimer : public exec::MemoryReclaimer {
   ...
   private:
    WriterReclaimer(HiveDataSink* dataSink, HiveWriterInfo* writerInfo)
      ...
      BOLT_CHECK_NOT_NULL(dataSink_);
      BOLT_CHECK_NOT_NULL(writerInfo_);
    }
  ...
  FOLLY_ALWAYS_INLINE void checkRunning() const {
    BOLT_CHECK_EQ(state_, State::kRunning, "Hive data sink is not running");
  }
  ...
  filesystems::FileOptions fileOptions_;
...
template <>
struct fmt::formatter<bytedance::bolt::connector::hive::HiveDataSink::State>
    : formatter<int> {
  auto format(
      bytedance::bolt::connector::hive::HiveDataSink::State s,
      format_context& ctx) {
...
struct fmt::formatter<
    bytedance::bolt::connector::hive::LocationHandle::TableType>
    : formatter<int> {
  auto format(
      bytedance::bolt::connector::hive::LocationHandle::TableType s,
      format_context& ctx) {
    return formatter<int>::format(static_cast<int>(s), ctx);
  }
};
