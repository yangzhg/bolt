 ...
 private:
  ...
  struct KindDispatcher {
    static simdjson::error_code apply(Input, exec::GenericWriter&, bool) {
      BOLT_NYI("Parse json to {} is not supported.", TypeTraits<kind>::name);
        ...
        for (const auto& fieldResult : object) {
          ...
          if (!field.value().is_null()) {
            ...
            if (it != fieldIndices.end() && it->second >= 0) {
              ...
              const auto res = BOLT_DYNAMIC_TYPE_DISPATCH(
                  ExtractJsonTypeImpl<simdjson::ondemand::value>::apply,
...
TypePtr FromJsonCallToSpecialForm::resolveType(
  ...
  BOLT_FAIL("from_json function does not support type resolution.");
}
...
exec::ExprPtr FromJsonCallToSpecialForm::constructSpecialForm(
  ...
  BOLT_USER_CHECK_EQ(args.size(), 1, "from_json expects one argument.");
  BOLT_USER_CHECK_EQ(
      args[0]->type()->kind(),
      TypeKind::VARCHAR,
  ...
  return std::make_shared<exec::Expr>(
      type, std::move(args), func, kFromJson, trackCpuUsage);
}
} // namespace bytedance::bolt::functions::sparksql
