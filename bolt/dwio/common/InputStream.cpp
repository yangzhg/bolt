...
ReadFileInputStream::ReadFileInputStream(
    std::shared_ptr<bolt::ReadFile> readFile,
...
void ReadFileInputStream::read(
  ...
  if (stats_) {
    auto scanTime = (getCurrentTimeMicro() - readStartMicros) * 1000;
    stats_->incRawBytesRead(length);
    stats_->incTotalScanTime(scanTime);
    stats_->incIOInfo(length, scanTime);
  }
...
void ReadFileInputStream::read(
  ...
  if (stats_) {
    auto scanTime = (getCurrentTimeMicro() - readStartMicros) * 1000;
    stats_->incIOInfo(size, scanTime);
  }
  DWIO_ENSURE_EQ(
      size,
      bufferSize,
...
void ReadFileInputStream::vread(
  ...
  if (stats_) {
    auto totalScanTime = (getCurrentTimeMicro() - readStartMicros) * 1000;
    stats_->incRawBytesRead(length);
    stats_->incTotalScanTime(totalScanTime);
    stats_->incIOInfo(length, totalScanTime);
  }
}
