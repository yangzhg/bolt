...
class SelectiveColumnReader {
 public:
  static constexpr uint64_t kStringBufferSize = 128 * 1024;
  ...
  next(uint64_t /*numValues*/, VectorPtr& /*result*/, const Mutation*) {
    BOLT_UNSUPPORTED("next() is only defined in SelectiveStructColumnReader");
  }
  ...
  virtual void getValues(
      const RowSet& rows,
      VectorPtr* FOLLY_NONNULL result) = 0;
  ...
  template <typename T>
  inline void addNull() {
    BOLT_DCHECK_NE(valueSize_, kNoValueSize);
    BOLT_DCHECK_LE(
  ...
  virtual bool useBulkPath() const {
    ...
    return hasBulkPath() && (process::hasAvx2() || process::hasNeon()) &&
        (!filter ||
 ...
 protected:
  ...
  void compactComplexValues(const RowSet& rows, Move move, bool isFinal);
  ...
  bolt::common::ScanSpec* FOLLY_NONNULL scanSpec_;
