...
WriterContext::WriterContext(
  ...
  BOLT_CHECK(!(forceLowMemoryMode && disableLowMemoryMode));
...
void WriterContext::validateConfigs() const {
  ...
  BOLT_CHECK(indexEnabled(), "index is required");
  if (indexEnabled()) {
    BOLT_CHECK_GE(indexStride_, MIN_INDEX_STRIDE);
    // Java works with signed integer and setting anything above the int32_max
    // will make the java reader fail.
    BOLT_CHECK_LE(indexStride_, INT32_MAX);
  }
  BOLT_CHECK_GE(
...
memory::MemoryPool& WriterContext::getMemoryPool(
  ...
  switch (category) {
    ...
    case MemoryUsageCategory::GENERAL:
      return *generalPool_;
    default:
      BOLT_FAIL("Unreachable: {}", static_cast<int>(category));
  }
}
