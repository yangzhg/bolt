...
class MapZipWithFunction : public exec::VectorFunction {
 public:
  ...
  void apply(
    ...
    if (leftKeys->type()->isPrimitiveType() && !leftKeys->mayHaveNulls() &&
      ...
      index = BOLT_DYNAMIC_SCALAR_TYPE_DISPATCH_ALL(
 ...
 private:
  ...
  static VectorPtr materializeMergedKeys(
    ...
    mergedKeys->copy(
        leftKeys.get(), copyRows, mergeResults.rawLeftKeyIndices, false);

    // Copy right keys.
    copyRows.setAll();
    copyRows.deselectNonNulls(mergeResults.rawLeftKeyNulls, 0, size);
    mergedKeys->copy(
        rightKeys.get(), copyRows, mergeResults.rawRightKeyIndices, false);
