  ...
  void testLike(
      ...
      const std::string& errorMessage = "") {
    {
      ...
      if (expected.has_value()) {
        ...
        BOLT_ASSERT_THROW(eval(), errorMessage);
      }
    }
...
void testRe2Match(F&& regexMatch) {
  ...
  EXPECT_EQ(false, regexMatch(" abc", "abcd*"));
  // Invalid regex.
  EXPECT_THROW(regexMatch("abc", "*"), BoltException);
  // Null cases.
...
TEST_F(Re2FunctionsTest, regexMatchConstantPattern) {
  ...
  // mapping into bolt is expected to provide types explicitly. Therefore we
...
void testRe2Search(F&& regexSearch) {
  ...
  EXPECT_EQ(true, regexSearch(" abc", "abcd*"));
  // Invalid regex.
  EXPECT_THROW(regexSearch("abc", "*"), BoltException);
  // Null cases.
...
void testRe2Extract(F&& regexExtract) {
  ...
  EXPECT_THROW(regexExtract(" 123 ", "\\d+", -1), BoltException);
  ...
  EXPECT_EQ(
      ...
      "stringwithmorethan");
  EXPECT_THROW(regexExtract(" 123 ", " (\\d+)", 2), BoltException);
  // Regex doesn't match.
  EXPECT_THROW(regexExtract("abc", "\\d+", -1), BoltException);
  EXPECT_EQ(regexExtract("abc", "\\d+", 0), std::nullopt);
  EXPECT_THROW(regexExtract("abc", "\\d+", 1), BoltException);
  // Invalid regex.
  EXPECT_THROW(regexExtract("abc", "*", 0), BoltException);
  // NULL cases.
...
TEST_F(Re2FunctionsTest, regexExtractAllNoMatch) {
  ...
  testRe2ExtractAll({""}, {"[0-9]+"}, noGroupId, {emptyArray});
  testRe2ExtractAll({"(╯°□°)╯︵ ┻━┻"}, {"[0-9]+"}, noGroupId, {emptyArray});
  testRe2ExtractAll({"abcde"}, {"[0-9]+"}, groupIds0, {emptyArray});
  testRe2ExtractAll(
...
TEST_F(Re2FunctionsTest, regexExtractAllBadArgs) {
  ...
  EXPECT_THROW(eval("123", std::nullopt, 0), BoltException);
  EXPECT_THROW(eval(std::nullopt, "(\\d+)", 0), BoltException);
  EXPECT_THROW(eval("", "", 123), BoltException);
  EXPECT_THROW(eval("123", "", 99), BoltException);
  EXPECT_THROW(eval("123", "(\\d+)", 1), BoltException);
  EXPECT_THROW(eval("123", "[a-z]+", 1), BoltException);
}
...
TEST_F(Re2FunctionsTest, likeRegexLimit) {
  auto testNonLiteralLike = [&](int count) {
    ...
    auto getPatternAtIdx = [&](PatternKind patternKind,
      ...
      switch (patternKind) {
        ...
        case PatternKind::kSubstring:
          return fmt::format("%%abc{}%%%", idx);
        default:
          BOLT_UNREACHABLE("like is not optimized for pattern {}", patternKind);
      }
    };
    ...
    // Infer regex compilation does not happen for optimized patterns by
    // verifying less than kMaxCompiledRegexes are compiled for each optimized
    // pattern type.
    ...
    assertEqualVectors(
        BaseVector::createConstant(BOOLEAN(), false, count, pool()), result);

    // All are complex but the same
    for (int i = 0; i < count; i++) {
  ...
  testNonLiteralLike(46);
  testNonLiteralLike(100);
  testNonLiteralLike(1024);
}
