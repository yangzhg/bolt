...
class DataBuffer {
 public:
  explicit DataBuffer(bolt::memory::MemoryPool& pool, uint64_t size = 0)
      : pool_(&pool),
        ...
        size_(size),
        capacity_(size) {
    BOLT_CHECK(buf_ != nullptr || size_ == 0);
  }

  DataBuffer(DataBuffer&& other) noexcept
      : pool_{other.pool_},
        boltRef_{other.boltRef_},
        buf_{other.buf_},
        size_{other.size_},
  ...
  const T& at(uint64_t i) const {
    BOLT_CHECK_LT(i, size_, "Accessing index out of range");
    return data()[i];
  }
  ...
  void reserve(uint64_t capacity) {
    if (capacity <= capacity_) {
      ...
      BOLT_CHECK_NOT_NULL(buf_);
      return;
    }
    if (boltRef_ != nullptr) {
      BOLT_FAIL("Can't reserve on a referenced buffer");
    }
    ...
    BOLT_CHECK(buf_ != nullptr || newSize == 0);
    capacity_ = capacity;
  }
  ...
  void append(
    ...
    BOLT_CHECK_GE(src.size(), srcOffset + items);
  ...
  void clear() {
    if ((boltRef_ == nullptr) && (buf_ != nullptr)) {
  ...
  static std::shared_ptr<const DataBuffer<T>> wrap(
      const bolt::BufferPtr& buffer) {
    return std::shared_ptr<DataBuffer<T>>(new DataBuffer<T>(buffer));
  }

 private:
  explicit DataBuffer(const bolt::BufferPtr& buffer)
      : pool_{nullptr}, boltRef_{buffer} {
    buf_ = const_cast<T*>(boltRef_->as<T>());
    size_ = boltRef_->size() / sizeof(T);
    capacity_ = size_;
  }
  ...
  // The referenced bolt buffer. 'buf_' owns the memory when 'boltRef_' is
  // nullptr.
  const bolt::BufferPtr boltRef_{nullptr};
