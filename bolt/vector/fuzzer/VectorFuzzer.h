...
#include "bolt/vector/fuzzer/GeneratorSpec.h"
...
enum UTF8CharList {
  ...
  ALL_OTHERS = 4 // All other characters in
...
class VectorFuzzer {
 public:
  struct Options {
    ...
    /// Control the format of timestamps generated.
    enum class TimestampFormat : int8_t { kStandard = 0, kSpecial = 1 };
    std::vector<TimestampFormat> timestampFormat{
        TimestampFormat::kStandard,
        TimestampFormat::kSpecial};
    ...
    bool genTimestampString{false};

    // If true, the ConstantVector and DictionaryVector generated by fuzz() will
    // only nest FlatVector. It is used when flattenDictionary and
    // flattenConstant of ArrowOptions set to true when Vector is converted to
    // arrow format, because bridge::exportFlattenedVector does not support
    // other situations.
    bool fuzzForArrowFlatten{false};

    /// If true, existing generated data will be reused to generate new input
    /// strings. E.g., first generate part1, and then generate input =
    /// concat(part1, part1), input = concat(part1, "rdn", part1).
    bool enableStringIncrementalGeneration{true};

    /// If true, reuse the existing tests for generating inputs with duplicates.
    bool enableDuplicates{false};

    /// If false, disable dictionary encodings.
    bool enableDictionary{true};

    /// If false, disable HUGEINT generation for dwrf::ColumnWriter.
    bool enableHugeint{true};
  };
  ...
  // the future.
  TypePtr randType(int maxDepth = 5, bool withDecimal = true);
