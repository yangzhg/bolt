  ...
  struct Stats {
    ...
    uint32_t getRank() {
      ...
      if (hasVariadic && !hasGeneric) {
        BOLT_DCHECK(!hasVariadicOfGeneric);
        return 2;
      }
      ...
      if (hasVariadicOfGeneric) {
        BOLT_DCHECK(hasVariadic);
        BOLT_DCHECK(hasGeneric);
        return 4;
      }

      BOLT_UNREACHABLE("unreachable");
    }
 ...
 public:
  ...
  struct CreateType {
    static auto create(const exec::FunctionSignaturePtr& signature) {
      auto type = bolt::exec::SignatureBinder::tryResolveType(
...
class UDFHolder final
 ...
 public:
  ...
  std::string debugString() const {
    auto typeName = [](const std::type_info& typeInfo) -> std::string {
    ...
    auto typesList = [&]() -> std::string {
      std::string types = "(";
      int unused[] = {
          0,
          (void(types += (types == "(" ? "" : ", ") + typeName(typeid(TArgs))),
           0)...};
    ...
    return fmt::format(
        ...
        "  Return Type: {:<30}\n"
        "  Arguments: {:<30}\n"
        "  Call Method (bool return): {:<5}\n"
        "  Call Method (void return): {:<5}\n"
        "  CallNullable (bool return): {:<5}\n"
        "  CallNullable (void return): {:<5}\n"
        "  CallNullFree (bool return): {:<5}\n"
        "  CallNullFree (void return): {:<5}\n"
        "  CallAscii (bool return): {:<5}\n"
        "  CallAscii (void return): {:<5}\n"
        "  Initialize Method: {:<5}\n"
        "  Default Null Behavior: {:<5}\n"
        "  Null Output Possible: {:<5}\n"
        "  Contains Nulls Default: {:<5}\n"
        "  ASCII Support: {:<5}\n"
        "  ASCII Default Behavior: {:<5}",
        typeName(typeid(Fun)),
        typeName(typeid(TReturn)),
        typesList(),
        udf_has_call_return_bool,
        udf_has_call_return_void,
        udf_has_callNullable_return_bool,
        udf_has_callNullable_return_void,
        udf_has_callNullFree_return_bool,
        udf_has_callNullFree_return_void,
        udf_has_callAscii_return_bool,
        udf_has_callAscii_return_void,
        udf_has_initialize,
        is_default_null_behavior,
        can_produce_null_output,
        is_default_contains_nulls_behavior,
        has_ascii,
        is_default_ascii_behavior);
  }
};
