...
#include "bolt/expression/ExprCompiler.h"
...
/// FLAGS_bolt_save_input_on_expression_failure_path
...
std::string onTopLevelException(BoltException::Type exceptionType, void* arg) {
  auto* context = static_cast<ExprExceptionContext*>(arg);

  const char* basePath =
      FLAGS_bolt_save_input_on_expression_any_failure_path.c_str();
  if (strlen(basePath) == 0 && exceptionType == BoltException::Type::kSystem) {
    basePath = FLAGS_bolt_save_input_on_expression_system_failure_path.c_str();
  }
...
/// sub-expression.
std::string onException(BoltException::Type /*exceptionType*/, void* arg) {
...
void Expr::evalFlatNoNullsImpl(
  ...
  for (int32_t i = 0; i < inputs_.size(); ++i) {
    if (inputIsConstant_[i]) {
      ...
      BOLT_CHECK_NULL(inputValues_[i]);
    }
  }
  releaseInputValues(context);
}
...
void Expr::evaluateSharedSubexpr(
  ...
  BOLT_DCHECK(missingRows->hasSelections());
...
Expr::PeelEncodingsResult Expr::peelEncodings(
  ...
  // Attempt peeling.
  BOLT_CHECK(!vectorsToPeel.empty());
...
void Expr::evalWithMemo(
  ...
  if (base.get() != baseOfDictionaryRawPtr_ ||
    ...
    context.exprSet()->dictionaryCacheTotalSize() -= dictionaryCacheSize_;
    dictionaryCacheSize_ = 0;
    evalWithNulls(rows, context, result);
    return;
  }
  ...
  if (uncached->hasSelections()) {
    ...
    if (!context.exprSet()->dictionaryCacheSizeExceedLimit()) {
      ...
      dictionaryCache_->copy(result.get(), *uncached, nullptr, false);
      auto newRetainedSize = dictionaryCache_->retainedSize();
      context.exprSet()->dictionaryCacheTotalSize() +=
          newRetainedSize - dictionaryCacheSize_;
      dictionaryCacheSize_ = newRetainedSize;
    }
  }
...
void computeIsAsciiForInputs(
  ...
  for (auto& index : indices) {
    ...
    if (index < inputValues.size() &&
      ...
      BOLT_CHECK(vector, inputValues[index]->toString());
...
void Expr::evalAllImpl(
    ...
    EvalCtx& context,
    VectorPtr& result) {
  BOLT_DCHECK(rows.hasSelections());
  ...
  MutableRemainingRows remainingRows(rows, context);
  if (defaultNulls) {
    ScopeNegatedRelationSetter negatedSetter(
        context, vectorFunction_->isNegated());
...
#ifdef ENABLE_BOLT_EXPR_JIT
  if (hasJITed()) {
    // TODO: move logic to evalAll()
    // TODO: non-compilable sub expressions.
    std::vector<VectorPtr> args;
    for (auto&& f : distinctFields()) {
      VectorPtr v;
      f->evalSpecialForm(rows, context, &v);
    ...
    // Apply JITed vector function
    jitVectorFunction_(args, result);

    // Now, handle SelectivityVector
    // TODO:
    // 1. evaluate non-compilable sub expressions as inputs
    // 2. visit the expr tree for defaultNulls
    bool defaultNulls = vectorFunction_ != nullptr
        ? vectorFunction_->isDefaultNullBehavior()
        : true;
    auto* remainingRows = &rows;
    ...
    for (auto&& arg : args) {
      if (defaultNulls && arg->mayHaveNulls()) {
...
#ifdef ENABLE_BOLT_EXPR_JIT
bool Expr::hasJITed() {
  return jitVectorFunction_.jitFunc != 0;
}
#endif
...
void ExprSet::eval(
  ...
  if (FLAGS_bolt_experimental_save_input_on_fatal_signal) {
...
void SimpleExpressionEvaluator::evaluate(
    ...
    VectorPtr& result) {
  const std::string kDefaultCurrentSplitValue =
      "empty_split_str__unkown_evaluate_call";
  evaluate(exprSet, rows, input, result, kDefaultCurrentSplitValue);
}
...
void SimpleExpressionEvaluator::evaluate(
    ...
    const RowVector& input,
    VectorPtr& result,
    const std::string& currentSplitStr) {
  EvalCtx context(ensureExecCtx(), exprSet, &input, currentSplitStr);
