...
inline bool isBitSet(const T* bits, int32_t idx) {
  const uint32_t width = sizeof(T) << 3;
  return bits[idx / width] & (T(1) << (idx & (width - 1)));
}
...
inline void setBit(T* bits, uint32_t idx) {
  ...
  bitsAs8Bit[idx >> 3] |= (1 << (idx & 7));
}

template <typename T>
inline void clearBit(T* bits, uint32_t idx) {
  auto bitsAs8Bit = reinterpret_cast<uint8_t*>(bits);
  bitsAs8Bit[idx >> 3] &= kZeroBitmasks[idx & 7];
}
...
inline void forEachWord(
  ...
  if (lastWord < firstWord) {
    partialWordFunc(
        lastWord >> 6, lowMask(end - lastWord) & highMask(firstWord - begin));
    return;
  }
  if (begin != firstWord) {
    partialWordFunc(begin >> 6, highMask(firstWord - begin));
  }
  for (int32_t i = firstWord; i + 64 <= lastWord; i += 64) {
    fullWordFunc(i >> 6);
  }
  if (end != lastWord) {
    partialWordFunc(lastWord >> 6, lowMask(end - lastWord));
  }
}
...
forEachWord(int32_t begin, int32_t end, PartialWordFunc partialWordFunc) {
  ...
  int32_t firstIndex = (begin >> 6);
  int32_t lastIndex = (roundUp(end, 64) - 64) >> 6;
  for (auto index = firstIndex; index <= lastIndex; ++index) {
    uint64_t mask = ~0UL;
    if (index == firstIndex && begin != (firstIndex << 6)) {
      // We do not start at 64 bit boundary, and off the bits below start.
      mask = highMask(((firstIndex + 1) << 6) - begin);
    }
    if (index == lastIndex && (lastIndex << 6) + 64 != end) {
      // The last word is partial, and off the bits at and above 'end'.
      mask &= lowMask(end - (lastIndex << 6));
    }
...
void forEachBit(
    ...
    bool isSet,
    Callable func,
    const uint64_t* const nulls = nullptr) {
  ...
  if (nulls == nullptr) {
    forEachWord(
        ...
        [isSet, bits, func](int32_t idx, uint64_t mask) {
          ...
          while (word) {
            func((idx << 6) + __builtin_ctzll(word));
            word &= word - 1;
          }
        },
    ...
    forEachWord(
        ...
        [isSet, bits, func, nulls](int32_t idx, uint64_t mask) {
          auto word = (isSet ? bits[idx] : ~bits[idx]) & mask & nulls[idx];
          if (!word) {
            return;
          }
          while (word) {
        ...
        [isSet, bits, func, nulls](int32_t idx) {
          auto word = (isSet ? bits[idx] : ~bits[idx]) & nulls[idx];
          if (kAllSet == word) {
            const size_t start = (idx << 6);
            const size_t end = (start + 64);
            for (size_t row = start; row < end; ++row) {
...
inline void forEachSetBit(
    ...
    Callable func,
    const uint64_t* const nulls = nullptr) {
  forEachBit(bits, begin, end, true, func, nulls);
}
...
// Changing to specific unsigned-integer-overflow since folly adds this
// attribute when either of ASan, MSan, or TSan is enabled. This is incorrect
// because these sanitizers do not check for unsigned-integer-overflow and
// CLang and GCC now supports check for undefined behaviour sanitizer
// Should be updated in folly/CPortability.h to
...
#define FOLLY_DISABLE_SANITIZE_UNDEFINED_BEHAVIOR(...) \
...
// #endif
/// TODO: Revert after change to folly
#if defined(FOLLY_DISABLE_SANITIZE_UNDEFINED_BEHAVIOR)
FOLLY_DISABLE_SANITIZE_UNDEFINED_BEHAVIOR("unsigned-integer-overflow")
#endif
...
void storeBitsToByte(uint8_t bits, uint8_t* bytes, unsigned index) {
  BOLT_DCHECK_EQ(index % kSize, 0);
  BOLT_DCHECK_EQ(bits >> kSize, 0);
  if constexpr (kSize == 8) {
    bytes[index / 8] = bits;
  } else {
    BOLT_DCHECK_EQ(kSize, 4);
    if (index % 8 == 0) {
      bytes[index / 8] = bits;
    } else {
