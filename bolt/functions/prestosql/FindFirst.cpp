...
void recordInvalidStartIndex(vector_size_t row, exec::EvalCtx& context) {
  try {
    BOLT_USER_FAIL("SQL array indices start at 1. Got 0.");
  } catch (const BoltUserError& exception) {
 ...
 private:
  ...
  struct StartIndexProcessor {
    ...
    void process(
        ...
        exec::EvalCtx& context) {
      BOLT_CHECK_NOT_NULL(startIndex);
        ...
        *predicateVector,
        ...
        [&](vector_size_t row, vector_size_t firstMatchingIndex) {
          if (flatArray->elements()->isNullAt(firstMatchingIndex)) {
            try {
              BOLT_USER_FAIL("find_first found NULL as the first match");
            } catch (const BoltUserError& exception) {
              context.setBoltExceptionError(row, std::current_exception());
            }
          } else {
