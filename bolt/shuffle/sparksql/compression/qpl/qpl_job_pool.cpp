...
void QplJobHWPool::InitJobPool() {
  ...
  for (uint32_t index = 0; index < MAX_JOB_NUMBER; ++index) {
    qpl_job* qplJobPtr =
        reinterpret_cast<qpl_job*>(hwJobsBuffer.get() + index * jobSize);
    if (auto status = qpl_init_job(qpl_path_hardware, qplJobPtr);
        status != QPL_STS_OK) {
      jobPoolReady = false;
      ARROW_LOG(WARNING)
          << "Initialization of hardware-assisted DeflateQpl codec failed at index: "
          << index
          << ", falling back to SW codec. (Details: QplJobHWPool->qpl_init_job with error code: "
          << status
  ...
  ARROW_LOG(WARNING)
      << "Initialization of hardware-assisted DeflateQpl codec succeeded.";
  jobPoolReady = true;
}
