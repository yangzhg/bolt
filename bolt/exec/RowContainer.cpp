...
Accumulator::Accumulator(
    ...
    : isFixedSize_{isFixedSize},
      ...
      spillType_{std::move(spillType)},
      finalType_(std::move(finalType)),
      spillExtractFunction_{std::move(spillExtractFunction)},
      outputExtractFunction_{std::move(outputExtractFunction)},
...
const TypePtr& Accumulator::finalOutputType() const {
  return finalType_;
}
...
void Accumulator::extractForOutput(
    folly::Range<char**> groups,
    VectorPtr& result) const {
  outputExtractFunction_(groups, result);
}
...
int32_t RowContainer::combineAlignments(int32_t a, int32_t b) {
  BOLT_CHECK_EQ(__builtin_popcount(a), 1, "Alignment can only be power of 2");
  BOLT_CHECK_EQ(__builtin_popcount(b), 1, "Alignment can only be power of 2");
  return std::max(a, b);
}

const std::vector<column_index_t> RowContainer::kEmptySortKeyIndexes = {};
...
RowContainer::RowContainer(
  ...
  for (const auto& accumulator : accumulators) {
    ...
    hasVariableAccumulator_ |= !accumulator.isFixedSize();
...
void RowContainer::eraseRows(folly::Range<char**> rows) {
  ...
  numRows_ -= rows.size();
  for (auto* row : rows) {
    BOLT_CHECK(!bits::isBitSet(row, freeFlagOffset_), "Double free of row");
...
void RowContainer::checkConsistency() {
  ...
  for (auto free = firstFreeRow_; free; free = nextFree(free)) {
    ++numFree;
    BOLT_CHECK(bits::isBitSet(free, freeFlagOffset_));
  }
  BOLT_CHECK_EQ(numFree, numFreeRows_);
  BOLT_CHECK_EQ(allocatedRows, numRows_);
}
...
void RowContainer::extractSerializedRows(
  ...
  BOLT_CHECK_EQ(totalWritten, totalBytes);
}
...
void RowContainer::storeSerializedRow(
    ...
    vector_size_t index,
    char* row) {
  BOLT_CHECK(!vector.isNullAt(index));
...
int32_t RowContainer::compareStringAsc(
  ...
  leftPrefix = *(reinterpret_cast<const uint32_t*>(&left) + 1);
  rightPrefix = *(reinterpret_cast<const uint32_t*>(&right) + 1);
...
int32_t RowContainer::compareStringAsc(StringView left, StringView right) {
  ...
  if (leftPrefix < rightPrefix) {
    ...
    int32_t size =
        std::min(left.size(), right.size()) - StringView::kPrefixSize;
    if (size <= 0) {
      // One ends within the prefix.
      return left.size() - right.size();
    }
    if (left.isInline() && right.isInline()) {
      uint64_t inlined = reinterpret_cast<const int64_t*>(&left)[1];
      uint64_t otherInlined = reinterpret_cast<const int64_t*>(&right)[1];
      if constexpr (std::endian::native == std::endian::little) {
...
void RowContainer::skip(RowContainerIterator& iter, int32_t numRows) {
  ...
  if (!iter.endOfRun) {
    // Set to first row.
    BOLT_DCHECK_EQ(0, iter.rowNumber);
    BOLT_DCHECK_EQ(0, iter.allocationIndex);
...
int32_t RowContainer::listPartitionRows(
  ...
  while (numResults < maxRows && iter.rowNumber < numRows_) {
    ...
    int32_t runIndex;
    int32_t offsetInRun;
    BOLT_CHECK_LT(startRow, numRows_);
...
std::string RowContainer::toString() const {
  ...
  for (auto i = 0; i < keyTypes_.size(); ++i) {
    ...
    out << keyTypes_[i]->toString() << "(" << rowColumns_[i].offset() << ")";
  }
  ...
  if (types_.size() > keyTypes_.size()) {
    ...
    for (auto i = keyTypes_.size(); i < types_.size(); ++i) {
      ...
      out << types_[i]->toString() << "(" << rowColumns_[i].offset() << ")";
    }
  }
  ...
  if (nullOffsets_.size()) {
    out << " NullOffsets start bit " << nullOffsets_[0];
  }
  return out.str();
}
...
RowContainer::codegenCompare(
    const std::vector<TypePtr>& keyTypes,
    const std::vector<CompareFlags>& compareFlags,
    bytedance::bolt::jit::CmpType cmpType,
    bool hasNullKeys,
    const std::vector<column_index_t>& sortKeyIndexs) {
  // Setup the CodeGenerator
  bytedance::bolt::jit::RowContainerCodeGenerator codeGenerator;
  {
    std::vector<TypePtr> keyTypesCopy;
    std::vector<bytedance::bolt::jit::CompareFlags> flags;
    std::vector<int32_t> nullByteOffsets;
    std::vector<int8_t> nullByteMasks;
    std::vector<int32_t> keyOffsets;
    if (FOLLY_LIKELY(sortKeyIndexs.size() == 0)) {
      for (auto i = 0; i < keyTypes.size(); ++i) {
        keyTypesCopy.emplace_back(keyTypes[i]);
        if (cmpType != bytedance::bolt::jit::CmpType::EQUAL) {
          flags.emplace_back<bytedance::bolt::jit::CompareFlags>(
              {compareFlags[i].nullsFirst, compareFlags[i].ascending});
        }
        auto col = this->columnAt(i);
        keyOffsets.emplace_back(col.offset());
        nullByteOffsets.emplace_back(col.nullByte());
        nullByteMasks.emplace_back(col.nullMask());
      }
    } else {
      for (auto i = 0; i < keyTypes.size(); ++i) {
        keyTypesCopy.emplace_back(keyTypes[i]);
        ...
        auto col = this->columnAt(sortKeyIndexs[i]);
        keyOffsets.emplace_back(col.offset());
        nullByteOffsets.emplace_back(col.nullByte());
        nullByteMasks.emplace_back(col.nullMask());
      }
  ...
  auto moduleSP = codeGenerator.codegen();
  return {std::move(moduleSP), std::move(fnName)};
}

std::tuple<bytedance::bolt::jit::CompiledModuleSP, std::string>
RowContainer::codegenRowEqVectors(
    const std::vector<TypePtr>& keyTypes,
    bool haveNulls) {
  // Setup the CodeGenerator
  bytedance::bolt::jit::RowEqVectorsCodeGenerator codeGenerator;
  {
    std::vector<TypePtr> keyTypeKinds;
    std::vector<int32_t> nullByteOffsets;
    std::vector<int8_t> nullByteMasks;
    std::vector<int32_t> keyOffsets;

    for (auto i = 0; i < keyTypes.size(); ++i) {
      keyTypeKinds.emplace_back(keyTypes[i]);
      auto col = this->columnAt(i);
      keyOffsets.emplace_back(col.offset());
      nullByteOffsets.emplace_back(col.nullByte());
      nullByteMasks.emplace_back(col.nullMask());
    }
    codeGenerator.setKeyTypes(std::move(keyTypeKinds))
        .setKeyOffsets(std::move(keyOffsets))
        .setNullByteOffsets(std::move(nullByteOffsets))
        .setNullMasks(std::move(nullByteMasks))
...
void RowPartitions::appendPartitions(folly::Range<const uint8_t*> partitions) {
  ...
  int index = 0;
  BOLT_CHECK_LE(size_ + toAdd, capacity_);
  while (toAdd) {
    int32_t run;
    int32_t offset;
...
__attribute__((__visibility__("default"))) int32_t ComplexTypeRowCmpRow(
    ...
    int8_t ascending) {
  auto leftStream =
      bytedance::bolt::exec::RowContainer::prepareRead(left, offset);
  auto rightStream =
      bytedance::bolt::exec::RowContainer::prepareRead(right, offset);
  return bytedance::bolt::exec::ContainerRowSerde::compare(
      *leftStream,
      *rightStream,
      reinterpret_cast<bytedance::bolt::Type*>(typePtr),
...
__attribute__((__visibility__("default"))) int32_t RowBasedComplexTypeRowCmpRow(
    ...
    int8_t nullFirst,
    int8_t ascending) {
  // as contiguous memory, it can't use RowContainer::prepareRead()
  auto leftSv = reinterpret_cast<std::string_view*>(left + offset);
  auto rightSv = reinterpret_cast<std::string_view*>(right + offset);
  ...
  bytedance::bolt::ByteRange leftRange{leftPtr, leftSize, 0};
  bytedance::bolt::ByteRange rightRange{rightPtr, rightSize, 0};

  bytedance::bolt::ByteInputStream leftStream(
      std::vector<bytedance::bolt::ByteRange>{leftRange}),
      rightStream(std::vector<bytedance::bolt::ByteRange>{rightRange});
  return bytedance::bolt::exec::ContainerRowSerde::compare(
      leftStream,
      rightStream,
      reinterpret_cast<bytedance::bolt::Type*>(typePtr),
...
CmpRowVecTimestamp(char* vec, int32_t index, char* rowPtr) {
  ...
  auto rts =
      bytedance::bolt::exec::RowContainer::valueAt<bytedance::bolt::Timestamp>(
          rowPtr, 0);
  return ts == rts;
}
...
__attribute__((__visibility__("default"))) int8_t ComplexTypeRowEqVectors(
    ...
    int32_t index) {
  auto decoded = reinterpret_cast<bytedance::bolt::DecodedVector*>(vec);
  auto stream = bytedance::bolt::exec::RowContainer::prepareRead(row, offset);
  return bytedance::bolt::exec::ContainerRowSerde::compare(
             *stream, *decoded, index, bytedance::bolt::CompareFlags()) == 0;
}
