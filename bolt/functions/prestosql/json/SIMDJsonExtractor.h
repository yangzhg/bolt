...
simdjson::error_code simdJsonExtract(
    const bolt::StringView& json,
    const bolt::StringView& path,
    TConsumer&& consumer);
...
class SIMDJsonExtractor {
 ...
 private:
  ...
  explicit SIMDJsonExtractor(const std::string& path) {
    if (!tokenize(path)) {
      BOLT_USER_FAIL("Invalid JSON path: {}", path);
    }
  }
...
simdjson::error_code simdJsonExtract(
    ...
    const bolt::StringView& path,
    TConsumer&& consumer) {
  // If extractor fails to parse the path, this will throw a BoltUserError, and
...
simdjson::error_code simdJsonExtract(
  ...
  return simdJsonExtract(
      bolt::StringView(json),
      bolt::StringView(path),
