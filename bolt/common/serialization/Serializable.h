...
#include "bolt/common/serialization/DeserializationRegistry.h"
...
class ISerializable {
 public:
  ...
  static std::shared_ptr<const T> deserialize(
    ...
    const auto& registryWithContext =
        bolt::DeserializationWithContextRegistryForSharedPtr();
    if (registryWithContext.Has(name)) {
      return std::dynamic_pointer_cast<const T>(
          registryWithContext.Create(name, obj, context));
    }

    const auto& registry = bolt::DeserializationRegistryForSharedPtr();
    BOLT_USER_CHECK(
        registry.Has(name),
  ...
  static T deserialize(const folly::dynamic& obj, void* context = nullptr) {
    auto raw = obj.asInt();
    BOLT_USER_CHECK_GE(raw, std::numeric_limits<T>::min());
    BOLT_USER_CHECK_LE(raw, std::numeric_limits<T>::max());
    return (T)raw;
  }
 ...
 private:
  ...
  static auto deserializeAsUniquePtr(const folly::dynamic& obj) {
    auto name = obj["name"].asString();
    const auto& registry = bolt::deserializationRegistryForUniquePtr();
    using deserializeType =
