...
///
/// (1) define the function body between BOLT_UDF_BEGIN and BOLT_UDF_END
/// macros.
/// (2) declare it using BOLT_DECLARE_VECTOR_FUNCTION macros.
/// (3) register it using BOLT_DECLARE_VECTOR_FUNCTION macro.
///
...
class VectorArithmetic : public VectorFunction {
 public:
  ...
  void apply(
    ...
    BOLT_CHECK(args.size() == 2);
    BOLT_CHECK(args[0]->typeKind() == args[1]->typeKind());
    ...
    switch (args[0]->typeKind()) {
      ...
      case TypeKind::DOUBLE:
        ...
        return;
      default:
        BOLT_CHECK(false, "Bad type for arithmetic");
    }
  }
