...
ExprPtr compileRewrittenExpression(
  ...
  if (dynamic_cast<const core::ConcatTypedExpr*>(expr.get())) {
    ...
    BOLT_CHECK(!compiledInputs.empty());
    ...
    BOLT_UNSUPPORTED("InputTypedExpr '{}' is not supported", row->toString());
  } else if (
      auto constant =
...
#ifdef ENABLE_BOLT_EXPR_JIT
  // Try to JIT it
  if (isRootExpr && config.isBoltJitExprEnabled()) {
    auto jitFn = jit::compileExprToJitVectorFunc(result.get());
    if (jitFn.jitFunc) {
      result->setJitFunction(jitFn);
    }
  }
#endif
