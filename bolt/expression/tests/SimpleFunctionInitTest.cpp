...
struct NonDefaultWithArrayInitFunction {
  ...
  void initialize(
      ...
      const arg_type<bolt::Array<int32_t>>* second) {
    if (second == nullptr) {
      return;
    }
...
struct NonDefaultWithMapInitFunction {
  ...
  void initialize(
      ...
      const core::QueryConfig& /*config*/,
      const arg_type<int32_t>* /*first*/,
      const arg_type<bolt::Map<int32_t, int64_t>>* second) {
    if (second == nullptr) {
      return;
    }
...
struct InitAlwaysThrowsFunction {
  ...
  void initialize(
      ...
      const core::QueryConfig& /*config*/,
      const arg_type<int32_t>* /*first*/) {
    BOLT_USER_FAIL("Unconditional throw!");
  }
...
TEST_F(SimpleFunctionInitTest, initException) {
  ...
  EXPECT_THROW(evaluate("init_throws(c0)", rowVector), BoltUserError);
