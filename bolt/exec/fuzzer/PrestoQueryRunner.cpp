...
T extractSingleValue(const std::vector<RowVectorPtr>& data) {
  BOLT_CHECK_EQ(1, data.size());
  BOLT_CHECK_EQ(1, data[0]->childrenSize());

  auto simpleVector = data[0]->childAt(0)->as<SimpleVector<T>>();
  BOLT_CHECK(!simpleVector->isNullAt(0));
...
std::string toAggregateCallSql(
    ...
    bool distinct) {
  BOLT_CHECK_EQ(sortingKeys.size(), sortingOrders.size());
  std::stringstream sql;
  sql << call->name() << "(";
...
std::optional<std::string> PrestoQueryRunner::toSql(
  ...
  BOLT_CHECK(aggregationNode->step() == core::AggregationNode::Step::kSingle);
...
std::vector<bolt::RowVectorPtr> PrestoQueryRunner::executeVector(
    const std::string& sql,
    const std::vector<bolt::RowVectorPtr>& input,
    const bolt::RowTypePtr& resultType) {
...
bool PrestoQueryRunner::supportsBoltVectorResults() const {
  return true;
}
