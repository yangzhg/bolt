...
TEST_F(MemoryUsageTrackerTest, update) {
  ...
  ASSERT_THROW(child1->reserve(2 * kMaxSize), BoltRuntimeError);
...
TEST_F(MemoryUsageTrackerTest, reserve) {
  ...
  EXPECT_THROW(child->reserve(2 * kMaxSize), BoltRuntimeError);
...
TEST_F(MemoryUsageTrackerTest, memoryLeakCheck) {
  ...
  FLAGS_bolt_memory_leak_check_enabled = false;
  child.reset();
}
...
DEBUG_ONLY_TEST_F(MemoryUsageTrackerTest, grow) {
  ...
  }
  {
    ...
    SCOPED_TESTVALUE_SET(
        "bytedance::bolt::memory::MemoryUsageTracker::incrementReservation::AfterGrowCallback",
  ...
  ASSERT_THROW(child->update(100 * kMB), BoltRuntimeError);
  ...
  ASSERT_THROW(child->update(100 * kMB);, BoltException);
  ASSERT_EQ(child->currentBytes(), 30 * kMB);
...
TEST_F(MemoryUsageTrackerTest, concurrentAllocates) {
  ...
  SCOPED_TESTVALUE_SET(
      "bytedance::bolt::memory::MemoryUsageTracker::reserve",
      std::function<void(MemoryUsageTracker*)>(
