...
struct TableInsertPartitionInfo {
  ...
  TableInsertPartitionInfo(
    ...
    BOLT_CHECK_GE(start, 0);
    BOLT_CHECK_LT(start, end);
  }
...
struct HashLookup {
  ...
  explicit HashLookup(
      ...
      bool jitEqual)
      : hashers(h), jitRowEqVectors(jitEqual) {}
  ...
  /// Stores new group indices per batch for distinct aggregations
  std::vector<vector_size_t> distinctNewGroups;
...
class BaseHashTable {
 public:
  ...
  // suppose rows are new and directly add them into rowcontainer, like
  // inserting new entries into hash table in the normal probing phase.
  virtual void directAddRows(
      HashLookup& lookup,
      const RowVectorPtr& input,
      SelectivityVector& rows,
 ...
 protected:
  ...
  void checkHashBitsOverlap(int8_t spillInputStartPartitionBit) {
    if (spillInputStartPartitionBit != kNoSpillInputStartPartitionBit &&
      ...
      BOLT_CHECK_LE(sizeBits(), spillInputStartPartitionBit);
    }
  }
...
class HashTable : public BaseHashTable {
 public:
  ...
  // that matches join condition for right and full outer joins, or meaning
  // a equal next rows in a sorted spilled stream for spilled aggregation.
  HashTable(
  ...
  int32_t listJoinResultsInternal(
      JoinResultIterator& iter,
      bool includeMisses,
      folly::Range<vector_size_t*> inputRows,
      folly::Range<char**> hits,
 ...
 private:
  ...
  Bucket* bucketAt(int64_t offset) const {
    BOLT_DCHECK_EQ(0, offset & (kBucketSize - 1));
  ...
  int64_t nextBucketOffset(int64_t bucketOffset) const {
    BOLT_DCHECK_EQ(0, bucketOffset & (kBucketSize - 1));
    BOLT_DCHECK_LT(bucketOffset, sizeMask_);
  ...
  // hash table optimize from haiyan
  std::vector<std::list<std::vector<char*>>> nexts_;

  bool enableJit_{false};
  RowEqVectors rowEqVectorsFunc_{nullptr};
  RowRowCompare rowEqRowFunc_{nullptr};
...
template <>
struct fmt::formatter<bytedance::bolt::exec::BaseHashTable::HashMode>
    : formatter<std::string> {
  auto format(
      bytedance::bolt::exec::BaseHashTable::HashMode s,
      format_context& ctx) {
    return formatter<std::string>::format(
        bytedance::bolt::exec::BaseHashTable::modeString(s), ctx);
  }
};
