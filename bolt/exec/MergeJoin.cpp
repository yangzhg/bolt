...
int32_t MergeJoin::compare(
  ...
  for (auto i = 0; i < keys.size(); ++i) {
    CompareFlags compareFlags = {
        .equalsOnly = false,
        .nullHandlingMode =
            CompareFlags::NullHandlingMode::kNullAsSpeciallValue};
...
void MergeJoin::addOutputRowForLeftJoin(
  ...
  for (const auto& projection : rightProjections_) {
    ...
    if (target->isConstantEncoding()) {
      BaseVector::flattenVector(target);
    }
...
void MergeJoin::addOutputRowForRightJoin(
  ...
  BOLT_USER_CHECK(isRightJoin(joinType_) || isFullJoin(joinType_));
...
RowVectorPtr MergeJoin::filterOutputForSemiJoin(const RowVectorPtr& output) {
  ...
  return wrapAndCombineDict(index, indices, output);
}
...
RowVectorPtr MergeJoin::filterOutputForAntiJoin(const RowVectorPtr& output) {
  ...
  BOLT_CHECK_EQ(index, numPassed);

  // Some, but not all rows passed.
  return wrapAndCombineDict(numPassed, indices, output);
}
      ...
      while (!noMoreRightInput_ && !rightInput_) {
        ...
        if (rightInput_) {
          ...
          if ((isRightJoin(joinType_) || isFullJoin(joinType_)) &&
            ...
            auto remainBatchSize = outputBatchSize_ - outputSize_;
            BOLT_CHECK(remainBatchSize > 0);
            auto batchSize =
                std::min<vector_size_t>(firstNonNullIndex, remainBatchSize);
            for (auto i = 0; i < batchSize; ++i) {
              addOutputRowForRightJoin(rightInput_, i);
            }
            if (firstNonNullIndex > remainBatchSize) {
              rightIndex_ = remainBatchSize;
              return std::move(output_);
            }
          }
...
RowVectorPtr MergeJoin::doGetOutput() {
  ...
  if (leftMatch_ && leftMatch_->cursor) {
    BOLT_CHECK(rightMatch_ && rightMatch_->cursor);
  ...
  // ready for output.
  if (leftMatch_) {
    BOLT_CHECK(leftMatch_->complete);
    BOLT_CHECK(rightMatch_ && rightMatch_->complete);
  ...
  for (;;) {
    ...
    if (compareResult == 0) {
      ...
      auto oldNum = outputSize_;
      if (addToOutput()) {
        if (oldNum == output_->size()) {
          previousLeftMatch_.reset();
        } else {
          previousLeftMatch_ = leftMatch_;
        }
        return std::move(output_);
...
RowVectorPtr MergeJoin::applyFilter(const RowVectorPtr& output) {
  // as the shared vector ptr may be changed, we need to reset the vector ptr
  for (auto id : filterInputToOutputChannel_) {
    filterInput_->childAt(id.first) = output->childAt(id.second);
  }
  ...
  if (joinTracker_) {
    ...
    //
    // 3. previousLeftMatch_ may be nullopt, leave an output
  ...
  BOLT_CHECK_LT(numPassed, numRows);
