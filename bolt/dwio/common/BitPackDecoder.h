...
static inline uint32_t unpackNaive(
    ...
    uint8_t bitWidth,
    T* FOLLY_NONNULL& result) {
  BOLT_CHECK(bitWidth >= 1 && bitWidth <= sizeof(T) * 8);
  BOLT_CHECK(inputBufferLen * 8 >= bitWidth * numValues);
...
inline void unpack<uint8_t>(
    ...
    uint8_t bitWidth,
    uint8_t* FOLLY_NONNULL& result) {
  BOLT_CHECK(bitWidth >= 1 && bitWidth <= 8);
  BOLT_CHECK(inputBufferLen * 8 >= bitWidth * numValues);
...
inline void unpack<uint16_t>(
    ...
    uint8_t bitWidth,
    uint16_t* FOLLY_NONNULL& result) {
  BOLT_CHECK(bitWidth >= 1 && bitWidth <= 16);
  BOLT_CHECK(inputBufferLen * 8 >= bitWidth * numValues);
  ...
  switch (bitWidth) {
    ...
    case 16:
      ...
      break;
    default:
      BOLT_UNREACHABLE("invalid bitWidth");
  }
#else
...
inline void unpack<uint32_t>(
    ...
    uint8_t bitWidth,
    uint32_t* FOLLY_NONNULL& result) {
  BOLT_CHECK(bitWidth >= 1 && bitWidth <= 32);
  BOLT_CHECK(inputBufferLen * 8 >= bitWidth * numValues);
...
inline uint64_t safeLoadBits(
  ...
  int32_t byteWidth =
      bytedance::bolt::bits::roundUp(bitOffset + bitWidth, 8) / 8;
  return bytedance::bolt::bits::loadPartialWord(
