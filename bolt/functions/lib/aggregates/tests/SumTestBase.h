...
void testHookLimits(bool expectOverflow = false) {
  ...
  for (const auto& [limit, overflow] : limits) {
    ...
    if (expectOverflow) {
      BOLT_ASSERT_THROW(hook.addValue(0, &overflow), "overflow");
    } else {
...
void verifyAggregates(
  ...
  for (const auto& [agg, expectedResult] : aggsToTest) {
    if (expectOverflow) {
      BOLT_ASSERT_THROW(
          bytedance::bolt::exec::test::readSingleValue(agg), "overflow");
    } else {
      auto result = bytedance::bolt::exec::test::readSingleValue(agg);
...
void SumTestBase::testAggregateOverflow(
  ...
  partialAggsToTest.push_back(
      {bytedance::bolt::exec::test::PlanBuilder()
