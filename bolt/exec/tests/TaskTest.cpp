...
class TestSkewedJoinBridge : public exec::JoinBridge {
 public:
  ...
  std::optional<bool> buildFinishedOrFuture(ContinueFuture* future) {
    std::lock_guard<std::mutex> l(mutex_);
    BOLT_CHECK(!cancelled_, "Getting data after the build side is aborted");
...
class TestSkewedJoinBuild : public exec::Operator {
 public:
  ...
  void noMoreInput() override {
    ...
    BOLT_FAIL("Last driver should not finish successfully.");
  }
...
// A test node creates operator that allocate memory from bolt memory pool on
// construction.
...
class TaskTest : public HiveConnectorTestBase {
 protected:
  ...
  executeSerial(
    ...
    BOLT_CHECK(task->supportSerialExecutionMode());
    ...
    BOLT_CHECK(waitForTaskCompletion(task.get()));

    auto planNodeStats = toPlanStats(task->taskStats());
    BOLT_CHECK(planNodeStats.count(plan.planNode->id()));
    BOLT_CHECK_EQ(numRows, planNodeStats.at(plan.planNode->id()).outputRows);
    BOLT_CHECK_EQ(
...
TEST_F(TaskTest, wrongPlanNodeForSplit) {
  ...
  BOLT_ASSERT_THROW(task->setMaxSplitSequenceId("1", 9), errorMessage)

  BOLT_ASSERT_THROW(task->noMoreSplits("1"), errorMessage)

  BOLT_ASSERT_THROW(task->noMoreSplitsForGroup("1", 5), errorMessage)
  ...
  BOLT_ASSERT_THROW(task->setMaxSplitSequenceId("12", 9), errorMessage)

  BOLT_ASSERT_THROW(task->noMoreSplits("12"), errorMessage)

  BOLT_ASSERT_THROW(task->noMoreSplitsForGroup("12", 5), errorMessage)
...
TEST_F(TaskTest, singleThreadedExecution) {
  ...
  {
    auto [task, results] = executeSerial(plan, {{scanId, {filePath->path}}});
    assertEqualResults({expectedResult}, results);
  }
...
DEBUG_ONLY_TEST_F(TaskTest, inconsistentExecutionMode) {
  {
    ...
    cursor->start();
    BOLT_ASSERT_THROW(task->next(), "Inconsistent task execution mode.");
...

  {
    ...
    task->next();
    BOLT_ASSERT_THROW(task->start(4, 1), "Inconsistent task execution mode.");
    ...
    SCOPED_TESTVALUE_SET(
        ...
        std::function<void(Operator*)>([&](Operator* testOp) {
          ...
          for (auto* op : ops) {
            ...
            BOLT_ASSERT_THROW(driver->findOperator(-1), "");
            BOLT_ASSERT_THROW(driver->findOperator(numDriver + 10), "");
          }
...
DEBUG_ONLY_TEST_F(
  ...
  auto thread = std::thread(
      [&]() { BOLT_ASSERT_THROW(blockingTask->next(), abortErrorMessage); });

  try {
    BOLT_FAIL(abortErrorMessage);
...
DEBUG_ONLY_TEST_F(TaskTest, driverEnqueAfterFailedAndPausedTask) {
  ...
  SCOPED_TESTVALUE_SET(
      "bytedance::bolt::exec::Task::enter",
      std::function<void(const bolt::exec::ThreadState*)>(
          ([&](const bolt::exec::ThreadState* /*unused*/) {
