...
template <typename BoltType, typename T>
auto materializeElement(const T& element) {
  if constexpr (MaterializeType<BoltType>::requiresMaterialization) {
    return element.materialize();
  } else {
    using unwrapped_type = typename UnwrapCustomType<BoltType>::type;
...
class MapView {
 public:
  ...
  ValueAccessor at(const key_element_t& key) const {
    auto it = find(key);
    BOLT_USER_CHECK(it != end(), "accessed key is not found in the map");
    return it->second;
  }
...
namespace std {
template <>
struct hash<bytedance::bolt::exec::GenericView> {
  size_t operator()(const bytedance::bolt::exec::GenericView& x) const {
    return x.hash();
  }
};
} // namespace std
