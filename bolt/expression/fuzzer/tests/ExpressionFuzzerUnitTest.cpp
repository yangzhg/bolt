...
namespace bytedance::bolt::fuzzer::test {
...
TEST_F(ExpressionFuzzerUnitTest, reproduceExpressionWithSeed) {
  ...
  BOLT_CHECK_EQ(firstGeneration.size(), 10);
  BOLT_CHECK_EQ(secondGeneration.size(), 10);
  for (auto i = 0; i < 10; ++i) {
    BOLT_CHECK_EQ(firstGeneration[i], secondGeneration[i]);
  }
}
