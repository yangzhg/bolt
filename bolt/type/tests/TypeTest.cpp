...
void testTypeSerde(const TypePtr& type) {
  ...
  auto copy = bolt::ISerializable::deserialize<Type>(
      bolt::ISerializable::serialize(type));
...
TEST(TypeTest, constructorThrow) {
  ...
  EXPECT_THROW(RowType({"a"}, {VARCHAR(), INTEGER()}), BoltRuntimeError);
  BOLT_ASSERT_THROW(
      RowType({"a"}, {VARCHAR(), INTEGER()}),
      "Mismatch names/types sizes: "
      "[names: {'a'}, types: {VARCHAR, INTEGER}]");

  EXPECT_THROW(RowType({"a", "b"}, {}), BoltRuntimeError);
  BOLT_ASSERT_THROW(
      RowType({"a", "b"}, {}),
      "Mismatch names/types sizes: "
      "[names: {'a', 'b'}, types: { }]");

  EXPECT_THROW(RowType({"a", "b"}, {VARCHAR(), nullptr}), BoltRuntimeError);
  BOLT_ASSERT_THROW(
...
TEST(TypeTest, array) {
  ...
  EXPECT_THROW(arrayType->childAt(1), BoltUserError);
...
TEST(TypeTest, timestampToString) {
  Timestamp epoch(0, 0);
  EXPECT_EQ(epoch.toString(), "1970-01-01 00:00:00.000000000");

  Timestamp beforeEpoch(-1, 890);
  EXPECT_EQ(beforeEpoch.toString(), "1969-12-31 23:59:59.000000890");

  Timestamp year2100(4123638000, 123456789);
  EXPECT_EQ(year2100.toString(), "2100-09-03 07:00:00.123456789");

  Timestamp wayBeforeEpoch(-9999999999, 987654321);
  EXPECT_EQ(wayBeforeEpoch.toString(), "1653-02-10 06:13:21.987654321");
}
...
TEST(TypeTest, map) {
  ...
  EXPECT_THROW(mapType->childAt(2), BoltUserError);
...
TEST(TypeTest, row) {
  BOLT_ASSERT_THROW(ROW({{"a", nullptr}}), "Child types cannot be null");
  ...
  EXPECT_THROW(row0->findChild("not_exist"), BoltUserError);
  ...
  BOLT_ASSERT_THROW(createScalarType(TypeKind::ROW), "not a scalar type");
  BOLT_ASSERT_THROW(
...
TEST(TypeTest, opaque) {
  ...
  EXPECT_THROW(foo->getSerializeFunc(), BoltException);
  EXPECT_THROW(foo->getDeserializeFunc(), BoltException);
  EXPECT_THROW(bar->serialize(), BoltException);
  EXPECT_THROW(bar->getSerializeFunc(), BoltException);
  EXPECT_THROW(bar->getDeserializeFunc(), BoltException);
...
TEST(TypeTest, cpp2Type) {
  ...
  EXPECT_EQ(*CppToType<bolt::StringView>::create(), *VARCHAR());
...
TEST(TypeTest, dynamicTypeDispatch) {
  auto val1 =
      BOLT_DYNAMIC_TYPE_DISPATCH(returnKindIntPlus, TypeKind::INTEGER, 1);
  EXPECT_EQ(val1, (int32_t)TypeKind::INTEGER + 1);

  auto val2 =
      BOLT_DYNAMIC_SCALAR_TYPE_DISPATCH(returnKindIntPlus, TypeKind::BIGINT, 2);
...
TEST(TypeTest, isVariadicType) {
  ...
  EXPECT_FALSE(isVariadicType<bolt::StringView>::value);
