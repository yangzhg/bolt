...
class SelectivityVector {
 public:
  ...
  explicit SelectivityVector(vector_size_t length, bool allSelected = true)
      : bits_(bits::nwords(length), allSelected ? ~0ULL : 0),
        size_(length),
        begin_(0),
        end_(allSelected ? length : 0),
  ...
  void setValid(vector_size_t idx, bool valid) {
    BOLT_DCHECK_LT(idx, bits_.size() * sizeof(bits_[0]) * 8);
  ...
  void setValidRange(vector_size_t begin, vector_size_t end, bool valid) {
    BOLT_DCHECK_GE(end, begin);
    if (begin == end) {
      return;
    }
    BOLT_DCHECK_LE(end, bits_.size() * sizeof(bits_[0]) * 8);
  ...
  void applyToSelected(Callable func, const uint64_t* const nulls = nullptr)
      const;
...
inline void SelectivityVector::applyToSelected(
    Callable func,
    const uint64_t* const nulls) const {
  if (!nulls && isAllSelected()) {
    // use const variable to help compiler generate more effective code
    const vector_size_t end = end_;
    for (vector_size_t row = begin_; row < end; ++row) {
      func(row);
    }
  } else {
    bits::forEachSetBit(bits_.data(), begin_, end_, func, nulls);
  }
}
