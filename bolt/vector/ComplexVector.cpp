...
std::shared_ptr<RowVector> RowVector::createEmpty(
  ...
  BOLT_CHECK_NOT_NULL(type, "Vector creation requires a non-null type.");
  BOLT_CHECK(type->isRow());
...
std::optional<int32_t> RowVector::compare(
  ...
  for (int32_t i = 0; i < compareSize; ++i) {
    ...
    BaseVector* otherChildLoadedVec = otherChild->loadedVector();
    auto wrappedOtherIndex = other->wrappedIndex(otherIndex);
    auto result =
        child->compare(otherChildLoadedVec, index, wrappedOtherIndex, flags);
...
void RowVector::copy(
  ...
  if (decodedSource.isIdentityMapping()) {
    if (source->mayHaveNulls()) { // TODO(fzh) check null ratio
      auto rawNulls = source->rawNulls();
      rows.applyToSelected([&](auto row) {
        auto idx = toSourceRow ? toSourceRow[row] : row;
        BOLT_DCHECK_GT(source->size(), idx);
    ...
    for (auto i = 0; i < childrenSize_; ++i) {
      if (rowSource->childAt(i)) {
        ...
        children_[i]->copy(
            rowSource->childAt(i)->loadedVector(),
            nonNullRows,
            toSourceRow,
            canCopyAll);
      } else {
...
bool isAllNullVector(const BaseVector& vector) {
  ...
  if (leafVector->isConstantEncoding()) {
    ...
    BOLT_CHECK(leafVector->isNullAt(0));
    return true;
  }
  return false;
}
...
std::string RowVector::toString(vector_size_t index) const {
  BOLT_CHECK_LT(index, length_, "Vector index should be less than length.");
  if (isNullAt(index)) {
    return "null";
  }
  std::stringstream out;
...
void ArrayVectorBase::copyRangesImpl(
  ...
  BOLT_CHECK_EQ(leafSource->encoding(), encoding());
  ...
  if (ranges.size() == 1 && ranges.back().count == 1) {
    ...
    BOLT_DCHECK(BaseVector::length_ >= range.targetIndex + range.count);
    ...
    if (source->isNullAt(range.sourceIndex)) {
      ...
      if (copySize > 0) {
        ...
        auto newSize = checkedPlus(childSize, copySize);
        targetValues->get()->resize(newSize);
        targetValues->get()->copy(
            sourceValues, childSize, copyOffset, copySize);
        if (targetKeys) {
          targetKeys->get()->resize(newSize);
    ...
    applyToEachRange(
        ranges, [&](auto targetIndex, auto sourceIndex, auto count) {
          if (count > 0) {
            BOLT_DCHECK(BaseVector::length_ >= targetIndex + count);
            totalCount += count;
          }
        });
...
void ArrayVectorBase::validateArrayVectorBase(
  ...
  BOLT_CHECK_GE(sizes_->size(), bufferByteSize);
  BOLT_CHECK_GE(offsets_->size(), bufferByteSize);
...
std::optional<int32_t> MapVector::compare(
  ...
  BOLT_DCHECK(result.has_value(), "Keys may not have nulls or nested nulls");
...
void MapVector::canonicalize(MapVector* map, bool useStableSort) {
  if (map->sortedKeys_) {
    return;
  }
  BufferPtr indices;
...
// static
CompositeRowVectorPtr CompositeRowVector::createInternal(
    const TypePtr& type,
    vector_size_t size,
    bolt::memory::MemoryPool* pool,
  ...
  children.resize(rowType.size());
  if (validColumns) {
    validColumns->applyToSelected([&](vector_size_t row) {
      children[row] = BaseVector::create(rowType.childAt(row), size, pool);
    });
  }
  return std::make_shared<CompositeRowVector>(
      type, size, pool, std::move(validColumns), std::move(children));
}

void CompositeRowVector::resize(vector_size_t newSize, bool setNotNull) {
  const auto oldSize = size();
  BaseVector::resize(newSize, setNotNull);
  if (validChildren_) {
    validChildren_->applyToSelected([&](vector_size_t index) {
      auto& child = children_[index];
      BOLT_CHECK(child != nullptr);
      if (child->isLazy()) {
        BOLT_FAIL("Resize on a lazy vector is not allowed");
      }
      if (newSize > oldSize) {
  ...
  rows_.resize(newSize);
  rawRows_.clear();
}

void CompositeRowVector::prepareForReuse() {
  rows_.clear();
  rawRows_.clear();
  // to be optimized
  rowBuffer_.reset();
...
uint64_t CompositeRowVector::usedSize() const {
  auto size = BaseVector::usedSize();
  size += rowBuffer_->allocatedBytes();
  if (validChildren_) {
    validChildren_->applyToSelected(
        [&](vector_size_t index) { size += children_[index]->usedSize(); });
  }
  return size;
}
