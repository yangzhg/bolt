...
#include "bolt/cudf/CudfEnable.h"
#endif
...
class PlanNode : public ISerializable {
 public:
  ...
  std::string toString(
    ...
    toString(stream, detailed, recursive, 0, withId, addContext);
    return stream.str();
  }
...
class ArrowStreamNode : public PlanNode {
 public:
  ArrowStreamNode(
    ...
    BOLT_USER_CHECK_NOT_NULL(arrowStream_);
  }
  ...
  folly::dynamic serialize() const override {
    folly::dynamic obj = PlanNode::serialize();
    obj["outputType"] = outputType_ ? outputType_->toString() : "";
    return obj;
  }
...
class TraceScanNode final : public PlanNode {
 public:
  ...
  folly::dynamic serialize() const override {
    folly::dynamic obj = PlanNode::serialize();
    obj["invalid"] = "TraceScanNode is not serializable";
    return obj;
  }
...
#if defined BOLT_HAS_CUDF && BOLT_HAS_CUDF == 1
class AggregationNode : public PlanNode, public bolt::cudf::CudfEnable {
#else
...
class TableWriteNode : public PlanNode {
 public:
  TableWriteNode(
    ...
    BOLT_USER_CHECK_EQ(columns->size(), columnNames.size());
...
class LocalPartitionNode : public PlanNode {
 public:
  ...
  LocalPartitionNode(
    ...
    BOLT_USER_CHECK_NOT_NULL(partitionFunctionSpec_);
  ...
  LocalPartitionNode(
      ...
      PartitionFunctionSpecPtr partitionFunctionSpec,
      std::vector<PlanNodePtr> sources)
      : LocalPartitionNode(id, 0, type, partitionFunctionSpec, sources) {}
 ...
 public:
  ...
  PartitionedOutputNode(
      ...
      : PlanNode(id),
        ...
        partitionFunctionSpec_(std::move(partitionFunctionSpec)),
        outputType_(std::move(outputType)) {
    BOLT_USER_CHECK_GT(numPartitions, 0);
    if (numPartitions == 1) {
      BOLT_USER_CHECK(
          keys_.empty(),
...
class HashJoinNode : public AbstractJoinNode {
 public:
  ...
  void setSource(size_t pos, std::shared_ptr<const PlanNode> newNode) {
    sources_[pos] = newNode;
  }
...
#if defined BOLT_HAS_CUDF && BOLT_HAS_CUDF == 1
class OrderByNode : public PlanNode, public bolt::cudf::CudfEnable {
#else
 ...
 public:
  OrderByNode(
      ...
      : PlanNode(id),
        ...
        sources_{source} {
    BOLT_USER_CHECK(!sortingKeys.empty(), "OrderBy must specify sorting keys");
    BOLT_USER_CHECK_EQ(
        sortingKeys.size(),
        sortingOrders.size(),
...
// Represents the 'LocalShuffle' node in the plan.
class LocalShuffleNode : public PlanNode {
 public:
  LocalShuffleNode(
      const PlanNodeId& id,
      int64_t seed,
      const PlanNodePtr& source)
      : PlanNode(id), seed_(seed), sources_{source} {}
...
class WindowNode : public PlanNode {
 public:
  ...
  bool canSpill(const QueryConfig& queryConfig) const override {
    return queryConfig.windowSpillEnabled();
  }
...
class GeneratorNode : public PlanNode {
 public:
  GeneratorNode(
      const PlanNodeId& id,
  ...
  void setGeneratorType(const std::string& funcName) {
    if (funcName == "explode" || funcName == "posexplode") {
      genType_ = GeneratorType::Explode;
    } else if (funcName == "json_tuple") {
      genType_ = GeneratorType::JsonTuple;
    } else {
      genType_ = GeneratorType::Undefined;
    }
  }
  ...
  const std::vector<FieldAccessTypedExprPtr>& replicateCols() const {
    return replicateCols_;
  }

  const CallTypedExprPtr& generateFunction() const {
    return generateFunc_;
  }
  ...
  const bool isOuter() const {
    return isOuter_;
  }

  bool isExplode() const {
    return genType_ == GeneratorType::Explode;
  }
  bool isJsonTuple() const {
    return genType_ == GeneratorType::JsonTuple;
  }

 private:
  void addDetails(std::stringstream& stream) const override;

  const std::vector<FieldAccessTypedExprPtr> replicateCols_;
  CallTypedExprPtr generateFunc_;
  const bool withOrdinality_;
  const std::vector<PlanNodePtr> sources_;
  RowTypePtr outputType_;
  const bool isOuter_;
...
template <>
struct fmt::formatter<bytedance::bolt::core::PartitionedOutputNode::Kind>
    : formatter<std::string> {
  auto format(
      bytedance::bolt::core::PartitionedOutputNode::Kind s,
      format_context& ctx) {
    return formatter<std::string>::format(
        bytedance::bolt::core::PartitionedOutputNode::kindString(s), ctx);
  }
};

template <>
struct fmt::formatter<bytedance::bolt::core::JoinType> : formatter<int> {
  auto format(bytedance::bolt::core::JoinType s, format_context& ctx) {
