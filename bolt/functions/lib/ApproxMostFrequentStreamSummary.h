...
void ApproxMostFrequentStreamSummary<T, A>::setCapacity(int capacity) {
  ...
  if (capacity_ == 0) {
    ...
    BOLT_CHECK_EQ(capacity, capacity_);
  }
}

template <typename T, typename A>
int ApproxMostFrequentStreamSummary<T, A>::size() const {
  BOLT_DCHECK_EQ(values_.size(), counts_.size());
  BOLT_DCHECK_EQ(values_.size(), generations_.size());
  BOLT_DCHECK_EQ(values_.size(), heap_.size());
  BOLT_DCHECK_EQ(values_.size(), heapIndices_.size());
  return values_.size();
}
...
void ApproxMostFrequentStreamSummary<T, A>::topK(
  ...
  boost::sort::pdqsort(posBeg, posEnd, gt);
...
void ApproxMostFrequentStreamSummary<T, A>::serialize(char* out) const {
  ...
  *reinterpret_cast<int32_t*>(cur) = size();
  ...
  BOLT_DCHECK_EQ(cur - out, serializedByteSize());
}
