...
#include "bolt/dwio/dwrf/common/wrap/orc-proto-wrapper.h"
...
class PostScript {
 ...
 public:
  ...
  const proto::PostScript* getDwrfPtr() const {
    BOLT_CHECK_EQ(format_, DwrfFormat::kDwrf);
    return dwrfPtr();
  }

  const proto::orc::PostScript* getOrcPtr() const {
    BOLT_CHECK_EQ(format_, DwrfFormat::kOrc);
    return orcPtr();
  }
...
class FooterWrapper : public ProtoWrapperBase {
 public:
  ...
  const ::google::protobuf::RepeatedPtrField<
      ::bytedance::bolt::dwrf::proto::ColumnStatistics>&
  statistics() const {
    // BOLT_CHECK_EQ(format_, DwrfFormat::kDwrf);
    return dwrfPtr()->statistics();
  }

  const ::bytedance::bolt::dwrf::proto::ColumnStatistics& statistics(
      int index) const {
    // BOLT_CHECK_EQ(format_, DwrfFormat::kDwrf);
  ...
  const ::bytedance::bolt::dwrf::proto::Encryption& encryption() const {
    BOLT_CHECK_EQ(format_, DwrfFormat::kDwrf);
...
template <>
struct fmt::formatter<bytedance::bolt::dwrf::DwrfFormat> : formatter<int> {
  auto format(bytedance::bolt::dwrf::DwrfFormat s, format_context& ctx) {
