...
/// rows. Returns only non-null values.
///
  ...
  void destroy(folly::Range<char**> groups) override {
    if constexpr (!numeric) {
      for (auto group : groups) {
        auto accumulator = Aggregate::value<TAccumulator>(group);
        // If groups are all null, accumulator will not set.
...
template <typename TData, bool numeric>
...
class LastAggregate : public FirstLastAggregateBase<numeric, TData> {
 public:
  ...
  FLATTEN void addIntermediateResults(
    ...
    rows.applyToSelected([&](vector_size_t i) {
      return updateValue(i, groups[i], this->decodedValue_);
    });
  }
...
template <template <typename T, bool B1> class TClass>
      ...
      [name](
        ...
        switch (dataKind) {
          case TypeKind::BOOLEAN:
            return std::make_unique<TClass<bool, true>>(resultType);
          case TypeKind::TINYINT:
            return std::make_unique<TClass<int8_t, true>>(resultType);
          ...
          case TypeKind::TIMESTAMP:
            return std::make_unique<TClass<Timestamp, true>>(resultType);
          case TypeKind::HUGEINT:
            return std::make_unique<TClass<int128_t, true>>(resultType);
          case TypeKind::VARBINARY:
          case TypeKind::VARCHAR:
          case TypeKind::ARRAY:
          case TypeKind::MAP:
          case TypeKind::ROW:
            return std::make_unique<TClass<ComplexType, false>>(resultType);
          default:
            BOLT_FAIL(
...
void registerFirstLastAggregates(
    ...
    bool withCompanionFunctions,
    bool overwrite) {
  registerFirstLast<FirstAggregate>(
      prefix + "first", withCompanionFunctions, overwrite);
  registerFirstLast<LastAggregate>(
