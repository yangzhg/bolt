...
TEST_F(JsonExtractScalarTest, invalidPath) {
  EXPECT_THROW(jsonExtractScalar(R"([0,1,2])", ""), BoltUserError);
  EXPECT_THROW(jsonExtractScalar(R"([0,1,2])", "$[]"), BoltUserError);
  EXPECT_THROW(jsonExtractScalar(R"([0,1,2])", "$[-1]"), BoltUserError);
  EXPECT_THROW(jsonExtractScalar(R"({"k1":"v1"})", "$k1"), BoltUserError);
  EXPECT_THROW(jsonExtractScalar(R"({"k1":"v1"})", "$.k1."), BoltUserError);
  EXPECT_THROW(jsonExtractScalar(R"({"k1":"v1"})", "$.k1]"), BoltUserError);
  EXPECT_THROW(jsonExtractScalar(R"({"k1":"v1)", "$.k1]"), BoltUserError);
}
...
TEST_F(JsonExtractScalarTest, overflow) {
  if (!queryCtx_->queryConfig().enableSonicJsonExtractScalar()) {
    EXPECT_EQ(
        jsonExtractScalar(
    ...
    EXPECT_EQ(
        jsonExtractScalar(
            R"(184467440737095516151844674407370955161518446744073709551615)",
            "$"),
        "1.8446744073709552e+59");
  }

  GTEST_SKIP() << "NO overflow with simdjson";
  EXPECT_EQ(
      jsonExtractScalar(
          R"(184467440737095516151844674407370955161518446744073709551615)",
          "$"),
