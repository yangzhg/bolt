...
TEST_F(ArrayContainsTest, arrayCheckNulls) {
  bytedance::bolt::functions::prestosql::registerInternalFunctions();
...

  {
    ...
    BOLT_ASSERT_THROW(contains("[3, 3]", data), kErrorMessage);
    ASSERT_FALSE(contains("[3, 3]", data, true));
    // Null equal, [6, 6] vs [6, null].
    BOLT_ASSERT_THROW(contains("[6, null]", data), kErrorMessage);
...

  {
    ...
    BOLT_ASSERT_THROW(contains("[null, 3]", data), kErrorMessage);
    // [null, 3] = [null, null] is indeterminate.
    BOLT_ASSERT_THROW(contains("[null, null]", data), kErrorMessage);
...
TEST_F(ArrayContainsTest, rowCheckNulls) {
  ...
  // Null equal, (3, null) vs (3, 3).
  BOLT_ASSERT_THROW(contains({3, 3}), kErrorMessage);
  ASSERT_FALSE(contains({3, 3}, true));
  // Null equal, (6, 6) vs (6, null).
  BOLT_ASSERT_THROW(contains({6, std::nullopt}), kErrorMessage);
  ASSERT_FALSE(contains({6, std::nullopt}, true));
  // (3, null) = (3, null) is true in $internal$contains.
  ASSERT_TRUE(contains({3, std::nullopt}, true));
}

TEST_F(ArrayContainsTest, rewrite2in) {
  auto target = makeNullableFlatVector<int>({1, 2, 33});
  auto data = makeRowVector({target});
  auto result = evaluate("contains(ARRAY [1, 2, 3], c0)", data);
  assertEqualVectors(makeNullableFlatVector<bool>({true, true, false}), result);
  auto expr =
      makeTypedExpr("contains(ARRAY [1, 2, 3], c0)", asRowType(data->type()));
  auto rexpr = bytedance::bolt::functions::rewriteArrayContains2In("", expr);
  ASSERT_TRUE(rexpr != nullptr);
}
