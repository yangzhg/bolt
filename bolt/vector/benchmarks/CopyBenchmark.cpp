...
BENCHMARK_MULTI(copyStructAll) {
  ...
  auto source = vectorMaker.rowVector(children);
  auto target = BaseVector::create(source->type(), kSize, pool.get());
  suspender.dismiss();
  target->copy(source.get(), 0, 0, kSize);
  target->copy(source.get(), 0, kSize / 2, kSize / 2);
  return target->size();
}
