...
const bool* FlatVector<bool>::rawValues() const {
  BOLT_UNSUPPORTED("rawValues() for bool is not supported");
}
...
void FlatVector<StringView>::setStringViewValue(
  ...
  if (value.isInline()) {
    ...
    Buffer* buffer = getBufferWithSpace(value.size(), exactSize);
...
void FlatVector<StringView>::acquireSharedStringBuffers(
  ...
  auto bufferCount = stringBuffers_.size();
  ...
  // if stringbuffers_.size() reach the integer multiple of logFreq, LOG the
  // memory information of this vector
  constexpr auto logFreq = 1024;
  if (stringBuffers_.size() / logFreq > bufferCount / logFreq) {
    uint64_t totalCapacity = 0, totalSize = 0;
...
uint64_t FlatVector<StringView>::estimateExportArrowSize() const {
  auto offsetSize = BaseVector::length_ + 1;
  auto exportSize = BaseVector::estimateExportArrowSize() + offsetSize;
  auto size = BaseVector::size();
  if (mayHaveNulls()) {
    for (int i = 0; i < size; ++i) {
      if (!isNullAt(i)) {
        exportSize += valueAtFast(i).size();
      }
    }
  } else {
    for (int i = 0; i < size; ++i) {
