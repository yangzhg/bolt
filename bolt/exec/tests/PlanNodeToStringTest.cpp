...
TEST_F(PlanNodeToStringTest, basic) {
  ASSERT_EQ("-- Project[4]\n", plan_->toString(false, false, true));
}
...
TEST_F(PlanNodeToStringTest, withContext) {
  ...
  ASSERT_EQ(
      "-- Project[4]\n"
      "   Context for 4\n",
      plan_->toString(false, false, true, addContext));

  ASSERT_EQ(
      "-- Project[4][expressions: (out3:BIGINT, plus(cast ROW[\"out1\"] as BIGINT,10))] -> out3:BIGINT\n"
      "   Context for 4\n",
      plan_->toString(true, false, true, addContext));
  ...
  ASSERT_EQ(
      ...
      "        -- Values[0]\n"
      "           Context for 0\n",
      plan_->toString(false, true, true, addContext));
  ...
  ASSERT_EQ(
      ...
      "        -- Values[0][5 rows in 1 vectors] -> c0:SMALLINT, c1:INTEGER, c2:BIGINT\n"
      "           Context for 0\n",
      plan_->toString(true, true, true, addContext));
}
...
TEST_F(PlanNodeToStringTest, orderBy) {
  ...
  plan = PlanBuilder()
             .values({data_})
             .orderBy({"c1 ASC NULLS FIRST", "c0 DESC NULLS LAST"}, false, true)
             .planNode();

  ASSERT_EQ("-- OrderBy\n", plan->toString(false, false, true));
  ASSERT_EQ(
      "-- OrderBy[1][c1 ASC NULLS FIRST, c0 DESC NULLS LAST USE_CUDF] -> c0:SMALLINT, c1:INTEGER, c2:BIGINT\n",
      plan->toString(true, false, true));
#endif
}
...
TEST_F(PlanNodeToStringTest, tableScan) {
          ...
          {DOUBLE(), DOUBLE(), VARCHAR(), VARCHAR()})};
  {
    ...
    ASSERT_EQ(output, plan->toString(true, false, true));
  }
  {
    auto plan =
