...
TEST_F(GenericViewTest, invalidMultiCast) {
  ...
  { // Reader for vector of map(bigint, bigint).
    ...
    // allowed togother.
    EXPECT_THROW((reader[0].castTo<Map<int64_t, Any>>().size()), BoltUserError);
  }
}
...
struct ToStringFuncCastTo {
  ...
  void print(out_type<Varchar>& out, const arg_type<Any>& arg) {
    // Note: BOLT_DYNAMIC_SCALAR_TYPE_DISPATCH can be used to simplify
    ...
    switch (arg.kind()) {
      ...
      case TypeKind::ROW: {
        ...
        BOLT_CHECK(rowSize == 2, "print only supports rows of width 2");
        auto rowView = arg.template castTo<Row<Any, Any>>();
        printRow(out, rowView);
        break;
      }
      default:
        BOLT_UNREACHABLE("not supported");
    }
  }
...
struct ToStringFuncTryCastTo {
  ...
  void print(out_type<Varchar>& out, const arg_type<Any>& arg) {
    if (auto bigIntValue = arg.template tryCastTo<int64_t>()) {
      ...
      BOLT_UNREACHABLE("type not supported in this function");
    }
  }
