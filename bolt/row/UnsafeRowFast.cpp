...
int32_t UnsafeRowFast::serializeAsArray(
  ...
  *reinterpret_cast<int64_t*>(buffer) = size;
  ...
  // values stored in memory of null element is non-deterministic
  // which will cause group by's data inconsistency
  if (elements.supportsBulkCopy_ && !elements.decoded_.mayHaveNulls()) {
