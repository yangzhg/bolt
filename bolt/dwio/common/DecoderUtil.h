...
void fixedWidthScan(
  ...
  constexpr bool hasFilter = !std::is_same_v<TFilter, bolt::common::AlwaysTrue>;
  ...
  loopOverBuffers<T>(
      ...
      [&](T value, int32_t rowIndex) {
        if constexpr (!hasFilter) {
          ...
          if (bolt::common::applyFilter(filter, value)) {
...
bool useFastPath(Visitor& visitor) {
  ...
  // Ensure the data type is not int128_t.
  constexpr bool isNotInt128 =
      !std::is_same_v<typename Visitor::DataType, int128_t>;

  // The filter must be deterministic and a bulk path must be available.
  constexpr bool filterDeterministic = Visitor::FilterType::deterministic;
  constexpr bool hasBulkPath = Visitor::kHasBulkPath;

  // If the filter is always true or there are no nulls, then no additional
  // filter check is needed.
  constexpr bool filterAlwaysTrueOrNoNulls =
      std::is_same_v<typename Visitor::FilterType, bolt::common::AlwaysTrue> ||
      !hasNulls;

  // For hooks: either no hook is needed, or if there are nulls, the hook must
  // skip them.
  constexpr bool hookCondition =
      std::is_same_v<typename Visitor::HookType, NoHook> || !hasNulls ||
      Visitor::HookType::kSkipNulls;

  // Combine compile-time conditions.
  constexpr bool compileTimeCondition =
      isNotInt128 && filterDeterministic && hasBulkPath && hookCondition;

  // If compile-time conditions fail, immediately return false.
  ...
  if (!process::hasAvx2() && !process::hasNeon()) {
    return false;
  }

  // Evaluate the filter condition.
  // If the condition is fully determined at compile time (filter is always true
  // or no nulls exist), then we can immediately return true. Otherwise, check
  // at runtime whether nulls are allowed.
  if constexpr (filterAlwaysTrueOrNoNulls) {
    return true;
  } else {
    return !visitor.allowNulls();
  }
}
