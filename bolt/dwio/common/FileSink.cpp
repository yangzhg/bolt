...
std::unique_ptr<FileSink> FileSink::create(
  ...
  BOLT_FAIL("FileSink is not registered for {}", filePath);
}
...
LocalFileSink::LocalFileSink(const std::string& name, const Options& options)
  ...
  if (!fs::exists(dir)) {
    DWIO_ENSURE(bolt::common::generateFileDirectory(dir.c_str()));
  }
...
BOLT_REGISTER_DATA_SINK_METHOD_DEFINITION(LocalFileSink, localFileSink);
