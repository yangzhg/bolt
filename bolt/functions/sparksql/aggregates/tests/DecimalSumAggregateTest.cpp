...
TEST_F(DecimalSumAggregateTest, sumDecimal) {
  ...
  //       {makeNullableFlatVector<int64_t>(shortDecimalRawVector, DECIMAL(10,
  //       1)),
  //        makeNullableFlatVector<int128_t>(longDecimalRawVector, DECIMAL(23,
  //        4))});
  auto input = makeRowVector(
  ...
  testAggregations({input}, {}, {"spark_sum(c0)"}, "SELECT sum(c0) FROM tmp");
