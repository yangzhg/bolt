...
#include "bolt/common/caching/StringIdMap.h"
...
#define BOLT_CACHE_LOG_PREFIX "[CACHE] "
#define BOLT_CACHE_LOG(severity) LOG(severity) << BOLT_CACHE_LOG_PREFIX
#define BOLT_CACHE_LOG_EVERY_MS(severity, ms) \
  FB_LOG_EVERY_MS(severity, ms) << BOLT_CACHE_LOG_PREFIX
...
namespace std {
template <>
struct hash<::bytedance::bolt::cache::FileCacheKey> {
  size_t operator()(const ::bytedance::bolt::cache::FileCacheKey& key) const {
    return bytedance::bolt::bits::hashMix(key.fileNum.id(), key.offset);
  }
};

template <>
struct hash<::bytedance::bolt::cache::RawFileCacheKey> {
  size_t operator()(
      const ::bytedance::bolt::cache::RawFileCacheKey& key) const {
    return bytedance::bolt::bits::hashMix(key.fileNum, key.offset);
  }
};
...
class AsyncDataCacheEntry {
 public:
  ...
  void setExclusiveToShared(bool cacheable = true);
 ...
 private:
  ...
  tsan_atomic<bool> cacheable_{true};
...
class CoalescedLoad {
 public:
  ...
  virtual uint64_t getMemoryBytes() {
    return 0;
  }
  ...
  int64_t preloadBytes() const {
    return preloadBytes_;
  }
 ...
 protected:
  ...
  void setEndState(State expectState, State tState, State fState);
 ...
 public:
  ...
  void makePins(
    ...
    for (auto i = 0; i < keys.size(); ++i) {
      auto pin = findOrCreate(keys[i], sizeFunc(i), nullptr, cacheable);
...
template <>
struct fmt::formatter<bytedance::bolt::cache::CoalescedLoad::State>
    : formatter<int> {
  auto format(
      bytedance::bolt::cache::CoalescedLoad::State s,
      format_context& ctx) {
