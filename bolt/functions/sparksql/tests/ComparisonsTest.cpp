...
TEST_F(ComparisonsTest, compareForArray) {
  // For two null
  auto arr1 = makeNullableArrayVector<int32_t>({std::nullopt});
  auto arr2 = makeNullableArrayVector<int32_t>({std::nullopt});
  auto data = makeRowVector({arr1, arr2});
  auto expected = makeNullableFlatVector<bool>({std::nullopt});
  runAndCompare(
      "equalto", {arr1, arr2}, makeNullableFlatVector<bool>({std::nullopt}));
  runAndCompare(
      "lessthan", {arr1, arr2}, makeNullableFlatVector<bool>({std::nullopt}));
  runAndCompare(
      "lessthanorequal",
      {arr1, arr2},
  ...
  // NullableArrays with array encoding
  arr1 = makeNullableArrayVector<int32_t>(
      {std::nullopt, {{1, 2}}, {{1, 2}}, {{1, 2, 3}}});
  arr2 = makeNullableArrayVector<int32_t>(
      {std::nullopt, {{1}}, {{1, 2}}, {{1, 2, 3}}});
  runAndCompare(
      "equalto",
      {arr1, arr2},
      makeNullableFlatVector<bool>({std::nullopt, false, true, true}));
  runAndCompare(
      "lessthan",
      {arr1, arr2},
      makeNullableFlatVector<bool>({std::nullopt, false, false, false}));
  runAndCompare(
      "lessthanorequal",
      {arr1, arr2},
      makeNullableFlatVector<bool>({std::nullopt, false, true, true}));
  runAndCompare(
      "greaterthan",
      {arr1, arr2},
      makeNullableFlatVector<bool>({std::nullopt, true, false, false}));
  runAndCompare(
      "greaterthanorequal",
      {arr1, arr2},
      makeNullableFlatVector<bool>({std::nullopt, true, true, true}));

  // NullableArrays with constant encoding
  vector_size_t size = 10;
  vector_size_t firstRow = 2;
  expected = makeNullableFlatVector<bool>({std::nullopt, false, true, true});
  auto constantInput1 = BaseVector::wrapInConstant(10, firstRow, arr1);
  auto constantInput2 = BaseVector::wrapInConstant(10, firstRow, arr1);
  runAndCompare(
      "equalto",
      {constantInput1, constantInput2},
      BaseVector::wrapInConstant(10, 2, expected));
  runAndCompare(
      "lessthan",
      {constantInput1, constantInput2},
      BaseVector::wrapInConstant(10, 1, expected));
  runAndCompare(
      "lessthanorequal",
      {constantInput1, constantInput2},
      BaseVector::wrapInConstant(10, 2, expected));
  ...
  // A nullableArray with array encoding and a nullableArrays with constant
  // encoding
  arr2 = makeNullableArrayVector<int32_t>({
      {{1, 2}},
      {{1}},
  ...
  expected = makeNullableFlatVector<bool>(
      {true,
       ...
       true,
       true});
  runAndCompare("equalto", {constantInput1, arr2}, expected);
  runAndCompare("equalto", {arr2, constantInput1}, expected);
  runAndCompare(
      "lessthan",
      {constantInput1, arr2},
  ...
  // NullableArrays with dictionary encoding
  auto arr3 = makeNullableArrayVector<int32_t>(
      {{{1, 2, 3}}, std::nullopt, {{1, 2, 4}}});
  auto indices1 = makeIndices({0, 1, 2, 2, 1, 2, 0, 1, 0, 2});
  auto indices2 = makeIndices({2, 0, 2, 2, 1, 2, 0, 1, 0, 0});
  auto dictionaryArr1 =
      BaseVector::wrapInDictionary(nullptr, indices1, 10, arr3);
  auto dictionaryArr2 =
      BaseVector::wrapInDictionary(nullptr, indices2, 10, arr3);
  expected = makeNullableFlatVector<bool>(
      {false,
       std::nullopt,
       true,
       ...
       true,
       false});
  runAndCompare("equalto", {dictionaryArr1, dictionaryArr2}, expected);
  runAndCompare(
      "lessthan",
      {dictionaryArr1, dictionaryArr2},
      makeNullableFlatVector<bool>(
          {true,
           std::nullopt,
           false,
  ...
  // A nullableArray with array encoding and a nullableArrays with dictionary
  // encoding
  arr2 = makeNullableArrayVector<int32_t>({
      {{1, 2, 4}},
      {{1, 2, 3}},
  ...
  runAndCompare("equalto", {dictionaryArr1, arr2}, expected);
  runAndCompare("equalto", {arr2, dictionaryArr1}, expected);
  runAndCompare(
      "lessthan",
      {dictionaryArr1, arr2},
      makeNullableFlatVector<bool>(
  ...
  // Two array vectors without null
  arr1 = makeArrayVector<int32_t>({{{1, 2}}, {{1, 2}}, {{1, 2, 3}}});
  arr2 = makeArrayVector<int32_t>({{1}, {{1, 2}}, {{1, 2, 3}}});
  expected = makeNullableFlatVector<bool>({false, true, true});
  runAndCompare("equalto", {arr1, arr2}, expected);
  runAndCompare(
      "lessthan",
      {arr1, arr2},
      makeNullableFlatVector<bool>({false, false, false}));
  runAndCompare(
      "lessthanorequal",
      {arr1, arr2},
      makeNullableFlatVector<bool>({false, true, true}));
  runAndCompare(
      "greaterthan",
      {arr1, arr2},
      makeNullableFlatVector<bool>({true, false, false}));
  runAndCompare(
      "greaterthanorequal",
      {arr1, arr2},
      makeNullableFlatVector<bool>({true, true, true}));
}
...
TEST_F(ComparisonsTest, testflat) {
  ...
  bytedance::bolt::test::assertEqualVectors(expectedResult, actualResult);
