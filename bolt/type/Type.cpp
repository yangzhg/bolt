...
namespace std {
template <>
struct hash<bytedance::bolt::TypeKind> {
  size_t operator()(const bytedance::bolt::TypeKind& typeKind) const {
...
TypeKind mapNameToTypeKind(const std::string& name) {
  ...
  if (found == getTypeStringMap().end()) {
    BOLT_USER_FAIL("Specified element is not found : {}", name);
  }
...
std::string mapTypeKindToName(const TypeKind& typeKind) {
  ...
  if (found == typeEnumMap.end()) {
    BOLT_USER_FAIL("Specified element is not found : {}", (int32_t)typeKind);
  }
...
std::vector<TypePtr> deserializeChildTypes(const folly::dynamic& obj) {
  return bolt::ISerializable::deserialize<std::vector<Type>>(obj["cTypes"]);
}
} // namespace
...
TypePtr Type::create(const folly::dynamic& obj) {
  ...
  switch (typeKind) {
    case TypeKind::ROW: {
      BOLT_USER_CHECK(obj["names"].isArray());
...
std::string MapType::jitName() const {
  return "M<" + keyType()->jitName() + "," + valueType()->jitName() + ">_";
}
...
const TypePtr& RowType::findChild(folly::StringPiece name) const {
  ...
  BOLT_USER_FAIL(makeFieldNotFoundErrorMessage(name, names_));
}
...
std::string RowType::jitName() const {
  std::stringstream ss;
  ss << "S<";
  for (size_t i = 0; i < children_.size(); ++i) {
    ss << children_[i]->jitName();
...
folly::dynamic RowType::serialize() const {
  ...
  obj["names"] = bolt::ISerializable::serialize(names_);
  obj["cTypes"] = bolt::ISerializable::serialize(children_);
  return obj;
}
...
#define BOLT_DEFINE_SCALAR_ACCESSOR(KIND)                    \
  std::shared_ptr<const ScalarType<TypeKind::KIND>> KIND() { \
    return ScalarType<TypeKind::KIND>::create();             \
  }

BOLT_DEFINE_SCALAR_ACCESSOR(INTEGER);
BOLT_DEFINE_SCALAR_ACCESSOR(BOOLEAN);
BOLT_DEFINE_SCALAR_ACCESSOR(TINYINT);
BOLT_DEFINE_SCALAR_ACCESSOR(SMALLINT);
BOLT_DEFINE_SCALAR_ACCESSOR(BIGINT);
BOLT_DEFINE_SCALAR_ACCESSOR(HUGEINT);
BOLT_DEFINE_SCALAR_ACCESSOR(REAL);
BOLT_DEFINE_SCALAR_ACCESSOR(DOUBLE);
BOLT_DEFINE_SCALAR_ACCESSOR(TIMESTAMP);
BOLT_DEFINE_SCALAR_ACCESSOR(VARCHAR);
BOLT_DEFINE_SCALAR_ACCESSOR(VARBINARY);
...
TypePtr createScalarType(TypeKind kind) {
  return BOLT_DYNAMIC_SCALAR_TYPE_DISPATCH(createScalarType, kind);
}
...
TypePtr createType(TypeKind kind, std::vector<TypePtr>&& children) {
  ...
  return BOLT_DYNAMIC_TYPE_DISPATCH(createType, kind, std::move(children));
}
...
TypePtr createType<TypeKind::ARRAY>(std::vector<TypePtr>&& children) {
  BOLT_USER_CHECK_EQ(children.size(), 1, "ARRAY should have only one child");
  return ARRAY(children.at(0));
}

template <>
TypePtr createType<TypeKind::MAP>(std::vector<TypePtr>&& children) {
  BOLT_USER_CHECK_EQ(children.size(), 2, "MAP should have only two children");
...
void toTypeSql(const TypePtr& type, std::ostream& out) {
  switch (type->kind()) {
    ...
    default:
      ...
      BOLT_UNSUPPORTED("Type is not supported: {}", type->toString());
  }
}
...
int32_t DateType::toDays(folly::StringPiece in, bool* nullOutput) const {
  return util::fromDateString(in.data(), in.size(), nullOutput);
}

int32_t DateType::toDays(const char* in, size_t len, bool* nullOutput) const {
  return util::fromDateString(in, len, nullOutput);
}
...
class DecimalParametricType {
 public:
  static TypePtr create(
      ...
      [[maybe_unused]] std::vector<std::string> rowParameterNames) {
    BOLT_USER_CHECK_EQ(2, parameters.size());
    BOLT_USER_CHECK(parameters[0].kind == TypeParameterKind::kLongLiteral);
    BOLT_USER_CHECK(parameters[0].longLiteral.has_value());
    BOLT_USER_CHECK(parameters[1].kind == TypeParameterKind::kLongLiteral);
    BOLT_USER_CHECK(parameters[1].longLiteral.has_value());
...
class ArrayParametricType {
 public:
  static TypePtr create(
      const std::vector<TypeParameter>& parameters,
      [[maybe_unused]] std::vector<std::string> rowParameterNames) {
    BOLT_USER_CHECK_EQ(1, parameters.size());
    BOLT_USER_CHECK(parameters[0].kind == TypeParameterKind::kType);

    BOLT_USER_CHECK_NOT_NULL(parameters[0].type);
...
class MapParametricType {
 public:
  static TypePtr create(
    ...
    BOLT_USER_CHECK(parameters[1].kind == TypeParameterKind::kType);
    BOLT_USER_CHECK_NOT_NULL(parameters[1].type);
 ...
 public:
  static TypePtr create(
    ...
    for (const auto& parameter : parameters) {
      BOLT_USER_CHECK(parameter.kind == TypeParameterKind::kType);
      BOLT_USER_CHECK_NOT_NULL(parameter.type);
    }
    ...
    if (!rowParameterNames.empty()) {
      return ROW(std::move(rowParameterNames), std::move(argumentTypes));
    } else {
...
class FunctionParametricType {
 public:
  static TypePtr create(
      const std::vector<TypeParameter>& parameters,
      [[maybe_unused]] std::vector<std::string> rowParameterNames) {
    BOLT_USER_CHECK_GE(parameters.size(), 1);
    for (const auto& parameter : parameters) {
      BOLT_USER_CHECK(parameter.kind == TypeParameterKind::kType);
      BOLT_USER_CHECK_NOT_NULL(parameter.type);
    }
...
TypePtr getType(
  ...
  BOLT_USER_CHECK(
      rowParameterNames.empty() ||
      rowParameterNames.size() == parameters.size());
  if (parametricBuiltinTypes().count(name)) {
    return parametricBuiltinTypes().at(name)(
        parameters, std::move(rowParameterNames));
  }
