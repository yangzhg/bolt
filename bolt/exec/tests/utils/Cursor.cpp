  ...
  TaskCursorBase(
    ...
    if (!params.spillDirectory.empty()) {
      ...
      if (taskSpillDirectoryCb_ == nullptr) {
        auto fileSystem =
            bolt::filesystems::getFileSystem(taskSpillDirectory_, nullptr);
        BOLT_CHECK_NOT_NULL(fileSystem, "File System is null!");
        try {
...
class MultiThreadedTaskCursor : public TaskCursorBase {
 public:
  explicit MultiThreadedTaskCursor(const CursorParameters& params)
    ...
    BOLT_CHECK(!params.serialExecution);
    BOLT_CHECK(
 ...
 public:
  explicit SingleThreadedTaskCursor(const CursorParameters& params)
    ...
    task_ = Task::create(
        ...
        std::function<BlockingReason(
            RowVectorPtr, ContinueFuture*, uint32_t)>{},
        0,
        std::move(spillDiskOpts));
  ...
  bool hasNext() override {
    ...
    while (true) {
      ...
      if (!future.valid()) {
        BOLT_CHECK(!task_->isRunning() || !noMoreSplits_);
        return false;
      }
      // Task is blocked for some reason. Wait and try again.
      BOLT_CHECK_NULL(next);
      future.wait();
    }
  };
