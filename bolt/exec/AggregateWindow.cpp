...
#include "bolt/exec/AggregateWindow.h"
 ...
 private:
  ...
  void fillArgVectors(vector_size_t firstRow, vector_size_t lastRow) {
    ...
    for (int i = 0; i < argIndices_.size(); i++) {
      if (argVectors_[i]->size() == 0 && argIndices_[i] != kConstantChannel) {
        // clear argVectors_ memory to avoid OOM
        BaseVector::prepareForReuse(argVectors_[i], numFrameRows);
      } else {
  ...
  void computeSpillableAggregate(bool doInitialize) override {
    ...
    int32_t remainingRowCount = partition_->numRows();
    vector_size_t startRow = 0, endRow = 0;
    SelectivityVector rows(kBatchSize);
    do {
      auto numRows = std::min(remainingRowCount, kBatchSize);
      rows.resize(numRows);
      endRow = startRow + numRows - 1;
      fillArgVectors(startRow, endRow);
...
void registerAggregateWindowFunction(const std::string& name) {
  ...
  if (aggregateFunctionSignatures.has_value()) {
    ...
    exec::registerWindowFunction(
        ...
        {exec::ProcessingUnit::kRows, false, false, true});
  }
}
} // namespace bytedance::bolt::exec
