...
bytedance::bolt::exec::FunctionSignaturePtr makeCastSignature(
    const std::string& fromType,
    const std::string& toType) {
  return bytedance::bolt::exec::FunctionSignatureBuilder()
...
void addCastFromIntegralSignatures(
    const std::string& toType,
    std::vector<bytedance::bolt::exec::FunctionSignaturePtr>& signatures) {
...
std::vector<bytedance::bolt::exec::FunctionSignaturePtr>
    ...
    kSpecialForms = {
        {"and",
         std::vector<bytedance::bolt::exec::FunctionSignaturePtr>{
             ...
             bytedance::bolt::exec::FunctionSignatureBuilder()
...
static void filterSignatures(
    bytedance::bolt::FunctionSignatureMap& input,
...
static void appendSpecialForms(
    bytedance::bolt::FunctionSignatureMap& signatureMap,
  ...
  for (const auto& [name, signatures] : kSpecialForms) {
    ...
    std::vector<std::shared_ptr<const bytedance::bolt::exec::FunctionSignature>>
        rawSignatures;
    for (const auto& signature : signatures) {
      rawSignatures.push_back(signature);
    }
...
std::optional<CallableSignature> processConcreteSignature(
  ...
  BOLT_CHECK_NOT_NULL(callable.returnType);
...
uint32_t levelOfNesting(const TypePtr& type) {
  ...
  switch (type->kind()) {
    ...
    default:
      BOLT_UNREACHABLE("Not a supported type.");
  }
}
...
ExpressionFuzzer::ExpressionFuzzer(
  ...
  BOLT_CHECK(vectorFuzzer, "Vector fuzzer must be provided");
  seed(initialSeed);
    ...
    for (const auto& signature : function.second) {
      ...
      if (!signature->variables().empty()) {
        ...
        signatureTemplates_.emplace_back(SignatureTemplate{
            function.first, signature.get(), std::move(typeVariables)});
      } else if (
  ...
  registerFuncOverride(&ExpressionFuzzer::generateSpaceArgs, "space");
}
...
// Specialization for "space" function: to avoid OOM, the input integer value is
// randomized within 1000
std::vector<core::TypedExprPtr> ExpressionFuzzer::generateSpaceArgs(
    const CallableSignature& input) {
  std::vector<core::TypedExprPtr> inputExpressions = {
      std::make_shared<core::ConstantTypedExpr>(
          input.args[0], variant::create<int32_t>(rand32(1, 1000)))};
...
core::TypedExprPtr ExpressionFuzzer::generateExpressionFromSignatureTemplate(
  ...
  BOLT_CHECK_EQ(fuzzer.fuzzArgumentTypes(options_.maxNumVarArgs), true);
...
void ExpressionFuzzer::ExprBank::insert(const core::TypedExprPtr& expression) {
  ...
  BOLT_CHECK_LE(nestingLevel, maxLevelOfNesting_);
...
core::TypedExprPtr ExpressionFuzzer::ExprBank::getRandomExpression(
  ...
  BOLT_CHECK_LE(uptoLevelOfNesting, maxLevelOfNesting_);
  ...
  if (totalToConsider > 0) {
    ...
    BOLT_CHECK(false, "Should have found an expression.");
  }
  return nullptr;
}
...
TypePtr ExpressionFuzzer::fuzzReturnType() {
  ...
  if (signatures_.empty() && signatureTemplates_.empty() &&
    ...
    BOLT_CHECK(!signatures_.empty(), "No function signature available.");
