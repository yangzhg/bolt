...
std::string childName(
  ...
  switch (type->kind()) {
    ...
    case bolt::TypeKind::ROW: {
      auto name = std::dynamic_pointer_cast<const bolt::RowType>(type)->nameOf(
          childIdx);
...
namespace {
std::shared_ptr<const Type> getChildDiskType(
    const std::shared_ptr<const Type>& diskType,
    const std::shared_ptr<const Type>& requestType,
    size_t childIndex,
    bool useColumnNamesForColumnMapping) {
  if (!diskType) {
    return nullptr;
  }
  if (!useColumnNamesForColumnMapping) {
    return childIndex < diskType->size() ? diskType->childAt(childIndex)
                                         : nullptr;
  }
  if (diskType->kind() != TypeKind::ROW ||
      requestType->kind() != TypeKind::ROW) {
    return childIndex < diskType->size() ? diskType->childAt(childIndex)
                                         : nullptr;
  }

  const auto& requestedRow = requestType->asRow();
  const auto& diskRow = diskType->asRow();
  auto childName = requestedRow.nameOf(childIndex);
  auto diskChildIdx = diskRow.getChildIdxIfExists(childName);
  if (!diskChildIdx.has_value()) {
    return nullptr;
  }
  return diskType->childAt(diskChildIdx.value());
}
} // namespace
...
ColumnSelector ColumnSelector::apply(
  ...
  ColumnSelector cs(
      ...
      true);
  cs.setUseColumnNamesForColumnMapping(useColumnNamesForColumnMapping);

  // visit file schema and decide in content call
  copy(
      cs.nodes_[0],
      fileSchema,
      onlyFilter ? nullptr : origin->nodes_[0],
      useColumnNamesForColumnMapping);
  return cs;
}
