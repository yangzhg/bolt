...
void writeEncoding(VectorEncoding::Simple encoding, std::ostream& out) {
  switch (encoding) {
    ...
    case VectorEncoding::Simple::LAZY:
      ...
      return;
    default:
      BOLT_UNSUPPORTED("Unsupported encoding: {}", mapSimpleToName(encoding));
  }
}
...
Encoding readEncoding(std::istream& in) {
  ...
  switch (encoding) {
    ...
    case Encoding::kLazy:
      return encoding;
    default:
      BOLT_UNSUPPORTED("Unsupported encoding: {}", encoding);
  }
}
...
int32_t computeStringOffset(
  ...
  BOLT_FAIL("String view points outside of the string buffers");
}
...
const char* computeStringPointer(
  ...
  BOLT_FAIL("String offset is outside of the string buffers: {}", offset);
}
...
class LoadedVectorShim : public VectorLoader {
 public:
  ...
  void loadInternal(
    ...
    *result = vector_;
    BOLT_CHECK_EQ((*result)->size(), resultSize);
  }
...
VectorPtr restoreVectorFromFile(
  ...
  BOLT_CHECK(!inputFile.fail(), "Cannot open file: {}", filePath);
...
template <>
struct fmt::formatter<bytedance::bolt::Encoding> : formatter<int> {
  auto format(bytedance::bolt::Encoding s, format_context& ctx) {
