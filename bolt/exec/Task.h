...
class Task : public std::enable_shared_from_this<Task> {
 public:
  ...
  bolt::memory::MemoryPool* addConnectorPoolLocked(
      const core::PlanNodeId& planNodeId,
      int pipelineId,
      uint32_t driverId,
      const std::string& operatorType,
  ...
  /// raw pointer.
  bolt::memory::MemoryPool* addMergeSourcePool(
      const core::PlanNodeId& planNodeId,
      uint32_t pipelineId,
      uint32_t sourceId);
  ...
  void createLocalExchangeQueuesLocked(
      ...
      std::function<void(const bool)> finish_driver_creator_callback = nullptr);
  ...
  int32_t numDrivers(DriverCtx* driverCtx) {
    return driverFactories_[driverCtx->pipelineId]->numDrivers;
  }

  /// Returns the number of created and deleted tasks since the bolt engine
  ...
  /// [Morsel-driven] check if pipeline is morsel-driven
  bool isMorselDrivenPipeline(int pipelineId) {
    return driverFactories_[pipelineId]->isMorselDriven();
  }
  ...
  // dynamic parallelism
  ContinueFuture taskReadyToRunFuture(bool doSchedule);
  void notifyReadyToRun();
  void setSourceFragmentUnlocked() {
    taskStats_.isSourceFragment_ = true;
  }
  // get taskStats_ with no lock and no copy
  // currently used only in Task::terminate by Spark
  const TaskStats& taskStatsImmutable() const {
    return taskStats_;
  }

  void setConcurrency(int32_t val) {
    concurrency_ = val;
  }
  ...
  // visible for gluten to diagnostic a hang issue where the future returned by
  // requestCancel() didn't finish
 ...
 private:
  ...
  bolt::memory::MemoryPool* addExchangeClientPool(
      const core::PlanNodeId& planNodeId,
      uint32_t pipelineId);
  ...
  // Calculate the task-level "coefficient of variation" upon Task terminating.
  // The coefficient of variation is based on 1 "group" being a combination of
  // 1 pipeline and 1 operator. Then, the skewness of drivers in each "group" is
  // calculated.
  // Let the pipeline skewnesses be the average skewness over all the operator
  // in that pipeline and call them P1, P2, P3.
  // Let the "weight" of each pipeline be the sum of its operators'
  // inputPositions and call them W1, W2, W3.
  // Then, the "taskInputSizeCoefficientOfVariation"
  // = (W1P1 + W2P2 + W3P3) / (W1 + W2 + W3).
  void calculateInputPositionsCoefficientsOfVariation();
  ...
  /// [morsel-driven]
  /// Create callback function for creating a new driver consuming from a
  /// specific queue.
  std::function<
      void(std::shared_ptr<LocalExchangeQueue>, const int, const bool)>
  makeCreateDriverCallback(
      std::unique_ptr<DriverFactory>& factory,
      uint32_t splitGroupId,
      uint32_t pipeline);

  std::function<void(const bool)> makeFinishDriverCreatorCallback(
      std::unique_ptr<DriverFactory>& factory);

  uint32_t numMorselDrivenDrivers(int pipelineId) {
    // Return numMorselDrivenDrivers only if all driver schedulers finishes,
    // otherwise, numMorselDrivenDrivers should be unknown (unlimited)
    if (driverFactories_[pipelineId]->numFinishedPushBasedDriverSchedulers ==
        driverFactories_[pipelineId]->numDrivers) {
      return driverFactories_[pipelineId]->numMorselDrivenDrivers;
    } else {
  ...
  // [morsel-driven]
  bool hasMorselDrivenPipeline{false};
  uint32_t numPushBasedDriverSchedulers_{0};
  std::atomic_uint numFinishedPushBasedDriverSchedulers_{0};

  // dynamic parallelism
  std::vector<ContinuePromise> taskReadyToRunPromises_;
  bool readyToRun_{false};
  int32_t concurrency_{0};
};
...
template <>
struct fmt::formatter<bytedance::bolt::exec::Task::ExecutionMode>
    : formatter<std::string> {
  auto format(bytedance::bolt::exec::Task::ExecutionMode m, format_context& ctx)
      const {
    return formatter<std::string>::format(
        bytedance::bolt::exec::executionModeString(m), ctx);
  }
};
