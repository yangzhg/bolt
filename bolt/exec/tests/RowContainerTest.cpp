...
class RowContainerTest : public exec::test::RowContainerTestBase {
 protected:
  ...
  /*
   * We changed the way to measure memory usage in the
   * RowContainer::storeComplexType function. Previously, RowSizeTracker was
   * used, but now prepareRead(row, offset).size() is used directly.
   *
   * We changed the way to measure memory usage mainly to prevent the value
   * recorded by RowSizeTracker from being too small, which would cause errors
   * in row based spill.
   */
  void ensureGreateThan(std::vector<char*>& rows, RowContainer& data) {
    int64_t sum = 0;
    for (auto row : rows) {
      sum += data.rowSize(row) - data.fixedRowSize();
    }
    auto usage = data.stringAllocator().cumulativeBytes();
    // diff from checkSizes()
    EXPECT_GE(usage, sum);
  }
  ...
  void storeAndExtract(const VectorPtr& input) {
    ...
    DecodedVector decoded(*input);
    auto rows = store(*rowContainer, decoded, size);
    CHECK_EQ(size, rows.size());
    auto result = BaseVector::create(input->type(), size, pool_.get());
    rowContainer->extractColumn(rows.data(), size, 0, result);
    CHECK_EQ(size, result->size());
  }
  ...
  void benchmark(
    ...
    uint64_t average_duration = duration.count() / iterations;
    const std::string green_text = "\033[1;32m";
    const std::string reset_text = "\033[0m";
    std::cout << green_text;
    std::cout << "======== " << name << " ========" << std::endl;
    std::cout << "  Total iterations: " << iterations << std::endl;
    std::cout << "  Total time: " << succinctNanos(duration.count())
              << std::endl;
    std::cout << "  Time/iter: " << succinctNanos(average_duration)
              << std::endl;
    std::cout << "=============================" << std::endl;
    std::cout << reset_text;
  }
...
void makeNonNull(RowVectorPtr batch, int32_t column) {
  ...
  BOLT_CHECK(!nonNulls.empty(), "Whole column must not be null");
...
TEST_F(RowContainerTest, storeExtractArrayOfBigint) {
  // Make a string vector with two rows each having 2 elements.
  // Here it is important, that one of the 1st row's elements has more than 12
  // characters (14 and 13 in this case), since 12 is the number of chars being
  // inlined in the String View.
  ...
  std::vector<std::vector<int64_t>> elements(2);
  elements[0].emplace_back(ints[0]);
  elements[0].emplace_back(ints[1]);
  elements[1].emplace_back(ints[2]);
  elements[1].emplace_back(ints[3]);
  auto input = vectorMaker.arrayVector<int64_t>(elements);
  roundTrip(input);
}

TEST_F(RowContainerTest, storeExtractArrayOfBoolean) {
  // Make a string vector with two rows each having 2 elements.
  // Here it is important, that one of the 1st row's elements has more than 12
  // characters (14 and 13 in this case), since 12 is the number of chars being
  // inlined in the String View.
  ...
  std::vector<std::vector<bool>> elements(2);
  elements[0].emplace_back(bools[0]);
  elements[0].emplace_back(bools[1]);
  elements[1].emplace_back(bools[2]);
...
TEST_F(RowContainerTest, toString) {
  ...
  EXPECT_EQ(
      ...
      "Keys: BIGINT(0), VARCHAR(8) Dependents: TINYINT(25), REAL(26), ARRAY<BIGINT>(30) Num rows: 0 NullOffsets start bit 192");
  ...
  EXPECT_EQ(
      rowContainer->toString(),
      "Keys: BIGINT(0), VARCHAR(8) Dependents: TINYINT(25), REAL(26), ARRAY<BIGINT>(30) Num rows: 3 NullOffsets start bit 192");
...
DEBUG_ONLY_TEST_F(RowContainerTest, eraseAfterOomStoringString) {
  ...
  SCOPED_TESTVALUE_SET(
      ...
      std::function<void(memory::MemoryPool*)>(
          [&](memory::MemoryPool*) { BOLT_FAIL(exceptionMsg); }));
  BOLT_ASSERT_THROW(rowContainer->store(decoded, 0, row, 0), exceptionMsg);

  std::vector<char*> rows(1);
  RowContainerIterator iter;
  auto numRows = rowContainer->listRows(&iter, 1, rows.data());
  BOLT_CHECK_EQ(numRows, 1);
...
TEST_F(RowContainerTest, DISABLED_ConvertBenchmark) {
  ...
  // int64 10K
  {
    auto data = fuzzer.fuzzFlat(BIGINT());
    auto lambda = [&](auto data) { roundTrip(data); };
    benchmark("int64_10K", lambda, 1000, data);
  }
  // float 10K
  {
    auto data = fuzzer.fuzzFlat(REAL());
    auto lambda = [&](auto data) { roundTrip(data); };
    benchmark("float_10K", lambda, 1000, data);
  }
  // string 10K
  {
    auto data = fuzzer.fuzzFlat(VARCHAR());
    auto lambda = [&](auto data) { roundTrip(data); };
    benchmark("string_10K", lambda, 1000, data);
  }
  // int64 Array 10K
  {
    auto data = fuzzer.fuzzArray(fuzzer.fuzzFlat(BIGINT(), 100000), 1000);
    auto lambda = [&](auto data) { roundTrip(data); };
    benchmark("int64_array_10K", lambda, 1000, data);
  }
  // float Array 10K
  {
    auto data = fuzzer.fuzzArray(fuzzer.fuzzFlat(REAL(), 100000), 1000);
    auto lambda = [&](auto data) { roundTrip(data); };
    benchmark("float_array_10K", lambda, 1000, data);
  }
  // string Array 10K
  {
    auto data = fuzzer.fuzzArray(fuzzer.fuzzFlat(VARCHAR(), 100000), 1000);
    auto lambda = [&](auto data) { roundTrip(data); };
