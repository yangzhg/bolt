...
class OutputBuffer {
 public:
  ...
  // [morsel-driven]
  void addFinishedPushBasedDriverSchedulers() {
    BOLT_CHECK(
        is_morsel_driven_,
        "Set buffer end should not happen if the driver is created as planned, not dynamically.");
    ++numFinishedPushBasedDriverSchedulers_;
  }

  // [morsel-driven]
  void setMorselDriven() {
    is_morsel_driven_ = true;
  }
 ...
 private:
  ...
  // [morsel-driven]
  bool is_morsel_driven_ = false;
  uint32_t numFinishedPushBasedDriverSchedulers_ = 0;
