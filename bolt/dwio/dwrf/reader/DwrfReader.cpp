...
DwrfRowReader::DwrfRowReader(
    ...
    : StripeReaderBase(reader),
      ...
      columnSelector_{std::make_shared<ColumnSelector>(ColumnSelector::apply(
          opts.getSelector(),
          reader->getSchema(),
          opts.getUseColumnNamesForColumnMapping()))} {
  if (executor_) {
  ...
  if (!opts.getUseColumnNamesForColumnMapping()) {
...
void DwrfRowReader::checkSkipStrides(uint64_t strideSize) {
  ...
  if (currentRowInStripe_ == 0 || recomputeStridesToSkip_) {
    ...
    selectiveColumnReader_->filterRowGroups(
        strideSize, context, res, getReader().getBufferedInput());
...
std::optional<std::vector<bolt::dwio::common::RowReader::PrefetchUnit>>
...
DwrfReader::DwrfReader(
    ...
    : readerBase_(std::make_unique<ReaderBase>(
      ...
      options_(options),
      columnReaderOptions_(dwio::common::makeColumnReaderOptions(options)) {
  ...
  // (options_.getFileSchema() != nullptr) always equal to false, because gluten
  // don't pass fileSchema in HiveTableHandle Constructor
  if ((not options_.isUseColumnNamesForColumnMapping() ||
       readerBase_->isOldSchema()) and
...
TypePtr updateColumnNames(
  ...
  if (fileType->isRow()) {
    return updateColumnNames<RowType>(fileType, tableType, recursive);
  }

  if (fileType->isMap()) {
    return updateColumnNames<MapType>(fileType, tableType, recursive);
  }

  if (fileType->isArray()) {
    return updateColumnNames<ArrayType>(fileType, tableType, recursive);
  }
...
void DwrfReader::updateColumnNamesFromTableSchema() {
  ...
  auto updateRecursive = !options_.useNestedColumnNamesForColumnMapping();
...
std::unique_ptr<DwrfRowReader> DwrfReader::createDwrfRowReader(
    const RowReaderOptions& opts) const {
  auto rowReader =
      std::make_unique<DwrfRowReader>(readerBase_, opts, columnReaderOptions_);
