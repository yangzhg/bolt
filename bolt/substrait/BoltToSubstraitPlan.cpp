...
void BoltToSubstraitPlanConvertor::toSubstrait(
    google::protobuf::Arena& arena,
  ...
  if (auto torchNode =
          std::dynamic_pointer_cast<const ::bytedance::bolt::torch::TorchNode>(
              planNode)) {
    auto extension_rel = rel->mutable_extension_single();
    toSubstrait(arena, torchNode, extension_rel);
    return;
  }
#endif
  BOLT_UNSUPPORTED("Unsupported plan node '{}' .", planNode->name());
}

#if defined BOLT_HAS_TORCH && BOLT_HAS_TORCH == 1
void BoltToSubstraitPlanConvertor::toSubstrait(
    google::protobuf::Arena& arena,
    const std::shared_ptr<const ::bytedance::bolt::torch::TorchNode>& torchNode,
    ::substrait::ExtensionSingleRel* extensionRel) {
  const auto& source = getSingleSource(torchNode);
  toSubstrait(arena, source, extensionRel->mutable_input());

  ::substrait::bolt::TorchRelDetail torch_detail;
  torch_detail.set_script(torchNode->moduleScript());
  torch_detail.set_type_signature(
      exec::TypeSignature(*torchNode->outputType()).toString());
  extensionRel->mutable_detail()->PackFrom(torch_detail);
}
#endif
...
void BoltToSubstraitPlanConvertor::toSubstrait(
  ...
  for (int64_t i = 0; i < projectionSize; i++) {
    const auto& boltExpr = projections.at(i);

    projectRel->add_expressions()->MergeFrom(
        exprConvertor_->toSubstraitExpr(arena, boltExpr, inputType));
  ...
  for (int64_t i = 0; i < aggregatesSize; i++) {
    ...
    for (const auto& expr : aggregate.call->inputs()) {
      ...
      if (auto aggregatesExprInput =
        ...
        BOLT_NYI("In Bolt Plan, the aggregates type cannot be CallTypedExpr");
      } else {
...
const ::substrait::SortRel& BoltToSubstraitPlanConvertor::processSortFields(
    google::protobuf::Arena& arena,
...
const core::PlanNodePtr& BoltToSubstraitPlanConvertor::getSingleSource(
