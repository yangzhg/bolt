...
std::ostream& operator<<(std::ostream& os, const ParquetVersion::type& version);
...
class PARQUET_EXPORT ColumnProperties {
 public:
  ColumnProperties(
      ...
      int64_t dictionary_pagesize_limit = DEFAULT_DICTIONARY_PAGE_SIZE_LIMIT,
      int64_t pagesize = kDefaultDataPageSize)
      : encoding_(encoding),
        codec_(codec),
  ...
  void set_dictionary_pagesize_limit(int64_t size) {
    dictionary_pagesize_limit_ = size;
  }

  void set_pagesize(int64_t size) {
    pagesize_ = size;
  }
  ...
  int64_t dictionary_pagesize_limit() const {
    return dictionary_pagesize_limit_;
  }

  int64_t data_pagesize() const {
    return pagesize_;
  }
...
class PARQUET_EXPORT WriterProperties {
 public:
  class Builder {
   public:
    ...
    Builder* dictionary_pagesize_limit(int64_t dictionary_psize_limit) {
      default_column_properties_.set_dictionary_pagesize_limit(
          dictionary_psize_limit);
      return this;
    }
    ...
    Builder* dictionary_pagesize_limit(
        ...
        int64_t dictionary_psize_limit) {
      dictionary_pagesize_limit_[path] = dictionary_psize_limit;
      return this;
    }
    ...
    Builder* dictionary_pagesize_limit(
        const std::shared_ptr<schema::ColumnPath>& path,
        int64_t dictionary_psize_limit) {
      return this->dictionary_pagesize_limit(
          path->ToDotString(), dictionary_psize_limit);
    }
    ...
    Builder* data_pagesize(int64_t pg_size) {
      default_column_properties_.set_pagesize(pg_size);
      return this;
    }

    /// Specify the data page size.
    /// Default 1MB.
    Builder* data_pagesize(const std::string& path, int64_t pg_size) {
      pagesize_[path] = pg_size;
      return this;
    }
    ...
    /// Target size of the RowGroup
    Builder* set_parquet_block_size(int64_t size) {
      parquet_block_size_ = size;
      return this;
    }
    ...
    std::shared_ptr<WriterProperties> build() {
      ...
      for (const auto& item : dictionary_pagesize_limit_)
        get(item.first).set_dictionary_pagesize_limit(item.second);
      for (const auto& item : pagesize_)
        get(item.first).set_pagesize(item.second);
  ...
  int64_t dictionary_pagesize_limit(
      const std::shared_ptr<schema::ColumnPath>& path) const {
    return column_properties(path).dictionary_pagesize_limit();
  }

  int64_t dictionary_pagesize_limit() const {
    return default_column_properties_.dictionary_pagesize_limit();
  }
  ...
  int64_t data_pagesize(const std::shared_ptr<schema::ColumnPath>& path) const {
    return column_properties(path).data_pagesize();
  }

  int64_t data_pagesize() const {
    return default_column_properties_.data_pagesize();
  }
