...
void DirectInputStream::BackUp(int32_t count) {
  ...
  BOLT_CHECK(unsignedCount <= offsetInRun_, "Can't backup that much!");
...
void DirectInputStream::seekToPosition(PositionProvider& seekPosition) {
  ...
  BOLT_CHECK_LE(offsetInRegion_, region_.length);
}
...
void DirectInputStream::loadPosition() {
    ...
    if (load) {
      ...
      uint64_t usecs = 0;
      {
        ...
        if (load->state() == cache::CoalescedLoad::State::kPlanned) {
          ...
          LOG(WARNING) << "sync reload load " << loadAddr_ << " after waiting "
                       << usecs << " us";
          auto res = load->loadOrFuture(nullptr);
          BOLT_CHECK(
              res && load->state() == cache::CoalescedLoad::State::kLoaded);
        }
        loadedRegion_.offset = region_.offset;
        loadedRegion_.length = load->getData(region_.offset, data_, tinyData_);
      }
      ioStats_->queryThreadIoLatencyAsync().increment(usecs);
      ioStats_->queryThreadIoLatency().increment(usecs);
    } else {
  ...
  BOLT_CHECK_LT(offsetInRun_, runSize_, "failed in load {}", loadAddr_);
}
