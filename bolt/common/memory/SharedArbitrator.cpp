...
#define LOCAL_MEM_ARBITRATION_FAILED(errorMessage, requestPool) \
  BOLT_MEM_ARBITRATION_FAILED(fmt::format(                      \
...
SharedArbitrator::SharedArbitrator(const Config& config)
  ...
  BOLT_CHECK_EQ(kind_, config.kind);
  BOLT_CHECK_LE(reservedCapacity_, capacity_);
  BOLT_CHECK_GT(maxArbitrationTimeNs_, 0, "maxArbitrationTimeNs can't be zero");
  ...
  BOLT_MEM_LOG(INFO) << "Start memory reclaim executor with "
  ...
  BOLT_MEM_LOG(INFO) << "Shared arbitrator created with "
                     << succinctBytes(capacity_) << " capacity, "
                     << succinctBytes(reservedCapacity_)
                     << " reserved capacity";
  if (globalArbitrationEnabled_) {
    BOLT_MEM_LOG(INFO) << "Arbitration config: max arbitration time "
  ...
  BOLT_MEM_LOG(INFO) << "Memory pool participant config: "
...
void SharedArbitrator::shutdown() {
  {
    ...
    BOLT_CHECK(globalArbitrationWaiters_.empty());
...
void SharedArbitrator::setupGlobalArbitration(
  ...
  BOLT_CHECK(
      ...
      abortCapacityLimit);
  BOLT_CHECK_NULL(globalArbitrationController_);
  ...
  BOLT_MEM_LOG(INFO) << "Global arbitration abort capacity limits: "
                     << folly::join(",", globalArbitrationAbortCapacityLimits_);

  BOLT_CHECK_GE(spillCapacityLimit, participantConfig_.minReclaimBytes);
  ...
  BOLT_MEM_LOG(INFO) << "Global arbitration spill capacity limits: "
                     << folly::join(",", globalArbitrationSpillCapacityLimits_);
...
void SharedArbitrator::shutdownGlobalArbitration() {
  ...
  BOLT_CHECK(!globalArbitrationAbortCapacityLimits_.empty());
  BOLT_CHECK(!globalArbitrationSpillCapacityLimits_.empty());
  BOLT_CHECK_NOT_NULL(globalArbitrationController_);

  BOLT_MEM_LOG(INFO) << "Stopping global arbitration controller";
  globalArbitrationThreadCv_.notify_one();
  globalArbitrationController_->join();
  globalArbitrationController_.reset();
  BOLT_MEM_LOG(INFO) << "Global arbitration controller stopped";
}
...
void SharedArbitrator::addPool(const std::shared_ptr<MemoryPool>& pool) {
  ...
  std::vector<ContinuePromise> arbitrationWaiters;
  {
    ...
    if (allocatedBytes > 0) {
      BOLT_CHECK_LE(allocatedBytes, maxBytesToReserve);
      try {
...
void SharedArbitrator::removePool(MemoryPool* pool) {
  BOLT_CHECK_EQ(pool->reservedBytes(), 0, "{}", pool->name());
  const uint64_t freedBytes = shrinkPool(pool, 0);
  BOLT_CHECK_EQ(pool->capacity(), 0, "{}", pool->name());
  freeCapacity(freedBytes);

  std::unique_lock guard{participantLock_};
  const auto ret = participants_.erase(pool->name());
  BOLT_CHECK_EQ(ret, 1, "{}", pool->name());
}
...
std::optional<ArbitrationCandidate> SharedArbitrator::findAbortCandidate(
  ...
  if (!force) {
    BOLT_MEM_LOG(WARNING) << "Can't find an eligible abort victim";
    return std::nullopt;
  }
  ...
  // bucket.
  BOLT_CHECK(!candidateGroups.empty() && !candidateGroups[0].empty());
...
uint64_t SharedArbitrator::allocateCapacityLocked(
    ...
    uint64_t maxAllocateBytes,
    uint64_t minAllocateBytes) {
  BOLT_CHECK_LE(requestBytes, maxAllocateBytes);
...
uint64_t SharedArbitrator::shrinkCapacity(
  ...
  BOLT_CHECK(pool->isRoot());
  auto participant = getParticipant(pool->name());
  BOLT_CHECK(participant.has_value());
...
uint64_t SharedArbitrator::shrinkCapacity(
  ...
  BOLT_MEM_LOG(INFO) << "External shrink reclaimed "
...
void SharedArbitrator::startAndWaitGlobalArbitration(ArbitrationOperation& op) {
  ...
  if (arbitrationWaitFuture.valid()) {
    ...
    BOLT_CHECK_NOT_NULL(arbitrationWait);
...
void SharedArbitrator::globalArbitrationMain() {
  BOLT_MEM_LOG(INFO) << "Global arbitration controller started";
  while (true) {
    {
...
void SharedArbitrator::runGlobalArbitration() {
  ...
  BOLT_MEM_LOG(INFO) << "Global arbitration reclaimed "
...
void SharedArbitrator::checkIfTimeout(ArbitrationOperation& op) {
  if (FOLLY_UNLIKELY(op.hasTimeout())) {
    BOLT_MEM_ARBITRATION_TIMEOUT(fmt::format(
...
std::optional<ScopedArbitrationParticipant> SharedArbitrator::getParticipant(
  ...
  BOLT_CHECK(it != participants_.end(), "Arbitration pool {} not found", name);
...
uint64_t SharedArbitrator::reclaimUsedMemoryBySpill(
  ...
  for (auto& reclaimTask : reclaimTasks) {
    ...
    if (reclaimResult->reclaimedBytes == 0) {
      ...
      BOLT_CHECK_EQ(failedParticipants.count(reclaimResult->participantId), 0);
      failedParticipants.insert(reclaimResult->participantId);
    }
    reclaimedBytes += reclaimResult->reclaimedBytes;
  }
  BOLT_CHECK_LE(prevReclaimedBytes, reclaimedUsedBytes_);
  return reclaimedBytes;
}
...
uint64_t SharedArbitrator::reclaim(
  ...
  BOLT_MEM_LOG(INFO) << "Reclaimed from memory pool " << participant->name()
                     << " with target of " << succinctBytes(targetBytes)
                     << ", reclaimed " << succinctBytes(reclaimedBytes)
                     << ", spent " << succinctNanos(reclaimTimeNs)
                     << ", local arbitration: " << localArbitration << " stats "
                     << succinctBytes(stats.reclaimedBytes)
...
uint64_t SharedArbitrator::abort(
  ...
  BOLT_CHECK(participant->aborted());
...
void SharedArbitrator::freeReservedCapacityLocked(uint64_t& bytes) {
  BOLT_CHECK_LE(freeReservedCapacity_, reservedCapacity_);
...
SharedArbitrator::ScopedArbitration::ScopedArbitration(
  ...
  BOLT_CHECK_NOT_NULL(operation_);
...
SharedArbitrator::GlobalArbitrationSection::GlobalArbitrationSection(
    ...
    : arbitrator_(arbitrator) {
  BOLT_CHECK_NOT_NULL(arbitrator_);
  BOLT_CHECK(!arbitrator_->globalArbitrationRunning_);
