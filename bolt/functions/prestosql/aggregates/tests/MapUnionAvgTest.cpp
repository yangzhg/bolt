...
class MapUnionAvgTest : public AggregationTestBase {};

TEST_F(MapUnionAvgTest, global) {
  ...
  auto expected = makeRowVector({
      makeMapVector<int64_t, int64_t>({
          {{1, 11}, {2, 20}, {3, 30}, {4, 40}, {5, 50}},
      }),
  });

  testAggregations({data}, {}, {"map_union_avg(c0)"}, {expected});
}

TEST_F(MapUnionAvgTest, nullAndEmptyMaps) {
  ...
  testAggregations({allEmptyMaps}, {}, {"map_union_avg(c0)"}, {expectedEmpty});
  ...
  testAggregations({allNullMaps}, {}, {"map_union_avg(c0)"}, {expectedNull});
  ...
  testAggregations(
      {emptyAndNullMaps}, {}, {"map_union_avg(c0)"}, {expectedEmpty});
}

TEST_F(MapUnionAvgTest, tinyintOverflow) {
  auto data = makeRowVector({
  ...
  auto plan = PlanBuilder()
                  .values({data})
                  .singleAggregation({}, {"map_union_avg(c0)"})
                  .planNode();
...
TEST_F(MapUnionAvgTest, smallintOverflow) {
...
TEST_F(MapUnionAvgTest, integerOverflow) {
...
TEST_F(MapUnionAvgTest, bigintOverflow) {
...
TEST_F(MapUnionAvgTest, floatNan) {
...
TEST_F(MapUnionAvgTest, doubleNan) {
...
TEST_F(MapUnionAvgTest, doubleBasic) {
  auto data = makeRowVector({
      makeNullableMapVector<int64_t, double>({
          // row-0
          {{{1, 10}, {2, 20}}},
...
TEST_F(MapUnionAvgTest, groupBy) {
  auto data = makeRowVector({
  ...
  testAggregations({data}, {"c0"}, {"map_union_avg(c1)"}, {expected});
}
