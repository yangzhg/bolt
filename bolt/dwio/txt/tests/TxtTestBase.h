...
class TxtTestBase : public testing::Test, public test::VectorTestBase {
 protected:
  ...
  static RowTypePtr arrayEmptySchema() {
    return ROW({"gpslocation"}, {ARRAY(VARCHAR())});
  }

  std::unique_ptr<bytedance::bolt::txt::reader::TxtReader> createReader(
      const std::string& path,
      const dwio::common::ReaderOptions& opts) {
    auto input = std::make_unique<dwio::common::BufferedInput>(
        std::make_shared<LocalReadFile>(path), opts.getMemoryPool());
    return std::make_unique<bytedance::bolt::txt::reader::TxtReader>(
        std::move(input), opts);
  }
  ...
  dwio::common::RowReaderOptions getReaderOpts(
    ...
    rowReaderOpts.setTimestampPrecision(timestampPrecision_);
  ...
  using FilterMap =
      std::unordered_map<std::string, std::unique_ptr<bolt::common::Filter>>;
  ...
  void assertReadWithReaderAndFilters(
    ...
    for (auto&& [column, filter] : filters) {
      scanSpec->getOrCreateChild(bolt::common::Subfield(column))
