...
struct FuncOne {
  ...
  FOLLY_ALWAYS_INLINE bool call(
      out_type<bolt::Varchar>& /* result */,
      const arg_type<bolt::Varchar>& /* arg1 */) {
    return true;
  }
};
...
struct VariadicFunc {
  ...
  FOLLY_ALWAYS_INLINE bool call(
      out_type<bolt::Varchar>& /* result */,
      const arg_type<Variadic<bolt::Varchar>>& /* arg1 */) {
    return true;
  }
};

class VectorFuncOne : public bolt::exec::VectorFunction {
 public:
  void apply(
      const bolt::SelectivityVector& /* rows */,
      std::vector<bolt::VectorPtr>& /* args */,
      const TypePtr& /* outputType */,
      bolt::exec::EvalCtx& /* context */,
      bolt::VectorPtr& /* result */) const override {}

  static std::vector<std::shared_ptr<bolt::exec::FunctionSignature>>
  signatures() {
    // varchar -> bigint
    return {bolt::exec::FunctionSignatureBuilder()
                .returnType("bigint")
...
class VectorFuncTwo : public bolt::exec::VectorFunction {
 public:
  void apply(
      const bolt::SelectivityVector& /* rows */,
      std::vector<bolt::VectorPtr>& /* args */,
...
class VectorFuncThree : public bolt::exec::VectorFunction {
 public:
  void apply(
      const bolt::SelectivityVector& /* rows */,
      std::vector<bolt::VectorPtr>& /* args */,
...
class VectorFuncFour : public bolt::exec::VectorFunction {
 public:
  void apply(
      const bolt::SelectivityVector& /* rows */,
      std::vector<bolt::VectorPtr>& /* args */,
...
inline void registerTestFunctions() {
  ...
  BOLT_REGISTER_VECTOR_FUNCTION(udf_vector_func_one, "vector_func_one");
  BOLT_REGISTER_VECTOR_FUNCTION(udf_vector_func_one, "Vector_Func_One_Alias");
  BOLT_REGISTER_VECTOR_FUNCTION(udf_vector_func_two, "vector_func_two");
  BOLT_REGISTER_VECTOR_FUNCTION(udf_vector_func_three, "vector_func_three");
  BOLT_REGISTER_VECTOR_FUNCTION(udf_vector_func_four, "vector_func_four");
}
} // namespace
...
class FunctionRegistryTest : public ::testing::Test {
 public:
  ...
  void testResolveVectorFunction(
    ...
    checkEqual(bolt::resolveFunction(functionName, types), expected);
    checkEqual(bolt::resolveVectorFunction(functionName, types), expected);
  }
