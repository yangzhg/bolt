...
TEST(SpillTest, spillStats) {
  ...
  BOLT_ASSERT_THROW(stats1 < stats2, "");
  BOLT_ASSERT_THROW(stats1 > stats2, "");
  BOLT_ASSERT_THROW(stats1 <= stats2, "");
  BOLT_ASSERT_THROW(stats1 >= stats2, "");
  ASSERT_TRUE(stats1 != stats2);
  ...
  ASSERT_EQ(
      stats2.toString(),
      "spillRuns[100] spilledInputBytes[2.00KB] spilledBytes[1.00KB] spilledRows[1031] spilledPartitions[1025] spilledFiles[1026] spillTotalTimeUs[0us] spillFillTimeUs[1.03ms] spillSortTime[1.03ms] spillConvertTime[0us] spillSerializationTime[1.03ms] spillWrites[1028] spillFlushTime[1.03ms] spillWriteTime[1.03ms] maxSpillExceededLimitCount[4]");
}
