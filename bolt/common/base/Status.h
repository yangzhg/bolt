...
///
///  Status operation() {
///    BOLT_RETURN_IF(noMoreMemory, Status::OutOfMemory(
...
///
///  ...
///  BOLT_RETURN_NOT_OK(operation1());
///  BOLT_RETURN_NOT_OK(operation2());
///  BOLT_RETURN_NOT_OK(operation3());
///  ...
...
///   in this context usually means the program using Bolt (or its end users).
///
...
#define _BOLT_STRINGIFY(x) #x

/// Return with given status if condition is met.
#define BOLT_RETURN_IF(condition, status) \
  do {                                    \
...
#define BOLT_RETURN_NOT_OK(status)                            \
  do {                                                        \
    ::bytedance::bolt::Status __s =                           \
        ::bytedance::bolt::internal::genericToStatus(status); \
...
/// Useful for status check macros such as BOLT_RETURN_NOT_OK.
...
template <>
struct fmt::formatter<bytedance::bolt::Status> : fmt::formatter<std::string> {
  auto format(const bytedance::bolt::Status& s, format_context& ctx) {
    return formatter<std::string>::format(s.toString(), ctx);
  }
};

template <>
struct fmt::formatter<bytedance::bolt::StatusCode>
    : fmt::formatter<std::string_view> {
  auto format(bytedance::bolt::StatusCode code, format_context& ctx) {
    return formatter<std::string_view>::format(
        bytedance::bolt::toString(code), ctx);
  }
};
